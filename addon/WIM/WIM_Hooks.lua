...
-- 任务日志标题点击处理
function WIM_QuestLogTitleButton_OnClick(button)
	if WIM_EditBoxInFocus then
		...
		qTitle = StripText(qTitle)
		if (button == "LeftButton") then
			QuestLog_SetSelection(qIndex);
			if (IsShiftKeyDown() and (not ChatFrameEditBox:IsVisible()) and not isHeader) then
				if pfQuest and (pfQuest_config["questlinks"] == "1") then
					...
					WIM_EditBoxInFocus:Insert(hex .. "|Hquest:" .. questid .. ":" .. qLevel .. "|h[" .. qTitle .. "]|h|r")
					QuestLog_Update()
				else
					if (qTag) then qTag = (" (" .. qTag .. ")  ") else qTag = ""; end
					WIM_EditBoxInFocus:Insert("[" .. qLevel .. "] " .. qTitle .. qTag);
				end
			end
			return;
		end
...
-- 任务奖励物品点击处理
function WIM_QuestLogRewardItem_OnClick(button)
	if ( IsShiftKeyDown() ) then
		if WIM_EditBoxInFocus then
			WIM_EditBoxInFocus:Insert(GetQuestLogItemLink(this.type, this:GetID()));
		end
	end
	WIM_QuestLogRewardItem_OnClick_orig(button)
end
...
function WIM_SetUpHooks()
	...
	QuestLogTitleButton_OnClick = WIM_QuestLogTitleButton_OnClick;

	WIM_QuestLogRewardItem_OnClick_orig = QuestLogRewardItem_OnClick;
	QuestLogRewardItem_OnClick = WIM_QuestLogRewardItem_OnClick;
