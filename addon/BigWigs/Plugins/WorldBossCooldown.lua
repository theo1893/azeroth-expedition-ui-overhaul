...
按参考分支的七天周期估算个人冷却；服务器锁定提示可补充时间未知的记录。
本插件通过两种方式检测冷却：
  1) 击杀时 BigWigs EndBossfight 钩子 → 记录精确时间 → 显示倒计时
  2) 系统提示"个人锁定权限" → 标记为 CD 中（时间不明）

记录存储：
  self.db.char.worldBossKills = { [bossName] = <time()秒数> }   -- 精确时间
  self.db.char.worldBossLocked = { [bossName] = true }          -- 仅知在CD，时间不明
...
L:RegisterTranslations("zhCN", function() return {
    KILLED          = "%s 已击杀！预计冷却：还剩 %d 天 %d 时",
    READY_SHORT = "估算已到期",
    COOLDOWN_SHORT = "预计 %d天%d时",
    LOCKED_SHORT = "锁定，时间未知",
...
L:RegisterTranslations("enUS", function() return {
    KILLED          = "%s defeated! Estimated cooldown: %d day(s) %d hr",
    READY_SHORT = "Estimate elapsed",
    COOLDOWN_SHORT = "Est. %dd %dh",
    LOCKED_SHORT = "Locked, time unknown",
...
local worldBosses = {
    ["Azuregos"]=true, ["Lord Kazzak"]=true, ["Emeriss"]=true, ["Lethon"]=true,
    ["Taerar"]=true, ["Ysondre"]=true, ["Ostarius"]=true, ["Doctor Weavil"]=true,
    ["Concavius"]=true, ["Nerubian Overseer"]=true,
}
local recentLocks = {}
local function IsWorldBoss(module)
    return module and worldBosses[module.bossSync] and not module.trashMod
end

function BigWigsWorldBossCooldown:EnsureData()
    self.db.char.worldBossKills = self.db.char.worldBossKills or {}
    self.db.char.worldBossLocked = self.db.char.worldBossLocked or {}
end
...
local function remaining(t)
    ...
    if days == 0 and hours == 0 then hours = 1 end
    return days, hours
end
...
function BigWigsWorldBossCooldown:OnEnable()
    -- 确保存储表存在
    if not self.db.char.worldBossKills then
        self.db.char.worldBossKills = {}
    end
    if not self.db.char.worldBossLocked then
        self.db.char.worldBossLocked = {}
    end
    ...
    self:ScheduleRepeatingEvent("BigWigsWorldBossCooldownRefresh", self.RefreshPanel, 60, self)
end

function BigWigsWorldBossCooldown:OnDisable()
    self:CancelScheduledEvent("BigWigsWorldBossCooldownRefresh")
...
local LOCKOUT_PATTERN_ZHCN = "你绝对不允许从%s*(.-)%s*处获得战利品.*个人锁定权限"
local LOCKOUT_PATTERN_ENUS = "You are not allowed to loot (.+) .* personal lock"

function BigWigsWorldBossCooldown:OnSystemMessage(msg)
    if type(msg) ~= "string" then return end
    local _, _, bossName = string.find(msg, LOCKOUT_PATTERN_ZHCN)
    if not bossName then _, _, bossName = string.find(msg, LOCKOUT_PATTERN_ENUS) end
    if not bossName then return end
    bossName = string.gsub(string.gsub(bossName, "^%s+", ""), "%s+$", "")
    local bosses = AceLibrary("Babble-Boss-2.2")
    local known
    for key in pairs(worldBosses) do
        if bossName == key or bossName == bosses[key] then known = true; break end
    end
    if not known then return end
    self:EnsureData()
    local kills, locked = self.db.char.worldBossKills, self.db.char.worldBossLocked
    recentLocks[bossName] = time()
    if lastKill.name == bossName and time() - lastKill.time <= 5 then
        kills[bossName] = lastKill.prev
    end
    if not kills[bossName] or kills[bossName] + COOLDOWN <= time() then
        kills[bossName] = nil
        locked[bossName] = true
    end
    self:RefreshPanel()
end

function BigWigsWorldBossCooldown:RemindOnLogin()
    local kills = self.db.char.worldBossKills
    local locked = self.db.char.worldBossLocked
    local ready = {}
...
function BigWigsWorldBossCooldown:EndBossfight(module)
    ...
    self:EnsureData()
    if recentLocks[module:ToString()] and time() - recentLocks[module:ToString()] <= 5 then return end
...
function BigWigsWorldBossCooldown:CreatePanel()
    ...
    for i = 1, 12 do
        ...
        row.bossText:SetWidth(170)
        row.bossText:SetHeight(rowH)
        row.bossText:Hide()
        ...
        row.statusText:SetWidth(160)
        row.statusText:SetHeight(rowH)
        row.statusText:Hide()
        ...
        row.killedText:SetWidth(90)
        row.killedText:SetHeight(rowH)
        row.killedText:Hide()
...
function BigWigsWorldBossCooldown:RefreshPanel()
    ...
    local kills = self.db.char.worldBossKills or {}
    local locked = self.db.char.worldBossLocked or {}
    ...
    for i, name in ipairs(allNames) do
        ...
        if isLockedOnly then
            -- 仅知在CD，时间不明
            row.statusText:SetText(L["LOCKED_SHORT"])
        ...
        elseif type(killTime) == "number" then
            ...
            if d == 0 and h == 0 then
                row.statusText:SetText(L["READY_SHORT"])
                row.statusText:SetTextColor(0.2, 1.0, 0.3) -- 绿色=可打
            else
                row.statusText:SetText(string.format(L["COOLDOWN_SHORT"], d, h))
