...
local has_superwow = SUPERWOW_VERSION or SUPERWOW_STRING or SetAutoloot
local tracked_guids = {}
...
module.defaultDB = {
	...
	magus_polymorph = playerClass == "PRIEST" or playerClass == "PALADIN",
	magus_flames = true,
...
function module:UNIT_FLAGS(unit)
	...
	if not tracked_guids[unit] and (name == "魔鳞巨龙" or name == "Manascale Drake") then
...
function module:EnvelopedFlames(player)
	if self.db.profile.magus_flames then
		...
		elseif playerClass == "PRIEST" or playerClass == "PALADIN" then
