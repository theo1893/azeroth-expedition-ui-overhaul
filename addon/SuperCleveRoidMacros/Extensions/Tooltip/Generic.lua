...
function CleveRoids.GetActionButtonInfo(slot)
    ...
    -- SuperMacroPlus places an internal native macro on the action bar as a
    -- carrier.  It is not a spell/item and must not enter CRM's slot indexes.
    if name == "SMP_Carrier" then
        name = nil
        rank = nil
    end
...
function CleveRoids.GetSpell(text)
    ...
    local LocaleRank = string.match(CleveRoids.Localized.SpellRank, "%((.-)%%%)") -- 新增多语言适配 by 武藤纯子酱 2025.11.27
