...
function CleveRoids.ValidateMacro(macroName)
    ...
    local success, result = pcall(function()
        ...
        -- Plus macros may share a name with their original migration source.
        -- Validate the editable Plus body first, matching Core.ParseMacro.
        if type(GetSuperMacroPlusInfo) == "function" then
            name, texture, body = GetSuperMacroPlusInfo(macroName)
        end

        if not body then
            local macroID = GetMacroIndexByName(macroName)
            if macroID and macroID ~= 0 then
                name, texture, body = GetMacroInfo(macroID)
            end
        end

        if not body and type(CleveRoids.GetExtendedMacroInfo) == "function" then
            name, texture, body = CleveRoids.GetExtendedMacroInfo(macroName)
        end
