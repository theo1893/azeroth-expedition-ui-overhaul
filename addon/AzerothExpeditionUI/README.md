...
`0.4.0` 继续使用 legacy 战地旧书主框／Tab／输入／未读 runtime：

- pfUI 负责聊天行为、停靠、拖动、滚动、历史、输入与信息 widget。
- 本插件提供主框九宫格、Tab 三状态、未读覆盖和输入条。
- pfUI 的公会／背包空间／延迟等 legacy 信息底栏默认不挂载；相关 widget
  实现仍保留在 `../pfUI/modules/panel.lua`。
- 周期维护只更新视觉，不持续改写 Frame 几何。
- 禁用本插件并重载会回到 pfUI 维护分支的公共材质基线。
