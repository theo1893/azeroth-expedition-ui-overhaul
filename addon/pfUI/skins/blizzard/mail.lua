pfUI:RegisterSkin("Mailbox", "vanilla:tbc", function ()
  ...
  -- TurtleMail 按钮美化（新增部分）
  do -- TurtleMail 插件按钮美化
    -- 为 TurtleMailOpenSelectedMailButton 和 TurtleMailOpenAllMailButton 添加美化
    local function SkinTurtleMailButtons()
      -- 检查按钮是否存在
      if TurtleMailOpenSelectedMailButton then
        if not TurtleMailOpenSelectedMailButton.skinned then
          SkinButton(TurtleMailOpenSelectedMailButton)
          
          -- 将按钮向左移动一点（例如向左移动10像素）
          local point, relativeTo, relativePoint, xOffset, yOffset = TurtleMailOpenSelectedMailButton:GetPoint()
          if point then
            -- 获取当前的位置信息，然后将xOffset减少10（向左移动）
            TurtleMailOpenSelectedMailButton:ClearAllPoints()
            TurtleMailOpenSelectedMailButton:SetPoint(point, relativeTo, relativePoint, xOffset - 29, yOffset)
          end
          
          TurtleMailOpenSelectedMailButton.skinned = true
        end
      end
      
      if TurtleMailOpenAllMailButton then
        if not TurtleMailOpenAllMailButton.skinned then
          SkinButton(TurtleMailOpenAllMailButton)
          TurtleMailOpenAllMailButton.skinned = true
        end
      end
    end
    
    -- 尝试立即美化
    SkinTurtleMailButtons()
    
    -- 监听邮件界面显示事件，确保按钮被美化
    local turtleMailSkinner = CreateFrame("Frame")
    turtleMailSkinner:RegisterEvent("MAIL_SHOW")
    turtleMailSkinner:SetScript("OnEvent", function()
      -- 延迟一点确保按钮已创建
      C_Timer.After(0.1, SkinTurtleMailButtons)
    end)
    
    -- 也尝试通过 HookAddonOrVariable 来处理 TurtleMail 插件
    HookAddonOrVariable("TurtleMail", function()
      -- TurtleMail 插件加载后美化按钮
      SkinTurtleMailButtons()
    end)
  end
