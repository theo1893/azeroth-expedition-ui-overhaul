...
local uf_fcache = {}
local function UF_AuraVisible(config, kind, name)
  if not config then return true end
  local mode = (kind == "buff" and config.bufffilter) or (kind == "debuff" and config.debufffilter) or "none"
  if mode == "none" then return true end
  local liststr
  if kind == "buff" then
    liststr = (mode == "whitelist" and config.buffwhitelist) or (mode == "blacklist" and config.buffblacklist) or ""
  else
    liststr = (mode == "whitelist" and config.debuffwhitelist) or (mode == "blacklist" and config.debuffblacklist) or ""
  end
  local index = tostring(config) .. ":" .. kind .. ":" .. mode .. ":" .. liststr
  if not uf_fcache[index] then
    uf_fcache[index] = {}
    for _, v in pairs({strsplit("#", liststr)}) do
      if v and v ~= "" then uf_fcache[index][string.lower(v)] = true end
    end
  end
  local lname = string.lower(name or "")
  if mode == "whitelist" then
    return uf_fcache[index][lname]
...
function pfUI.uf:UpdateConfig()
  ...
  if not f.hoverglow then
    f.hoverglow = CreateFrame("Frame", nil, f)
    f.hoverglow:SetFrameStrata("BACKGROUND")
    f.hoverglow:SetFrameLevel(1)  -- 略高于战斗/仇恨glow的0级
    f.hoverglow:SetBackdrop(glow2)  -- 复用glow2边框材质
    f.hoverglow:SetBackdropBorderColor(1, 1, 1, 0.8)  -- 白色半透明
    f.hoverglow:Hide()
  end

  -- 更新悬停边框的位置（与glow一致）
  f.hoverglow:ClearAllPoints()
  f.hoverglow:SetPoint("TOPLEFT", f, "TOPLEFT", -6 - default_border, 6 + default_border)
  f.hoverglow:SetPoint("BOTTOMRIGHT", f, "BOTTOMRIGHT", 6 + default_border, -6 - default_border)
  ...
  if f.config.buffs == "off" then
    ...
    for i=1, 32 do
      ...
      local buffoffx = f.config.buffoffx and tonumber(f.config.buffoffx) or 0
...
local perrow = f.config.debuffperrow
      local row = floor((i-1) / perrow)

      local invert_h, invert_v, af
      -- 修改锚点设置，团队框架保持原有逻辑，其他框架使用新的逻辑
      if f.label == "raid" then
        -- 团队框架保持原有逻辑
        if f.config.debuffs == "TOPLEFT" then
          invert_h = 1
          invert_v = 1
          af = "TOPLEFT"
        elseif f.config.debuffs == "BOTTOMLEFT" then
          invert_h = -1
          invert_v = 1
          af = "BOTTOMLEFT"
        elseif f.config.debuffs == "TOPRIGHT" then
          invert_h = 1
          invert_v = -1
          af = "TOPRIGHT"
        elseif f.config.debuffs == "BOTTOMRIGHT" then
          invert_h = -1
          invert_v = -1
          af = "BOTTOMRIGHT"
        end
      ...
      if f.label == "raid" then
        -- 团队框架保持原有逻辑
        if f.config.raidgroup == "1" and f.group and (f.config.debuffs == "TOP" or f.config.debuffs == "BOTTOM") then
          anchor = f.group
        else
          anchor = f.config.portraitheight ~= "-1" and f.hp or f
        end
      else
        -- 非团队框架使用新的锚点逻辑
        anchor = f.config.portraitheight ~= "-1" and f.hp or f
        if anchor == f.hp and (f.config.debuffs == "BOTTOMLEFT" or f.config.debuffs == "BOTTOMRIGHT") then
          anchor = f.power
        end
      end
...
function pfUI.uf.OnEnter()
  ...
  -- 显示悬停高亮边框（若配置允许）
  if this.hoverglow and (this.config.hoverglow == nil or this.config.hoverglow == "1") then
    this.hoverglow:Show()
  end
  ...
  if unit.buffs and ( component == "all" or component == "aura" ) then
    ...
    for i=1, unit.config.bufflimit do
      ...
      if unit.label == "player" then
        ...
        -- Get buff name for filtering
        local bid = GetPlayerBuff(PLAYER_BUFF_START_ID+i,"HELPFUL")
        if bid and libtipscan then
          scanner = scanner or libtipscan:GetScanner("unitframes")
          scanner:SetPlayerBuff(bid)
          name = scanner:Line(1)
        end
    ...
    -- Calculate visible buffs count
    local visible_buffs_count = 0
    for _ in pairs(visible_buffs) do visible_buffs_count = visible_buffs_count + 1 end

    -- Second pass: rearrange visible buffs
    local perrow = unit.config.buffperrow
    local multiply = C.appearance.border.force_blizz == "1" and 1 or 2
    ...
    if unit.config.buffs == "TOPLEFT" then
      ...
      invert_v = 1
      af = "BOTTOMLEFT"
    elseif unit.config.buffs == "BOTTOMLEFT" then
      invert_h = -1
      invert_v = 1
      af = "TOPLEFT"
    elseif unit.config.buffs == "TOPRIGHT" then
      invert_h = 1
      invert_v = -1
      af = "BOTTOMRIGHT"
    elseif unit.config.buffs == "BOTTOMRIGHT" then
      invert_h = -1
      invert_v = -1
      af = "TOPRIGHT"
    end
    ...
    for i=1, unit.config.bufflimit do
      ...
      if i <= visible_buffs_count then
        -- Show and position visible buffs
        local buff_data = visible_buffs[i]
        local row = floor((i-1) / perrow)
        ...
        if buff_data.stacks > 1 then
          unit.buffs[i].stacks:SetText(buff_data.stacks)
        else
  ...
  if unit.debuffs and ( component == "all" or component == "aura" ) then
    ...
    for i=1, unit.config.debufflimit do
      ...
      if unit.label == "player" then
        ...
        local bid = GetPlayerBuff(PLAYER_BUFF_START_ID+i, "HARMFUL")
        if bid and libtipscan then
          scanner = scanner or libtipscan:GetScanner("unitframes")
          scanner:SetPlayerBuff(bid)
          name = scanner:Line(1)
    ...
    local visible_debuffs_count = 0
    for _ in pairs(visible_debuffs) do visible_debuffs_count = visible_debuffs_count + 1 end

    -- Calculate buff rows for positioning
    local buffrow = 0
    if unit.config.buffs == unit.config.debuffs then
      for row=0, 3 do
        if unit.buffs[row*bperrow+1] and unit.buffs[row*bperrow+1]:IsShown() then
          buffrow = buffrow + 1
        end
      end
    end

    -- Second pass: rearrange visible debuffs
    local multiply = C.appearance.border.force_blizz == "1" and 1 or 2
    ...
    for i=1, unit.config.debufflimit do
      ...
      if i <= visible_debuffs_count then
        -- Show and position visible debuffs
        local debuff_data = visible_debuffs[i]
        local row = floor((i-1) / perrow)
        ...
        if unit:GetName() == "pfPlayer" then
          ...
          for j=1, unit.config.debufflimit do
            local bid = GetPlayerBuff(PLAYER_BUFF_START_ID+j, "HARMFUL")
            if bid and libtipscan then
              scanner = scanner or libtipscan:GetScanner("unitframes")
              scanner:SetPlayerBuff(bid)
              local debuffName = scanner:Line(1)
              if debuffName == debuff_data.name then
        ...
        elseif libdebuff and selfdebuff == "1" then
          ...
          for j=1, unit.config.debufflimit do
            local debuffName, rank, texture, stacks, dtype, duration, timeleft, caster = libdebuff:UnitOwnDebuff(unitstr, j)
            if debuffName == debuff_data.name and duration and timeleft then
              CooldownFrame_SetTimer(unit.debuffs[i].cd, GetTime() + timeleft - duration, duration, 1)
              break
            end
          end
        elseif libdebuff then
          -- For all debuffs, find the actual debuff with matching name
          for j=1, unit.config.debufflimit do
            local debuffName, rank, texture, stacks, dtype, duration, timeleft, caster = libdebuff:UnitDebuff(unitstr, j)
            if debuffName == debuff_data.name and duration and timeleft then
              CooldownFrame_SetTimer(unit.debuffs[i].cd, GetTime() + timeleft - duration, duration, 1)
              break
            end
        ...
        if debuff_data.stacks > 1 then
          unit.debuffs[i].stacks:SetText(debuff_data.stacks)
        else
          ...
          for _, filter in pairs(unit.indicators) do
            if filter == string.lower(texture) then
              if string.lower(texture) == "interface\\icons\\spell_nature_rejuvenation" then
                ...
                if string.lower(buffName) ~= "回春术" then break end
              ...
              elseif string.lower(texture) == "interface\\icons\\spell_nature_resistnature" then
                ...
                if string.lower(buffName) ~= "愈合" then break end
