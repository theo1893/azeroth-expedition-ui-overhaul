...
--     race[String]  - Returns race of unit (player only)
--
-- Internal functions:
...
function GetUnitData(name, active)
  if units["players"][name] then
    ...
    return ret.class, ret.level, ret.elite, true, ret.guild, ret.race
  elseif units["mobs"][name] then
    local ret = units["mobs"][name]
    return ret.class, ret.level, ret.elite, nil, nil, nil
  elseif active then
    queue[name] = true
    libunitscan:Show()
  end
...
local function AddData(db, name, class, level, elite, guild, race)
  ...
  units[db][name].race = race or units[db][name].race
  queue[name] = nil
end
...
libunitscan:SetScript("OnEvent", function()
  if event == "PLAYER_ENTERING_WORLD" then
    ...
    AddData("players", name, class, level, nil, guild, race)
  ...
  elseif event == "FRIENDLIST_UPDATE" then
    ...
    for i = 1, GetNumFriends() do
      ...
      AddData("players", name, class, level, nil, nil, nil)
    end
  ...
  elseif event == "GUILD_ROSTER_UPDATE" then
    ...
    for i = 1, GetNumGuildMembers() do
      ...
      class = L["class"][class] or nil
      AddData("players", name, class, level, nil, guild, nil)
    end
  ...
  elseif event == "PARTY_MEMBERS_CHANGED" then
    local name, class, level, unit, _, guild, race
  ...
  elseif event == "UPDATE_MOUSEOVER_UNIT" or event == "PLAYER_TARGET_CHANGED" then
    ...
    local name, class, level, elite, _, guild, race
    if UnitIsPlayer(scan) then
      ...
      level = level > 0 and level or nil
      name = UnitName(scan)
      AddData("mobs", name, class, level, elite, nil, nil)
    end
  end
end)
