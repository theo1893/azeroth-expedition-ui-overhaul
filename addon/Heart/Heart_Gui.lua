...
function Heart_ProfileDropDownMenuInitialize()
	...
	for profile,data in Heart_SavedProfiles do
		-- 过滤掉带有 _D、_S、_T 后缀的方案
		if not (string.find(profile, "_D$") or string.find(profile, "_S$") or string.find(profile, "_T$")) then
			info = {
				["text"] = profile,
...
function Heart_ClassDropDownMenuInitialize()
	...
	local show
        for class, data in Heart_Config["classes"] do
		-- 过滤掉带有 _D、_S、_T 后缀的方案
		if not (string.find(class, "_D$") or string.find(class, "_S$") or string.find(class, "_T$")) then
			info = {
				["text"] = class,
...
function Heart_ClassDropDownMenuUpdate(class)
	...
	local taken = {}
	-- 创建或获取已警告过的法术表
	Heart_warned_missing_spells = Heart_warned_missing_spells or {}
	for percent, data in Heart_Config["classes"][class] do
        do
            local spell = data["Spell"]
            local attachment = data["Attachment"]
            -- 只在法术存在时执行显示逻辑
            if (Heart_Spells and Heart_Spells[spell]) then
			...
			getglobal("Heart_GUIClass" .. slot .. "ScaleCheckBox"):SetChecked(data["Scale"])
                ...
                -- 法术不存在时显示提示，限制最多显示5次且每个法术只提示一次
                Heart_missing_spell_warnings = Heart_missing_spell_warnings or 0
                if Heart_missing_spell_warnings < 5 and not Heart_warned_missing_spells[spell] then
                    C_Print("[Heart Helper] 当前天赋下没有技能: " .. spell)
                    Heart_missing_spell_warnings = Heart_missing_spell_warnings + 1
                    Heart_warned_missing_spells[spell] = true
                end
            end
        end
		...
		if (aka) then
		   if ( aka ~= "target" or UnitIsUnit("player", aka)) then
			...
			C_Print("将只治疗 (" ..aka.."): ".. Heart_Tank["Name"])
                        ...
                        C_Print("坦克单位必须在你的团队或小队中: " .. C_UnitName(aka) .. " 不在")
                        if (Heart_Tank and Heart_Tank["Name"]) then
                               C_Print("当前坦克仍然是 (" ..Heart_Tank["Unit"].."): ".. Heart_Tank["Name"])
                        else
                               C_Print("没有指定治疗目标")
                        end
                   end
...
function Heart_ShowTooltip()
	...
	elseif (this:GetName() == "Heart_GUIRoguePrioritySlider" or this:GetName() == "Heart_GUIRoguePriorityRez") then
		title = Heart_GUI_help["RoguePriority"]["Title"]
		description = Heart_GUI_help["RoguePriority"]["Description"]
	elseif (this:GetName() == "Heart_GUIShamanPrioritySlider" or this:GetName() == "Heart_GUIShamanPriorityRez") then
...
-- 显示治疗条的预览效果
function Heart_ShowHealPreview()
	if not Heart_PreviewMode then
		return
	end
	
	-- 使用预览数据，并确保所有变量都已初始化
	local targetName = Heart_PreviewTarget or "预览目标"
	local spellName = Heart_PreviewSpell or "治疗术"
	local spellRank = Heart_PreviewRank or 1
	local healAmount = Heart_PreviewHeal or 1000
	local currentHP = Heart_PreviewCurrentHP or 3000
	local maxHP = Heart_PreviewMaxHP or 5000
	
	-- 设置法术名称显示，与Heart_Hooks.lua中的逻辑保持一致
	-- show_spell_rank_heal同时控制法术等级和治疗量预估的显示
	if Heart_Config["show_spell_rank_heal"] == 1 then
		Heart_GUIHealCurrentSpellText:SetText("预览: " .. spellName .. "(等级" .. spellRank .. ") - " .. math.floor(healAmount) .. "/0")
	...
	-- 计算目标当前生命值百分比
	local currentHPPercent = currentHP / maxHP
	local afterHealHP = currentHP + healAmount
	local overhealAmount = 0
	
	-- 计算过量治疗
	if afterHealHP > maxHP then
		overhealAmount = afterHealHP - maxHP
		afterHealHP = maxHP
	end
	local afterHealPercent = afterHealHP / maxHP
	local overhealPercent = overhealAmount / maxHP
	
	-- 设置治疗目标的健康条
	if Heart_Config["show_target_health"] == 1 then
		Heart_GUIHealCurrentTarget:SetMinMaxValues(0, 1)
		Heart_GUIHealCurrentTarget:SetValue(currentHPPercent)
		Heart_GUIHealCurrentTargetText:SetText(targetName .. " (" .. math.floor(currentHPPercent * 1000) / 10 .. "%)")
		Heart_GUIHealCurrentTarget:Show()
	else
		Heart_GUIHealCurrentTargetText:SetText("")
		Heart_GUIHealCurrentTarget:Hide()
	end
	
	-- 设置治疗后的健康条
	if Heart_Config["show_after_health"] == 1 then
		Heart_GUIHealCurrentAfter:SetMinMaxValues(0, 1)
		Heart_GUIHealCurrentAfter:SetValue(afterHealPercent)
		Heart_GUIHealCurrentAfterText:SetText("治疗后: " .. math.floor(afterHealPercent * 1000) / 10 .. "%")
		Heart_GUIHealCurrentAfter:Show()
	else
		Heart_GUIHealCurrentAfterText:SetText("")
		Heart_GUIHealCurrentAfter:Hide()
	end
	
	-- 设置过量治疗条
	if Heart_Config["show_overheal"] == 1 and overhealAmount > 0 then
		Heart_GUIHealCurrentOverheal:SetMinMaxValues(0, 1)
		Heart_GUIHealCurrentOverheal:SetValue(overhealPercent)
		Heart_GUIHealCurrentOverhealText:SetText("过量: " .. math.floor(overhealAmount) .. " (" .. math.floor(overhealPercent * 1000) / 10 .. "%)")
		Heart_GUIHealCurrentOverheal:Show()
	else
		Heart_GUIHealCurrentOverhealText:SetText("")
		Heart_GUIHealCurrentOverheal:Hide()
	end
	
	-- 根据Heart_UpdateHealCurrent中的逻辑，重新排列可见的计量条
	local bars = {
		Heart_GUIHealCurrentSpell,
		Heart_GUIHealCurrentTarget,
		Heart_GUIHealCurrentAfter,
		Heart_GUIHealCurrentOverheal
	}
	...
	for _, bar in ipairs(bars) do
		if bar:IsVisible() then
			if not lastVisibleBar then
				-- 第一个可见条锚定到父框架顶部
				bar:SetPoint("TOP", Heart_GUIHealCurrent, "TOP", 0, 0)
			else
				-- 后续可见条锚定到上一个可见条底部
				bar:SetPoint("TOP", lastVisibleBar, "BOTTOM", 0, 0)
			end
			lastVisibleBar = bar
		end
	end
...
function Heart_UpdateHealCurrent(elapsed)
	-- 在预览模式下不需要更新实际治疗条
	if Heart_PreviewMode and Heart_PreviewMode == true then
		return
	end
	
	-- do nothing if current heal bars are disabled
	if (Heart_Config["show_current_heal_bars"] ~= 1) then
	...
	elseif (unit) then
		-- 只有当show_target_health为1时才显示法术目标和当前生命值
		if Heart_Config["show_target_health"] == 1 then
		...
		if (not Heart_healing[healingtarget][C_my_name]["Complete"]) then
			...
			-- 只有当show_after_health为1时才显示预计治疗后生命值
			if Heart_Config["show_after_health"] == 1 then
				Heart_GUIHealCurrentAfterText:SetText("治疗后 (" .. math.floor(afterhp / UnitHealthMax(unit) * 1000) / 10 .. "%)")
				Heart_GUIHealCurrentAfter:Show()
			else
				Heart_GUIHealCurrentAfterText:SetText("")
				Heart_GUIHealCurrentAfter:Hide()
			...
			-- 只有当show_overheal为1时才显示预计过量治疗值
			if Heart_Config["show_overheal"] == 1 then
				Heart_GUIHealCurrentOverhealText:SetText("过量治疗 (" .. math.floor(overheal * 1000) / 10 .. "%)")
				Heart_GUIHealCurrentOverheal:Show()
			else
				Heart_GUIHealCurrentOverhealText:SetText("")
				Heart_GUIHealCurrentOverheal:Hide()
...
function Heart_UpdateHealing(elapsed)
	...
	for who, data in Heart_healing do
		for author, moredata in Heart_healing[who] do
			...
			Heart_warned_missing_spells = Heart_warned_missing_spells or {}
			-- 检查法术是否存在，限制施法梯形警告次数
			if moredata["Spell"] and not Heart_Spells[moredata["Spell"]] then
				Heart_cast_warnings = Heart_cast_warnings or 0
				if Heart_cast_warnings < 5 and not Heart_warned_missing_spells[moredata["Spell"]] then
					C_Print("[Heart Helper] 施法梯形警告: 当前天赋下没有技能: " .. moredata["Spell"])
					Heart_cast_warnings = Heart_cast_warnings + 1
					Heart_warned_missing_spells[moredata["Spell"]] = true
				end
			end
			...
			if (Heart_Config["show_heal_bars"] == 1 and ((GetNumPartyMembers() >= 0 or GetNumRaidMembers() >= 0) and (Heart_Config["show_healing_all"] == 1 or (Heart_Config["show_healing_me"] == 1 and who == C_my_name) or not Heart_healing[who][author]))) then
...
function Heart_UpdateClassButtons()
         ...
         for class, data in Heart_Config["classes"] do
                 -- 过滤掉带有 _D、_S、_T 后缀的方案
                 if not (string.find(class, "_D$")) and not (string.find(class, "_S$")) and not (string.find(class, "_T$")) then
                     if x <= 5 then
...
function Heart_FixDeleteClassButton()
    if Heart_GUIDeleteClassButton then
        Heart_GUIDeleteClassButton:SetScript("OnClick", function()
            Heart_DeleteClassAndRelated(Heart_current_class)
        end)
    end
end
