...
function Heart_SpellTargetUnit(unit,fromheart)
	   ...
	   if not Heart_Spells[spell] or not Heart_Spells[spell][rank] then
		...
		local spell_key = spell .. "_rank" .. rank
		if Heart_cast_warnings < 5 and not Heart_warned_missing_spells[spell_key] then
			C_Print("[Heart Helper] 施法梯形警告: 当前天赋下没有技能或等级: " .. spell .. " (等级" .. rank .. ")")
			Heart_cast_warnings = Heart_cast_warnings + 1
			Heart_warned_missing_spells[spell_key] = true
		end
		clean_spell_awaiting_target()
		return
	   end
