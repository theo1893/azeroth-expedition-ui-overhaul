...
local function AddLocales(tooltip, type, sid, count)
    ...
    if not RDbItemsCfg.NoPrice and lv > 1 and right and not right:IsVisible() then
		...
		for current_line=1,tooltip:NumLines() do
			local current =getglobal(tooltip:GetName().."TextLeft"..current_line)
			if current and current:GetText() and strfind(current:GetText(), "套装：") then
				first_set_line = current_line
				break
			end
		end
		...
		if tooltip:NumLines() < 27 then
			...
			if first_set_line then 
				right = getglobal(tooltip:GetName().."TextRight".. first_set_line-1)
				right:SetText("|cffddddddLv|r".. lv)
				local right2 = getglobal(tooltip:GetName().."TextLeft".. first_set_line-1)
				local price = toMoney(itemInfo[IDX_PRICE])
				right2:SetText(price)
				right2:Show()
			end
		end
        right:Show()
