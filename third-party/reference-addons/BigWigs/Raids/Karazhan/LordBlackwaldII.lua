...
function module:PLAYER_TARGET_CHANGED()
	...
	if target == "Lord Blackwald II" or target == "布莱克沃尔德勋爵二世" then
		if not BigWigs:IsModuleActive(module) then
			BigWigs:EnableModule(module.name)
		end
	end
end
...
function module:CheckStatus()
	...
	-- Strict Coordinate Check + Combat
	if UnitAffectingCombat("player") then
		if posX >= 56.7 and posX <= 71.4 and posY >= 39.9 and posY <= 52.9 then
			self:Engage()
			return
		end
	end
	...
	-- Mouseover Check (Extra safety)
	local mouseover = UnitName("mouseover")
	if mouseover == "Lord Blackwald II" or mouseover == "布莱克沃尔德勋爵二世" then
		self:Engage()
		return
	end
end
