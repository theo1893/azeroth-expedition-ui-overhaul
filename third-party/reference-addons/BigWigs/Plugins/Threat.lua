	...
	args = {
		active = {
			...
			set = function(v)
				...
				-- 1.12 移植环境下模块没有 Ace2 的 Enable/Disable 方法，
				-- 参考 CatThreatLite 的做法：不依赖框架生命周期，
				-- 直接用本模块自带的 StartListening/StopListening 管理监听。
				if v then
...
function BigWigsThreat:OnEnable()
	-- 仅在玩家手动启用（db.profile.active）时才监听，
	-- 防止 BigWigs 核心自动启用所有非 Boss 模块时无视开关状态。
