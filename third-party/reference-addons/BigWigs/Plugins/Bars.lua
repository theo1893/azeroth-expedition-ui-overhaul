...
function BigWigsBars:BigWigs_StartBar(module, text, time, icon, otherc, c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, emphasize, target, spell)
	...
	-- 竖行条（VerticalBars）启用时，不再显示原版倒计时计时条。
	-- 但 MonitorBar（血/蓝条）会通过 target 参数传入 GUID，这类条仍需走原版。
	if BigWigsVerticalBars and BigWigsVerticalBars.db and BigWigsVerticalBars.db.profile and BigWigsVerticalBars.db.profile.enabled and not target then
		return
	end
...
-- 停止并清理当前所有正在运行的计时条（供竖行条启用时调用）
function BigWigsBars:StopAllBars()
	self:BigWigs_HideCounterBars()
	for i = table.getn(barCache), 1, -1 do
		if barCache[i] and barCache[i][1] then
