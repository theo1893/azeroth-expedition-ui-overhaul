pfUI:RegisterSkin("Character", "vanilla:tbc", function ()
  ...
  if PlayerTitleDropDown then -- tbc, wotlk
    ...
    -- Character Tab - 经典旧世版本的下拉菜单
    if PaperDollFrameTitlesDropdown then
      SkinDropDown(PaperDollFrameTitlesDropdown)
      PaperDollFrameTitlesDropdown:SetPoint("TOP", CharacterLevelText, "BOTTOM", 0, -2)
      PaperDollFrameTitlesDropdownText:SetPoint("LEFT", PaperDollFrameTitlesDropdown.backdrop, "LEFT", 6, 2)
    end
    
    -- Honor Tab
    StripTextures(HonorFrame)

    -- 美化HonorFrame的标签页（与ArenaFrameTab样式一致）
    for i = 1, 2 do -- HonorFrame通常有2个标签页
      local tab = _G["HonorFrameTab"..i]
      if tab then
        SkinTab(tab)
        
        -- 如果是第二个标签页，确保它的位置正确
        if i == 2 and HonorFrameTab1 then
          tab:ClearAllPoints()
          tab:SetPoint("LEFT", HonorFrameTab1, "RIGHT", border*2 + 1, 0)
        end
      end
    end
    ...
    -- Arena Frame 美化 - 适用于所有版本
  do -- Arena Tab
    if ArenaFrame then
      StripTextures(ArenaFrame)
      
      -- 美化ArenaFrame的标签页
      for i = 1, 2 do -- ArenaFrame通常有2个标签页
        local tab = _G["ArenaFrameTab"..i]
        if tab then
          SkinTab(tab)
          
          -- 如果是第二个标签页，确保它的位置正确
          if i == 2 and ArenaFrameTab1 then
            tab:ClearAllPoints()
            tab:SetPoint("LEFT", ArenaFrameTab1, "RIGHT", border*2 + 1, 0)
          end
        end
      end
      
      -- 美化Arena分数条等元素
      if ArenaFramePointsBar then
        StripTextures(ArenaFramePointsBar)
        CreateBackdrop(ArenaFramePointsBar, nil, true)
        ArenaFramePointsBar:SetStatusBarTexture(pfUI.media["img:bar"])
      end
      
      -- 美化竞技场队伍框架
      for i = 1, 5 do
        -- 尝试两种可能的命名模式
        local teamFrame = _G["ArenaTeam"..i] or _G["ArenaFrameTeam"..i]
        if teamFrame then
          StripTextures(teamFrame)
          CreateBackdrop(teamFrame)
        end
      end
    end
  end
