...
if IsSpellUsable and C.unitframes.reactive_indicator == "1" then
  ...
  if spells then
    ...
    for i, spell in ipairs(spells) do
      -- 获取法术 ID（如果 Nampower 可用）
      local spellId
      if pfUI.api and pfUI.api.GetMaxRankSpellId then
        spellId = pfUI.api.GetMaxRankSpellId(spell.name)
      end
      -- 如果无法获取 ID，则跳过该图标（无法检测）
      if not spellId then
        spellId = nil  -- 不存储字符串，避免后续错误
      end
      ...
      CreateBackdrop(icon)
      icon:Hide()
      icon.spellId = spellId  -- 存储 ID 或 nil
    ...
    pfUI.reactive:SetScript("OnUpdate", function()
      ...
      for _, icon in ipairs(this.icons) do
        local usable = false
        -- 只有当我们有数字 ID 时才检查可用性，否则视为不可用
        if type(icon.spellId) == "number" then
          usable = IsSpellUsable(icon.spellId) == 1
        end
        if usable then
          icon:Show()
          anyVisible = true
  ...
  if GetUnitField and pfUI.uf and pfUI_config.unitframes.druidmanabar == "1" then
    ...
    local function CreateDruidManaBar(parent, unit)
      ...
      -- ===== 德鲁伊法力条独立字体大小设置 =====
      if DC.druidmanafont == "1" then
          fontsize = tonumber(DC.druidmanafontsize) or fontsize
          -- 如需独立字体样式可在此扩展
      end
      -- =================================
      ...
      -- ===== 字体颜色设置（支持自定义） =====
      local tr, tg, tb, ta
      -- 仅当启用了“使用自定义字体”时，才应用自定义颜色
      if DC.druidmanafont == "1" and DC.druidmanafontcolor and DC.druidmanafontcolor ~= "" then
        tr, tg, tb, ta = pfUI.api.strsplit(",", DC.druidmanafontcolor)
        tr, tg, tb, ta = tonumber(tr), tonumber(tg), tonumber(tb), tonumber(ta)
        if not tr or not tg or not tb then
          -- 解析失败时回退
          tr, tg, tb = ManaBarColor[0].r, ManaBarColor[0].g, ManaBarColor[0].b
          ta = 1
        else
          ta = ta or 1
        end
      else
        -- 无自定义设置或未启用自定义字体，使用默认的 mana 颜色
        tr, tg, tb = ManaBarColor[0].r, ManaBarColor[0].g, ManaBarColor[0].b
