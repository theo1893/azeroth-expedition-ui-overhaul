...
-- 任务怪姓名板提示图标
-- 检测单位是否为任务目标
function IsQuestUnit(name)
  if not name or not IsAddOnLoaded("pfQuest") then return end
  local zone = pfMap:GetMapID(GetCurrentMapContinent(), GetCurrentMapZone())
  if pfMap.tooltips[name] then
    local meta
    ...
    if meta["quest"] then
      ...
      if meta["texture"] then
        resultcluster = meta["texture"]
      elseif meta.description and strfind(meta.description, "拾取") then
        resultcluster = pfQuestConfig.path.."\\img\\cluster_item"
      else
        resultcluster = pfQuestConfig.path.."\\img\\cluster_mob"
      end
...
-- 显式导出到 pfUI.api 表
pfUI.api.IsQuestUnit = IsQuestUnit
...
function pfUI.api.GetColoredTimeString(remaining)
  ...
  -- 获取精确时间模式配置（从buff模块的配置中读取）
  local exactMode = C and C.buffs and C.buffs.exacttime == "1"
  ...
  -- Show hours if remaining is > 2 hours (7200 seconds)
  elseif remaining > 7200 then
    -- 精确时间模式：2小时以上显示小时
  ...
  -- Show minutes if remaining is > 99 Seconds (99) and <= 2 hours
  elseif remaining > 99 then
    ...
    if exactMode and remaining <= 7200 then
      -- 精确模式：2小时以内显示分钟
      local totalMinutes = floor(remaining / 60)
      return color_minute .. totalMinutes .. "|rm"
    else
      -- 普通模式：显示分钟
