...
| `build_chat_component_textures.py` | legacy 必需 | V1 母版 → Tab、输入、底栏、蜡封 TGA |
| `build_chat_v3_layout_preview.py` | V3 验收 | V3 三张母版 → `440 × 320` 临时预演 |
| `build_chat_v3_runtime_assets.py` | V3 迁移草稿 | V3 三张母版 → 六张临时 runtime 图集、manifest 与预览 |
...
脚本依赖见根目录 [`requirements-tools.txt`](../requirements-tools.txt)；当前离线验证使用
`Pillow 12.2.0`。对 exporter 的任何 crop／UV 变更必须同步
