...
2. 为每张锁定基准解析对应的 prototype／provenance prompt；记录基准图路径、
   原始提示词路径和唯一职责。
3. 在生产提示词中写出“必须继承／组件级转译／明确不继承／冲突裁决”块，
   并把实质约束融合进最终执行正文。不得只写“保持同一风格”，也不得把
   派生 `assets/source/` 提升为最高视觉权威。
4. 先依据上述权威链把需求重写进版本化提示词文件。
5. 用户看到执行正文并明确确认具体版本后，将正文原样交给 `$imagegen`；
   “继续”或“下一步”本身不构成执行授权。
6. 记录固定执行器会话、结果、内部失败和执行器实际报告的 revised prompt；
   已执行正文不得原地覆盖。
7. 结果只写入被 Git 忽略的 `generated/`。
8. 候选先通过物件身份、物理结构、透视、图层、装配和技术审查；美术审查
   同时对照锁定图和从原始提示词提取的视觉 DNA，技术指标不
   等于综合色觉通过。
9. 用户明确确认具体透明母版后才进入 `assets/source/`。
10. 每次执行、退回、确认和路径变化同时更新 overhaul tracker。
