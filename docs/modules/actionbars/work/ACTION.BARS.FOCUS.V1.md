...
- 固定执行器：`imagegen-0-143-0 / @openai/codex@0.143.0`；本轮没有位图生产或
  修图，因此未调用执行器。
- ImageGen：`0/0`
- runtime：AEUI `0.8.22`／`focus-layout-contract=2.3`／
  `sidebar-group-contract=1.0`／`fieldkit-contract=2.2`。Field Kit v2.2 只修正
  AutoBar 配置兼容与同事件锚点收敛；V11 focus 几何不变并继续直接写 Turtle WoW
...
`0.8.22` 的 ActionBars adapter 内将 Player／Target 保持 `240×60 / 0.8`，
...
- 当前 AEUI `0.8.22` entrypoints：ActionBars `6c1b54af…7078`、Bootstrap
  `3c62eaee…cec4`、TOC `f8327129…fd38`。四张既有 TGA 字节不变，ImageGen `0/0`。
- AEUI `0.8.22` fresh-checkout addon package `pass`、violations `0`、report
...
包含 `version 0.8.22`、`focus-layout-contract=2.3`、
...
`fieldkit-contract=2.2` 与 `fieldkit-binding=bound`。先开关一次 pfUI unlock，确认无
