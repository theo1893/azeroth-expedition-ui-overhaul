...
| `AB.FIELDKIT bridge-v1.6` | runtime-v1.5 source／TGA／AutoBar／TrinketMenu 合同不变；可选 ArchiTotem 根加入 Bar 1 唯一 mover，绑定态在主栏下方，拖动回位，`unbind` 恢复；显式 focus preset 请求向下，普通 refresh 只读 | 用户接受 V4；ArchiTotem provider 级审计、Lua smoke 与 focus runtime display `7/7` pass，`pending-game-validation / P5` | 实机验证四元素施放、右键、Air 七层、Recall、拖动／锁定、向下 popup、bind／unbind 及非萨满／缺失 fail-open |
...
2. Turtle WoW 启动或 `/reload` 后确认 `/aeui status` 含 `version 0.8.9`、
   `fieldkit-contract=1.6`、`fieldkit-binding=bound` 与 `actionbar-stack=12x2-bound`。
   同时确认 `focus-layout-contract=1.3`、`focus-ui-scale-tier=8`、
   `focus-layout-display-scale=0.82`、`architotem-dock=bottom` 与
