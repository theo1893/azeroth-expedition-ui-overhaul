...
- DDPS／pfUI 白字日志统一由 `/ddps debug` 控制，默认关闭；开启后才创建并追加
  ...
  白字事件、执行记录和天赋快照。待实机复核默认游玩不创建／增长日志、开启记录、
  关闭停止及白字／输出循环正常；DDPS 缺失时不写日志。
  Debug 同时采样当前选中的敌对目标，以 Debuff 条目数（不累加层数）维护
  `DoiteDPSDB.debugMaxTargetDebuffs`；扫描直到客户端 API 返回空，不预设 16／32 上限。
  每次开启清零并开始记录，关闭保留本段峰值，`/ddps status` 可查看；数值按角色保存。
  待实机验证换目标后峰值只增不减、关闭停止采样及重新开启清零；这是客户端可见峰值，
  只能作为服务器 Debuff 上限的观测下界。

- DDPS 双手输出试验配置已接入，待实机：`/ddps config` 按非斩杀／斩杀阶段分别控制
  猛击、压制、旋风和嗜血／致死参与，默认全部开启；单体／群体都可调整猛击卡条
  `0–0.30` 秒（默认 `0.17`）、武器斩杀提前时间 `0.25–1.00` 秒（默认 `0.55`）、
  狂暴斩杀额外怒气上限 `0–30`（默认 `10`），并可关闭默认开启的“保护下轮猛击”。
  关闭技能同步取消它的预测、等待和预留怒气，不改变真实技能书、天赋、距离探针或
  已排队攻击的实际成本。下轮猛击保护开关只影响狂暴斩杀，当前猛击／近期瞬发、GCD
  和白字末端 `0.20` 秒保护保留。配置按角色和单体／群体独立保存，可恢复默认；
  旧值保留并补齐新默认。`/ddps test`、假图标轮换、TEST 标记及斩杀替换断筋均已移除。
  更新后 `/reload`，复核阶段切换、技能关闭后的留怒／预测、斩杀参数、群体猛击卡条及
  恢复默认；相邻回归防战／萨满真实推荐，并验证缺失白字 provider 时不提交自动斩杀，
  禁用 AEUI 后仍可独立配置和执行。

- DDPS 新白字周期提前排英勇／顺劈已增加统一时间限制，待实机：两者只允许在白字后半周期、
  剩余大于 `0.20` 秒时提交，推荐与执行共用条件；两者均避开猛击窗口并保留核心怒气预算。
  施法期间同步推荐刷新导致的动作错认也已修复，执行入口固定本次提交动作。
  待 `/reload` 复核双手狂暴单体／AOE 新周期高怒与 GCD 中的猛击保护、后半周期英勇／顺劈
  队列，以及禁用 AEUI 后的独立运行。

- DDPS 战士武器／职责切换已接入，待实机：`/ddps config` 的“战士武器方案”区域
  显示输出主手、坦克主手与坦克盾牌的图标／名称；点击装备位可从背包和已装备物品中
  选择或清除，也可在打开配置页后直接从背包／角色装备栏拖入对应装备位。
  拖入只保存物品引用并将物品放回原位；类型不符时保留原配置。
  自动过滤双手输出武器、坦克单手武器和盾牌，无需先装备。
  同时保留保存当前输出武器、当前剑盾的快捷按钮；也可使用 `/ddps weapons save dps`、
  `/ddps weapons save tank`，方案保存在每角色 `WARRIOR_ALL.weaponSets`。
  动作栏宏 `/ddps role toggle`（或 `/run DoiteDPS_WarriorRole()`）在输出／坦克之间切换；
  `/ddps role tank` 与 `/ddps role dps` 可固定切向。确认装备就位后同时更新单体／AOE
  出口，保留当前单体或 AOE 选择；后续继续使用原输出宏，由原循环处理姿态与施法。
  切换期间暂停 DDPS 执行，连续点击不反向切换；缺失武器拒绝开始，超时保留原循环并
  提示检查可能已变更的部分装备。切回双手武器时自动寻找所有普通背包的空格收盾，
  跳过专用袋与类型未缓存的背包；所有普通背包都满时提示留出一个空格。
  仅用户命令启动短暂换装任务，AEUI 不接管装备或循环。实机检查战斗中一次点击完成
  剑盾／双手双向切换、连续点击、单体／AOE 出口、主背包满与缺失武器；相邻回归原
  输出宏和防战姿态切换，确认另一角色配置隔离及禁用 AEUI 时 DDPS 仍可独立使用。
...
6. 移走／耗尽成员后确认主格与候选数量都在右下角，红色 `0` 常驻，任意正库存均为图腾计时同款的 12 UI 浅金描边字，主格左上没有额外
...
9. 所有角色的 DDPS 时间线与资源排应统一到 `TOPLEFT (650,-615)`，内部相对位置、
   ...
   应只列双手战士与防战各自的单体／群体入口，原宏和配置保持，切换天赋后自动识别
   武器／狂暴策略。深武器普通阶段验证致死／旋风保持就绪优先、每白字最多一次猛击、“猛击最大
   卡条”默认 `0.17` 秒且按实际攻速生效、移动时仍可建议猛击，以及压制只在 `25`
   怒气内切姿态；设置页不应再出现“斩杀贴刀窗口”。深武器斩杀阶段验证普通定时斩杀只在
   ...
   白字剩余等于或小于 `0.20` 秒时都不得排队。
   无尽怒气角色应按真实 `130` 上限判断溢出；切换怒不可遏等级后
   白字预测应无需重载即时刷新。群体验证横扫准备、回狂暴姿态、旋风／致死／安全猛击，
   顺劈默认 `95` 怒且只在计划技能后仍高怒或下次白字将封顶时排队；横扫末层应留给旋风，
   深武器定时斩杀到点不得被顺劈替代。双手狂暴用实际 `2.5–2.6` 秒 swing 复核：
   斩杀阶段不再为贴刀斩杀空等旋风，安全猛击继续参与；低怒斩杀不得破坏下一轮猛击／
   近期旋风资金；满强化斩杀复核 `20` 怒允许、`21` 怒不再斩杀，高怒仅按原预算用英勇／顺劈泄怒，低怒预计溢怒也不得绕过下一轮猛击保护。无嗜血天赋应仅使用已学技能，有嗜血时再验证
   推荐、六秒冷却与时间线；乱舞 Buff 消失不改变流派，洗天赋后无需重载即可切换。
   狂暴群体验证横扫／旋风／猛击与顺劈预算，缺失横扫技能时不等横扫；关闭调试后
   不应继续输出天赋快照，缺少白字 provider 时保留可用常规瞬发并暂停自动斩杀。
   相邻回归防战循环；再手动选中超距存活敌人确认不切换，清空目标后确认可自动取得近战候选，且 DDPS 自己取得的目标超距后仍可切到可信候选；无可信候选时保留原目标的红色“远”。
