...
- 底部布局试用已接入（待实机）：主动作栏 `BOTTOM (-18,80)`，Player／Target
  ...
  左下、标记控制按钮左侧，读条 X 恢复 `-22`。消耗品以真实动作栏外框对齐底边，
  两排总高随双排动作栏外框缩放，使上下边缘同时对齐；保持每排四格、按原槽位
  顺序向上增长，不挤压下方姿态／标记按钮。
  实机出现补给栏异常放大，已移除屏幕上下坐标差参与缩放的路径，改用 pfUI
  明确设置的两排高度、重叠量与外框 inset；尺寸未就绪时恢复主栏比例，待重载复核。
  补给图标按真实主动作 Button 的尺寸与有效缩放匹配，冷却／高亮随图标更新；
  两排外框对齐保持，解除绑定恢复原图标尺寸。待复核可见图标等大与数量／冷却显示。
  ...
  Target Markers `2.9` 为 `8×1`，绑定时左右外缘贴合主动作栏，八格高度填满栏下至
  屏幕底边的可用空间；左侧盾牌连同外框等高放大，姿态跟随盾牌／一键按钮左侧。
  宠物动作条保留独立行，解除绑定恢复原单格高度。全部沿用 accepted 媒体。
  外框大小已获用户实机确认；上下皮面以自身折边接合，保留原材料明度，
  共同底皮填满原 `3 UI` 排版分隔，连续外圈统一收口。独立横梁、压暗与重阴影
  已撤销；空态和有目标的局部预演已核对，当前结构仍待实机确认。上段固定显示八枚标记，空态 `70%` 亮度、
  占用态满亮且不移动；下段显示至多两行名字、全宽居中血量和底部 `3 UI` 细血条，
  过长名字省略且 Tooltip 保留全名。常规图标 `28 UI`、字号 `10 UI`，较矮双段使用
  `9 UI` 字号及按净空适配的图标；当前目标以顶部短暖金线提示。全空时两段仍显示，
  整体边界、盾牌与命中区不变。解除绑定或宠物栏造成低于 `72 UI` 的矮格时，恢复
  原单块紧凑布局。待 `/reload` 复核全空接缝、混合占用、全满、长名字、
  `100%`、死亡／失去目标后恢复空态，以及点选、禁用和解除绑定回退。
...
- 用户确认乌龟服先排英勇／顺劈再读猛击，两者可在同周期生效，无论猛击是否延迟白字。
  DDPS 已接入单体先英勇、AOE 先顺劈，下一次输出按键接猛击（循环待实机）：英勇要求当前
  满怒或当前怒气加预计下一刀回怒触顶，不预扣尚未施放的猛击来取消泄怒机会；顺劈沿用
  配置怒气线（默认 `95`）或扣除计划技能成本后预计溢怒。两者均预留排队技能、当前猛击与
  下次普通白字回怒前的核心技能成本，按
  冷却可用次数计入并扣除已预算的计划动作；狂暴额外预留下轮猛击。
  待确认／已确认队列均允许可负担的猛击，并阻止重复排队。排队独立于 GCD、猛击可用性与
  白字前后半周期；猛击来不及、已用或关闭时，仍可按泄怒条件排队。等待瞬发时也预留当前
  可落地猛击的成本。猛击自身仍遵守 GCD、卡条上限与每周期一次；顺劈保留横扫准备与旋风
  层数保护，深武器斩杀保持原策略。所有排队仍要求白字数据有效并保留末端 `0.20` 秒保护，
  已开始读条时执行入口仍暂停。
  实机日志已定位新周期先猛击、随后满怒的竞态：白字事件先重置 swing，怒气 API 稍后才刷新，
  旧流程会按旧怒气提前读猛击，随后被读条锁挡住泄怒。已增加白字命中后的短暂资源同步保护：
  收到玩家怒气事件或读到数值变化后立即重新推荐，最多等 `0.30` 秒；只延后尚未
  排英勇／顺劈且未满怒时的猛击，已有队列、已满怒和其他技能不因此等待。实机日志已确认
  旧怒气等待后能先英勇再猛击；当前待验证的是高怒前置瞬发后仍先英勇再猛击的泄怒门槛调整。
  组合仍需两次按键。
  `/ddps debug` 下猛击执行记录包含按键前的泄怒预算、队列标记与待确认时长。
  施法期间同步推荐刷新导致的动作错认也已修复，执行入口固定本次提交动作。
  待 `/reload` 复核单体／AOE 高怒先英勇／顺劈后猛击、GCD 结束后已来不及猛击仍能排队、
  关闭猛击后独立泄怒、低怒保留核心资金、合法
  卡条时两次伤害及无重复取消；相邻回归横扫末层与深武器斩杀，缺失白字 provider 时不提前
  排队，禁用 AEUI 后仍可独立运行。
...
- 合同：Slot `1.0`、Rail `1.0`、Field Kit `3.0`、Supply `2.1`、Combat Focus `3.6`、
  Sidebar Group `1.0`、Target Markers `2.9`。
...
| Target Markers | `P5` | runtime `2.9`；骷髅优先的 `8×1` 整体尺寸已实机确认。内部为上下常驻分区，共用完整连续的皮革九切片外框，以上下皮面自身的折边区分，共同底皮与连续外圈保持连接，保留原材料明度，上段放固定标记、下段放两行名字／居中百分比／细血条；空态图标 `70%` 亮度，全空仍保留两段，整体位置、盾牌尺寸和点击区域不变。常规图标 `28 UI`，较矮布局按净空适配；低于 `72 UI` 回退单块紧凑排版。新结构待实机确认。死亡／失去目标只清空本地显示，不修改真实团队标记；完整名字保留在 Tooltip。左侧盾牌继续使用 DDPS 坦克 API，缺失时保留不可用态；一键标记依赖 HDLRaidTools／SuperWoW。全部复用 accepted 媒体，不新增位图、不改变手动左右键语义 |
...
1. 完整退出客户端并重启后确认插件列表不再出现 AutoBar，且 `/aeui status` 含 `version 0.9.0`、
   ...
   `architotem-dock=upper-left-upward`；另有 `markers contract=2.9`、
   `anchor=bottom-single-row`、
   `layout=8x1-row`、`style=joined-fold-leather-board`（矮格为 `compact-leather-board`）、`strata=BACKGROUND`、`bulk=hdl-one-click`、
   `bulk-layout=conditional-left-of-tank`、`tank=ddps-assist`、
...
2. 在有标记权限的队伍／团队中确认八个透明命中位共用一块连续缝制皮革底板，
   ...
   Combination 一起移动；空态与占用态的图标均固定顶部中央，后者满亮；混合占用与
   八格全满时，`5–8` 字中文怪物名最多两行且不得遮住图标，超长名字省略后 Tooltip
   仍显示全名，`100%` 完整显示；当前选中只有顶部短暖金线。解除绑定后矮格不得出现
   名字、数字或血条重叠。再验证空格、设标、
