...
| DoiteDPS | `P5` | live provider `0.9.0`；战士目录只保留双手深武器单体／群体与防战单体／群体，旧战斗姿态武器战及狂暴战四套循环已删除，旧模式和按键绑定一次性迁移到对应双手武器战。单体以狂暴姿态为常驻姿态，普通阶段保持致死／旋风等就绪瞬发优先，每个白字周期最多一次猛击；“猛击最大卡条”默认 `0.17` 秒并按实际攻速动态计算，猛击后转好的瞬发仍可使用；压制只在战术掌握可保留的 `25` 怒气内切姿态，英勇只在预测下次白字将触及真实怒气上限时排队。斩杀阶段保留高效率核心，使用压制／斩杀 `20`、猛击／斩杀 `30`、旋风／斩杀 `40`、致死／斩杀 `45` 的最低怒气边界，并把斩杀放到白字周期后段；短命目标才立即斩杀，期间不再新排英勇。群体以横扫准备、回狂暴姿态、旋风／顺劈／致死及定时斩杀为主；撕裂、破甲和移动继续手动负责。怒气读取使用角色真实上限，支持无尽怒气的 `130` 上限；原精确近战候选与手动目标保护保持不变 |
...
8. 所有角色的 DDPS 时间线与资源排应统一到 `TOPLEFT (650,-615)`，内部相对位置、
   ...
   卡条”默认 `0.17` 秒且按实际攻速生效、猛击后转好的瞬发仍可使用，以及压制只在
   `25` 怒气内切姿态；斩杀阶段分别验证 `45` 怒致死后斩杀、`40` 怒旋风后斩杀、
   长线目标猛击后按白字时序斩杀、短命目标立即斩杀且不新排英勇，无尽怒气角色
   应按真实 `130` 上限判断溢出。群体验证横扫准备、回狂暴姿态、旋风／顺劈／致死及
   定时斩杀；相邻回归防战循环、精确近战换目标和无可信候选时保留原目标的红色“远”。
