...
| DoiteDPS | `P5` | live provider `0.9.0`；战士目录只保留双手深武器单体／群体与防战单体／群体，旧战斗姿态武器战及狂暴战四套循环已删除，旧模式和按键绑定一次性迁移到对应双手武器战。单体以狂暴姿态为常驻姿态，普通阶段保持致死／旋风等就绪瞬发优先，每个白字周期最多一次猛击；“猛击最大卡条”默认 `0.17` 秒并按实际攻速动态计算，移动时不建议猛击；压制只在战术掌握可保留的 `25` 怒气内切姿态，英勇只在按“暴击加权伤害项＋固定基础速度项”预测的下次白字将触及真实怒气上限时排队。斩杀阶段先保留高效率猛击：瞬发后仍能在白字前完成猛击与最低斩杀时，致死／旋风分别使用 `60／55` 怒气边界；猛击不可用、已用或无法落地时才回退到致死／斩杀 `45`、旋风／斩杀 `40`，并在周期后段清空余怒。可信 TTD 显示目标（包括 Boss）即将死亡时立即斩杀，期间不再新排英勇。群体以横扫准备、回狂暴姿态、旋风／顺劈／致死及定时斩杀为主；撕裂和破甲继续手动负责。怒气读取使用角色真实上限，支持无尽怒气的 `130` 上限；原精确近战候选与手动目标保护保持不变 |
