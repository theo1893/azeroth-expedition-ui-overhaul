...
- 底部布局试用已接入（待实机）：主动作栏 `BOTTOM (-18,80)`，Player／Target
  ...
  Target Markers `2.6` 为 `8×1`，绑定时左右外缘贴合主动作栏，八格高度填满栏下至
  屏幕底边的可用空间；左侧盾牌连同外框等高放大，姿态跟随盾牌／一键按钮左侧。
  宠物动作条保留独立行，解除绑定恢复原单格高度。全部沿用 accepted 媒体。
  外框大小已获用户实机确认；内部按已确认预演接入：常规高度下标记固定在顶部中央
  `28 UI`，空态降亮度、占用态满亮且不移动；中段使用固定字号、按实际字体宽度均衡
  分为至多两行的名字，过长末尾省略且 Tooltip 保留全名；血量独占整格宽度并居中，
  底部保留 `3 UI` 细血条，当前目标以顶部短暖金线提示。解除绑定或宠物栏造成的矮格
  使用紧凑内容布局，外框和命中区不变。待 `/reload` 复核混合占用、全满、长名字、
...
- 合同：Slot `1.0`、Rail `1.0`、Field Kit `3.0`、Supply `2.1`、Combat Focus `3.6`、
  Sidebar Group `1.0`、Target Markers `2.6`。
...
1. 完整退出客户端并重启后确认插件列表不再出现 AutoBar，且 `/aeui status` 含 `version 0.9.0`、
   ...
   `architotem-dock=upper-left-upward`；另有 `markers contract=2.6`、
