...
- AEUI 版本：`0.8.30`。
- 合同：Slot `1.0`、Rail `1.0`、Field Kit `2.8`、Combat Focus `2.5`、
  Sidebar Group `1.0`。
...
| Combat Focus | `P5` | Player／Target `240×60 / 0.8`，TargetTarget `240×60 / 0.68`；系统字体 `18 UI OUTLINE`；施法／Swing `260×12`；姿态真实 icon `25 UI / scale 1` |
...
1. `/reload` 后确认 `/aeui status` 含 `version 0.8.30`、
...
5. 战士三枚姿态按钮应真实放大到 `25 UI / scale 1`，高亮、命中区和快捷键同步；
   同时复查三条施法／攻击计时、Aura、Boss Debuff 与 DoiteDPS 净空。
