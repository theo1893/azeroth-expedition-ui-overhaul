...
- AEUI 版本：`0.9.0`。
- 合同：Slot `1.0`、Rail `1.0`、Field Kit `2.9`、Combat Focus `3.0`、
...
| Combat Focus | `P5` | Player／Target `480×48 / 0.8`、底锚点 `408 UI`，中心分别为 `-224／224`，完整框体之间保留 `64 UI` 视线缝；`23 UI` Aura 保持原尺寸、每排 `16` 枚，Player 向左、Target 向右展开，`32` 枚最多两排。TargetTarget 保持 `240×60 / 0.68`、每排 `8` 枚并向右展开；三框合同按角色版本应用并保存独立回退备份，后续同版本刷新不覆盖手调。施法／Swing 保持 `260×12` 与原坐标，两排下置 Aura 和玩家施法条之间约留 `32 UI`；姿态真实 icon `25 UI / scale 1`；所有启用 DDPS 的角色统一 `TOPLEFT (650,-615)`，各自 scale／功能配置不变 |
| DoiteDPS | `P5` | live provider `0.8.39`；战士输出键使用 `0.5 秒`超距容错、只读近战候选筛选和 `1.25 秒`切换锁，不再循环调用 Tab 目标；时间线以黄／红边框及“远”标记区分短暂超距和持续超距。武器战默认“智能斩杀”：短命目标、高怒／怒气封顶风险及核心空档才提高斩杀，长线目标保留致死／旋风／安全猛击，斩杀阶段不再新排英勇；仅在玩家再次按输出键、目标 TTD 可信且短于猛击剩余时间时允许中断猛击，群体循环继续优先多目标核心。配置仍提供“斩杀强优先”和“仅手动按键”两种显式策略 |
...
1. `/reload` 后确认 `/aeui status` 含 `version 0.9.0`、
   `fieldkit-contract=2.9`、`focus-layout-contract=3.0`、
   `focus-unit-default-version=2`、`focus-unit-default=profile-applied` 或
   `profile-saved`、`focus-layout-unit-size=480x48`、
   `focus-layout-unit-y=408`、`focus-layout-primary-gap=64`、
   `focus-layout-aura-per-row=16`、
   ...
   玩家／目标法术并让任一主框显示 `17–32` 个 Aura，确认图标尺寸没有缩小、只占
   两排，Player 向左、Target 向右展开，中央 `64 UI` 视线缝保持清空，Aura backdrop
   与三条读条没有相碰；再登录另一个启用 AEUI 的角色，确认 v2 三框布局只应用一次。
...
8. 所有角色的 DDPS 时间线与资源排应统一到 `TOPLEFT (650,-615)`，内部相对位置、
   ...
   目标刚跑出近战时先显示黄色“远”且不跳目标，持续超过约 `0.5 秒`后一次性切到
   已证明在近战内的坦克目标／团队标记／交战目标，附近没有可信候选时保留原目标并
