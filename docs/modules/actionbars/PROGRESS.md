...
- AEUI 版本：`0.8.32`。
- 合同：Slot `1.0`、Rail `1.0`、Field Kit `2.9`、Combat Focus `2.9`、
...
| Combat Focus | `P5` | Player／Target `240×48 / 0.8`、底锚点 `480 UI`，TargetTarget 保持 `240×60 / 0.68` 并只同步下移对齐；该三框合同按角色首次加载应用一次并保存独立回退备份，后续刷新不覆盖手调；系统字体 `18 UI OUTLINE`；施法／Swing 保持 `260×12` 与原坐标，主框下方两排 Aura 连同外框仍有净空；姿态真实 icon `25 UI / scale 1`；所有启用 DDPS 的角色统一 `TOPLEFT (650,-615)`，各自 scale／功能配置不变 |
...
1. `/reload` 后确认 `/aeui status` 含 `version 0.8.32`、
   `fieldkit-contract=2.9`、`focus-layout-contract=2.9`、
   `focus-unit-default=profile-saved`、
   `focus-layout-unit-size=240x48`、`autobar-anchor-basis=button-grid-4col-up`、
   ...
   玩家／目标法术并显示两排下置 Aura，确认紧凑主框、Aura backdrop 与三条读条
   之间没有相碰；再登录另一个启用 AEUI 的角色，确认相同三框布局只首次应用一次。
