...
- 用户于 `2026-08-12` 提供 `1337×542 RGB` 战士实机截图（SHA-256
  `7af6c0de4d868f7ecbcbe03c6e306a08ab0c3514f4562d01359f27b6a718c014`），确认
  bridge-v2.7／focus runtime-v2.5 未通过实机门禁：AutoBar 仍停在左上自由坐标，
  三枚姿态按钮已放大为约 `97×33 UI`，但整栏跨压主动作栏。退出游戏后的只读
  SavedVariables 显示活动 AutoBar 共享 display 仍保存 `(409.156,514.312)`，Combat
  Focus 已为 profile v16、`bar11.icon_size=25`、stance `BOTTOM (0,255) / scale 1`；
  MoveAnything 无 AutoBar／pfActionBarStances 条目，排除第三方位置恢复。源码全路径审计
  确认两组最终写入者：AutoBar `DisplayCopy` 可在嵌套 `ProfileChanged／OnShow` 中让真实
  `AutoBar.display` 与缓存 `profile.layoutProfile` 暂时不同；pfUI 除公开 `bars:UpdateConfig`
  外，还会在 stance `OnEvent`、pet `OnEvent／OnShow／OnHide` 与 unlock 关闭时重建／加载
  Bar 11 movable。AEUI `0.8.26`／`fieldkit-contract=2.8` 因此按活动 display 表身份解析
  配置，并创建全局 `AzerothExpeditionUIAutoBarDockAnchor`：代理与 AutoBar handle 等效
  缩放、相对主栏 `BOTTOMLEFT` 保存一次已验证偏移，provider 原生 docking 始终只写
  `CENTER → proxy.CENTER (0,0)`；自由 position 仅作可逆回退，logout 前仍移除运行时
  token。`focus-layout-contract=2.6`／profile v17 保存安全 fallback `BOTTOM (0,130)`，live
  强制 `TOP → pfActionBarMain.BOTTOM (0,-12)`；覆盖完整 bars rebuild、stance／pet 事件、
  pet 显隐和 unlock 关闭，并在解锁界面隐藏 stance 独立 mover。v14／v15 旧尺寸签名与
  实机失败的 exact v16 签名只升级姿态合同，其他手调坐标不变；无 OnUpdate 维护循环。
  Lua smoke 已逐条模拟所有最终写入边界与 stale layoutProfile；ActionBars／Bootstrap／TOC
  SHA 分别为 `18bcaa63…e2c3e`／`68206faa…e5ed0`／`b3d93f60…f7ef6`。Focus display
  `13/13 pass`（report SHA `d04c4631…1280`），Field Kit display `9/9＋10/10 pass`，
  fresh-checkout package `pass`（SHA `e1ca9054…0a35`，records `64`，tracked addon
  files `554`，目标设备无需构建）。位图字节不变，ImageGen `0/0`。当前仍为
...
- 用户于 `2026-08-12` 提供新的 `1408×633 RGB` 战士实机截图（SHA
  ...
  `554`、目标设备无需构建。该轮随后被 `1337×542` 实机证据否决，由 bridge-v2.8／
  focus runtime-v2.6 接续；阶段保持 `P5`。
...
| `AB.STANCE／PET` | `P1–P5` | Bar `11／12` 与全部重建边界已审计；focus runtime-v2.6 使用 `25 UI / scale 1`，SavedVariables fallback 为 `BOTTOM (0,130)`，live 以 `TOP → 主栏 BOTTOM / 12 UI` 绑定；stance／pet 事件、pet 显隐、bars rebuild 与 unlock 关闭后同事件恢复，stance 独立 mover 隐藏。姿态／宠物内容、点击与状态仍归 pfUI | 战士 `/reload` 确认三姿态真实按钮与命中区同步放大且在主栏下方；切姿态、召唤／解散宠物、开关 unlock 均不跳位；再覆盖职业最少／最多姿态数量与宠物自动施法排版 |
| `AB.CONSUMABLE.RACK／POCKET／POPUP／CONFIG` | `P5 / runtime-asset-v1.5 / bridge-v2.8 / pending-game-validation / 1/5` | [source](../../../assets/source/actionbars/ab-consumable-kit/ActionConsumableKit_Master_v1.png)／[runtime manifest](../../../assets/source/actionbars/ab-consumable-kit/AB-CONSUMABLE-KIT-V1_RuntimeManifest_v1.json)／[work](work/ACTION.BARS.FIELDKIT.V1.md)；TGA `c48f6292…320e`、像素 `658f826f…e30d` 不变；24 个逻辑类别、当前可用类别动态显示、最大 `4×6`；绑定底边比主栏低 `20 UI`。v2.8 以活动 `AutoBar.display` 身份为权威，把 provider docking 指向与主栏相对绑定的等效缩放全局代理；自由坐标仅用于 unbind 回退，运行时 token 在 logout 前移除 | `/reload` 后连续两次 apply、开关／点击配置页并移动主栏，确认 `autobar-anchor-basis=provider-dock`、`autobar-provider-dock=bound` 且卷袋不跳；再验证职业槽、当前 13 格、候选 `1／6／7／12`、NATIVE 与非 exact 回退 |
| `AB.CONSUMABLE.GROUP` | `P5 / runtime-asset-v1.5 / bridge-v2.8 / pending-game-validation / 1/5` | 仅精确 `24 Button / 4×6 / 推荐 profile` 显示三组分隔，不创建三段文字；少于 24 个当前主格时隐藏分隔但保留 external drawer；v2.8 只改变整组根停靠，不改变职业层与分组合同 | 实机确认库存变化时外壳动态收缩／扩展、无文字且少于 24 格无错误分隔；验证职业槽、item ID、抽屉与非 exact 原生回退 |
| `AB.TRINKET.DOCK` | `P5 / runtime-asset-v1.5 / bridge-v2.8 / pending-game-validation / 4/5` | [source](../../../assets/source/actionbars/ab-trinket-kit/ActionTrinketKit_Master_v1.png)／[runtime manifest](../../../assets/source/actionbars/ab-trinket-kit/AB-TRINKET-KIT-V1_RuntimeManifest_v1.json)／[work](work/ACTION.BARS.FIELDKIT.V1.md)；TGA `3614d9a8…f455`、像素 `0961d750…aef` 不变；主栏右侧 `8 UI` 强绑定并与消耗品共用低 `20 UI` 的底线，Queue／换装／候选不变；v2.8 不改 TrinketMenu | `/reload` 验证双槽与消耗品同步下移、拖动松手回位、候选向外、横／竖／scale 与 Queue 行为保持；不执行 attempt 5 |
...
| `AB.FOCUS.UNITFRAMES` | `P5 / runtime-v2.6 / pending-game-validation` | runtime-v2.6 只修正姿态绑定；Player／Target `240×60 / 0.8`、TargetTarget `240×60 / 0.68`、系统字体 `OUTLINE / 18 UI`、Aura `23 UI`／每排 `8` 均不变 | `/reload` 验证既有三框、字体、长名字、卷袋净空、TargetTarget、Aura 八枚与 Boss 双排 |
| `AB.FOCUS.CASTBAR` | `P5 / runtime-v2.6 / pending-game-validation`；v1.4／v1.5 `game-geometry-failed` | 玩家／目标／Focus 真实对象；玩家／目标施法仍为 `260×12 / 1.0`，共用 `x=0`，分别落在 `y=316／300`；Focus 仍跟随自身 Frame | 实机验证读数、延迟区、可打断状态、与 Swing 同轴且三排不重叠、Boss Debuff 净空 |
| `AB.FOCUS.SWING` | `P5 / runtime-v2.6 / pending-game-validation`；v1.4／v1.5 `game-geometry-failed` | 主手／副手／ranged 真对象；主手／ranged 仍为 `260×12 / 1.0`、`BOTTOM (0,284)`，副手以 `2 UI` 间距紧贴其下；无维护循环 | 实机验证近战双条、远程复用、攻速变化、同轴与中央视野 |
| `AB.FOCUS.STANCE` | `P5 / runtime-v2.6 / pending-game-validation` | 真实 `bar11.icon_size=25 UI`、local scale `1.0`，三按钮约 `97×33 UI`；fallback `BOTTOM (0,130)`，live `TOP → pfActionBarMain.BOTTOM (0,-12)`。exact v14／v15 与失败 v16 一次升级到 v17，仅改姿态合同；bars／stance／pet／unlock 全边界后置恢复并隐藏独立 mover | 战士 `/reload` 确认按钮状态／点击／快捷键正常且位于主栏下方；切姿态、宠物显隐、unlock 与 `/pfui` apply 后均保持 `12 UI` 净空 |
| `AB.DOITEDPS.TIMELINE` | `P5 / runtime-v2.6 / pending-game-validation`；v1.4／v1.5 `game-geometry-failed` | `318×46 UI` provider 根与 `178×22 UI` 资源排保持 `0.82`，union 仍位于 `TOPLEFT (850,-615)`；v2.6 不改其行为 | 实机验证上下两排与 Player Buff 净空、锁定态、显隐和 fail-open |
...
- AEUI `0.8.25`／Field Kit bridge-v2.7／Combat Focus runtime-v2.5 历史失败轮：新实机截图
...
2. `AB.FIELDKIT.V1` 保持 `runtime-exported / P5`，当前为
   ...
   TGA SHA `3614d9a8…f455`／`c48f6292…320e` 均未改变。bridge-v2.8 延续 popup guard、
   ...
   偏移。v2.8 以 `AutoBar.display` 的表身份解析实际活动配置，把偏移放在与 handle
   等效缩放、相对主栏绑定的全局代理 Frame 上，再让 provider docking 固定写
   `CENTER → proxy.CENTER (0,0)`；因此 profile 缓存切换、`SetupVisual` 与主栏移动都
   不再回到自由坐标。显式 `unbind`／AEUI 关闭时
...
3. Combat Focus 当前共享 AEUI `0.8.26` entrypoint，为
   `focus-layout-contract=2.6`／profile v17。
   `/reload` 会把仍匹配完整旧签名的 v7／v8／v9／v10／v11 游戏坐标 profile、
   仍使用旧全局 unit face／`14 UI` 的 exact v12 profile，以及完整匹配上一版系统
   字体／几何签名的 exact v13 profile 继续一次性迁移；copied v14／v15 与最新实机
   失败的 exact v16 profile 只把姿态保存 fallback 改为 `BOTTOM (0,130)`，并让 live
   `TOP` 绑定到主动作栏 `BOTTOM`、间距 `12 UI`；`bar11.icon_size=25 UI`、scale `1.0`
   保持，Player 等非姿态手调坐标不变。bars rebuild、stance／pet 事件、pet 显隐与
   unlock 关闭均在同一边界恢复该相对锚，且不使用维护循环。显式 apply 仍可覆盖完整 preset。
...
4. Field Kit、Combat Focus 与已确认的四栏组合在 V11 同轮复测：先确认
   ...
   mover，Bar 6 与 TargetTarget 均不跳位；姿态独立 mover 不显示。战士三枚姿态按钮
   清晰放大、位于主栏下方且点击／状态正常；切换姿态、召唤／解散宠物以及反复开关
   unlock 后都保持 `12 UI` 间距。
...
5. `AB.RAIL.V1` 已达到 `game-validated / P6`。长期证据为
   ...
   共享 AEUI `0.8.26` adapter／bootstrap／TOC 哈希，P5→P6 ImageGen `0`。
