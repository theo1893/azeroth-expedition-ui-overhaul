...
- 配置页在单聊天框规则生效时收起右框开关／尺寸；AEUI 书体已挂载时收起
  已被接管的输入宽高、正文描边与背景／边框选项。消息行为和正文大小继续保留，
  原值不删除。仅禁用 Chat 外观不会关闭 pfUI 的独立单聊天框规则；该规则仍生效
  时，右框相关设置继续收起。`/aeui config` 提供现有聊天外观切换入口。
