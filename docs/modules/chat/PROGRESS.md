...
- 暗色候选方向：`CHAT-DARK-SIM-V1 / simulation-confirmed / P2`。用户提出把
  ...
  ImageGen `0/5`，下一门禁为固定执行器 attempt 1 和完整内部审查。
...
| `CHAT.FRAME`／`CHAT.TEXT` 暖黑候选 | `P3` `CHAT-DARK-SIM-V1` confirmed／`CHAT.FRAME.PAPER.V1` prompt-authorized | B 已确认；正式产物只生成单一暖黑旧纸 donor，确定性 mask 保留 V3 母版、Alpha、皮革／黄铜、九宫格、`440 × 320` 布局与独立 Tab／Input atlas；用户已授权唯一固定 Image 1 和最多 5 次实际生成；当前 `0/5` | 提交已授权 work，然后用固定 `imagegen-0-143-0` 执行 attempt 1；每个输出从对象身份开始完整审查 |
