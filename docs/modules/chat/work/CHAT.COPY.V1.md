...
- 项目阶段：`P3`
- 处置：`user-deferred`；用户于 `2026-07-30` 判断该功能对体验提升不明显，
  明确要求暂缓并继续后续功能
...
- 若未来恢复，下一版本必须改变：先在 `22 × 26` 锁定能读出的
  closed／open 独立轮廓，
  增大上层纸叶开合差与层间接触阴影的屏幕像素占比，并避免硬 polygon
  把页边裁成规则矩形；再反推高分辨率 mask。暂缓期间不准备 V1.4，
  未经用户明确恢复与新版本授权不执行。
- 用户结论与日期：`2026-07-30`；用户认为 Chat Copy 的体验收益不明显，
  明确暂缓该功能并转向后续聊天功能
- 下一门禁：无活跃生产门禁；仅在用户明确恢复 Chat Copy 后重新准备版本
