...
远端合并后，本地 Combat Focus 已确认的 Player／Target 几何为 `240×60`，与
当前 V4 固定 `214×42`、禁止纵向拉伸的接受合同不兼容。Player／Target 的
下一门禁因此先是建立并确认兼容合同；不得直接缩放外壳、静默改写 Combat
Focus 几何，或把只支持另一种高度的媒体标为已接入。兼容合同确认后，才从
两张已接受 source 确定性导出 runtime、横向三切片与状态边，完成真实排版、
display region、addon 接入和 fresh-checkout package 校验。本次“确认, 进入
下一阶段”只授权并完成 P4，不应被解释为 P5 或实机验收授权。P5 仍不得复用
V1／V2／V3 失败像素，也不得改变动态内容或交互所有权。
