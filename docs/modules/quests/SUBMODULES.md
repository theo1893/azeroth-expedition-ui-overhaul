...
| `QUEST.LOG.DETAIL.TITLE` | ScrollChild 标题 FontString，需实机确认名 | Theme `1.8`：Noto Serif SC `14px`，无 outline／shadow |
| `QUEST.LOG.DETAIL.DESCRIPTION` | 叙述 FontString 集 | Theme `1.8`：`pfUI.font_default` `12px`，无 outline／shadow |
...
| `QUEST.LOG.ACTION.SEAL_MENU` | planned adapter-owned `QuestLogDetailScrollChild` 子树；未接入 | `QUEST-LOG-SEAL-ACTIONS-SIM-V12` 已于 `2026-08-05` 获拓扑方向确认：非模态即时开合；漆章、动态承载条、七张独立透明纹章与七个独立 Button 随详情内容滚动并由真实 `[366,64,246,324]` viewport 裁切。不创建页外 outset、popup 背板或第二纸面，也不改变正文 `214px`／缩进 `204px`。hidden 无空洞收拢，disabled 留位但不响应；任一代理 parity 缺失时全部旧入口原子 fail-open。V5-A attempt 4 composite 仍作为可恢复 fallback 保留。V6-A..G 授权批次已封闭：实际 ImageGen `28/35`、流程错误 `7`；A／B／C 未获用户接受，D／E／F／G 各自耗尽。用户保留七张独立 source／七个 Button 拓扑，但否决近黑布、双钝缺口和蜡体悬浮方向。用户于 `2026-08-06` 确认 `QUEST-LOG-SEAL-PURITY-RIBBON-SIM-V17 / QS-B1 V7-A`：载体为较窄、略有挺度的烟熏旧骨褐誓约纸／亚麻，QS-A1 漆章在其上后绘并相交 `24px`，尾端约五个不等距尖锐破口；真实排版 `42/42`、展示区 `6/6`、ImageGen `0/0`。当前 `simulation-confirmed / production-prepared / P2`；无新 source/runtime/addon，旧入口继续原子 fail-open；下一门禁是用户独立授权最终 production 正文与五次上限 |
| `QUEST.LOG.ACTION.SEAL_MENU.SUBSTRATE.MAX` | planned adapter-owned 无鼠标 Texture；一张连续 source | V5-A accepted fallback 仍为 [`128×696px` 空白连续暗色旧布](../../../assets/source/quests/qs-b1/QuestLogSealMenuSubstrate_Master_v1.png)，manifest 见 [`QS-B1_SourceManifest_v1.json`](../../../assets/source/quests/qs-b1/QS-B1_SourceManifest_v1.json)；其历史 runtime 合同为 `32×174px`，但用户已取代其近黑布／双钝缺口可见方向，不得直接导出为目标菜单。V17 已确认一张最大 `32×192px` 的较窄挺质旧骨褐誓约载体，按 `visible_count` 取连续前缀并接同一不等距尖锐 tail；不平铺、不重复、不属于任何 action，不含纹章、文字或状态，火漆仍由 QS-A1 独立上层对象持有。V7-A production 只允许 ImageGen 生成连续材料 donor；tracked `128×768` mask 独占轮廓／Alpha，accepted QS-A1 Alpha 只用于确定性接触压暗。当前 `simulation-confirmed / production-prepared / P2 / ImageGen 0/0`，没有 V7 target source、runtime 或 addon 接入；独立生产授权前不得覆盖 fallback |
