...
| `QUEST.LOG.CHROME.SEAL` | `QuestLogFrame.aeuiQuestChromeSeal`，当前是 adapter-owned 无鼠标 `OVERLAY` Texture | QS-A1 V1.r4 美术与四态 atlas 仍 accepted；旧 `[600,-18,28,28]` 顶部悬空锚点只作为待替换 P5 fallback。V1／V2 已否决，V3 又因横向方位误读被否决；当前 `QUEST-LOG-SEAL-ACTIONS-SIM-V4 / simulation-reviewed` 使用从 detail 右页下缘竖直夹入的一整张柔性羊皮纸，根部由真实 QL-A1 shell 下页缘像素覆盖，火漆压在底框外末端，未确认前不得接入 |
...
| `QUEST.LOG.ACTION.SEAL_MENU` | proposed adapter-owned transient Frame；未接入 | `QUEST-LOG-SEAL-ACTIONS-SIM-V4` 沿用分享、详情开合、pfQuest 四个地图操作与放弃，共七项；左键开关，选择／书外点击／Esc 关闭，禁用态镜像 provider；入口由底部竖向书签末端的漆章承担，打开态菜单必须从下方向同一右页内连续展开，不得从侧面弹出或成为断开的卡片 |
