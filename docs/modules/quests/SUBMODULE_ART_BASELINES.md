...
V3 必须把“连续背景”和“功能纹章”分为两种资产所有权：背景只由无鼠标
root、三个不属于任何功能的无缝 body variant 与无鼠标 tail 动态装配；七个
功能各有一张透明纹章 source，并各由一个真实 Button 独立叠放。runtime 可将
七张纹章确定性打包 atlas，但不得合并 UV、Button 或 manifest ID，也不得把
纹章重新烘焙进背景。

背景材料冻结为厚实、柔软、做旧的远征公会誓约亚麻布，以暖赭、烟熏旧棕和
深褐边缘保持卷宗综合色。质感必须由宽而低频的不对称褶皱、少量综合色块、
非周期污渍、手裁边缘偏差和稀疏受力磨损构成；禁止满幅均匀颗粒、程序化微型
织纹、压花壁纸、相同斑点按 `22px` 节距重复、全长平直高光或左右精确镜像。
三个 body variant 的上下明暗和纤维必须连续，拼接后不得看见横缝、七格卡片
或复制粘贴节奏。不规则必须保留克制：仍是一条可弯曲的旧布，而非碎布、旗穗、
破洞网或随机噪声。

七张独立纹章依次为双羽笔／结约、折页、公会罗盘、遮蔽罗盘、清扫地图线、
回环路线结和断裂契约线。使用深乌公会印墨，只有放弃任务使用克制暗酒红；
每枚采用少量粗笔画、局部缺墨、轻微渗化、非恒定线宽和独立 `±1px` 视觉重心
...
的完整 hitbox，完全滚出时命中数必须为零。V12 只是 V3 的几何与层序模拟，
尚待用户确认；确认前不得生图、上传、导出、接入或隐藏旧按钮。V1、V2 和 V10
候选均不得成为 V3 edit input、source、runtime 或确定性美术例外。
