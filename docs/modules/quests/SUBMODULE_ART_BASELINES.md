...
或图标矩阵。V9 于 `2026-08-03` 通过的外侧展开、七个真实 Button、页边遮根、
正文／奖励零占用和克制尺寸仍只作为交互与几何参考。`QS-B1 V1` 的暗胡桃
矩形皮革母版在五次生成耗尽后仍呈现现代按钮式均匀压纹、连续亮边和不合格
比例，用户于 `2026-08-05` 明确判定“不可接受”；V1 没有 source、runtime 或
菜单接入，全部候选不得成为 V2 edit input 或确定性几何例外。V2 的可见物件
身份、材质、轮廓、重复节奏和视觉重量不再由 V1 冻结，必须先以本地确定性
模拟重新确认；但动态标签与七个真实 Button 的功能所有权仍不得烘焙或合并。
