...
追踪圈当前隐藏。当前选中任务使用行内细墨线，保留难度／完成文字墨色，不增加整行底板。accepted 暗酒红织物选择书签 source 仍保留，但 runtime 隐藏；
...
底部放弃、分享、退出、详情开合及四个 pfQuest 地图操作使用嵌入下封皮接缝的
短皮革签：同源深胡桃皮革、粗裁短边、边缘局部磨损与接触暗面，上缘两处粗短、
长短不等的旧赭缝线。中央字区连续、低纹理，禁止完整金框、逐项铆钉、现代圆角
按钮或跨过文字的划痕。左上暖光与书封一致，文字使用暖骨色，放弃仅以文字警示色区别。

source 为 `assets/source/quests/ql-actions/QuestLogActionTabs_Master_v1.png`；
三枚 normal donor 分别对应 `64×20`、`58×20`、`50×20 UI`，独立等比导出及
