...
悬空充当红色图标。V11 的当前用户指向把漆章与菜单共同挂到右页
`QuestLogDetailScrollChild`：scroll `0` 时保持现有 `[576,68,32,32]` 可见位置，
收起态只在蜡体下露出 `6px` 折叠根；展开态向下形成七段相接的
`32×22px`“远征公会授印绶带”，随后与正文一起滚动并由真实 viewport 裁切。
绶带不能形成页外书签、第二纸面、共享 popup、书框或常驻侧栏。

七段分别使用双羽笔／结约、折页、公会罗盘、遮蔽罗盘、清扫地图线、回环
路线结和断裂契约线的整段纹章式墨迹。材料冻结为粗织、做旧的远征公会誓约
亚麻布，不再使用羊皮／布的模糊二选一，
以暖赭、旧棕、深乌墨和克制暗酒红保持卷宗综合色；每段边缘略不规则、明暗
切面宽而低频，不使用现代微型 icon、规则卡片、圆角、完整金框、逐项铆钉、
玻璃、霓虹、科幻金属、骷髅、双头鹰或任何战锤帝国符号。视觉可以借用“蜡封
压住垂直誓约带”的物理逻辑，但必须第一眼仍属于 2004 年前后艾泽拉斯公会
文书，而不是对其他 IP 的图形复制。

展开时绶带只临时覆盖正文最右 `14..24px`，不得改变既有 `214px` 正文宽度、
`204px` 缩进宽度或触发绕排；尾端必须在真实 `108×41px` 奖励槽前至少留
`32px`。七段始终是七个独立 Button／状态皮肤，动态名称、enabled、Tooltip、
原生确认与点击逻辑仍由游戏／provider 持有，不得烘焙或合并。部分裁切段不得
留下不可见的完整 hitbox，完全滚出时命中数必须为零。V1 的现代矩形皮革候选
和 V10 的页外索引签均已被当前方向取代，不得成为 V2 edit input、source、
runtime 或确定性几何例外。V11 的具体可见方向已于 `2026-08-05` 获用户确认；
V2 完整生产正文与五次实际调用边界已于 `2026-08-05` 获独立授权；新
source／atlas／addon 接入仍必须等待具体候选通过内部审查并由用户明确接受。
