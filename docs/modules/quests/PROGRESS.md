...
- `QS-B1 V6-A..G` 已完成授权批次：实际 ImageGen `28/35`、流程错误 `7`；
  ...
  近黑布质感过轻、双钝缺口过于规整。当前已回到生成前模拟
  `QUEST-LOG-SEAL-PURITY-RIBBON-SIM-V17 / QS-B1 V7-A`：漆章后绘并与较窄
  旧骨褐誓约条带相交 `24px`，尾端改为约五个不等距尖锐破口；真实排版
  `42/42 pass`、展示区 `6/6 pass`、ImageGen `0/0`。状态为
  `simulation-reviewed / P2 / awaiting-user-direction-confirmation`；确认前不得
  执行新 production，也不改 source、runtime 或 addon。V5-A accepted source
  仅保留为可恢复 fallback，旧功能按钮继续原子 fail-open。
...
- 当前 `QUEST-LOG-SEAL-ACTIONS-SIM-V12 / QS-B1 V3` 已把资产拆为动态空白
  ...
  为 `QUEST-LOG-SEAL-PURITY-RIBBON-SIM-V17 / QS-B1 V7-A`，等待方向确认。
...
| `QL-C` | 两套 ScrollBar、关闭、Collapse All、操作、辅助按钮与 pfQuest 六控件兼容 | `P5 runtime-integrated`；QS-B1 V7-A `P2 simulation-reviewed` | V6 授权批次已结束：ImageGen `28/35`、流程错误 `7`；A／B／C 未获用户接受，D／E／F／G 各自耗尽。用户保留七项独立代理结构但否决 V5-A 可见承载方向。V17 已预演火漆跨压旧骨褐誓约条带与不等距尖锐破口，自动 `42/42`、展示区 `6/6`；无新 source/runtime/addon | 用户确认或否决 V17 可见方向；确认前不准备授权执行，七项代理 parity 前旧按钮继续 fail-open |
...
| `QS-B1` | Quest Log ScrollChild 内火漆 Button、动态承载条、七张独立纹章与七个代理 Button | V7-A `simulation-reviewed / P2 / ImageGen 0/0`；V6 closed `28/35`；V5-A source retained as fallback | 用户确认结构但否决 V6 可见承载层。V17 以 `24px` 漆章／载体相交、较窄烟熏旧骨褐誓约条带和约五个不等距尖锐破口重开材料方向；真实排版 `42/42`、展示区 `6/6`。无新 source/runtime，菜单未接入 | 用户确认或否决 V17；确认后才冻结 V7-A 最终 production 正文并另行请求授权。旧按钮继续 fail-open |
...
当前唯一 QS-B1 门禁是用户审视
`QUEST-LOG-SEAL-PURITY-RIBBON-SIM-V17 / QS-B1 V7-A`。本地模拟已证明漆章
跨压载体、七项功能独立、动态收拢、滚动裁切、正文零重排和奖励前 `32px`
安全距，自动 `42/42`、展示区 `6/6`；但模拟材质、蜡影、Alpha 和正式纹章
仍非权威。用户确认前不调用 ImageGen；确认后只冻结可见方向、完成正式
production 正文与确定性 mask 复核，再单独请求生图授权。若用户否决，则继续
本地几何模拟。V5-A source 与旧按钮保留 fail-open，不改 addon。
