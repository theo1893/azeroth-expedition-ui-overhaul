...
- 同日后续两张实机图继续暴露三项独立问题：左页本地化
  ...
  `production-active / internal-rejected / retry-prepared / P3 / 2/5`。
  attempt 1 因 non-square、aspect `3.3484` 和现代精密微纹退回；attempt 2
  为 `1254²`，技术 `18/19`、display `5/5`，但 aspect `3.0576`，左图标井占宽
  仍不足且外框／微纹偏工整，因此同样无 Image 3 资格。V2.r3 已用逐像素两区
  比例冻结为固定 Image 1／2 fresh regenerate；尚无合格候选、source 或 runtime。
...
现为 `production-active / internal-rejected / retry-prepared / P3 / 2/5`。
attempt 1／2 分别因 non-square／aspect `3.3484` 与 aspect `3.0576` 退回；
V2.r3 使用固定 Image 1／2 fresh regenerate。自主循环内允许按固定合同调用 ImageGen，
