...
- 同日后续两张实机图继续暴露三项独立问题：左页本地化
  ...
  `production-active / internal-rejected / retry-prepared / P3 / 3/5`。
...
现为 `production-active / internal-rejected / retry-prepared / P3 / 3/5`。
attempt 1／2／3 分别因 non-square／aspect `3.3484`、aspect `3.0576` 与
aspect `2.56085` 退回；V2.r4 使用固定 Image 1／2 fresh regenerate。自主循环内允许按固定合同调用 ImageGen，
