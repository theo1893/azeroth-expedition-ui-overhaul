...
- 同日用户否决 Quest Log 漆章的旧顶部悬空锚点，并要求把底部分享／放弃等
  ...
  V2 预演因伪页唇、二段式硬质按钮轮廓和断开的弹窗语义被用户否决；V3 虽
  修复真实遮挡和单张纸连续性，又把“右页下方”误解成图层下方，仍从右侧
  中段横向伸出，也已被用户否决。现已完成
  `QUEST-LOG-SEAL-ACTIONS-SIM-V4` 本地确定性关闭／展开两态预演：一整张
  柔性羊皮纸书签竖直夹在 detail 右页下缘，根部由已接受 QL-A1 shell 的真实
  下页缘像素覆盖，窄身向下越过底框，QS-A1 火漆完全压在书外纸张末端；菜单
  由同一事务签从下方向右页内连续展开，共七项原 Button 代理。内部初图发现
  page-lip 覆盖第四奖励槽，最终已下移根部并新增全奖励避让门禁。ImageGen `0/0`，
...
- `QL-C runtime`：provider 兼容子合同 `1.7` 已完成第一批书本外控件
  ...
  fail-open fallback；`QUEST-LOG-SEAL-ACTIONS-SIM-V4` 未确认前不隐藏。
...
| `QL-C` | 两套 ScrollBar、关闭、Collapse All、操作、辅助按钮与 pfQuest 六控件兼容 | `P5 runtime-integrated`；seal-menu `P2 simulation-reviewed` | 既有 late-load 兼容和原 Button fallback 保留；V2／V3 均已否决，V4 把单张羊皮纸竖直夹在 detail 右页下缘，七项事务由下向上连续展开，关闭态不遮奖励，尚未接入 | 用户确认 `QUEST-LOG-SEAL-ACTIONS-SIM-V4` 后实现菜单；再做实机点击／禁用／确认框验证 |
...
| 批次 | 范围 | 阶段 | 下一门禁 |
|---|---|---:|---|
| `QS-A1` | 两处共用漆章母版；Quest Log 新承载／事务菜单不改母版 | asset `P5 runtime-exported / 5/5`；Quest Log placement `P2 simulation-reviewed` | 漆章 source／atlas 保持 accepted；旧悬空锚点及 V1／V2／V3 承载均不再作为方向；V4 右页下缘竖向书签与七项向上连续事务签等待确认 | 用户确认或退回 `QUEST-LOG-SEAL-ACTIONS-SIM-V4`；确认前不改 runtime、不隐藏 fallback |
...
当前离线门禁是用户审查 `QUEST-LOG-SEAL-ACTIONS-SIM-V4`。可确认的是：单张
羊皮纸书签是否正确从 detail 右页下缘竖着夹入、真实 shell 下页缘像素压住
根部且不遮四个奖励槽、`32px` 漆章是否直接落在底框外纸张末端、菜单是否由
同一张事务签从下方向右页内连续展开，以及七项功能分组与底部固定按钮移除
后的综合色；模拟不确认最终纸
