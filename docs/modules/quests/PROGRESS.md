...
- `QL-B0 V2`：新的 [左页卷宗目录 work](work/QUEST.LOG.LEFTPAGE.md)
  已于 `2026-07-30` 获用户明确授权，进入 `prompt-authorized / P3`。
  A 段只生成固定 `262 × 340` 列表内框；
  B 段只生成 `224 × 18` 地区条与任务条 base，四态确定性派生。顶部控件、
  ScrollBar 与按钮继续归 QL-C；QL-B1／B2 只在 V2 source 接受后从现有
  accepted source 确定性重导出，QL-B3 继续暂停。A／B 当前各 `0/5`，
  最坏 `10` 次；下一门禁是提交授权正文后执行 A attempt 1。
- `QL-B1 V1` 的固定执行循环在
...
| `QL-B0` | 左页列表几何、内框、地区条与任务条底板 | V1 `P5 fallback`；V2 `P3 prompt-authorized` | V1 的 23 行 `323px` 几何仍可回退；用户已授权 `18 × 18` V2 的 A／B 两段、固定 SHA、repair envelope 与各 `0/5` 预算，见 [work](work/QUEST.LOG.LEFTPAGE.md) | 提交授权正文后执行 A attempt 1；内部通过只到 P3 |
...
`QL-B0-A V2`／`QL-B0-B V2` 两段完整正文、固定 SHA 的 Image 1／2 上传和
每段最多五次／最坏十次预算。下一步先提交授权正文，再执行 A attempt 1；
A 内部通过或预算耗尽后再执行 B。当前批次只覆盖列表内框、地区条与任务条
底板；顶部真实控件和 ScrollBar 继续留给 QL-C 独立拆分。
QL-B2 V1 已在 `5/5` 停止；
