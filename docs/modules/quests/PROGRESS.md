...
- `QL-B0 V2`：新的 [左页卷宗目录 work](work/QUEST.LOG.LEFTPAGE.md)
  已于 `2026-07-30` 获用户明确授权。A attempt 1 已由固定
  `@openai/codex@0.143.0` 执行并完成透明／100% 真实排版审查；它虽满足
  单一正面空框语义，但 raw 为 `1254 × 1254 RGB`、背景不是精确
  `#00FF00`，归一化 bbox／开口与目标严重不符，厚羊皮纸／砂岩框和大面积
  对称黄铜包角也偏离薄目录装订唇边，因此退回。当前为
  `repair-prepared / P3`，A `1/5`，完整 `V2.r1` 正文待提交后执行。
  ...
  仍为 `10` 次；下一门禁是提交 A attempt 1 审查与 `V2.r1` 后执行
  A attempt 2。
- `QL-B1 V1` 的固定执行循环在
...
| `QL-B0` | 左页列表几何、内框、地区条与任务条底板 | V1 `P5 fallback`；V2 `P3 repair-prepared` | V1 的 23 行 `323px` 几何仍可回退；用户已授权 `18 × 18` V2 的 A／B 两段、固定 SHA 与各 `5` 次预算。A attempt 1 因设计网格／bbox／开口／色键和厚框美术退回，A `1/5`、B `0/5`，见 [work](work/QUEST.LOG.LEFTPAGE.md) | 提交 A V2.r1 完整正文后执行 attempt 2；内部通过只到 P3 |
...
五次／最坏十次预算。A attempt 1 已计数并完成全套内审，因设计网格、bbox、
开口、色键与厚框身份退回；下一步提交 `QL-B0-A V2.r1` 完整正文后执行
attempt 2，仍不上传 attempt 1 raw。A 内部通过或预算耗尽后再执行 B。
