...
- 同日后续两张实机图继续暴露三项独立问题：左页本地化
  ...
  `candidate-rejected / P3 / production 1/5 / repairing`。attempt 1 的真实排版
  `5/5 pass`，但技术 `17/19`，并因微型摊书身份、规则缝圈与密集纹理拒绝。
  ...
  当前 `1/5`、流程错误 `1`；`QL-D V3.r1` 固定 Image 1／2 fresh-regenerate
  正文已冻结，下一步执行 attempt 2。
...
| `QL-D` | 奖励槽、分隔与文字安全区 | current geometry／fallback `P6 game-validated`；V3 final art `P3 repairing` | runtime `1.26` 保持真实 Button 的 `108×41px` 双列／`64px` 名称／`8px` 列距／`4px` 行距、setter 锁、无鼠标程序化容器、原生 `NameFrame` 抑制和可见数量兜底。V2 `5/5` 未晋级且被用户否决。QL-D V3 已确认／授权；attempt 1 display `5/5`、技术 `17/19`，因 `2.5714` aspect、微型摊书身份、等距缝圈、密集交叉纹理及绿边拒绝，且无 Image 3 资格。当前 `1/5`、流程错误 `1`；V3.r1 只用固定 Image 1／2 fresh regenerate，奖励只读、无 selected、无 source/runtime/addon 接入 | 固定执行器执行 V3.r1 attempt 2；每次候选完成完整内审，内部通过即停 |
...
`QL-D-SIM-V3` 确认，处于
`candidate-rejected / P3 / production 1/5 / process errors 1 / repairing`。
本地稿保持真实排版和 safe area，只把外观改成粗裁皮革补片、撕边纸签、
不等手扎与错位暗铜痕。完整 production 正文和确定性合同已写入唯一 work；
用户已于 `2026-08-07` 独立授权固定正文与五次修复边界；attempt 1 因错误
比例与物件／纹理问题拒绝，下一步以完整 `QL-D V3.r1` 固定 Image 1／2执行
attempt 2。
