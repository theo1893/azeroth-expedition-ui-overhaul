...
- 子状态：`repair-prepared`。
- 项目阶段：`P3`。
- 授权正文状态：`production`；A attempt 1／2 已完成并退回，下一次必须
  逐字使用本文件 `QL-B0-A V2.r2` 完整正文；B 首次执行仍必须逐字使用
  `QL-B0-B V2` 完整正文。
- 当前实际生图：A `2/5`；B `0/5`。
- 单段预算：最多 `5` 次实际生成／编辑。
- 最坏总预算：`10` 次实际生成／编辑。
...
- 用户授权：`2026-07-30` 明确授权 `QL-B0-A V2` 与 `QL-B0-B V2`，
  ...
  `10` 次。
- 当前门禁：先提交 A attempt 2 审查与 `QL-B0-A V2.r2` 完整正文，再以
  相同 Image 1／2 执行 A attempt 3。A 内部通过或预算耗尽后再执行 B。
...
构图和厚度是本次最高几何门禁。唯一物件以 `(512, 512)` 为中心。它的宽度
必须只占整张画布的 `51.17%`，高度只占 `66.41%`：可见外接盒严格位于
`x=250..774, y=172..852`，恰为 `524 × 680px`；左、右各留
`250px` 纯绿，上、下各留 `172px` 纯绿。盒外不得有任何物件像素、投影或
光晕，外框不得超过画布宽度的 `52%` 或高度的 `67%`。

中央开口必须是从 `x=266..758, y=188..836` 的完整矩形纯绿色开口，恰为
`492 × 648px`，即画布宽度的 `48.05%`、高度的 `63.28%`。外框与开口
之间只允许一层结构：上、下、左、右均为精确 `16px` 薄带。禁止再叠第二圈
内框、暗色衬框、阴影边或凸起边；任何视觉笔触、磨损和接缝都必须被压进这
一层 `16px` 带内。它缩到 `262 × 340 UI px` 后只能形成四边各 `8px`
的目录嵌边，并完整露出 `246 × 324 UI px` 的十八行阅读区。

这个物件是嵌在暖赭纸页接缝处的“薄目录装订唇边”，不是完整书壳、窗框、
门框、肖像框或独立厚面板。主体只用深胡桃木色旧皮与烟黑硬边；氧化黄铜只可
作为两个小而不同的接缝夹片：一个靠近左上角，一个靠近右下角；另外两个角
不放黄铜。每个夹片都必须完全落在 `16 × 16px` 角部薄带内，不能成为包角、
三角大金属片、铆钉阵列或连续金属边。禁止四角相同或对角镜像。禁止把浅色
羊皮纸、砂岩、石板或厚木条当作四边主体。四边可有轻微手工起伏、缺口和
不均匀磨损，但必须始终留在 `16px` 薄带中；装饰密度不能规律重复。

只保留低频、概括的二维手绘明暗，不要连续高光线、深黑内阴影、均匀压花、
高清裂纹或照片级皮革。轮廓要能看出手工切边，但不能弯成波浪或破坏矩形
开口。整体应像 Image 1 书页上的小型目录装订细节，而不是单独展示的豪华
奇幻边框。
...
输出前逐项自检：只有一个单层薄目录内框；外框只占画布
`51.17% × 66.41%`，开口占 `48.05% × 63.28%`；中心、外盒、开口和
四边 `16px` 厚度全部正确；只有左上与右下两个不同的小黄铜接缝，且都在
薄带内；外盒以外与开口以内都是同一个精确 `#00FF00`；没有第二圈内框、
投影、绿色纹理或抗锯齿污染扩散到安全区；内框正面、无透视、无拉伸；在
`262 × 340 UI px` 显示时不会遮挡 `246 × 324 UI px` 的十八行内容；
没有任何被禁止的动态或交互内容。
...
| A | 3/5 | `QL-B0-A V2.r2`／待执行前 commit | generate |  |  |  |  | 待执行 |
...
下一门禁为：提交 attempt 2 审查与 `QL-B0-A V2.r2` 完整正文后执行
A attempt 3；仍只上传固定 SHA 的 Image 1／2，不上传任何前次 raw。
