...
- QS-B1 V5-B 模拟 ImageGen：`0/0`；production `5/5`；流程错误 `7`。E1
  ...
  工具发现错误已复现，按工作流停止重试。用户随后明确要求“重试”；新的
  ...
  `V5-B.r2 / repair-prepared / 2/5`。attempt 3 已把第八空格修到 `0` 可见
  像素并把自动门禁提升到 `10/12`，但两排纵向锚点错误导致七格仍全部被
  固定 cell 裁断，DETAIL 含假文字且 RESET 成为现代撤销箭头；当前进入
...
- 固定 child 为 `@openai/codex@0.143.0`，执行前 commit
  `87903e8ec9accb6cf7594b52659670e6ed477b49`。终端打印的 `user` block 完整包含
  已提交正文 `5450` 字符，正文 UTF-8 SHA-256
  `16357f0e979e79365c725a36ff8b49c7b062a2cd06e77fb8ba8cfdba4904e2d9`；输入仅为
  固定 SHA 的 Image 1／2，没有 Image 3、模拟、失败稿或其他 reference。
- child 从本次 session cache 原样复制 result 到临时 `generated/`；项目 raw
  与 child copy SHA 完全一致。provider 已返回图片，因此本次明确计为 `1/5`。
  PowerShell 旧运行时不支持 `[Convert]::ToHexString`，导致生成前提示词 SHA
  显示为空；它没有改变正文、输入、child exit 或候选，随后用同一 tracked
  正文确定性复算出上述 SHA。该非阻塞记录告警不属于“无候选流程错误”，当前
  流程错误仍为 `0`。
- raw：`1254×1254 RGB`、全不透明 square，SHA
  `c313ca608921189396b9c72c10bbfbe10be8c2c30373a92d1f05016dd966c45f`。
  normalized donor SHA `d6f15e85c9089e65f74d625572382b327b3b3bb2ee5687eb61cecc4d010b99f3`；
  fixed crop SHA `c6044b1cb753d446afb054f8c66e7509b9135b2a538793649436ae049f2b22e5`；
  fixed `4×` mask SHA `8dec7e19e51d80cda8c38c9c665662d988c8e13211c89931deaa17bfd8d704fd`。
- composite：`128×696 RGBA`，visible bbox `[0,0,128,696]`，Alpha `>8` 为单一
  连通对象，全透明 RGB 为零；SHA
  `b64cce638e1f8f7ec86b0bce0baca3006e534b9899989ae282ae9acdf6e5e9b9`。
  runtime review master：`32×174 RGBA`、同样全 bbox／透明 RGB 零，SHA
  `ae82d07e5cbe65dfe2e6533a6f8cf2cfa5dc059a77cba391680a2f514dc03794`。
- reviewer report：
  `generated/quests/QUEST-SEALS/QS-B1-V5-A/attempt-01/review/QS-B1-V5-A.attempt-01.review.json`，
  SHA `da89e4f7e9170095cba735367bf2cdf00f73c0ea15f590502741e2a3b89b1ea4`；
  automated `18/18 pass`、真实六态布局 `29/29 pass`。真实排版 SHA
  `fe8ad4f44837e014fbcb8ca16bbf08848ff3139adeac44ffd04319c3c08876a8`。
  display-region report SHA
  `175da8570d737453648ac145d362b1b1e2b740ff5954a6a6f088f84ef0c2f97b`，
  `6/6 pass`、violations `0`。这些机器结果只证明 crop／mask／Alpha／动态长度／
  命中与展示几何，不能推翻下述美术失败。

#### Attempt 1 完整目视审查

- 范围／物件：通过。raw 是全画幅连续单一暗布面，正交、无 Alpha、无外轮廓、
  书、页、火漆、纹章、按钮、文字、投影或额外对象；ImageGen 与 deterministic
  mask 的所有权分离成立。
- 语义／透视：通过。布面没有条带／旗帜轮廓和悬浮层，综合色为低饱和深胡桃、
  烟熏炭褐与暗暖面；宽幅大明暗、左上暖光和材料重量可保留。
- 第一失败为美术一致性。raw 全幅覆盖连续均匀的细颗粒／毛毡式噪声，局部呈
  旧皮革或石面，而不是 Image 1 锁定的低分辨率粗颗粒二维手绘大面；多个污渍
  接近圆斑。固定 crop 中约 crop-local y=`224` 与 `400`（normalized y≈`388` 与
  `564`）的两道亮折面都从左到右贯穿，违反“短、broken、不得触及两侧”，并
  分别压入对应的安静带。
- 装配／像素：通过。固定 crop、polygon、两处不等宽钝浅缺口、runtime
  `32×174`、prefix＋tail、closed／7／5／3／partial／fully-out、奖励间距和
  display-region 全部成立；runtime 高频残差 `2.051`，但缩小后不显眼不能豁免
  source 本身的均匀微纹与横贯折面。
- 修复决定：允许 Image 3 edit。attempt 1 仍是有效 full-frame 单一暗旧布面，
  总体画笔尺度、光向与综合色正确，失败严格属于授权列出的“微纹过密、折痕
  过强、中央裁片不够安静和圆斑”。attempt 2 固定 Image 1／2，并把本 raw 作为
  唯一 Image 3；保持暗色大面，重绘表面与中央 crop，不改变 canvas、crop、mask、
  runtime 或对象合同。
...
WoW 1.18.1, Interface 11200 quest-log overhaul. This output is NOT a ribbon
sprite and NOT a finished UI object. It is only an edge-to-edge continuous
painted cloth surface. A tracked deterministic crop and alpha mask will later
create the exact narrow substrate silhouette. Do not draw, imply, or reserve
an outer ribbon contour.

REFERENCE AUTHORITY, EDIT SCOPE, AND PRESERVATION
Image 1 is the highest visual authority. Inherit only its circa-2004 vanilla
World of Warcraft low-resolution 2D hand-painted bitmap language, broad
readable value planes, substantial material weight, muted Azeroth expedition
palette, restrained upper-left warm light, sparse wear, and deliberately
...
Image 3 is the immediately preceding donor and is an edit input only. Preserve
its square full-frame single-cloth ownership, flat orthographic view, opaque
edge-to-edge coverage, dark smoke-aged charcoal-brown and walnut temperature,
restrained upper-left light, broad low-frequency value balance, and absence of
UI objects. Repaint its surface where required below. Do not preserve its
uniform fine granular texture, round blotches, or either cross-width bright
fold inside the future central crop. Image 3 is not visual authority and does
not change any fixed contract.
...
Paint heavy, soft, old expedition-guild oath linen in low-saturation smoke-aged
charcoal brown, dark walnut, and deep umber. It is a quiet material accent for
an old quest ledger, not leather, stone, felt, carpet, or a ceremonial banner.
Use exactly one dominant dark middle mass, one deeper shadow mass, and one
restrained dim warm mass. Keep all three broad, irregular, softly hand-painted,
and continuing beyond the future crop. The brightest cloth plane is only
modestly lighter than the base. Never use bright ochre, gold, orange, ivory,
cream, yellow edging, saturated red, a bright rim, or a full-height highlight.

SURFACE REPAINT
Replace the all-over fine granular or fuzzy field in Image 3 with chunky,
low-resolution hand-painted value planes. The cloth may have two or three broad
low-frequency folds across the whole square and three to six large diffuse
irregular noncircular age stains, but their edges must be soft and painted.
Reshape every round dark blotch into a broad asymmetric diffuse stain. Use only
a few sparse coarse fiber suggestions in separated stressed areas. There must
be no individually resolved threads, visible weave, even speckle field,
uniform grain, hair-like noise, photographic nap, leather pores, stone grain,
repeated curls, dense scratches, or procedural microtexture.

FUTURE CENTRAL CROP REPAIR
The normalized central crop x = 448..575 must become calm, matte, heavy, and
readable behind future independent ink motifs. Remove both current horizontal
fold bands that run from one crop edge to the other. Inside that crop, paint
exactly two short, broad, broken dim fold planes at clearly different heights:
one centered near y = 254 and one centered near y = 696. Each is a localized
soft plane less than half the 128-pixel crop width, fades out before both crop
side edges, has an interrupted irregular interior, and is only modestly lighter
or darker than its surroundings. Neither may become a stripe, seam, hem, row,
button plate, stain edge, or cross-width contrast boundary. Do not add any
other localized horizontal fold plane inside the future crop.

FUTURE CROP SAFETY
Within the normalized 1024 x 1024 composition, keep y = 212, 300, 388, 476,
564, 652, 740, and 828, plus at least 8 pixels above and below each row, quiet
and continuous across the central crop x = 448..575. Completely repaint the
current bright bands near normalized y = 388 and y = 564 so both safety bands
are continuous and low contrast. No strong fold boundary, horizontal highlight, stain edge,
crack, seam, hem, row, repeated cell, contrast jump, or action-sized mark may
occur in any safety band. Do not create a 22-pixel runtime rhythm or seven
stacked sections. The crop must read as one continuous material from top to
bottom.

STYLE LOCK
Make an original coarse hand-painted bitmap surface compatible with a 2004-era
vanilla WoW Azeroth expedition ledger. Preserve chunky readable painting,
material weight, restrained wear, broad value masses, and low-resolution
clarity. It must not look photorealistic, scanned, airbrushed, vector-clean,
procedural, mobile-toolbar-like, modern flat UI, Diablo-3-like,
Skyrim-minimalist, Warhammer, gothic sci-fi, ceremonial military regalia, or a
generic modern fantasy ribbon.
...
page, spine, leather, metal, brass, rivet, jewel, skull, eagle, aquila, weapon,
chain, popup, panel, paper shadow, glow, glass, photographic burlap, visible
weave, textile scan, uniform microtexture, fuzzy nap, leather pores, stone
grain, repeated curl, procedural grain, dense scratches, round regular stains,
cross-width band, full-height bright stripe, gold, orange, ivory, saturated
red, or extra object.
...
surface; flat orthographic front view; no object boundary or background; dark
low-saturation smoked-brown palette; exactly three broad value masses; two or
three broad folds across the full surface; three to six diffuse noncircular
stains; exactly two short localized broken dim fold planes inside the future
central crop, near y = 254 and y = 696, with neither touching both crop side
edges; all eight future crop rows quiet; no uniform fine grain, threads, weave,
round blotch, repeating cell, icon, text, wax, book, shadow, bright color,
modern styling, or extra object.
```

### V5-A.r1 修复边界复核与下一调用

- 子状态：`repair-prepared / P3 / 1/5`。attempt 2 操作为 `edit`；固定 Image 1／2
  路径、顺序与 SHA 不变，唯一 Image 3 为 attempt 1 raw SHA
  `c313ca608921189396b9c72c10bbfbe10be8c2c30373a92d1f05016dd966c45f`。
- r1 只强化已授权的微纹密度、圆斑形态、中央 crop 安静度与两段局部折面；对象、
  权威、综合色、canvas、归一化、crop、polygon、Alpha、runtime、prefix＋tail、
  provider／Button 拓扑和展示场景都不变，未新增参考或上传范围。
- 下一次实际调用前必须提交本节与 attempt 1 完整记录；若 attempt 2 仍在同一
  首要失败上退回，下一次必须改变修复策略，不能复用同一正文抽卡。

#### Attempt 2 执行与审查

- 固定 child `@openai/codex@0.143.0`，执行前 commit
  `cee911f7376ea8a90c7c033c244f69bf903dbbd8`；完整 r1 正文 `7307` 字符、UTF-8
  SHA `1787c6abd82ff62a11b5c16c23dcf1c9890c2f3680c53bbb37cc439c9bdef361`。
  child `user` block 完整显示固定 Image 1／2 与唯一 Image 3 attempt 1 raw，输入
  SHA 和职责与冻结记录一致。session
  `019fd25a-b613-76b1-a261-7283d11d7cb0`，result
  `ig_0e43b77fb0420fbe016a734b4bf508819181f5b7d77b66ee07.png`。
- child 在 provider 图片已完成后首次递归枚举 `$CODEX_HOME` 时遇到
  `.sandbox-secrets` 读取拒绝，随后直接从本 session `generated_images` 目录原样
  复制成功并 exit `0`。该落盘恢复发生在生成后，只对应同一 result，不是额外
  调用或“无候选流程错误”；production 现为 `2/5`，流程错误仍为 `0`。
- raw `1254×1254 RGB`、SHA
  `7ca5117006ce6d5712f30deb5d785f25013394bb7add85ad5d9aaf00023cdca9`；
  composite `128×696 RGBA`、SHA
  `00d3fd1f6e44de0b6f93b140a04ed0b69fdf1717ad8ce05befebcb3e2663d736`；
  runtime review master SHA
  `4db79da521447c2df2a4adb1d70748918459eaf82e9553d4838fc880c2d9fa05`。
  reviewer report SHA
  `d976eb07dfbcaa2c9c1287b58c35b4218f776cd1ee05d76fde33295c09bd4a4a`，
  automated `18/18 pass`、六态布局 `29/29 pass`；real-layout SHA
  `e3b8b048037cd69ffe1c22ae29a5c314ca19b629b39f17e128982d6e39ef1bf8`；
  display-region report SHA
  `f8208e0386f803ecb29fde635565aadda03185efabd774e7acdf90f7de59c9b0`，
  `6/6 pass`、violations `0`。
- 范围、square、正交、全不透明单一布面、固定 crop／mask、Alpha、双钝缺口、
  runtime、动态 prefix＋tail、六态命中与奖励距离继续通过；暗烟熏胡桃综合色、
  左上暖光和材料重量也仍可作为文字方向。
- 第一失败仍是 `美术一致性／STYLE LOCK`，且比 attempt 1 更严重：整张 raw
  铺满规则重复的卷曲／编织状微图案，明确呈 procedural textile wallpaper；
  宽亮褶皱数量超过允许的 `2–3`，固定 crop 下半仍存在横贯整宽的亮折面。它没有
  达成短局部折面和八条安静带的综合色要求。
- 相同首要失败连续两稿出现，禁止继续用相同 Image 3 edit 策略。attempt 2 像素
  不保留；attempt 3 只从固定 Image 1／2 regenerate，并把“完全无纹理的中心
  paint-only 区”“两道侧区宽折”“两个坐标化局部 dim plane”写成新的完整 r2，
  不改变任何冻结合同。

### QS-B1 V5-A.r2 完整修复执行正文

```text
Create one new production material-donor bitmap for a Turtle WoW 1.18.1,
Interface 11200 quest-log overhaul. Generate it from the fixed visual
authorities only; do not edit or preserve any previous generated candidate.
This output is NOT a ribbon sprite and NOT a finished UI object. It is only an
edge-to-edge continuous painted cloth surface. A tracked deterministic crop
and alpha mask will later create the exact narrow substrate silhouette. Do not
draw, imply, or reserve an outer ribbon contour.
...
pixels. If the references conflict, Image 1 and the Azeroth quest-ledger
baseline win. Neither reference authorizes copying an object or composition.
There is no Image 3 in this regenerate operation. Previous donors are negative
evidence only and must not be uploaded, copied, or visually preserved.

CANVAS AND OWNERSHIP
Return one square RGB bitmap filled edge to edge by one continuous opaque cloth
surface. There is no background and no separate object: every canvas pixel is
cloth. Keep the view flat, orthographic, front-facing, evenly scaled, and free
of perspective. This is a painted material board, not a hanging drapery scene.
Do not add transparency, green screen, checkerboard, vignette, frame, border,
isolated strip, banner outline, torn silhouette, cast shadow, contact shadow,
floating fibers, or detached fragments. The downstream deterministic compiler
will normalize the whole square on both axes together to 1024 x 1024, then crop
the fixed central rectangle [448,164,576,860], exactly 128 x 696 pixels. Do not
draw a guide, border, highlight, seam, column, or compositional frame around
that rectangle.

PAINT-ONLY MATERIAL LANGUAGE
Paint heavy, soft, old expedition-guild oath linen using only broad matte color
and value shapes. The surface must read as a circa-2004 hand-painted game
bitmap, never as a textile photograph or generated fabric texture. At 100%
source scale there is no small repeated surface information at all: no visible
thread, weave, loop, curl, pore, speckle, stipple, nap, grain, scratch field,
embossed pattern, noise, or fiber carpet. Do not simulate cloth by repeating a
micro-pattern. Cloth identity comes only from weight, two broad folds, softened
painted transitions, and a few large diffuse age stains.

COLOR AND VALUE MASSES
Use low-saturation smoke-aged charcoal brown, dark walnut, and deep umber. Use
exactly three broad irregular value masses over the square: one dominant dark
middle mass, one deeper shadow mass, and one restrained dim warm mass. Each
mass spans hundreds of normalized pixels and blends with coarse painted edges.
The brightest plane is only modestly lighter than the base. Never use bright
ochre, gold, orange, ivory, cream, yellow edging, saturated red, a bright rim,
or a full-height highlight.

LARGE FOLDS AND STAINS
Paint exactly two broad low-frequency folds on the whole 1024 x 1024 normalized
composition. Keep one fold in the left side area x = 48..408 and one fold in
the right side area x = 616..992. Each fold is a single wide soft change over
hundreds of pixels, not a ridge repeated in parallel. Both folds fade out before
x = 432 and x = 592 respectively, so neither crosses or touches the future
central crop. Do not add a third fold, layered drapery arcs, sagging bands,
parallel ridges, or cross-width fold lines. Add exactly four large, diffuse,
asymmetric, noncircular age stains, each low contrast and at least 140 normalized
pixels across. Keep every stain edge soft; no round blotch or action-sized mark.

CENTRAL CROP AS A QUIET PAINTED ZONE
Across normalized x = 432..591 and y = 148..876, make a plain matte painted
zone with no microtexture, weave, speckle, loop, curl, grain, stain edge, broad
fold ridge, or repeating detail. It may contain only the three very broad value
masses plus the two explicitly allowed local dim planes below. This quiet zone
is wider than the actual crop x = 448..575 so the fixed crop receives a safety
margin.

Inside the actual central crop x = 448..575, paint exactly two short, broad,
broken dim local planes. Plane A occupies approximately x = 462..512 and
y = 244..264. Plane B occupies approximately x = 510..558 and y = 686..706.
Each plane is less than half the crop width, has a soft interrupted interior,
fades to the surrounding base before every edge of its approximate box, and is
only slightly lighter or darker than its surroundings. Neither plane is a fold
ridge, stripe, seam, hem, row, stain edge, button plate, or cross-width contrast
boundary. Do not add any other localized horizontal plane inside the crop.

FUTURE CROP SAFETY
Within the normalized 1024 x 1024 composition, keep y = 212, 300, 388, 476,
564, 652, 740, and 828, plus at least 8 pixels above and below each row, quiet
and continuous across x = 448..575. No fold boundary, horizontal highlight,
stain edge, crack, seam, hem, row, repeated cell, contrast jump, or action-sized
mark may occur in any safety band. Do not create a 22-pixel runtime rhythm or
seven stacked sections. The central crop must read as one continuous painted
material from top to bottom.

STYLE LOCK
Make an original chunky hand-painted bitmap surface compatible with a 2004-era
vanilla WoW Azeroth expedition ledger. Preserve broad readable painting,
material weight, restrained wear, softened coarse brush transitions, and
low-resolution clarity. It must not look photorealistic, scanned, airbrushed,
vector-clean, procedural, mobile-toolbar-like, modern flat UI, Diablo-3-like,
Skyrim-minimalist, Warhammer, gothic sci-fi, ceremonial military regalia, or a
generic modern fantasy ribbon.

STRICT EXCLUSIONS
No previous donor, outer silhouette, narrow ribbon object, banner, pennant,
fishtail, notch, fringe, tassel, hem, green background, alpha, wax, seal, stamp,
icon, motif, rune, glyph, text, letter, number, button, state, card, tile,
divider, book, page, spine, leather, metal, brass, rivet, jewel, skull, eagle,
aquila, weapon, chain, popup, panel, paper shadow, glow, glass, photographic
burlap, visible weave, textile scan, uniform microtexture, loop pattern, curl
pattern, fuzzy nap, leather pores, stone grain, repeated grain, dense scratches,
round stains, layered drapery arcs, cross-width band, full-height bright stripe,
gold, orange, ivory, saturated red, or extra object.

BEFORE RETURNING, VERIFY IN ORDER
One square RGB image; every pixel belongs to one continuous opaque cloth
surface; flat orthographic painted material board; no object boundary or
background; dark low-saturation smoked-brown palette; exactly three broad value
masses; exactly two broad side folds that never enter x = 432..591; exactly four
large diffuse noncircular stains; central x = 432..591 contains no microtexture
or broad fold ridge; exactly two short broken dim local planes at the declared
central coordinates; all eight future crop rows quiet; no visible or implied
repeating surface pattern, threads, weave, loops, curls, grain, round blotch,
icon, text, wax, book, shadow, bright color, modern styling, or extra object.
```

### V5-A.r2 修复边界复核与下一调用

- 子状态：`repair-prepared / P3 / 2/5`。attempt 3 改为 `generate`，只上传固定
  Image 1／2；attempt 1／2 raw、composite、review、runtime 与模拟均不得上传。
- r2 把同一授权内的美术禁项转为“paint-only、无任何小尺度重复信息、两道侧区
  宽折、中心坐标化局部 dim plane”；不改变视觉综合色、对象身份、参考职责、
  canvas、crop、mask、runtime、prefix＋tail、provider／Button 或展示合同。
- 本记录和完整 r2 必须提交后才能调用 attempt 3。若 regenerate 仍失败，必须按
  新稿重新完整审查；剩余预算不能改变冻结边界。
...
- 当前最高状态：`repair-prepared / P3 / 1/5`。用户明确要求重试后，固定
  `@openai/codex@0.143.0` child 已恢复并产生 attempt 1；该候选未通过
  safe-box、空格／绿残留与美术材料门禁。下方完整 `V5-B.r1` 正文已在同一
  冻结修复边界内准备，attempt 2 必须 fresh regenerate 且只上传固定
  Image 1／2。内部通过最高只到 `candidate-reviewed / P3`，不得自动写
  source/runtime/addon。
...
- 用户于 `2026-08-06` 明确要求“重试”；该指令只恢复既有固定执行路径，不
  改变 V5-B 正文、输入、修复边界或五次实际生图预算。
- 执行正文／前置 commit：`QS-B1 V5-B / 75877c9`；正文仍为上方 SHA
  `762930e574b4c3fdf0ec6898946e51742d61ccfd9ae09913e018e95035a1dc54`。
- fixed child session：`019fd504-7d9a-7fc1-8c14-9c35edc605ba`；事件流包含
  图片生成声明、cache 图片、项目临时目录复制与 `turn.completed`，故这次
  必须计为实际 ImageGen `1/5`。启动时另有
  `failed to load models cache: missing field supports_reasoning_summaries`，但
  它未阻止本次 provider 图片生成，不能记作非计数流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V5-B/attempt-01/quest_admin_emblems_worksheet_raw.png`；
  `1254×1254 RGB`；SHA
  `08b150a62804c93f247e09041a7dfa4861515854c94854472fa58cf3a7f0a95e`。
- 专用确定性 reviewer：
  `tools/review_quest_seal_menu_motifs_candidate_v1.py`。它只在 ignored
  `generated/` 中执行同轴 `1024²` 归一化、边缘连通色键、透明 RGB 清零、
  固定七格 review crop、等比 runtime bbox-fit、四态预演与真实排版；不写
  source、runtime 或 addon。
- 机器审查：`9/12 pass`；review JSON SHA
  `3bee457931997bff0039e5aaeb9b5425eb7a9ff7f0ce90fcd538eb5b6be84006`。
  第一失败门禁为 `all_visible_pixels_stay_inside_safe_boxes`：七格全部越界，
  第一排尤其向下触碰 cell 底边；第四色键空格仍有 `270` 个可见残留像素；
  SHARE／DETAIL／CLEAN／RESET 分别仍有 `45／48／29／8` 个可见绿色污染像素。
- 真实排版：
  `attempt-01.real-layout.png`，SHA
  `b6f8352ce662ddbb9fc1fe4b1a0a42a4a76d89134a2dd39d96811185dab6a890`；
  使用 accepted V5-A 布底、真实 `676×464` Quest Log、18 行、四奖励和六种
  closed／7／5／3-disabled／partial-scroll／fully-out 状态。display-region
  报告 SHA
  `375f605dcce964d86b871963fbe7b331f5d0971f75b3d22083640c9134b494d2`，
  `6/6 pass`、violations `0`；该通过只证明既有运行时几何，不挽救 source
  safe box 或美术失败。
- 语义／美术目视：七个功能大体可辨，但候选把 SHARE 画成完整立体羽笔、
  DETAIL 画成有皮革般厚边和纸纹的实体页、SHOW／HIDE 画成独立金属罗盘刃、
  CLEAN 画成带高光的实体条带、RESET 画成带圆形路点的现代地图路线、
  ABANDON 画成有编织体积和高光的圆柱绳。普通六项还有深外描、明亮高光、
  多明暗斜面和强综合色差，读取为金属／皮革物件，不是无底材、哑光、低饱和
  的旧赭金矿物颜料授印。这是美术材料身份失败，不能靠色键或缩小修复。
- 决策：`internal-rejected / repair-prepared / P3 / 1/5`。对象数与语义方向
  继续冻结，但 attempt 1 像素不保留。因为 cell／safe box、时代风格和材料
  身份均不正确，不满足 Image 3 使用前提；attempt 2 必须只上传固定
  Image 1／2 fresh regenerate。

### 完整 V5-B.r1 production Prompt — attempt 2

Use Image 1 and Image 2 only as fixed visual references. Create one production
worksheet containing exactly seven independent, small, flat hand-painted quest
...
complete interface, or presentation mockup.

NONNEGOTIABLE RENDERING MODE

Every emblem is a direct, flat, imperfect transfer of old guild pigment onto no
visible backing at all. Paint a two-dimensional pigment mark that merely depicts
the requested symbol. Do not render the depicted quills, ledger leaf, compass,
trails, route, or cord as physical three-dimensional objects. There is no object
thickness, bevel, rim, cast shadow, contact shadow, metallic face, paper surface,
leather edge, cylindrical rope volume, polished highlight, or separate dark
outline. Form comes from a compact flat silhouette, non-constant brush width,
slight pigment-density variation, small missing-pigment breaks, and restrained
soft bleed only. If any emblem looks like a collectible item, miniature prop,
metal badge, parchment object, or modern rendered icon, it is wrong.

REFERENCE AUTHORITY

Image 1 is the highest visual authority. Inherit its old Azeroth
expedition-ledger language: substantial but slightly irregular hand-painted
contours, broad value grouping, muted warm color, sparse believable wear,
upper-left warm-light logic in the surrounding UI, and the visual weight of a
formal adventurers' guild record from vanilla World of Warcraft. Translate that
weight into flat stamped pigment; do not copy its complete book, pages, leather
frame, brass, compass ornaments, wax seal, ribbons, text, reward slots, buttons,
or composition.

Image 2 is a secondary micro-scale reference only. Inherit only its economical
stroke count, softened hand-painted edges, and legibility when reduced to a tiny
game UI mark. Do not copy, trace, rotate, recolor, or rearrange its triangles,
circles, check mark, 2-by-2 layout, positions, transparency, or pixels.

CANVAS, CELLS, AND HARD SAFE BOXES
...
shadow, contamination, or color variation. Do not add grid lines, labels,
numbers, text, captions, frames, checkerboard, or a transparency preview.

Use the following invisible 4-by-2 layout. The safe boxes are hard clipping
boundaries, not approximate suggestions. Every visible pigment pixel,
antialiasing pixel, soft bleed, worn fragment, and dense pigment pool must remain
strictly inside its assigned safe box with green clearance to all four safe-box
edges. No mark may touch a cell boundary. Keep the eighth cell entirely empty
exact #00FF00.

1 SHARE: cell [32,160,256,384], hard safe box [64,208,224,336], optical center
near [144,264], maximum visual span about 144 by 96.
2 DETAIL: cell [272,160,496,384], hard safe box [304,208,464,336], optical center
near [392,272], maximum visual span about 112 by 112.
3 SHOW: cell [512,160,736,384], hard safe box [544,208,704,336], optical center
near [616,272], maximum visual span about 112 by 112.
4 HIDE: cell [752,160,976,384], hard safe box [784,208,944,336], optical center
near [864,280], maximum visual span about 128 by 112.
5 CLEAN: cell [32,608,256,832], hard safe box [64,656,224,784], optical center
near [152,712], maximum visual span about 144 by 80.
6 RESET: cell [272,608,496,832], hard safe box [304,656,464,784], optical center
near [376,728], maximum visual span about 136 by 96.
7 ABANDON: cell [512,608,736,832], hard safe box [544,656,704,784], optical
center near [624,720], maximum visual span about 128 by 72.
8 EMPTY: cell [752,608,976,832], completely uniform exact #00FF00.

SEVEN FLAT PIGMENT SYMBOLS

SHARE is a compact flat pigment silhouette of two short broad old quills
crossing at unequal angles and bound by one compact covenant knot. Suggest each
feather with one broad mass and only two or three coarse edge notches. Do not
render individual feather barbs, shafts with polished highlights, long needle
tips, a scroll, handshake, share arrow, or two physical feather objects.

DETAIL is one simplified flat guild-ledger-leaf mark with one short folded
corner. Use a broad economical filled or broken-brush page silhouette. Do not
render parchment grain, a thick page edge, curled leather-like corners, a
bordered card, a modern file, envelope, application window, menu, or writing.

SHOW is one unified flat four-direction expedition-guild compass mark. Four
broad, slightly unequal blunt points surround one very small quiet center and
read as one stamped symbol, not four separate metal arrowheads. No outer ring,
letters, cartographic ticks, central gem, bevel, or glow.

HIDE is an independently painted compass from the same flat pigment family,
crossed by one short quiet partially worn diagonal veil stroke. The veil is a
simple brush interruption, not a physical quill, plank, ribbon, or weapon. It
must not resemble an eye slash, prohibition circle, close X, or modern
visibility icon.

CLEAN is three broad flat cartographic trail strokes fanning from one motion.
Their lengths, curves, spacing, pigment density, and endpoints are unequal. They
are brush trails, not raised ribbons or blades. No broom, speed lines, water
waves, list, repeated chevrons, or mechanical parallel bars.

RESET is one continuous winding flat expedition-route brushstroke returning to
one compact irregular route knot. Keep the route visibly open rather than a
perfect circle. Do not add waypoint discs, detached stones, arrowheads, gear
teeth, a location pin, or a modern refresh symbol.

ABANDON is one short substantial flat old-wine pigment silhouette of a contract
cord snapped at the center, with a clear quiet gap and only a few coarse fibers
at the broken ends. Suggest cord identity with two or three broad twists at
most; do not render cylindrical braided rope, specular thread, depth shading,
an X, trash can, skull, blood, weapon, or warning triangle.

PIGMENT, COLOR, AND MICROSTRUCTURE

The first six emblems use matte low-saturation aged ochre mineral guild pigment.
Their dominant midtone should stay close to restrained earthy old ochre,
approximately RGB 174,138,78 in role, with only small local density changes.
They must remain visibly lighter than the smoked dark-brown cloth that will sit
behind them in game, but they are not metallic gold, polished brass, bright
yellow, orange, ivory, white, emissive, glowing, or celebratory. Do not create a
bright top edge and dark bottom edge; do not model directional object lighting.

ABANDON alone uses muted greyed dark old-wine pigment, approximately RGB
156,72,61 in role. It is distinct from the ochre family but never bright red,
blood-red, glossy, neon, or glowing. Do not tint the whole worksheet or any
other emblem wine red.

Use only a few broad hand-painted strokes per emblem. Allow non-constant line
width, one to three controlled missing-pigment interruptions, one or two small
denser pools at turns, and a restrained two-to-four source-pixel soft bleed.
Keep density variation shallow enough that every mark still reads as one flat
pigment transfer. No deep dark contour, inner rim, hard white highlight,
material texture, engraved detail, feather micro-barbs, parchment grain,
metal facets, woven rope microtexture, uniform noise, or repeated speckles.

The seven emblems need small independent differences in size, optical center,
angle, wear, and pressure while remaining inside their exact safe boxes. Do not
regularize them into seven same-size, same-center, same-weight vector icons, but
also do not enlarge any mark merely to fill its cell.

ERA AND STYLE

The result must feel native to a 2004-era Azeroth guild ledger: hand-painted 2D
game UI sprite art, broad readable shapes, slightly irregular but substantial
forms, low saturation, and quiet material weight. It must not resemble a modern
flat HUD, mobile icon set, web toolbar, Diablo III ornament, Skyrim minimalist
menu, Warhammer purity-seal iconography, photorealistic scan, procedural
texture, contemporary high-resolution concept-art presentation, or a sheet of
rendered inventory items.

STRICT EXCLUSIONS

No cloth strip, ribbon, parchment tag, page material, leather plate, wax, button
face, border, card, frame, backing tile, circular medallion, brass mount, rivet,
engraving plate, embossed badge, drop shadow, contact shadow, dark outline,
bevel, inner glow, outer glow, specular highlight, glass, neon, chrome, 3D
render, photographic fiber, uniform weave, repeated noise, perfect symmetry,
mechanical line weight, or text. Do not join any two emblems with pigment,
decoration, shadow, or a shared object. Do not paint green inside an emblem or
tint pigment edges green.

FINAL SELF-CHECK BEFORE OUTPUT

Confirm there are exactly seven and only seven visible flat pigment emblems.
Confirm each cell contains its requested semantic mark and the eighth cell is
fully empty. Confirm every visible, antialiased, bled, and worn pigment pixel is
strictly inside the stated hard safe box with green clearance. Confirm the first
six read as matte muted aged-ochre pigment and only ABANDON is muted greyed dark
wine. Confirm none reads as a physical feather, parchment item, metal compass,
raised ribbon, waypoint route, cylindrical rope, badge, or rendered inventory
object. Confirm there is no backing, text, copied Image-2 silhouette, modern icon
shorthand, or another IP symbol. Confirm every remaining canvas pixel is one
uniform exact #00FF00.
...
- 本正文仍只包含七枚原功能纹章、第八空格、原固定 cell／safe box、六赭金一
  ...
  外部输入。
- 修复只强化已授权的 safe-box 外溢、绿残留、笔画、颜料密度、对比和材料
  身份门禁；不改变 V15 可见方向、七张独立 source／七 Button 所有权、画布、
  runtime 几何或四态派生合同。
- attempt 2 操作为 fresh regenerate；Image 1／2 SHA 与职责保持不变，明确不
...
### QS-B1 V5-B attempt 3 执行与内部审查

- 执行正文／前置 commit：`QS-B1 V5-B.r2 / db7bf39`；实际传输正文
  `11498 bytes`，SHA
  `ade1424474350bb3cc5d9849018575409c8c4fcf942581970b16f348e7b4f0b6`。
- fixed child session：`019fd518-7330-7032-b2b4-7bef3def2c64`；provider 图片、
  原样复制与 `turn.completed` 完整，累计实际 ImageGen `3/5`。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V5-B/attempt-03/quest_admin_pigment_emblems_worksheet_raw.png`；
  `1254×1254 RGB`；SHA
  `987c5b4ddc8615262aad8cbf5c19d589fc70cac93756779eea1d2783d948e35a`。
- 机器审查：`10/12 pass`；review JSON SHA
  `4bd75f674104742e66f948a7994b4bbad3fc707e28e4f490b06d1a6df6a240a5`。
  第八空格已为 `0` 可见像素；但七项仍全部
  `inside_safe_box=false`，SHARE／SHOW／HIDE／CLEAN／ABANDON 各有
  `10／117／1／15／17` 个 visible green spill。
- 位置诊断：provider 把上排视觉中心放在画布约 `35..36%` 高度，固定目标为
  `26.5%`；把下排放在约 `64..66%`，固定目标为 `70.5%`。上排全部向下越过
  cell 底边，下排 CLEAN／RESET 向上越过 cell 顶边；review crop 因而截断
  SHARE 羽根、SHOW／HIDE 南向点和下排上缘。外列同时向中间收拢，第一列约
  `18%` 而非 `14%`，第四色键约 `80%` 而非 `84.4%`。
- 色彩诊断：普通六项中位约 `RGB 130..140 / 90..98 / 50..57`，已不再亮金，
  但比 V15 冻结的旧赭棕稍暗；ABANDON `148/71/64` 接近目标。
- 语义／美术：平面颜料身份继续成立，CLEAN 与 ABANDON 方向可保留；DETAIL
  仍有多条假文字且轮廓像现代文件；SHOW 中央封闭绿洞且近似罗盘武器；HIDE
  斜笔过长；RESET 直接成为现代逆时针撤销箭头，违反无箭头合同。SHARE 结
  形成封闭绿洞。
- 真实排版：`attempt-03.real-layout.png`，SHA
  `6807e9373b8b83f5ae341405a10d4bd560239e324b0157a90f01b37a6e64752f`；
  展示区报告 SHA
  `95e0538185b567f370f1b964afcc7fe4921ef8867a25f4d8db9e2109a22da223`，
  `6/6 pass`。运行时图标仍因 source crop 截断，不可晋级。
- 决策：`internal-rejected / repair-prepared / P3 / 3/5`。cell 仍错误，不
  满足 Image 3 前提；attempt 4 继续只上传固定 Image 1／2 fresh regenerate。
...
administration pigment emblems for a circa-2004 vanilla World of Warcraft UI
overhaul. This is a bitmap sprite-source worksheet, not a scene, screenshot,
complete interface, inventory sheet, presentation mockup, or modern icon set.

ABSOLUTE PRIORITY: PERCENTAGE ANCHORS AND SMALL BOUNDING BOXES

The exact row and column anchors below override any aesthetically balanced icon
sheet convention. Do not use default rows near one-third and two-thirds of the
canvas. The first row must be centered at exactly 26.5 percent of total canvas
height. The second row must be centered at exactly 70.5 percent of total canvas
height. This intentionally leaves a very large empty green gap between rows.

Column centers are not the common evenly inset 20/40/60/80 percent arrangement.
Use exactly 14.0, 37.5, 61.0, and 84.4 percent of total canvas width for the four
top-row centers. Use exactly 14.0, 37.5, and 61.0 percent for the three bottom
marks. Keep the bottom-right quadrant entirely empty.

Each emblem must be conspicuously small: normally only 6 to 8 percent of total
canvas width and 4 to 6 percent of total canvas height. No emblem may exceed 9
percent of canvas width or 7 percent of canvas height. Large unused exact-green
margins around every mark are mandatory. If necessary, make the artwork smaller
and simpler; never move a row toward the center or enlarge a mark to balance the
composition.

NONNEGOTIABLE FLAT-PIGMENT MODE

Every emblem is a direct two-dimensional transfer of old guild pigment onto no
visible backing. Depict each symbol only as a compact matte paint silhouette.
There is no object thickness, bevel, rim, cast or contact shadow, metallic face,
paper surface, leather edge, cylindrical rope volume, polished highlight,
separate dark outline, or directional object lighting. Form comes only from a
few broad strokes, non-constant brush width, restrained same-pigment density
changes, and a quiet dry-brush edge.

Never create an enclosed green hole or green cutout anywhere inside an emblem.
All knots, compass centers, cord bodies, and closed-looking intersections must
be filled with pigment. Wear must be a darker or lighter shade of the same
pigment, never exposed green. Do not leave green-tinted antialiasing inside or
along a visible edge. If a mark reads as a physical prop, badge, parchment item,
metal weapon, or rendered toolbar icon, it is wrong.
...
formal adventurers' guild record from vanilla World of Warcraft. Translate that
weight into flat stamped pigment only. Do not copy its complete book, pages,
leather, brass, compass ornaments, wax, ribbons, text, reward slots, buttons,
lighting scene, or composition.

Image 2 is a secondary micro-scale reference only. Inherit its economical
stroke count, softened hand-painted edges, and legibility at tiny in-game size.
Do not copy or rearrange its triangles, circles, check mark, 2-by-2 layout,
positions, transparency, or pixels.

CANVAS, CELLS, AND HARD SAFE BOXES

Render an exact square 1024 by 1024 RGB worksheet. Fill every pixel outside the
seven pigment emblems with one perfectly uniform exact chroma-key green
#00FF00. The field has no gradient, vignette, noise, texture, shadow, color
variation, grid, label, number, text, frame, checkerboard, or transparency
preview.

The following invisible safe boxes are hard clipping boundaries. Every visible
pigment, antialiasing, dry-brush edge, dense pool, and worn pixel must remain
strictly inside its assigned box, with at least 16 pixels of pure green to all
four safe-box edges. Never touch a cell boundary.

1 SHARE: cell [32,160,256,384], safe box [64,208,224,336], center [143,271],
target bbox about 78 by 62, absolute maximum 92 by 70.
2 DETAIL: cell [272,160,496,384], safe box [304,208,464,336], center [384,271],
target bbox about 62 by 68, absolute maximum 76 by 76.
3 SHOW: cell [512,160,736,384], safe box [544,208,704,336], center [625,271],
target bbox about 70 by 68, absolute maximum 82 by 76.
4 HIDE: cell [752,160,976,384], safe box [784,208,944,336], center [864,271],
target bbox about 76 by 68, absolute maximum 88 by 76.
5 CLEAN: cell [32,608,256,832], safe box [64,656,224,784], center [143,722],
target bbox about 82 by 42, absolute maximum 94 by 52.
6 RESET: cell [272,608,496,832], safe box [304,656,464,784], center [384,722],
target bbox about 76 by 50, absolute maximum 88 by 60.
7 ABANDON: cell [512,608,736,832], safe box [544,656,704,784], center [625,722],
target bbox for both ends together about 82 by 36, absolute maximum 94 by 46.
8 EMPTY: cell [752,608,976,832], completely uniform exact #00FF00.

SEVEN SPECIFIC PIGMENT SYMBOLS

SHARE is one compact flat pigment mark of two short broad old quills crossing
at unequal angles and held by one solid irregular covenant dab. The dab is not
a loop and contains no hole. Each feather is one broad mass with at most two
coarse edge notches and a short blunt shaft. No individual barbs, long tips,
polished shafts, scroll, handshake, modern share arrow, or rendered feather
props.

DETAIL is one slightly skewed old guild-ledger-leaf pigment silhouette with one
short uneven folded corner. Its face is blank: absolutely no horizontal lines,
writing, glyphs, rows, fake text, menu marks, or inner list. It must not be a
perfect modern document rectangle. No application icon, envelope, bordered
card, parchment texture, thick page edge, or curled leather corner.

SHOW is one old cartographer's four-direction expedition compass-rose pigment
mark, not a weapon. It has one longest blunt north point, a shorter south point,
and two broader shorter unequal side points around one solid pigment center.
There is no center hole. Use only those four main directions; no extra diagonal
blades. No ring, letters, ticks, gem, bevel, throwing-star symmetry, glow, or
detached arrowheads.

HIDE is a separately painted four-direction compass from the same family with
one short narrow partially worn diagonal pigment stroke over only its center.
The compass remains dominant. The stroke is at most half the compass width and
is not a long quill, pair of bands, plank, ribbon, weapon, eye slash,
prohibition circle, X, or modern visibility icon. Fill all intersections; no
green center hole.

CLEAN is exactly three broad compact cartographic trail strokes fanning from
one gesture. Their lengths, curves, spacing, density, and endpoints are unequal.
They are flat paint trails, not ribbons, blades, a broom, speed lines, waves,
list, repeated chevrons, or mechanical bars. No green distress holes.

RESET is one arrowless open returning expedition route. Paint a low, imperfect
open teardrop-shaped loop whose two blunt ends nearly meet near one compact
irregular route-knot dab. One side doubles back slightly so the route reads as a
journey returning to its record. Both ends are rounded and blunt. There is
absolutely no arrowhead, triangular end, pointed hook, clockwise/anticlockwise
arrow, undo icon, refresh icon, perfect circular ring, letter C, letter S,
waypoint disc, gear, or location pin.

ABANDON is one compact flat old-wine pigment symbol of a contract cord snapped
at the center. Both short blunt fragments stay together inside cell 7 with one
quiet central gap. Each is a broad simple silhouette with at most two shallow
diagonal cord hints and a few coarse broken fibers. No enclosed green holes,
cylindrical shading, braid volume, specular thread, far-separated end, bow-tie,
X, trash can, skull, blood, weapon, or warning triangle.

PIGMENT COLOR AND MICROSTRUCTURE

The first six emblems use matte low-saturation aged ochre-brown mineral guild
pigment, centered in role near RGB 158,124,76. It is dusty warm brown-ochre,
lighter than attempt-dark umber but far duller than yellow-gold. It must remain
readable on smoked dark-brown cloth without becoming metallic gold, polished
brass, bright yellow, orange, ivory, white, emissive, glowing, or celebratory.
No bright top edge, modeled dark bottom, or deep contour.

ABANDON alone uses muted greyed dark old-wine pigment near RGB 148,76,68. It is
dusty and brown-grey, never bright scarlet, blood-red, glossy, neon, or glowing.

Use only a few broad strokes, shallow same-pigment density variation,
non-constant width, one or two denser pools, and restrained dry-brush edges.
Every worn area remains pigment-colored. No green holes, deep outline, white
highlight, engraved detail, feather micro-barbs, parchment grain, metal facets,
woven-rope texture, uniform noise, repeated speckles, or high-frequency wear.

ERA AND STYLE

The result must feel native to a 2004-era Azeroth guild ledger: hand-painted 2D
game UI sprite art, tiny broad readable shapes, slightly irregular substantial
forms, low saturation, and quiet weight. It must not resemble a modern flat
HUD, mobile icon set, web toolbar, Diablo III ornament, Skyrim minimalist menu,
Warhammer purity-seal language, photorealistic scan, procedural texture,
contemporary concept-art sheet, or rendered inventory objects.

STRICT EXCLUSIONS

No cloth, ribbon, parchment tag, page material, leather plate, wax, button face,
border, card, frame, backing tile, medallion, brass mount, rivet, engraving
plate, badge, drop shadow, contact shadow, dark outline, bevel, glow, specular
highlight, glass, neon, chrome, 3D render, photographic fiber, perfect symmetry,
mechanical line weight, or text. Do not join emblems, place pigment outside a
safe box, put anything in cell 8, or paint green inside a mark.

FINAL SELF-CHECK BEFORE OUTPUT

Confirm exactly seven tiny emblems exist and cell 8 is fully empty. Confirm row
centers are 26.5 and 70.5 percent of canvas height, not one-third/two-thirds.
Confirm column centers are 14.0, 37.5, 61.0, and 84.4 percent. Confirm every bbox
is below its absolute maximum and surrounded by at least 16 pixels of pure
green inside its safe box. Confirm there are no enclosed green holes. Confirm
DETAIL is blank, SHOW has four directions and a filled center, HIDE uses one
short stroke, RESET has no arrowhead and is not an undo/refresh icon, and both
ABANDON fragments remain compact in cell 7. Confirm the first six are muted
ochre-brown and only ABANDON is old wine. Confirm every other canvas pixel is
one uniform exact #00FF00.

### V5-B.r3 完整性与修复边界复核

- 以上正文按两个三级标题边界提取、去除标题并按既有 shell 位置参数传输语义
  去除尾随换行后为 `10618 bytes`；传入参数正文 SHA-256
  `f70a3ddc4e19423a3b48b71f80092ae03a2748d917cee6932408194fa068c19b`；
  首句已核对为 `Use Image 1 and Image 2 only as fixed visual references.`。
- 本正文只修复已冻结七项中的行列锚点、bbox、绿洞、综合色与已量化语义；
  没有新增对象、功能、状态、参考或载体。
- 继续冻结七张独立 source、四态派生、accepted V5-A 布底、runtime 几何和
  香草魔兽旧公会账簿语言。百分比只是同一 fixed cell／safe-box 的补充表达，
  不改变布局合同。
- attempt 4 必须 fresh regenerate；只上传固定 Image 1／2，不上传 attempt
  1／2／3、V15、V5-A 布底或历史候选。
- 提交前须按标题边界提取并记录传输正文 bytes／SHA／首句；返回图片才把
  production 从 `3/5` 增加到 `4/5`。

### QS-B1 V5-B attempt 4 执行与内部审查

- 执行正文／前置 commit：`QS-B1 V5-B.r3 / 86b4164`；正文
  `10618 bytes`，SHA
  `f70a3ddc4e19423a3b48b71f80092ae03a2748d917cee6932408194fa068c19b`。
- fixed child session：`019fd51f-a58a-7d52-85ed-ba4ebf3b1e1e`；provider 图片、
  原样复制与 `turn.completed` 完整，累计实际 ImageGen `4/5`。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V5-B/attempt-04/quest_admin_pigment_emblems_worksheet_raw.png`；
  `1254×1254 RGB`；SHA
  `2c40de66474f3abe4c23f955aa337dd95e69465f5d8056c3a784dcb5e697c601`。
- 机器审查：`10/12 pass`；review JSON SHA
  `0a21ab0f7f9398e9e1d58c52d8ea8222edb45eb9add0c270c0819b02863eace4`。
  七项 green spill 均为 `0`，但七格仍全部 `inside_safe_box=false`，第八空格
  有 `5744` 个可见像素。
- 位置诊断：上排列位置大体接近四槽，但对象仍约为目标两倍；底排忽略“前
  三个四槽”，自动按三个对象横向三等分，CLEAN 约在 `17%`、RESET 约在
  `50%`、ABANDON 约在 `83%`，导致后两项分别跨向 cell 3／cell 4。
- 美术诊断：SHARE、DETAIL 恢复为有内高光／纸纹的实体物件；SHOW／HIDE
  近似规则金属四芒星；RESET 是有厚度的圆环和端点；ABANDON 重新出现明显
  编织体积。综合色也偏深黄褐，失去 attempt 3 更平的颜料身份。
- 真实排版：`attempt-04.real-layout.png`，SHA
  `2f2e2d890d64d1892992536ef89c3229e779f40fe0244bf2aff012113c94a1ec`；
  展示区报告 SHA
  `e5a1e0f0a3c793ec7686fc9d7d2d0e8f28becd6fc903020e0063e8a4794897f7`，
  `6/6 pass`；ABANDON 在固定 cell crop 后只剩极小红色残片，运行时不可用。
- 决策：`internal-rejected / repair-prepared / P3 / 4/5`。cell、空格与美术
  身份均不正确，不允许上传 attempt 4 为 Image 3。attempt 5 只上传固定
  Image 1／2 fresh regenerate；这是已授权预算的最后一次实际调用。

### 完整 V5-B.r4 production Prompt — attempt 5

Use Image 1 and Image 2 only as fixed visual references. Create one production
bitmap worksheet with exactly seven tiny flat hand-painted quest-administration
pigment marks for a circa-2004 vanilla World of Warcraft UI overhaul.

MOST IMPORTANT COMPOSITION RULE

This is an eight-slot 4-by-2 sprite grid even though only seven marks are drawn.
The bottom row still has four equal placement slots. Draw CLEAN in bottom slot
1, RESET in bottom slot 2, ABANDON in bottom slot 3, and deliberately reserve
bottom slot 4 as completely empty green. Never redistribute or center the three
bottom marks across the row. ABANDON must stay near 61 percent canvas width,
not near 83 percent. The empty fourth slot is a required structural placeholder.

All seven marks are thumbnail-small. Deliberately undersize them: each visible
bbox should be only about 3.5 to 4.5 percent of total canvas width and 3 to 4
percent of total canvas height. No mark may exceed 5.5 percent of width or 5
percent of height. They must look almost too small on the source worksheet,
surrounded by very large unused green areas. This undersizing is intentional
because later deterministic bbox-fit creates the in-game 32-by-22 sprites.

FIXED SLOT CENTERS

Top row centers: SHARE [14.0%,26.5%], DETAIL [37.5%,26.5%], SHOW
[61.0%,26.5%], HIDE [84.4%,26.5%]. Bottom row centers: CLEAN [14.0%,70.5%],
RESET [37.5%,70.5%], ABANDON [61.0%,70.5%]. The empty slot center is
[84.4%,70.5%]. Do not use one-third/two-thirds rows, 20/40/60/80 columns, or a
three-column bottom row.

FLAT TWO-TONE PIGMENT ONLY

Every mark is a direct two-dimensional guild-pigment transfer with no visible
backing. Use one dominant matte pigment tone plus at most one closely related
darker dry-brush tone. No third tone and no modeled lighting. There is no
thickness, bevel, rim, cast shadow, contact shadow, dark outline, metal, paper,
leather, rope volume, bright highlight, texture relief, or three-dimensional
material. Do not render physical objects. Keep silhouettes broad, compact,
slightly irregular, and readable like old 2004-era painted UI sprites.

Never leave an enclosed green hole inside a mark. Knots, compass centers,
intersections, and cord bodies are solid pigment. Wear is same-pigment color,
not green. Do not tint antialiased edges green.

REFERENCE AUTHORITY

Image 1 is the highest visual authority only for the solemn old-Azeroth guild
ledger feeling: broad hand-painted shapes, muted warmth, believable restrained
wear, and substantial vanilla-era weight. Translate that feeling into tiny flat
pigment marks. Do not copy its book, pages, leather, brass, compass decoration,
wax, ribbons, text, rewards, buttons, scene, or composition.

Image 2 is secondary only for economical stroke count and tiny-size
legibility. Do not copy its triangles, circles, check mark, 2-by-2 positions,
black field, material, transparency, or pixels.

CANVAS AND HARD CELLS

Render an exact square 1024-by-1024 RGB image. Every pixel outside the seven
marks is one uniform exact #00FF00 with no gradient, vignette, noise, texture,
shadow, grid, label, text, frame, checkerboard, or transparency preview.

Each mark must be completely contained by its hard safe box with at least 24
pixels of pure green clearance on all four sides. All visible pigment,
antialiasing, and dry-brush pixels count. The cells and safe boxes are:

SHARE cell [32,160,256,384], safe [64,208,224,336], center [143,271].
DETAIL cell [272,160,496,384], safe [304,208,464,336], center [384,271].
SHOW cell [512,160,736,384], safe [544,208,704,336], center [625,271].
HIDE cell [752,160,976,384], safe [784,208,944,336], center [864,271].
CLEAN cell [32,608,256,832], safe [64,656,224,784], center [143,722].
RESET cell [272,608,496,832], safe [304,656,464,784], center [384,722].
ABANDON cell [512,608,736,832], safe [544,656,704,784], center [625,722].
EMPTY cell [752,608,976,832], entirely exact #00FF00.

SEVEN MARKS

SHARE: two short broad old-quill silhouettes crossing at unequal angles, held
by one solid irregular pigment dab. Each feather is one mass with at most two
coarse notches. No loop, hole, barbs, long tips, polished shaft, scroll,
handshake, share arrow, highlight, or physical feather texture.

DETAIL: one blank, slightly skewed old guild-record leaf silhouette with one
small uneven folded corner. The face is empty pigment: no lines, fake writing,
rows, glyphs, list, modern document icon, paper grain, page thickness, border,
card, envelope, or menu.

SHOW: one four-direction old cartographer compass-rose pigment silhouette with
one longest blunt north point, shorter south, and two shorter unequal side
points around a solid center. Exactly four points. No center hole, diagonal
blades, ring, letters, ticks, gem, metal, weapon symmetry, or glow.

HIDE: a second four-direction compass of the same family with one short narrow
diagonal pigment stroke over only its solid center. The stroke is less than
half the compass width. No long bar, two bands, quill, plank, ribbon, X, eye,
prohibition sign, center hole, or modern visibility icon.

CLEAN: exactly three compact broad flat cartographic trail strokes fanning from
one gesture. Unequal length, curve, spacing, and endpoints. No ribbon, blade,
broom, speed lines, waves, list, chevrons, parallel bars, highlight, or green
wear holes.

RESET: one tiny arrowless open returning expedition route. Use a low imperfect
open teardrop loop with two blunt ends nearly meeting beside one small irregular
pigment knot. No arrowhead, pointed hook, undo arrow, refresh icon, complete
circle, letter C, letter S, gear, waypoint, pin, thick ring, or raised cord.

ABANDON: two very short flat old-wine pigment cord fragments snapped at one
quiet central gap, both together inside bottom slot 3. Each fragment is a plain
broad silhouette with two shallow diagonal pigment hints and a few coarse end
fibers. No braid volume, cylinder, highlights, far-separated pieces, bow-tie,
X, trash, skull, blood, weapon, or warning sign.

COLOR

SHARE through RESET use one matte, low-saturation dusty ochre-brown dominant
tone near RGB 158,124,76, with at most one close darker pigment tone. It must be
muted brown-ochre, not gold, brass, bright yellow, orange, ivory, white,
emissive, glossy, or glowing.

ABANDON alone uses matte muted greyed old-wine pigment near RGB 148,76,68 plus
at most one close darker wine tone. It is not scarlet, blood-red, glossy, neon,
or glowing.

STYLE AND EXCLUSIONS

The result must feel native to a solemn 2004-era Azeroth guild ledger, not a
modern HUD, mobile/web icon set, Diablo III ornament, Skyrim minimalist menu,
Warhammer purity-seal system, photorealistic scan, concept-art sheet, procedural
texture, or rendered inventory object.

No backing tile, cloth, ribbon, parchment tag, page material, leather plate,
wax, button, frame, card, medallion, brass, rivet, engraving, badge, shadow,
outline, bevel, glow, specular highlight, glass, neon, chrome, 3D rendering,
photographic fiber, perfect vector symmetry, repeated noise, or text. Do not
join marks or place any pixel in the empty eighth slot.

FINAL CHECK

Confirm the worksheet is a 4-by-2 grid with the bottom fourth slot empty, not a
three-column bottom row. Confirm seven marks and only seven marks. Confirm all
centers match the listed percentages. Confirm every bbox is tiny and below 5.5
by 5 percent of canvas. Confirm every mark stays inside its safe box with 24
pixels green clearance. Confirm there are no internal green holes. Confirm
DETAIL is blank; SHOW has exactly four points; HIDE has one short stroke; RESET
has no arrowhead; ABANDON remains fully in bottom slot 3. Confirm flat two-tone
pigment with no material volume. Confirm all other pixels are exact #00FF00.

### V5-B.r4 完整性与修复边界复核

- 以上正文按标题边界提取、去除标题并按既有 shell 位置参数传输语义去除
  尾随换行后为 `7755 bytes`；传入参数正文 SHA-256
  `13f4bbd07e38f3f15a8950cc770e1b612d04e2ae78a0e8413f64d058f3a838ac`；
  首句已核对为 `Use Image 1 and Image 2 only as fixed visual references.`。
- 本正文仍只包含冻结七项与第八空格；只是针对 attempt 4 的底排三等分、2×
  尺度、实体材料退化和跨格进行最后一次 in-envelope 强化。
- 继续冻结七 source、四态派生、V5-A 布底、runtime 几何、六赭棕一酒红和
  香草魔兽旧公会账簿语言；未新增对象、功能、状态、参考或载体。
- attempt 5 必须 fresh regenerate，只上传固定 Image 1／2；不上传任何历史
...
- 执行前 commit：`aff50cb`；完整正文版本：`QS-B1 V6-A.r2`；操作：fresh
  regenerate。
- 固定输入：Image 1／2；无 Image 3；提示词正文 `6598 bytes`、SHA-256
  `4c1d13389d0299d0dd5408ae99fba317b5931cb427070756a92a4461db35de33`。
- fixed child：`@openai/codex@0.143.0`、`gpt-5.5 / medium`；session
  `019fd58d-832d-7641-855c-47ed7fb1c4d5`；收到唯一实际图像并
  `turn.completed`，计为 V6-A `3/5`、整批 `3/35`。模型缓存 TTL 与 analytics
  警告未阻止 provider 结果，不另计流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-A/attempt-03/raw/QS-B1-V6-A.attempt-03.png`；
  `1254×1254 RGB`；SHA-256
  `c57c3eac4a145af55b20ea4c541d7032d621edc983cc92bed45b376f5faf2deb`。
- 确定性审查：归一化后可见 bbox `[165,193,835,792]`，位于 safe box
  `[160,160,864,864]`；可见绿色污染 `0`；自动 `9/9 pass`，review JSON SHA
  `9acd1e0f326fb2665375a8840990d18e87f6dd0951906e8e4c94e1661ae99454`。
  颜色 median `[121,66,28]`、luma p50 `74.94`、chroma p50 `93`，已从 attempt 2
  的明亮橙金收敛到暗赭褐。
- runtime 装配：等比 bbox-fit 为 `13×12px`，落在冻结 `[7,5,25,17]` content
  box；normal／hover／pressed／disabled 四态由确定性规则派生。technical board
  SHA `2dfaedb008b4bbe8048f0f5237f09bc1e1349ac93ff4e7c4251b970aec9da674`。
- 真实排版：六场景图 `attempt-03.real-layout.png`，SHA
  `71930bdeaae7ec76e8a6bb4d49a7049ec5cc2d4108d1505692fcdbd4c5455bab`；
  display-region `6/6 pass`、violations `0`，报告 SHA
  `92a3021a2b9dcef3152aa91b04c8ff4d5852e8bfb8d6e052fd04f0c4334dec30`。
- 美术内审：双旧羽笔＋不等角交叉语义清楚；暗赭褐、钝化边缘和不规则磨损与任务
  书页／暗旧布底协调；在最终 `13×12px` normal 及四态近邻放大中，高分辨率斑驳
  只形成少量宽像素变化，不产生现代高光、实体羽轴或摄影纹理读感。图标在开启、
  筛选、disabled、局部滚动和完全滚出场景中的层级与命中区均正确。
- 结论：`candidate-reviewed / P3 / production 3/5 / internal-pass`。依“单段内部
  通过即停”，V6-A attempt 4／5 不调用。当前只保留 ignored generated 候选与审查
  证据，不写 `assets/source`、runtime、addon 或 accepted manifest；需待整批候选
  由用户明确接受后再进入 P4。
...
- 执行前 commit：`a267756`；完整正文版本：`QS-B1 V6-D.r1`；操作：fresh
  regenerate。
- 固定输入：Image 1／2；无 Image 3；提示词正文 `8257 bytes`、SHA-256
  `1ae4c37fa7639a8b95dc909ec17228b9cfe6b6adc3d047df4ca13ef454b44fe3`；未上传或
  复用任何跨段生成像素。
- fixed child：`@openai/codex@0.143.0`、`gpt-5.5 / medium`；session
  `019fd5d0-5cb0-7ec1-b065-2423ab9535c6`；完整正文和两张固定参考职责已逐字
  回显，收到唯一实际图像并 `turn.completed`，计为 V6-D `2/5`、整批 `10/35`；
  本次没有新增流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-D/attempt-02/raw/QS-B1-V6-D.attempt-02.png`；
  `1254×1254 RGB`；SHA-256
  `cd44ae6e34837b08f2b3119f0f11a719348cf897d2c88133e4b7c87d0a59b680`。
- 确定性审查：归一化后可见 bbox `[222,204,818,808]`，完整位于 safe box；
  可见绿色污染 `20`，自动 `8/9`，唯一技术失败为
  `visible_green_spill_is_zero`。20 个像素分成四个极小局部：右上墨带末端
  `[717,362,720,366]`、中心右下单像素 `[607,609]`、左下墨带边缘
  `[342,628,345,632]` 与下缘单像素 `[360,705]`（均为归一化坐标）。review
  JSON SHA `64cf0e19fe8a972d668bc8a4f26db7984cedcf7fe32b1ee9e3fbf1b10cc6ab78`。
- 美术内审：median `[106,55,21]`、luma p50 `63.184`、chroma p50 `85`；
  四个方向块已成为厚、钝、不等的暗赭平面印记，中心的短粗深褐遮蔽墨带具有磨损
  与钝端，不再是金属徽章、细长武器或明亮禁止符号。身份、颜色家族、年代语言与
  微缩可读性内部成立；本次仅因四处局部封闭绿色孔洞失败。
- runtime 装配：等比 fit 为 `14×14px`，位于冻结 `[8,4,24,18]` content box；
  六场景 display-region `6/6 pass`、violations `0`，报告 SHA
  `d9077678b019b8715b6dcbffcae78726e69172d1040f267670842c53fafba124`；真实排版
  SHA `fd984949aa046b1094ee246f1c9d163a907f6e4ab8f385d46cb7a06e852b94cf`。
- 结论：`internal-rejected / repair-prepared / P3 / 2/5`。身份、构图、配色、
  材质和时代均正确，缺陷严格局限于 20 个绿色像素，满足同段紧邻 Image 3 edit
  条件。attempt 3 允许上传固定 Image 1／2，并把本次 raw 以固定 SHA 作为唯一
  Image 3 edit-identity 输入；禁止上传 attempt 1 或任何跨段候选。
- 下一版必须逐项保持：当前四个方向块、中央墨池、短粗斜向遮蔽墨带的相对位置、
  外轮廓、尺度、颜色、深浅关系、磨损分布、背景与所有冻结禁项。
- 下一版只允许改变：用紧邻的暗赭或深褐颜料填平上述四处微小绿色孔洞，并清除
  所有绿染可见边缘；不得移动、缩放、重画、增删或重新解释主体。
...
Warcraft quest-log administration menu. This is a surgical cleanup of one already
correct flat bitmap pigment sprite source, not a new generation, redesign, scene,
screenshot, UI mockup, worksheet, atlas, button, ribbon, map, physical badge, metal
object, inventory item, or complete interface.

REFERENCE AUTHORITY AND IMAGE ROLES

Image 1 remains the highest visual authority for the old Azeroth expedition-ledger era:
heavy handmade masses, muted earth color, sparse wear, and the serious weight of a
vanilla-era adventurers' guild record. Do not copy its book, pages, leather frame,
brass, wax seal, ribbons, text, reward slots, buttons, compass ornaments, or composition.

Image 2 remains a secondary micro-scale reference only for economical stroke count,
soft hand-painted edges, broad readable masses, and severe-downscale legibility. Do not
copy, trace, rotate, recolor, or rearrange any triangle, circle, check mark, 2-by-2
layout, position, transparency, or pixel from Image 2.

Image 3 is the immediately preceding V6-D attempt 2 raw image with fixed SHA-256
cd44ae6e34837b08f2b3119f0f11a719348cf897d2c88133e4b7c87d0a59b680. It is the only
edit-identity input. Preserve its complete HIDE identity, four blunt unequal direction
masses, small center, short thick lower-left-to-upper-right masking stroke, overall
silhouette, position, scale, dark umber-ochre color family, broad value groups, wear
pattern, softened perimeter, and flat pigment rendering. Do not use or reproduce any
pixel from V6-A, V6-B, V6-C, V6-D attempt 1, or another segment.

CANVAS, COUNT, AND PLACEMENT

Return one exact square 1024 by 1024 RGB image. Preserve the Image-3 emblem centered at
the same position and the same scale. Preserve the current complete visible bbox
relationship, approximately normalized [222,204,818,808], strictly inside safe box
[160,160,864,864]. Do not translate, rotate, stretch, crop, enlarge, shrink, mirror, or
recompose it. Use the same direct orthographic front view.

Fill every true exterior pixel with one perfectly uniform exact chroma-key green
#00FF00. Preserve the broad exterior green field and its connection to all four canvas
edges. It must have no gradient, lighting, vignette, texture, noise, paper, cloth, wax,
metal, frame, guide, label, checkerboard, or transparency preview. There must remain
exactly one connected semantic emblem and no additional mark.
...
FROZEN IDENTITY AND APPEARANCE

Preserve Image 3's four short broad blunt direction masses exactly in their current
relative positions: the broad flat-topped north mass, unequal rounded east and west
masses, compact rounded south mass, small central overlap, and open exterior channels
between adjacent directions. Preserve the current short thick diagonal masking stroke
from lower-left toward upper-right, including its blunt brushy ends, uneven width,
deep-smoked-umber value, central overlap, and broad old-ochre wear patches. Preserve the
fact that the stroke ends before the outer direction tips and reads as obscured guild
instructions rather than a weapon or modern slash.

Preserve the current flat dark mineral-pigment palette: dominant direction masses near
RGB [106,55,21] median family, deep smoked umber in the masking stroke, and sparse muted
old-ochre wear. Preserve all valid coarse wear, handmade asymmetry, broad softened edge
shape, value grouping, and non-metallic two-dimensional appearance. Do not add gold,
brightness, orange saturation, white, black, highlights, bevels, depth, rim light,
shadow, photographic relief, or new texture.

ONLY ALLOWED REPAIR

Repair only the four tiny isolated green-colored defects enclosed by or visibly caught
on the Image-3 pigment body after whole-square 1024 normalization:

1. the approximately nine-pixel green fleck at the upper-right end of the dark masking
   stroke, near normalized box [717,362,720,366];
2. the single green pixel inside the center-right/lower overlap near [607,609];
3. the approximately nine-pixel green fleck on the lower-left masking-stroke edge near
   normalized box [342,628,345,632];
4. the single green pixel on the lower-left/lower edge near [360,705].

Cover each defect with the immediately adjacent existing pigment color and edge logic.
Use deep smoked umber for defects touching the masking stroke and the nearest dark
umber-ochre for defects touching a direction mass. Blend no farther than the minimum
small local patch required to remove the green. The repaired spots must look like
continuous old pigment, not fresh dots, patches, jewels, holes, or a new wear pattern.

Do not change any exterior edge-connected green background. Do not fill the intentional
open green channels between adjacent directions. Do not alter any other pigment pixel,
outer contour, inner contour, wear mark, brush end, wedge shape, center, color block,
texture, position, or scale. Do not invent a fifth repair zone. After repair, no visible
green-like pixel may remain inside or on the keyed emblem: no enclosed green island,
green crack, green center, isolated green speck, or green-tinted antialiasing fringe.

STRICT EXCLUSIONS

No redesign; no new compass; no sharp diamond point, spearhead, arrowhead, four-point
star, five-point star, crosshair, D-pad, separate keys, shuriken, sunburst, flower, gem,
faceted badge, wind rose, outer circle, inner ring, prohibition ring, eye outline,
eyelid, pupil, letters N/E/S/W, ticks, map grid, coordinates, needle, location pin, X,
long slash, thin slash, sword, spear, axe, quill, crossed weapons, text, numbers, runes,
...
photographic grain, glass, neon, chrome, 3D rendering, photorealism, perfect fourfold
symmetry, vector-clean geometry, constant line weight, Diablo III ornament, Skyrim
minimalist menu language, or Warhammer iconography.

FINAL SELF-CHECK

Confirm the result preserves Image 3's existing HIDE emblem identity, silhouette,
placement, scale, four blunt unequal direction masses, short thick central masking
stroke, dark earthy palette, wear distribution, and flat hand-printed character.
Confirm the only visual changes are the minimum local pigment fills at the four listed
green defects. Confirm all four defects are fully covered with adjacent dark pigment,
no new defect or green fringe exists, and every remaining green pixel belongs to the
uniform exact #00FF00 exterior field connected to a canvas edge. Confirm all visible
...
- 执行前 commit：`610f134`；完整正文版本：`QS-B1 V6-D.r3`；操作：edit。
- 固定输入：Image 1／2；Image 3 仅为紧邻 attempt 3 raw，固定 SHA
  `b0e91e52b402c724d8be5085bddec2d7925b35008ddef470fba3b2c5d6255698`；提示词
  正文 `7184 bytes`、SHA-256
  `6bf743052421700e9813668ae9677493b62968f3c3987fcb68dd5b706fa2ce21`；未上传或
  复用任何跨段像素。
- fixed child：`@openai/codex@0.143.0`、`gpt-5.5 / medium`；session
  `019fd5de-78a1-7661-af36-73f89e214e7c`；完整正文和三图职责已逐字回显，收到
  唯一实际编辑结果并 `turn.completed`，计为 V6-D `4/5`、整批 `12/35`；本次
  没有新增流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-D/attempt-04/raw/QS-B1-V6-D.attempt-04.png`；
  `1254×1254 RGB`；SHA-256
  `e5cb30bf52425378b45bd33dd202a738494ed9cfb5c709e354ed6f42159a9955`。
- 确定性审查：归一化后可见 bbox `[215,194,833,823]`，仍位于 safe box，但
  相比 Image 3 发生约 7–16px 的非必要漂移；可见绿色污染由 `12` 反增至 `22`，
  自动 `8/9`，唯一技术失败仍为 `visible_green_spill_is_zero`。review JSON SHA
  `ee709c8cab5fb2192e86ec0fd9229bebc6795d4aa190f7b1418c8d5969db4ebb`。
- 剩余绿色位置：右上墨带末端 `[725,356,727,359]` 6px；左下墨带及邻缘
  `[341,632,351,643]` 13px；另有 `[435,707]` 1px 与 `[370,725,371,727]`
  2px。它们均是深褐主体上的局部绿色污染，不是合法外部通道。
- 美术内审：主体仍保持四个钝头方向块加粗短遮蔽墨带的 HIDE 身份，微缩可读；
  median `[87,27,10]`、luma p50 `38.099`、chroma p50 `78`，进一步变暗但尚未
  变成纯黑现代符号。由于模型重渲染了纹理、尺度和颜色，attempt 4 不优于 attempt 3。
- runtime 装配：等比 fit 为 `14×14px`，位于冻结 `[8,4,24,18]` content box；
  六场景 display-region `6/6 pass`、violations `0`，报告 SHA
  `a4a4b5487b70c0fbd5378e9b1ef6f8fe6725e2047c9a31fb1ce3f3e0b5b4534d`；真实排版
  SHA `b09569529711e78956d20cfbe30d699bf110f5d80af17c87801e034fa5c36450`。
- 结论：`internal-rejected / repair-prepared / P3 / 4/5`。attempt 5 是本段最后
  一次实际 ImageGen；依授权只可上传固定 Image 1／2 与紧邻 attempt 4 raw，禁止
  回退上传 attempt 3、上传更早稿或使用跨段像素。
- 下一版必须保持：attempt 4 的完整身份、构图、相对部件关系、normal state、
  safe box、无 carrier、runtime box 与动态排除。
- 下一版只允许改变：在四个明确的微型修复框内，用各自相邻的深褐或暗赭颜料覆盖
  22 个绿色像素及最多 1px 缓冲；不得重新渲染框外区域。

### QS-B1 V6-D.r4 complete production Prompt — repair-prepared / frozen final edit

Use Image 1, Image 2, and Image 3 with the authority and frozen repair boundaries below.
Perform the final surgical edit of exactly one independent normal-state HIDE emblem for
a circa-2004 vanilla World of Warcraft quest-log administration menu. The existing
Image-3 sprite is already compositionally correct. This operation is only a four-box
green-defect inpaint, not a new generation, redesign, re-render, scene, screenshot, UI
mockup, worksheet, atlas, button, ribbon, map, physical badge, or complete interface.

REFERENCE AUTHORITY AND IMAGE ROLES

Image 1 remains the highest era authority only for old-Azeroth ledger weight, muted
earth color, sparse wear, and circa-2004 visual language. Do not copy its book, pages,
leather, brass, wax, ribbons, text, reward slots, buttons, compass ornament, or layout.

Image 2 remains a secondary micro-scale authority only for economical stroke count,
soft hand-painted perimeter, broad readable masses, and tiny-sprite legibility. Do not
copy, trace, rotate, recolor, or rearrange its shapes, layout, transparency, or pixels.

Image 3 is the immediately preceding V6-D attempt 4 raw image with fixed SHA-256
e5cb30bf52425378b45bd33dd202a738494ed9cfb5c709e354ed6f42159a9955. It is the only
edit-identity input. Preserve its complete HIDE identity: four blunt unequal direction
masses, center, short thick lower-left-to-upper-right masking stroke, silhouette,
position, scale, dark umber-ochre palette, value groups, wear distribution, softened
perimeter, and flat pigment rendering. Do not use any pixel from another body or an
earlier V6-D attempt.

CANVAS, COUNT, AND PLACEMENT

Return one exact square 1024 by 1024 RGB image. Preserve Image 3 at the same position,
scale, orientation, and composition. Preserve its normalized visible bbox relationship
of approximately [215,194,833,823], strictly inside safe box [160,160,864,864]. Do not
translate, rotate, stretch, crop, enlarge, shrink, mirror, recenter, or recompose it.

Preserve the exterior field as one perfectly uniform exact chroma-key green #00FF00,
connected to all four canvas edges. Do not alter any valid exterior background pixel.
The field has no gradient, texture, lighting, vignette, noise, shadow, carrier, guide,
label, checkerboard, or transparency preview. There remains exactly one connected
semantic emblem and no additional mark.

This source owns only one normal-state pigment emblem. It will later be extracted and
isotropically bbox-fitted into content box [8,4,24,18] inside one 32 by 22 runtime
Button. Do not draw the Button, backing tile, other states, Tooltip, text, map, menu
background, ribbon substrate, or provider logic.

HARD FREEZE OUTSIDE FOUR REPAIR BOXES

Do not regenerate, repaint, reinterpret, sharpen, blur, relight, recolor, resize, or
retexture the full emblem. Copy every Image-3 pixel outside the four repair boxes below
unchanged. Preserve the broad flat-topped north mass, unequal rounded east and west
masses, compact south mass, center overlap, open exterior channels, short thick masking
stroke, both blunt brush ends, deep smoked-umber value, old-ochre wear, handmade
asymmetry, existing dark median family near RGB [87,27,10], and all valid contours.

Do not move or reshape a direction mass, center, brush end, wear patch, or exterior
channel. Do not brighten or darken the icon. Add no gold, bright orange, white, black,
highlight, bevel, depth, shadow, rim light, photographic relief, or new texture.

ONLY ALLOWED REPAIR: FOUR MICRO INPAINT BOXES

After whole-square 1024 normalization, exactly twenty-two green-like defect pixels
remain on or enclosed by the pigment body. Inpaint only these four tiny regions:

1. upper-right masking-stroke end, normalized box [723,354,729,361], containing pixels
   (725,356), (726,356), (725,357), (726,357), (725,358), and (726,358);
2. lower-left masking-stroke and adjacent edge, normalized box [339,630,353,645],
   containing (350,632), (343,638), (342,639), (343,639), (344,639), (342,640),
   (343,640), (344,640), (341,641), (342,641), (343,641), (341,642), and (342,642);
3. lower central overlap edge, normalized box [432,704,439,711], containing only
   (435,707);
4. lower-left/south edge, normalized box [367,722,374,729], containing only (370,725)
   and (370,726).

Within each box, replace the listed green-like pixels with the opaque pigment color
immediately surrounding them. Use deep smoked umber in boxes 1 and 2, and the nearest
local dark umber-ochre in boxes 3 and 4. A maximum one-pixel normalized buffer around a
listed defect may be blended solely to avoid green-tinted antialiasing. The repair must
merge invisibly into existing pigment and must not change the outer silhouette.

Do not edit any pixel outside these four boxes. Do not fill the intentional open green
channels between adjacent direction masses. Do not extend pigment into true exterior
background. Do not invent another repair. After the inpaint, every green-like pixel
must belong only to the exact exterior #00FF00 field connected to a canvas edge. No
enclosed green island, green crack, isolated green speck, green center, or green-tinted
visible fringe may remain.

STRICT EXCLUSIONS

No redesign, re-render, new compass, sharp point, star, crosshair, D-pad, shuriken,
sunburst, gem, badge, wind rose, circle, prohibition ring, eye, pupil, letters, ticks,
map grid, coordinates, needle, location pin, X, thin slash, weapon, quill, text,
numbers, runes, compass casing, gold, brass, cloth, ribbon, wax, backing tile, button,
border, card, frame, medallion, rivet, bevel, shadow, glow, highlight, gradient,
photographic grain, glass, neon, chrome, 3D rendering, photorealism, perfect symmetry,
vector-clean geometry, Diablo III ornament, Skyrim minimalist menu language, or
Warhammer iconography.
...
Confirm every Image-3 pixel outside the four listed repair boxes is visually unchanged.
Confirm all twenty-two listed green-like pixels are replaced by matching local opaque
pigment, with at most a one-pixel local blend and no green fringe. Confirm every
remaining green pixel is part of uniform exact #00FF00 exterior connected to a canvas
edge. Confirm the HIDE identity, silhouette, placement, scale, four blunt unequal
direction masses, short thick masking stroke, dark earthy palette, wear distribution,
and flat hand-printed character remain intact. Confirm all visible pixels stay inside
...
- 执行前 commit：`3c13fdb`；完整正文版本：`QS-B1 V6-E`；操作：generate。
- 固定输入：Image 1／2；无 Image 3；提示词正文 `4699 bytes`、SHA-256
  `6bc1d309645e2ee87da3be83e9e6332396dc704fc1d7f9262325721913d459ff`；未上传或
  复用 V6-A..D 的任何候选像素。
- fixed child：`@openai/codex@0.143.0`、`gpt-5.5 / medium`；session
  `019fd5f2-cf80-7be1-9c2c-35686dbc9fad`；完整正文与两图职责已逐字回显，内建
  `image_gen` 恰好调用一次并成功原样落位，计为 V6-E `1/5`、整批 `14/35`；
  本次没有新增流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-E/attempt-01/raw/QS-B1-V6-E.attempt-01.png`；
  `1254×1254 RGB`；SHA-256
  `187dd5aeae3d25e86ae4b9861590ee7707153613921d4633acfc10de87bd4db8`。
- 确定性审查：归一化后可见 bbox `[178,279,859,751]`，位于 safe box；可见绿色
  污染 `556`，自动 `8/9`，唯一技术失败为 `visible_green_spill_is_zero`。review
  JSON SHA `6b95bfb541dc51dc9b2a48b128c24a2299a7a8da86cd80241e90f1652e75ea1f`。
- 美术内审：三条弧形确实共享左侧起点，但均为明亮赭金、带连续明暗、边缘厚度和
  高光的硬质弧片；median `[178,136,60]`、luma p50 `139.229`、chroma p50 `117`。
  三条轨迹长度、弧度与间距过于规整，右端近似削尖／卷曲，微缩后更像风线、速度线
  或三片金属羽，而不是旧地图上的清理墨迹。内部磨损直接露出绿色，形成大量孔洞。
- runtime 装配：等比 fit 为 `14×10px`，位于冻结 `[7,6,25,16]` content box；
  六场景 display-region `6/6 pass`、violations `0`，报告 SHA
  `bfc0b7fa7ee39b36859b1b90bfa6c464c6f9d533f275236d66ab61eae3b9cce9`；真实排版
  SHA `d1878753844bbbf1219a40d6eee222945ebe8a4d4cb58830a30d8fcc9d44d042`。
- 结论：`internal-rejected / repair-prepared / P3 / 1/5`。身份、材质、配色和轮廓
  均需重做，不属于局部 edit；attempt 1 禁止作为 Image 3。attempt 2 必须 fresh
  regenerate，只上传固定 Image 1／2，禁止跨段像素。
- 下一版必须保持：CLEAN 功能语义、恰好三条不等地图轨迹、共享运动起源、normal
  state、1024² RGB、精确绿色背景、safe box、无 carrier、runtime content box 与
  所有动态排除。
- 下一版只允许修复：改为短、厚、钝、不同弧度和不同节奏的暗赭平面矿物墨迹；
  取消尖端、金属厚度、高光、连续渐变、机械扇形和绿色磨损孔洞。
...
quest-log administration menu. This is a flat bitmap pigment sprite source, not a
scene, screenshot, UI mockup, worksheet, atlas, button, ribbon, map, physical object,
metal ornament, inventory item, or complete interface. Do not use V6-E attempt 1 or any
V6-A, V6-B, V6-C, or V6-D generated pixel as an input, edit source, tracing source,
texture donor, or identity reference.
...
Return one exact square 1024 by 1024 RGB image. Fill every pixel outside the emblem with
one perfectly uniform exact chroma-key green #00FF00. The green field must have no
gradient, lighter center, darker corner, texture, lighting, vignette, noise, shadow,
paper, cloth, wax, metal, frame, guide, label, checkerboard, or transparency preview.
There must be exactly one semantic CLEAN emblem and no unrelated second mark.

Center the complete emblem near [512,512]. Keep every pigment pixel, antialiasing pixel,
soft bleed, and worn edge strictly inside safe box [160,160,864,864]. Target a complete
visible span between 420 and 500 pixels wide and between 240 and 310 pixels high. Leave
broad exact-green margin on every side. Use a direct orthographic front view with no
perspective, thickness, modeled depth, or directional lighting.
...
Paint exactly three independent but visually related old cartographic trail strokes.
They begin within one compact shared motion-origin region near the left side and sweep
toward the right, but they must not converge into a sharp point, arrowhead, fan handle,
broom binding, or physical joint. Keep enough separation at the origin to preserve
three readable pigment strokes while still making them one semantic CLEAN emblem.

Make every stroke short, broad, blunt, and distinctly different:

- the upper stroke is medium length, gently rises, then makes one slight uneven bend;
- the middle stroke is longest and travels mostly rightward with one shallow downward
  irregularity rather than a smooth mechanical arc;
- the lower stroke is shortest, settles downward early, and ends in a wide chipped
  blunt patch.

Use unequal widths approximately 58 to 86 normalized-source pixels, unequal start
offsets, unequal gaps, unequal curvature, unequal pigment density, and visibly different
blunt endpoints. Keep the vertical negative-space gaps broad enough to survive at
18-by-10 runtime scale. Do not make the three paths parallel, concentric, evenly spaced,
perfectly smooth, or equally long. Avoid a single shared tangent that creates a speed-
line fan. Avoid a pointed left apex and avoid hooked or curled right tips.

The emblem should read as three heavy route marks being broadly swept off an old guild
map: deliberate, earthy and administrative, not fast or airy. It must not read as wind,
waves, water, speed streaks, claw scratches, feathers, three blades, menu bars, Wi-Fi,
a back arrow, a broom, or a brush object.

MATERIAL, COLOR, EDGE, AND ERA

Render all three strokes as direct imperfect two- or three-pass woodblock or wax-stencil
transfers of matte mineral pigment on an absent carrier. They are flat painted marks,
not carved, embossed, cast, folded, beveled, metallic, illuminated, photographic, or
three-dimensional. Use only a few broad discrete pigment patches with no continuous
gradient and no repeated micro-texture.

Use a dominant dark muted umber-ochre near RGB [112,72,35], sparse old-ochre wear near
RGB [144,96,47], and a few deep smoked-umber pools near RGB [72,44,24]. The median
visible color must stay dark and earthy, approximately RGB [100,62,30] to [130,84,42].
No visible pixel may resemble bright yellow, bright orange, polished gold, brass,
ivory, white, neon, or emissive light.

Use non-constant hand-printed edges, softened blunt corners, slight asymmetry, a few
coarse edge nicks, and restrained two-to-four normalized-source-pixel pigment bleed.
Represent every internal worn interruption with a lighter ochre pigment patch, never
with background green. The interior of each of the three strokes remains fully pigment
colored. Any true missing-pigment notch must be open and connected to the exterior
silhouette. No enclosed green island, green gap, green crack, isolated green speck, or
green-tinted antialiasing is allowed. All edge antialiasing transitions through brown
or ochre pigment values before meeting exact green.

STRICT EXCLUSIONS

No broom, brush object, wind, waves, water, speed line, motion streak, claw, feather,
blade, fan, arrowhead, back arrow, pointed shared apex, three equal parallel lines,
concentric arcs, menu bars, horizontal list, Wi-Fi arcs, route arrow, map grid, text,
letters, numbers, runes, physical trail material, paper, cloth, ribbon, wax, metal
backing, button face, border, card, frame, medallion, rivet, bevel, cast shadow, drop
shadow, contact shadow, ambient occlusion, glow, specular highlight, rim light,
continuous gradient, photographic grain, glass, neon, chrome, 3D rendering,
photorealism, repeated procedural noise, perfect symmetry, constant vector line weight,
Diablo III ornament, Skyrim minimalist menu language, or Warhammer iconography. Do not
place green inside a pigment stroke or tint its edges green.

FINAL SELF-CHECK

Confirm the canvas contains exactly one CLEAN emblem composed of exactly three separate
short broad blunt unequal cartographic trail strokes sharing a compact motion-origin
region without forming a pointed apex. Confirm upper, middle, and lower strokes have
clearly different lengths, widths, curves, gaps, and blunt endpoints, and do not form
smooth concentric or mechanical speed lines. Confirm the complete visible span is 420
to 500 pixels wide and 240 to 310 pixels high and all visible pixels stay strictly
inside [160,160,864,864]. Confirm every exterior pixel is uniform exact #00FF00.
Confirm all three strokes use only broad flat dark earthy pigment patches near
[112,72,35], [144,96,47], and [72,44,24], with no gold brightness, thickness, bevel,
...
- 执行前 commit：`3fceefa`；完整正文版本：`QS-B1 V6-F.r1`；操作：fresh
  regenerate。固定 Image 1／2，无 Image 3；正文 `7396 bytes`、SHA-256
  `946d83a7303ba5701469f1d0a3c39dc940590b881627fa6bb57eac7bc161c484`；未上传或
  复用任何旧 V6 候选像素。
- fixed child：session `019fd626-a043-7912-9b5f-d7c1826288e3`；完整正文已回显，
  内建 `image_gen` 恰好调用一次并原样落位，计为 V6-F `2/5`、整批 `20/35`；
  无新增流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-F/attempt-02/raw/QS-B1-V6-F.attempt-02.png`；
  `1254×1254 RGB`；SHA-256
  `f19199c05a91c8b1239c4933fec9db9f19dad640b9a7c7211b27bc211531bc4f`。
- 确定性审查：bbox `[141,201,908,828]`，左／右越 safe box `19／44px`；可见
  绿色污染 `0`，自动 `8/9`，唯一失败为 safe-box。用户授权的等比 bbox-fit 可处理
  该几何溢出；review JSON SHA
  `ef0090300b4ac953480d7cbcf2fadea37592448d3c74eb97444f62a00773eff5`。
- 美术内审：median `[123,60,21]`，配色已进入暗土色且绿色孔洞清零；但大而光滑
  的拱顶、两根垂直下端和右下圆团在 `15×12px` 下明确读成问号／字母 C／耳机，
  仍不是路线回返。小路线结被画成圆形端头，整条路径仍有符号化连续圆弧。
- runtime 装配：等比 fit `15×12px`，位于 `[7,5,24,17]`；六场景
  display-region `6/6 pass`、violations `0`，报告 SHA
  `0b0036fbd4820c8c731898f32d34f71c5d5327dd228760a92ff78a917607ab1c`；真实排版
  SHA `010e1e35dbd4f2f4e218cbb082681297c99f1d301a65e1e78e34cb20f3cda708`。
- 结论：`internal-rejected / repair-prepared / P3 / 2/5`。轮廓与路线结均为全局
  身份问题；attempt 2 禁止作为 Image 3。attempt 3 必须 fresh，只上传固定
  Image 1／2。
- 下一版只允许修复：取消大拱顶和所有圆形路线结；使用四个粗钝折点构成开放的
  非圆形远征回返路线，末端以短交叠式不规则路线结收束；保持暗赭平面颜料与零
  绿色污染。
...
quest-log administration menu. This is one connected flat bitmap pigment route glyph,
not a scene, screenshot, UI mockup, worksheet, atlas, button, ribbon, map, physical
object, material sample, inventory item, or complete interface. Do not use V6-F attempt
1 or 2, or any V6-A through V6-E generated pixel as an input, edit source, tracing
source, texture donor, or identity reference.

REFERENCE AUTHORITY

Image 1 is the highest authority only for old Azeroth expedition-ledger weight: heavy
hand-painted masses, restrained earth color, slight asymmetry, sparse believable wear,
and vanilla-era seriousness. Do not copy its book, pages, leather, brass, wax, ribbon,
text, buttons, compass ornaments, or composition.

Image 2 is a secondary micro-scale reference only. Inherit only economical stroke
count, broad readable silhouette, softened handmade perimeter, and tiny legibility. Do
not copy, trace, rotate, recolor, or rearrange any triangle, circle, check mark, 2-by-2
layout, location, transparency, or pixel from Image 2.

CANVAS AND PLACEMENT

Return one exact square 1024 by 1024 RGB image. Fill every exterior pixel with one
perfectly uniform exact chroma-key green #00FF00. No gradient, lighting, texture,
vignette, noise, shadow, paper, cloth, wax, metal, frame, guide, label, checkerboard, or
transparency preview. Draw exactly one connected RESET pigment emblem and nothing else.

Keep every visible and antialiased pixel inside hard safe box [160,160,864,864]. Place
the complete emblem inside target envelope [220,300,805,710], around 585 by 410 pixels.
Leave broad green margin. Use direct orthographic front view with no perspective,
thickness, modeled depth, or light direction.

This source owns one normal-state emblem only. It will later be edge-keyed and
isotropically bbox-fitted into content box [7,5,24,17] inside one 32 by 22 Button. Do
not draw the Button, carrier, backing, states, Tooltip, text, map, menu, or provider
logic.

ANGULAR OPEN EXPEDITION ROUTE

Paint one stout irregular old-map route that travels out and then doubles back through
one right-side hairpin. It must be a coarse route with four obvious broad bends, not a
single smooth arch, circle, C, question mark, or spiral.

Follow this approximate connected centerline:

1. one wide blunt start near [255,655];
2. climb diagonally up-right to a first coarse bend near [330,485];
3. travel diagonally and then mostly right through a second bend near [500,365] toward
   [690,405];
4. descend to one broad right-side hairpin shoulder near [760,515];
5. double back leftward through [680,585] and terminate in an integrated short
   overlapping route knot near [555,610].

Do not draw a continuous semicircular arch over the center. Connect the centerline
segments with broad irregular shoulders whose turn radii are only about 45 to 75
pixels. The route changes direction at visible coarse bends. Keep a large exterior-
connected opening between the lower-left start and the returning segment, at least 155
pixels wide. The route encloses less than 170 degrees around any center and never loops
around an empty middle.

The end knot is not a circle or blob. Make it a short, dense, asymmetric overlap where
the returning route briefly crosses or folds alongside itself: roughly 115 by 75
pixels, oblong and jagged, with one shallow side notch. It remains connected to the
route and contains no enclosed green. It must read as an old cartographic route knot,
not a detached dot, central disk, lollipop end, question-mark dot, arrowhead, buckle,
gem, eye, or medallion.

Use a broad non-constant route width around 55 to 78 normalized-source pixels, with
coarse variation. Keep all terminals blunt. No smooth vector curve, full arch, round
cap pair, arrowhead, inward coil, second revolution, or pointed tip. At about 16 by 12
runtime pixels the result must read as a bent expedition path doubling back to an
irregular knot, not headphones, a question mark, C, g, 9, snake, refresh, undo,
spinner, power symbol, gear, or clock.

FLAT MINERAL PIGMENT

Render the route as an imperfect two- or three-pass transfer of flat matte guild
pigment on an absent carrier. Use dominant muted dark umber-ochre near RGB [119,76,38],
sparse broad worn-ochre patches near RGB [145,98,49], and a few deep smoked-umber pools
near RGB [73,44,24]. Keep median visible color between RGB [104,63,30] and [137,87,44].
No bright yellow, orange, polished gold, brass, ivory, white, neon, or emissive light.

Use only a few broad opaque color regions. No continuous gradient, directional
lighting, highlight, rim, dark underside, cast shadow, bevel, thickness, parchment,
leather, rope, raised paste, grain field, fibers, cracks, or repeated micro-texture.
Use irregular hand-stamped edges, softened blunt corners, slight asymmetry, a few coarse
open edge nicks, and restrained pigment bleed. Internal wear is flat lighter ochre,
never green. Every interior pixel remains pigment colored. Any missing notch is open to
the exterior. No enclosed green island, hole, crack, isolated speck, or green fringe.

STRICT EXCLUSIONS

No smooth arch, complete circle, ring, semicircle dominating the glyph, spiral, coil,
second revolution, letter C, lowercase g, numeral 9, question mark, headphones, snake,
standard refresh arrow, undo arrow, replay triangle, arrowhead, loading ring, power
symbol, gear, clock, compass rose, circular knot, central disk, detached dot, map grid,
text, letters, numbers, runes, paper, leather, cloth, ribbon, wax, metal backing, button,
border, frame, medallion, rivet, bevel, shadow, glow, highlight, continuous gradient,
photographic grain, glass, neon, chrome, 3D rendering, photorealism, procedural noise,
perfect symmetry, constant vector line weight, Diablo III ornament, Skyrim minimalist
menu language, or Warhammer iconography. Do not place green inside pigment or tint an
edge green.

FINAL SELF-CHECK

Confirm exactly one connected RESET glyph exists with a blunt lower-left start, four
visible coarse bends, one right-side hairpin, one leftward return, and one small oblong
integrated overlap-knot. Confirm no smooth overarching arch or circular knot exists;
the route encloses less than 170 degrees and the exterior-connected opening is at least
155 pixels wide. Confirm the silhouette cannot read as C, g, 9, question mark,
headphones, snake, circle, spiral, refresh, undo, replay, spinner, power, gear, clock,
or arrow. Confirm every visible pixel stays inside [220,300,805,710] and all exterior
pixels are exact uniform #00FF00. Confirm only flat muted dark-earth pigment is present,
...
- 执行前 commit：`c0d41e9`；完整正文版本：`QS-B1 V6-E.r3`；操作：fresh
  regenerate。固定 Image 1／2，无 Image 3；正文 `7865 bytes`、SHA-256
  `981fabe51c77fc9fd2dba527ca787cc8929aa8adc3d24170ee6d6598fa6a40da`；child
  user block 已完整回显首段与 FINAL SELF-CHECK，未上传任何旧候选像素。
- fixed child：`@openai/codex@0.143.0`、`gpt-5.5 / medium`；session
  `019fd614-253c-7b71-adea-0eaaad77f617`；内建 `image_gen` 恰好调用一次，
  provider 图片原样落位，正式计为 V6-E `5/5`、整批 `18/35`；无新增流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-E/attempt-05/raw/QS-B1-V6-E.attempt-05.png`；
  `1254×1254 RGB`；SHA-256
  `6f85cef0c106ab8efa769b3d08c3fed87780d58262eaa8b211412af4a4ac9eb9`。
- 确定性审查：归一化后可见 bbox `[115,316,922,725]`，向左／右越 safe box
  `45／58px`；可见绿色污染 `0`，自动 `8/9`，唯一技术失败为
  `visible_bbox_inside_safe_box`。按用户授权的等比 bbox-fit 可处理该几何溢出；review
  JSON SHA `4afe2f1c297972356537e26c7ad44ccd20d81ae85d48f7af569b0702c7646495`。
- 美术内审：median `[99,47,13]`，暗土色、三笔数量、钝端和宽矮方向均成立；但
  三笔几乎完全水平、间距规则且缺乏路线节奏，微缩后就是标准的三条菜单横栏。
  这直接命中 r3 的 hamburger-menu 否决条件，无法凭 bbox-fit 合同例外晋级。
- runtime 装配：等比 fit 为 `18×9px`，完整落入 `[7,6,25,16]`；六场景
  display-region `6/6 pass`、violations `0`，报告 SHA
  `c2b0ef1f71e30aae3e8a47e0e091cf90711000037ed01085db4733115b1ece07`；真实排版
  SHA `86e93eebe1192be1396075e986c6e489fe76e0fac4dc3188586fd4b951919448`。
- 终态：`candidate-rejected / repair-budget-exhausted / P3 / production 5/5`。
  不得执行 attempt 6，不写 source、runtime、addon 或 accepted manifest。保留
  ignored generated 证据供用户后续决定是否授权新版本或合同改向。
- 下一动作：依整批授权进入 V6-F RESET attempt 1；只上传固定 Image 1／2，无
  Image 3，禁止复用 V6-E 或其他段的任何候选像素。

### QS-B1 V6-F attempt 1 execution and review

- 执行前 commit：`86236a5`；完整正文版本：`QS-B1 V6-F`；操作：generate。
  固定 Image 1／2，无 Image 3；正文 `4724 bytes`、SHA-256
  `e55422df8f3d1c4aa9d6bb75b153c9432445a2b26f0570ea376f1278d2956bb3`；未上传或
  复用 V6-A..E 的任何候选像素。
- fixed child：`@openai/codex@0.143.0`、`gpt-5.5 / medium`；session
  `019fd61c-4f89-7320-959c-17d6e111e298`；完整正文与两图职责已回显，内建
  `image_gen` 恰好调用一次并原样落位，计为 V6-F `1/5`、整批 `19/35`；无新增
  流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-F/attempt-01/raw/QS-B1-V6-F.attempt-01.png`；
  `1254×1254 RGB`；SHA-256
  `8142a8fdc474fd71efc63d92fd9612f98674677b9404735da3b3bef3b52423b3`。
- 确定性审查：归一化后 bbox `[185,213,845,817]` 完整位于 safe box；可见绿色
  污染 `264`，自动 `8/9`，唯一失败为 `visible_green_spill_is_zero`。review JSON
  SHA `a919852acd0e4348b41071d40ac0395def753b0e9b0a66ece59e610745f726ab`。
- 美术内审：整体是近完整的浅金色螺旋，中央为大圆团，外圈连续收紧；median
  `[214,170,94]`、luma p50 `173.582`，明显接近亮金／羊皮而非暗赭矿物颜料。
  真实 `13×12px` 下直接读成字母 g／旋涡／loading 标记，命中 RESET 正文的多项
  明确禁区，既没有开放 hairpin 路线，也没有偏心的小型集成路线结。
- runtime 装配：等比 fit `13×12px`，位于 `[7,5,24,17]`；六场景
  display-region `6/6 pass`、violations `0`，报告 SHA
  `c3f52049b49ba278083e8b2232f3bed265dd75b7fc81f28f236902ebbdd9b6f2`；真实排版
  SHA `0bd110671061bb6de28d31b174f0262a24cbb528bf83e4a292e0dd27704cb8a4`。
- 结论：`internal-rejected / repair-prepared / P3 / 1/5`。身份、结构、颜色和污染
  均为全局问题，不属于局部 edit；attempt 1 禁止作为 Image 3。attempt 2 必须
  fresh regenerate，只上传固定 Image 1／2，禁止跨段像素。
- 下一版必须保持：RESET 路线回返语义、一个连接的平面颜料对象、normal state、
  1024² RGB、精确绿色背景、safe box、无 carrier、runtime content box 与动态排除。
- 下一版只允许修复：改为一次开放 hairpin 回返、偏心小路线结和明确钝头起点；
  杜绝完整圆、螺旋、字母 C／g 和中央大圆团；压暗到克制旧赭，使用少量平面颜料
  色块并消除全部绿色孔洞。
...
quest-log administration menu. This is a flat bitmap pigment sprite source, not a
scene, screenshot, UI mockup, worksheet, atlas, button, ribbon, map, physical object,
material sample, inventory item, or complete interface. Do not use V6-F attempt 1 or
any V6-A, V6-B, V6-C, V6-D, or V6-E generated pixel as an input, edit source, tracing
source, texture donor, or identity reference.
...
wear, and vanilla-era seriousness. Do not copy its book, pages, leather frame, brass,
wax seal, ribbons, text, reward slots, buttons, compass ornaments, or composition.

Image 2 is a secondary micro-scale reference only. Inherit only economical stroke
count, broad readable silhouette, softened handmade perimeter, and legibility after
severe downscaling. Do not copy, trace, rotate, recolor, or rearrange any triangle,
circle, check mark, 2-by-2 layout, location, transparency, or pixel from Image 2.
...
Keep every visible pigment, antialiasing pixel, soft bleed, and edge nick strictly
inside hard safe box [160,160,864,864]. Place the complete emblem inside target envelope
[235,300,790,715], about 555 pixels wide by 415 pixels high. Center its visual mass
slightly left of [512,512]. Leave broad exact-green margin. Use direct orthographic
front view with no perspective, thickness, modeled depth, or directional lighting.

This source owns only one normal-state pigment emblem. It will later be edge-keyed and
isotropically bbox-fitted into content box [7,5,24,17] inside a 32 by 22 runtime Button.
Do not draw that Button, carrier, backing tile, hover, pressed, disabled state, Tooltip,
text, map, menu background, ribbon substrate, or provider logic.

OPEN HAIRPIN ROUTE, NOT A SPIRAL

Paint one connected, stout, uneven expedition route with exactly one broad open return.
It is a route reconstructed back toward a small integrated route knot, not a circular
arrow or a coil. Use this directional sequence:

1. Start with one clearly visible wide blunt beginning in the lower-left quadrant near
   [285,650]. This beginning is not pointed and not feathered.
2. Travel upward and slightly left, then bend once across the upper region toward the
   right, passing roughly through [300,430], [460,340], and [650,385].
3. Make one broad hairpin turn on the right near [730,475]. Return leftward and slightly
   downward toward [585,585].
4. Merge the returning end into one small irregular dense route knot near [525,590].
   The knot is an integrated thickened patch on the same route, not a detached dot and
   not a central disk. Its diameter is only about 90 to 120 normalized-source pixels.

The route may surround at most about 210 degrees of open negative space. Keep a large,
obvious opening between the lower-left starting segment and the returning knot. Do not
continue the route around the knot. Do not wrap inward, make a second revolution, form
a coil, or place the knot at the geometric center. The silhouette should resemble an
irregular open expedition hairpin with a destination blot, never a complete ring,
spiral, letter C, lowercase g, numeral 9, snake, question mark, refresh, undo, replay,
loading spinner, power symbol, gear, or clock.

Use a broad non-constant route width about 58 to 86 normalized-source pixels. Vary it
coarsely along the route; keep the lower-left beginning, right hairpin shoulder, and
integrated knot optically distinct. Use blunt softened turns, not smooth vector arcs.
The route must remain one connected pigment mass, but the inner negative space remains
open to the exterior through a gap at least 125 pixels wide.

FLAT MINERAL PIGMENT, COLOR, AND EDGE

Render the route as a direct imperfect two- or three-pass transfer of matte mineral
guild pigment. It is flat paint on an absent carrier, never parchment, leather, rope,
metal, raised paste, embossed material, carved relief, or a physical trail object.

Use a dominant muted old umber-ochre near RGB [125,82,39], sparse broad worn-ochre
patches near RGB [151,104,52], and a few deep smoked-umber pools near RGB [78,47,25].
Keep median visible color approximately RGB [108,67,31] to [142,91,45]. No bright
yellow, bright orange, polished gold, brass, ivory, white, neon, or emissive light.
...
blunt corners, slight asymmetry, a few coarse open edge nicks, and restrained two-to-
four-pixel pigment bleed. Internal wear is always a lighter opaque ochre patch, never
background green. The entire connected route interior remains pigment colored. Any
missing-pigment notch must be open to the exterior silhouette. No enclosed green
island, hole, crack, isolated speck, or green-tinted antialiasing is allowed.

STRICT EXCLUSIONS

No complete circle, ring, spiral, coil, second revolution, letter C, lowercase g,
numeral 9, question mark, snake, standard refresh arrow, undo arrow, replay triangle,
arrowhead, loading ring, power symbol, gear teeth, clock, compass rose, central disk,
detached dot, map grid, text, letters, numbers, runes, parchment material, paper,
leather, cloth, ribbon, wax, metal backing, button face, border, card, frame, medallion,
rivet, bevel, cast shadow, drop shadow, ambient occlusion, glow, highlight, continuous
gradient, photographic grain, glass, neon, chrome, 3D rendering, photorealism,
procedural noise, perfect symmetry, constant vector line weight, Diablo III ornament,
Skyrim minimalist menu language, or Warhammer iconography. Do not place green inside
pigment or tint an edge green.

FINAL SELF-CHECK

Confirm exactly one connected RESET glyph exists: one blunt lower-left start, one broad
open right-side hairpin return, and one small offset integrated route knot. Confirm the
route surrounds no more than about 210 degrees, the inner negative space has an exterior
opening at least 125 pixels wide, and no second revolution or inward coil exists.
Confirm it cannot read as a circle, spiral, C, g, 9, question mark, snake, refresh,
undo, replay, spinner, power symbol, gear, clock, or arrow. Confirm every visible pixel
stays inside [235,300,790,715] and therefore inside [160,160,864,864]. Confirm the mark
uses only broad flat muted dark-earth pigment regions, with no bright gold, parchment
...
- 执行前 commit：`3080748`；完整正文版本：`QS-B1 V6-G`；fresh generate；固定
  Image 1／2，无 Image 3；正文 `4677 bytes`、SHA-256
  `4f8e51725271a730a0d7b8a87539a3faba393f8d708e3528fbb90f543c3cae12`。
- fixed child：`@openai/codex@0.143.0`、`gpt-5.5 / medium`；session
  `019fd654-a892-7171-a116-9bb3ead07e7e`；内建 `image_gen` 恰好一次并返回图片，
  计为 V6-G `1/5`、整批 `24/35`；无新增流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-G/attempt-01/raw/QS-B1-V6-G.attempt-01.png`；
  `1254×1254 RGB`；SHA-256
  `6adbb3c8243de4ee69b16d06ca0766f23327831cfaab0145d6756b28240870d9`。
- 技术：bbox `[105,337,912,688]`，超出原始 safe box；可见绿色污染 `0`；
  median `[92,31,35]`；自动 `8/9`，仅 safe-box 项失败。review JSON SHA
  `d61c1b472efec2c5f683c853b76c923d464f71de27d1be960ca25e1258aae6fe`；
  technical board SHA
  `0cb499886514cffb64fad091141c76aa4b417b7037ca3202ddab47799238f642`。
- 美术：两半是近乎镜像的宽大弧带，组合后优先读成在顶部断开的圆环／马蹄／
  一对括号；外端、弧度、长度与压力几乎相同，不符合不等长断裂契约绳。大图
  存在明显连续体积明暗和纤维式尖齿，接近实体断带；`16×7px` 下仍保留镜像
  括号语义，无法稳定表达 ABANDON。
- runtime：等比 fit 为 `16×7px`，落入 `[8,8,24,15]`；显示区域 `6/6 pass`，
  报告 SHA `23a1565e8d566e8f81d08949a5c6287c6330a93f27651a47ce210f39eb1bc1da`；
  真实排版 SHA
  `283132dc423e22237fa2cbb57ef6ce20e6b2d855b35dbbef35f5912295aaa0d0`。
- 结论：`internal-rejected / repair-prepared / P3 / 1/5`。safe-box 可由授权
  bbox-fit 处理，但不能覆盖镜像断环身份和实体材质失败；整幅构图必须重画，
  attempt 1 禁止作为 Image 3。attempt 2 fresh，只上传固定 Image 1／2。
...
Do not upload, inspect, edit, trace, imitate, or reuse V6-G attempt 1. Do not use any
V6-A, V6-B, V6-C, V6-D, V6-E, or V6-F generated pixel, silhouette, crop, texture, or
identity reference. The result must be a fresh independent construction guided only by
the two fixed references and the complete text below.
...
emblem inside target envelope [235,350,790,710], approximately 555 pixels wide by 360
pixels high. Center the combined visual mass near [510,530] with broad exact-green
margin. Use direct orthographic front view with no perspective, thickness, modeled
depth, or directional lighting.
...
Construct two short, stout, unequal pigment strokes that imply one old covenant cord
snapped across a quiet diagonal gap. They are not mirror halves and do not follow a
circle. Use this spatial anatomy:

1. The left half begins with a wide blunt outer end near [270,650], bends once near
   [350,565], then rises toward its inner broken end near [455,445]. It is about 260
   pixels long, visibly heavier near the outer third, and slightly bowed rather than
   straight.
2. The right half begins at its inner broken end near [575,505], bends gently through
   [670,550], and finishes at a shorter blunt outer end near [770,660]. It is a little
   shorter, sits about 55 pixels lower at the break, and has a different pressure and
   curvature from the left half.
3. Preserve an obvious diagonal central gap between the two inner ends, about 105 to
   135 pixels wide. The imagined continuation across the gap should feel like one line
   interrupted by a snap, but the visible halves never touch, cross, overlap, or point
   at one another like weapons.
4. Give each inner broken end only two or three broad irregular fiber-notches. Keep
   those notches chunky and short, not hairlike spikes. Keep both outer ends solid,
   blunt, unequal, and unfrayed.

The combined silhouette must be decisively asymmetric. It cannot be reflected across a
vertical or horizontal axis. It must not make a U, arch, horseshoe, broken ring, pair of
parentheses, moustache, wings, halo, eye, X, V, chevron, equals sign, pause bars,
quotation marks, parallel slash pair, wound, weapon, or warning mark. Each half bends
only once; neither curls around an empty center. At runtime around 16 by 8 pixels, the
gap and unequal endpoints remain visible.

FLAT OLD-WINE MINERAL PIGMENT

Render both halves as a direct imperfect two- or three-pass transfer of matte,
low-saturation greyed dark old-wine mineral guild pigment on an absent carrier. This is
flat paint, never physical rope, braided cord, leather thong, ribbon, wet blood, metal,
raised paste, embossed material, carved relief, or a photographed object.

Use dominant greyed dark old-wine near RGB [86,39,43], sparse broad worn-wine patches
near RGB [112,55,55], and a few smoked brown-red pools near RGB [58,31,32]. Keep median
visible color approximately RGB [72,34,37] to [108,50,52]. It must be darker and more
greyed than fresh blood: no bright red, crimson glow, magenta, pink, orange, polished
metal, ivory, white, neon, or emissive light.

Use only a few broad opaque pigment regions. No continuous gradient, center-to-edge
lighting, highlight, rim, dark underside, cast shadow, bevel, thickness, photographic
fiber, rope grain, repeated micro-texture, or specular detail. Use non-constant stamped
edges, softened blunt corners, slight hand pressure variation, one to three coarse open
edge nicks away from the break, and restrained pigment bleed. Internal wear is always
an opaque muted wine value, never background green. Each half has a fully opaque
pigment interior with no enclosed green hole, crack, isolated speck, or green-tinted
antialiasing.

STRICT EXCLUSIONS

No bilateral symmetry, U, arch, horseshoe, broken circle, broken ring, parentheses,
moustache, wings, eye, halo, X, cross, V, chevron, trash can, skull, blood, wound,
weapon, sword, slash pair, pause symbol, quotation marks, equals sign, exclamation mark,
warning sign, physical rope, braid, fibers across the whole length, paper, parchment,
cloth, ribbon, wax, leather, metal backing, button face, border, card, frame, medallion,
rivet, bevel, drop shadow, cast shadow, glow, specular highlight, glass, neon, chrome,
3D rendering, photorealism, procedural noise, perfect vector curves, constant line
weight, Diablo III ornament, Skyrim minimalist menu language, or Warhammer iconography.
Do not place green inside either pigment half or tint an edge green.

FINAL SELF-CHECK

Confirm exactly two visible unequal pigment halves form one semantic ABANDON emblem.
Confirm the left half rises from a blunt lower-left outer end and the shorter right half
falls toward a different lower-right outer end, with one diagonal gap and clearly
offset inner breaks. Confirm the halves never touch, cross, mirror, form a ring, U,
...
- 执行前 commit：`df9ccb0`；完整正文版本：`QS-B1 V6-G.r3`；受限 edit；固定
  Image 1／2，紧邻 attempt 3 raw 是唯一 Image 3；正文 `6350 bytes`、SHA-256
  `eabcd618ad835c8d3d5e97e6037b290b9ae451ff621dbb62c01a7910e6f15865`。
- fixed child：`@openai/codex@0.143.0`、`gpt-5.5 / medium`；session
  `019fd66c-e7bd-76e1-9efc-ea546b1533b0`；内建 `image_gen` 恰好一次并返回图片，
  计为 V6-G `4/5`、整批 `27/35`；无流程错误。
- raw：
  `generated/quests/QUEST-SEALS/QS-B1-V6/V6-G/attempt-04/raw/QS-B1-V6-G.attempt-04.png`；
  `1254×1254 RGB`；SHA-256
  `be702d450ba0e922fc490b18078ab915ed24ce6ea8a3cbcd5b55d6719ca517d7`。
- 技术：bbox `[57,398,970,622]`，超出 safe box；可见绿色污染 `0`；median
  `[107,23,37]`；自动 `8/9`，仅 safe-box 项失败。review JSON SHA
  `31cb0e310bc3b0356d71c033f62617a6243a1e28521c7c3d14abed14a748e435`；
  technical board SHA
  `3529e7baef95b43a2c73316e5d27d0f0e0a1fc7c23186ed15d379fe3b7afe8b9`。
- 冻结审计：左段变为 `[57,398,539,622]`、右段变为 `[580,474,970,615]`；
  相比冻结 `[117,451,544,612]`／`[585,513,916,606]`，两侧外缘分别漂移
  `60px` 与 `54px`，整体宽度由 `799px` 增至 `913px`。这不是保持 x 范围的
  纵向加厚，违反 r3 edit 合同。
- 美术：源图断裂语义和笔触重量较 G3 清楚，中央间隙仍存在；但更宽的构图抵消
  加厚收益，等比运行时仍只有 `16×4px`。综合色更鲜红且连续亮带更明显，也未
  达成克制灰暗旧酒红方向。
- runtime：等比 fit 为 `16×4px`，落入 `[8,9,24,13]`；显示区域 `6/6 pass`，
  报告 SHA `403da2cef8443d2976ab2a7cc7cd756de9f51f9061688d64cdde29a947dfc399`；
  真实排版 SHA
  `b2da5ea798568102d39e0c85fd54725e43a04445906691492aa79416737d7b08`。
- 结论：`internal-rejected / fresh-final-prepared / P3 / 4/5`。attempt 4 违反
  edit 冻结边界，不得成为候选或下一次 Image 3。attempt 5 是最终 fresh，
  只上传固定 Image 1／2；不复用任何旧 G 或跨段像素。

### QS-B1 V6-G.r4 complete production Prompt — fresh-final-prepared / frozen

Use Image 1 and Image 2 only as fixed visual references. Freshly generate exactly one
independent normal-state ABANDON emblem for a circa-2004 vanilla World of Warcraft
quest-log administration menu. This is the final authorized V6-G attempt. It is one
flat bitmap pigment sprite source, not a scene, screenshot, UI mockup, worksheet,
atlas, button, ribbon, physical rope, material sample, inventory item, or complete
interface.

Do not upload, inspect, edit, trace, imitate, or reuse V6-G attempts 1 through 4. Do not
use any V6-A through V6-F generated pixel, silhouette, crop, texture, or identity
reference. Build a fresh independent result from only the two fixed references and
this complete text.

REFERENCE AUTHORITY

Image 1 supplies only old Azeroth expedition-ledger weight: substantial handmade
masses, muted earth color, slight asymmetry, sparse wear, and serious vanilla-era guild
craftsmanship. Do not copy its book, pages, leather, brass, wax, ribbons, text, slots,
buttons, ornaments, or composition. Image 2 supplies only economical stroke count,
softened handmade perimeter, and tiny-sprite legibility. Do not copy, trace, rotate,
recolor, or rearrange its geometry, layout, transparency, or pixels.

CANVAS, COUNT, AND REQUIRED ASPECT

Return one exact square 1024 by 1024 RGB image. Fill every exterior pixel with perfectly
uniform exact #00FF00. The green field has no texture, gradient, lighting, noise,
shadow, paper, cloth, frame, guide, label, checkerboard, or transparency preview.

Draw exactly one semantic ABANDON emblem made from exactly two separated pigment pieces
of one snapped covenant pledge-stroke. Keep all visible and antialiased pixels inside
[160,160,864,864]. Keep the combined emblem inside [245,330,785,700], no wider than
540 pixels and between 300 and 360 pixels high. The combined visible aspect ratio must
be 1.55 to 1.80. Center it near [515,525] with broad green margin. Use orthographic
front view with no perspective, thickness, modeled depth, or lighting.

This normal-state source will be edge-keyed and isotropically fitted into [8,7,24,16]
of a 32 by 22 Button. Its required aspect must produce about 14 to 16 runtime pixels
wide by 8 or 9 high. Draw no Button, carrier, backing, state, Tooltip, text, map, menu,
ribbon substrate, or provider logic.

COMPACT HEAVY PLEDGE-STROKE, SNAPPED AT THE MIDDLE

Paint two compact, thick, unequal pieces that imply one shallow covenant line snapped
across a diagonal central gap. They are broad pigment masses with one gentle bend each,
not long thin ribbons, straps, dashes, or cords.

1. Keep the left piece inside [255,350,485,660]. Start at a solid blunt lower-left
   outer end near [275,610], pass one shallow bend near [360,535], and finish at the
   inner broken face near [475,465]. Use non-constant body thickness 135 to 175 pixels.
2. Keep the right piece inside [555,390,780,690]. Begin at the inner broken face near
   [565,505], pass one flatter bend near [655,555], and finish at a shorter blunt outer
   end near [755,635]. Use body thickness 125 to 165 pixels and different pressure.
3. Preserve a completely empty diagonal center gap from x 485 to 555. The left break is
   about 40 pixels higher than the right. The imagined centerline continues across the
   gap as one interrupted shallow pledge-stroke.
4. Each inner broken face has exactly two or three broad step-like tear notches, 30 to
   50 pixels deep, facing the gap and visible after downscaling. They are blunt torn
   steps, not fibers, spikes, arrowheads, claws, fingers, or teeth. Outer ends are
   solid, blunt, unequal, and unfrayed.

The pieces never touch, cross, overlap, point sharply, or leave their boxes. Their
centerlines stay within about 25 degrees of horizontal. They do not form a peak, roof,
caret, V, chevron, U, arch, horseshoe, broken ring, parentheses, moustache, wings, eye,
halo, X, equals sign, pause bars, quotation marks, slash pair, wound, weapon, or warning
mark. At runtime the first read is one heavy guild pledge-stroke visibly snapped in its
middle, with both pieces and the gap clearly present.

FLAT GREYED DARK OLD-WINE MINERAL PIGMENT

Render both pieces as a direct imperfect transfer of matte low-saturation greyed dark
old-wine mineral pigment on an absent carrier. It is flat paint, never physical rope,
braid, leather thong, ribbon, blood, metal, raised paste, relief, or a photographed
object.

Use exactly three broad opaque value families: dominant old-wine near RGB [78,38,42],
sparse worn-wine near RGB [99,51,52], and a few smoked brown-red pools near RGB
[54,30,31]. Keep median visible color RGB [68,34,37] to [100,49,50]. No bright red,
crimson glow, magenta, violet, pink, orange, metal, ivory, white, neon, or emission.

Use only two to five large flat pigment regions per piece. No continuous shading,
lengthwise highlight, center-to-edge lighting, rim, underside, cast shadow, bevel,
thickness, photographic fiber, rope grain, repeated micro-texture, or specular detail.
Use non-constant stamped edges, softened blunt corners, slight pressure variation, one
coarse open edge nick per piece away from the break, and restrained bleed. Interior
wear is opaque wine pigment, never green. No enclosed green hole, crack, isolated
speck, or green-tinted antialiasing.

STRICT EXCLUSIONS

No object wider than 540 pixels, source aspect outside 1.55 to 1.80, long thin dash,
steep diagonal, peak, roof, caret, V, chevron, symmetry, U, arch, horseshoe, broken
circle, ring, parentheses, moustache, wings, eye, halo, X, cross, trash can, skull,
blood, wound, weapon, slash pair, pause, quotations, equals, exclamation, warning,
arrowhead, claw, fingers, physical rope, braid, paper, parchment, cloth, ribbon, wax,
leather, metal backing, button, border, frame, medallion, bevel, shadow, glow, highlight,
gradient, glass, neon, chrome, 3D, photorealism, procedural noise, perfect vector curves,
constant line weight, Diablo III ornament, Skyrim minimalist language, or Warhammer
iconography. Do not place green inside pigment or tint an edge green.

FINAL SELF-CHECK

Confirm exactly two compact unequal old-wine pieces form one snapped ABANDON pledge-
stroke. Confirm the combined bbox is at most 540 pixels wide, 300 to 360 high, aspect
1.55 to 1.80, yielding about 14–16 by 8–9 runtime pixels. Confirm the left piece stays
inside [255,350,485,660], the right inside [555,390,780,690], and [485,555] stays empty.
Confirm the break is visible and the mark does not read as dashes, pause, V, chevron,
arch, ring, parentheses, X, wound, weapon, or warning. Confirm only broad flat matte
greyed old-wine pigment exists, with no bright red, gradient, violet highlight,
...
授权元数据后的授权快照 SHA-256 为
`89b24fc18109593b28f40b750fea96da6f65e323c1fcc3832bfd8aeed9b39192`；追加
attempt 1 账本与 repair gate 后的当前机器合同 SHA-256 为
`b9c81296a62b83064f82a2a43d154c9800875f6b15421ffe9e8393328762c6eb`；追加
attempt 2 与一次不计额度的 npm 流程错误后，当前机器合同 SHA-256 为
`b8f6c8b5360c5c8c036a77bc75e5ac91268f42d5fdf4a1df71c65e55a8cc32ce`。固定输入
职责、确定性装配合同与用户授权边界均未改变。追加 attempt 3 后的当前机器
合同 SHA-256 为
...
- 执行前 commit：`d96c7bc`；完整正文：`QS-B1 V7-A final production`；
  fixed Image 1／2 fresh generate，无 Image 3。child prompt SHA-256
  `83cd06869f7f2432dac6193433b3f0feb05dadd2868f9485c1c810badfa7aad9`。
- fixed child：`@openai/codex@0.143.0`，session
  `019fd6c3-9919-7880-bd06-4c4dd95dc356`；内建 ImageGen 返回一张图片，计入
  V7-A `1/5`；流程错误 `0`。
- raw：`generated/quests/QUEST-SEALS/QS-B1-V7-A/attempt-01/raw.png`；
  `1254×1254 RGB`；SHA-256
  `5db0c14af577c1e3aee230a47c8e2475a7517e441d99efd3f59ebe7f10804ea4`。
- 确定性技术门禁：`13/13 pass`；真实排版几何：`29/29 pass`；display-region：
  `6/6 pass`、violations `0`。review JSON SHA-256
  `6c2687fc7e30291e8350b704ac79e6cd8ed51c874153428145fcf1780f772a8a`；
  display report SHA-256
  `71727963836a73cc57a98163b45094d48bdad9a0f9415d93e86b134e54160b63`；
  real-layout SHA-256
  `5792fe490cc6de6df3ebe7b0e785d54feaa976b719b98c029107c0acb8c09977`。
- 固定 crop `[448,128,576,896]` 为 `128×768`；综合色均值
  `[145.02,109.12,53.67]`，明显偏金黄／橙褐，高于已确认的烟熏旧骨褐主色；
  high-frequency luma residual `8.11`。crop 与 `32×192` 运行时都存在连续
  纵向接缝／折线和均匀细密的照片式或程序式麻布微纹，破坏三至五块宽阔
  手绘明暗面的 2004 年代位图语言。
- 第一失败门禁：`art-inheritance-and-crop-safe-material-composition`。技术
  crop、tracked mask、QS-A1 Alpha 接触压暗、透明 RGB 清零、动态收短、
  七 Button 和六场景装配都正确，但它们不能抵消材质美术失败。
- 结论：`internal-rejected / repair-prepared / P3 / 1/5`。没有可冻结的正确
  材质区；attempt 2 必须 fresh regenerate，只上传固定 Image 1／2，不上传、
  不编辑、不复用 attempt 1。没有写入 source、runtime、atlas 或 addon。

### QS-B1 V7-A.r1 complete production Prompt — attempt 2 fresh repair

状态：`repair-prepared / authorized-by-bounded-loop`。这是 attempt 2 的完整、
自包含执行正文。它保持用户授权的 V7-A 组件、固定输入、deterministic crop、
mask、接触压暗、runtime 和真实排版边界，只加强 attempt 1 首个失败门禁。
attempt 2 只上传固定 Image 1／2；禁止上传 attempt 1 作为 Image 3。

> Create exactly one edge-to-edge square orthographic hand-painted material donor for
> the narrow oath carrier of an Azeroth expedition guild wax seal. The output is only
> one continuous material field. It is not a finished ribbon, purity-seal object, UI
> button, prop, fabric sample photographed in a studio, or complete interface. Fill
> the entire square with one coherent, matte, moderately stiff aged hybrid dominated
> by compressed vellum with only sparse coarse-linen fibres. Do not draw an exterior
> background, transparent area, isolated silhouette, torn outline, wax, contact
> shadow, stamp, emblem, icon, writing, rune, border, hole, button, or state.
>
> This is a fresh regeneration. Use only Image 1 and Image 2 as the fixed references
> described below. Do not upload, inspect, edit, trace, imitate, or reuse any pixel,
> crop, seam, weave, colour identity, or composition from V7-A attempt 1 or any earlier
> generated candidate.
>
> IMAGE AUTHORITY AND INHERITANCE
>
> Image 1 is the highest visual authority. Inherit its circa-2004 vanilla World of
> Warcraft two-dimensional hand-painted bitmap language: three to five broad readable
> light, middle, and shadow masses; slightly imperfect manual brushwork; subdued warm
> upper-left illumination; muted expedition-document colour; substantial material
> weight; and coarse, selectively placed wear that survives low-resolution display.
> Ignore the complete open book, page geometry, leather cover, brass parts, wax,
> bookmarks, text, buttons, rewards, and every finished-object silhouette in Image 1.
>
> Image 2 is secondary and material-only. Inherit only its quiet field-document age,
> a few sparse broad grey-brown handling stains, restrained broken fibre cues, and
> long-used expedition-record character. Ignore the tracker shape, leather strap,
> compass, quill, rivets, layered pages, text, buttons, and rectangular layout. If the
> references conflict, Image 1 controls painting era, broad value grouping, palette,
> upper-left light, material weight, and non-photographic finish. Image 2 contributes
> only sparse low-frequency ageing and occasional broken fibres.
>
> FIXED CROP-SAFE COMPOSITION
>
> The whole square is one uninterrupted flat material surface. A deterministic process
> will isotropically normalize the returned square to 1024 by 1024, then use only x 448
> through 576 and y 128 through 896. Treat this central 128 by 768 extraction band as
> the most important region of the image. It must be calm, seamless, representative,
> and useful from top to bottom. Broad painted masses must cross both sides of this
> band; the band must never coincide with a boundary between masses.
>
> Place no vertical seam, centre crease, fold, stitched join, fabric panel boundary,
> gutter, lengthwise cord, full-height fibre bundle, dark column, bright column, or
> colour transition anywhere through that crop. Place no horizontal seam, folded hem,
> join, band, row, action-slot rhythm, or abrupt value step through it. Do not divide
> the canvas into panels, strips, squares, lanes, folds, tiles, or woven sections. Do
> not mirror the left and right halves. Do not create a central spotlight, focal stain,
> hole, symbol, full-height gradient, repeated 22-pixel rhythm, or periodic weave. When
> the fixed crop is reduced to 32 by 192, it must still read as one old hand-painted
> material plane, never as a vertically striped or folded ribbon.
>
> DARKER SMOKED OLD-BONE PALETTE
>
> The first read must be muted smoked old-bone umber: an aged field-document brown with
> grey in it, not yellow, mustard, ochre-gold, orange, honey, caramel, tan fabric, or
> bright parchment. Across the important central crop, keep the dominant visual colour
> close to RGB [96,78,54] through [116,94,64]. Use broad shadowed compressed areas near
> RGB [52,40,30] through [70,54,39], a small amount of faded warm old-bone no brighter
> than RGB [132,106,72], and a few quiet grey-brown stains. Bright warm pixels above
> about RGB [140,112,76] must be rare accents, never the dominant field. Avoid the
> previous gold-orange family around RGB [145,109,54]. Keep saturation low and the
> value range substantial but restrained. It must not read as clean beige paper,
> golden cloth, orange burlap, yellow suede, near-black leather, or polished wood.
>
> BROAD HAND-PAINTED SURFACE, NOT PHOTOGRAPHIC WEAVE
>
> Construct the entire square from only three to five large asymmetric matte value
> regions. Each region should span roughly one quarter to one half of the canvas and
> blend through broken hand-painted transitions rather than straight boundaries. Add
> only two to four broad, interrupted compression marks and only a few blunt grey-brown
> handling stains. Reserve large quiet areas. The centre extraction band must contain
> broad low-frequency variation, not dense detail.
>
> Linen is only a sparse secondary cue. Show at most a few coarse broken fibre bundles
> in the whole square. Every fibre cue must be broad enough for an old game bitmap,
> irregularly spaced, softly hand-painted, and stop or dissolve quickly. No fibre,
> crease, scratch, or compression mark may run more than about one quarter of the
> canvas height. Do not cover the surface with fine threads, tight crosshatching,
> uniform nubs, tiny curls, all-over grain, repeated loops, regular warp and weft,
> photographic burlap, canvas, felt fuzz, procedural noise, embossed pores, leather
> grain, or wallpaper texture. At thumbnail size, the image must still show broad
> painted planes rather than a noisy textile photograph.
>
> MATERIAL WEIGHT AND LIGHT
>
> Make the material visibly firmer and more compressed than soft cloth, felt, velvet,
> or a hanging celebration ribbon. Compressed vellum controls the surface; coarse linen
> appears only as sparse embedded fibres. The donor should plausibly become a narrow
> medieval Azeroth guild oath strip beneath a heavy wax seal, but it remains an empty
> material field with no finished outline.
>
> Render orthographically with no perspective, thickness, cast shadow, edge lighting,
> or studio setup. Use Image 1's subdued upper-left warmth only as wide, broken painted
> masses. Never create a full-height highlight, full-height shadow, glossy streak,
> centred light beam, rim light, bevel, metallic edge, relief, or 3D fold. The surface
> is matte and hand-painted. Wear is old, sparse, blunt, and accumulated by handling;
> no fresh cuts, dramatic cracks, holes, burned edges, blood, or luminous magic.
>
> STRICT EXCLUSIONS
>
> No object silhouette, ribbon outline, torn outer edge, pointed tail, sawteeth,
> fishtail, fringe, tassel, wax, seal, medallion, emblem, motif, text, rune, prayer,
> icon, button, state, border, frame, card, book, page edge, leather, wood, metal,
> rivet, brass, transparent HUD glass, modern toolbar, Diablo III ornament, Skyrim
> minimalism, neon, glow, chrome, satin, velvet, clean canvas, macro photograph,
> photoreal material scan, procedural texture, repeating microtexture, tile, straight
> seam, stitched join, centre crease, vertical fold, horizontal fold, full-canvas
> directional stripe, perfect symmetry, or regular weave.
>
> Do not depict or copy Warhammer or any other franchise identity: no aquila,
> double-headed eagle, skull, armour, devotional wording, prayer text, purity-seal
> silhouette, science-fiction metal, heraldic badge, or religious insignia. Borrow no
> copyrighted symbol or finished-object design. The result remains an original Azeroth
> expedition-document material painted in the locked vanilla-era visual language.
>
> FINAL SELF-CHECK
>
> Confirm the output is exactly one square edge-to-edge matte material donor and
> nothing else. Confirm the central x 448..576, y 128..896 region is intentionally calm
> and seamless from top to bottom, with no vertical or horizontal join, no centre line,
> no full-height value band, no fold system, and no periodic weave. Confirm its dominant
> colour is subdued greyed smoked old-bone around RGB [96,78,54] to [116,94,64], not
> the rejected gold-orange around [145,109,54]. Confirm the whole image uses only three
> to five broad hand-painted value masses, large quiet areas, sparse short broken fibre
> cues, and no dense photographic or procedural micrograin. Confirm there is no object
> silhouette, transparency, wax, emblem, text, button, border, tail, torn outline,
> cast shadow, bright cream, golden cloth, orange burlap, near-black leather, metallic
> shine, modern HUD language, or Warhammer iconography.

### QS-B1 V7-A attempt 2 execution and internal rejection

- 调用前先发生一次不计额度流程错误：相同 V7-A.r1 固定 child 在 provider
  启动前因沙箱拒绝 npm registry／cache 访问返回 EPERM；没有 provider
  generation 证据、没有图片、没有输出文件，因此不占生图额度。原样重试正文，
  未修改提示词或输入。
- 执行前 commit：4eee370；完整正文：QS-B1 V7-A.r1；fixed
  Image 1／2 fresh regenerate，无 Image 3。child prompt SHA-256
  bcdd6ec5358cc9ee25702c4dc6e4c80bf212e1dd66587f4a5098237403ef4c82。
- fixed child：@openai/codex@0.143.0，session
  019fd6d3-fcac-7be0-9db6-d8c1711230a5；内建 ImageGen 返回一张图片，计入
  V7-A 2/5；累计流程错误 1。
- raw：generated/quests/QUEST-SEALS/QS-B1-V7-A/attempt-02/raw.png；
  1254×1254 RGB；SHA-256
  5b57f1c22c48f47a8e136b7d2c48ae20038e5feb3a7a065c046dd8602045e16b。
- 技术门禁：13/13 pass；真实排版几何：29/29 pass；display-region：
  6/6 pass、violations 0。review JSON SHA-256
  4af685a57b33ebf1b489745393f0275d228076abdc7c118ffe875ba70e62d187；
  display report SHA-256
  0abaeec1966547dc3631bba560d1a1df1fd15ee915962132c14350f766102372；
  real-layout SHA-256
  23910c4e9348d3dbbad967c2ccf6aa372d20f87e23e776354da9a8dcb4ef3d6b。
- 相比 attempt 1，固定 crop 的最大相邻列跳变由 3.05 降为 1.44，
  最大相邻行跳变由 10.29 降为 3.72，high-frequency residual 由
  8.11 降为 5.08；全高纵缝已消失，三至五块宽阔低频明暗结构与
  crop 连续性可以冻结。
- 仍失败的第一门禁：
  muted-smoked-old-bone-palette-and-hand-painted-surface-scale。crop 均值
  [141.16,106.76,57.52]，仍明显是金黄／焦糖旧布而不是灰暗烟熏旧骨褐；
  近看仍有均匀细密的程序式或照片式微纹，运行时读成浅棕装饰带，缺少沉重
  誓约文书感。
- 结论：internal-rejected / edit-repair-prepared / P3 / 2/5。attempt 3
  只冻结 attempt 2 的 edge-to-edge 单材质身份、宽阔非对称明暗拓扑、无接缝
  中央 crop 与低频污渍位置；只允许在这些边界内重新授色并覆盖密集微纹。
  固定 Image 1／2 加紧邻 attempt 2 raw 作为唯一 Image 3。禁止使用 attempt 1
  或任何 V5／V6／V17 像素。没有写入 source、runtime、atlas 或 addon。

### QS-B1 V7-A.r2 complete production Prompt — attempt 3 local edit

状态：repair-prepared / authorized-by-bounded-loop。这是 attempt 3 的完整、
自包含执行正文。固定 Image 1／2 继续提供美术权威；紧邻 attempt 2 raw 是唯一
Image 3，只用于保留已正确的连续构图并修复配色与表面尺度。禁止 fresh
redesign、禁止改变组件或确定性装配合同。

> Edit Image 3 into exactly one edge-to-edge square orthographic hand-painted material
> donor for the narrow oath carrier of an Azeroth expedition guild wax seal. Image 3
> is the immediately preceding V7-A attempt and is the target canvas. Preserve its
> single uninterrupted edge-to-edge surface, its broad asymmetric value topology, its
> large quiet central field, and the absence of any vertical or horizontal seam.
> Change only two connected surface properties across that same material: replace the
> gold-caramel colour family with darker greyer smoked old-bone umber, and repaint the
> dense uniform micrograin into sparse broad hand-painted material variation.
>
> The output remains only one continuous material field. It is not a finished ribbon,
> purity-seal object, UI button, prop, fabric photograph, or complete interface. Keep
> the entire square filled by one coherent matte moderately stiff hybrid dominated by
> compressed vellum with only sparse embedded coarse-linen fibres. Do not create an
> exterior background, transparency, isolated silhouette, torn outline, wax, contact
> shadow, stamp, emblem, icon, writing, rune, border, hole, button, or state.
>
> REFERENCE AUTHORITY AND EDIT BOUNDARY
>
> Image 1 remains the highest visual authority. Inherit its circa-2004 vanilla World
> of Warcraft two-dimensional hand-painted bitmap language: broad readable light,
> middle, and shadow masses; imperfect manual brushwork; subdued upper-left warmth;
> muted expedition-document colour; substantial material weight; and coarse selective
> wear that survives low-resolution display. Ignore its complete book, pages, leather,
> brass, wax, bookmarks, text, buttons, rewards, and every finished-object silhouette.
>
> Image 2 is secondary and material-only. Inherit only its quiet field-document age,
> sparse broad grey-brown handling stains, restrained broken fibre cues, and long-used
> expedition-record character. Ignore its tracker shape, strap, compass, quill, rivets,
> layered pages, text, buttons, and rectangular layout. If Image 1 and Image 2 conflict,
> Image 1 controls era, broad value grouping, palette discipline, light, material
> weight, and non-photographic finish.
>
> Image 3 controls only the already-correct spatial composition: one edge-to-edge
> surface; three to five broad asymmetric value masses; large quiet areas; no centre
> seam, panel join, fold system, horizontal band, tiled layout, silhouette, or empty
> exterior. Preserve those broad regions in place. Do not move, rotate, mirror, crop,
> reframe, introduce a new focal object, or redesign the value topology. Image 3 does
> not control its rejected gold-caramel hue or its rejected dense microtexture; repaint
> both across the full surface.
>
> FIXED CENTRAL CROP MUST REMAIN SEAMLESS
>
> A deterministic process will isotropically normalize the returned square to 1024 by
> 1024, then use only x 448 through 576 and y 128 through 896. Preserve Image 3's calm,
> uninterrupted central 128 by 768 extraction band. Keep its broad masses crossing
> both sides of the band. Do not add a vertical seam, centre crease, stitched join,
> panel boundary, gutter, cord, full-height fibre, dark column, bright column, colour
> boundary, horizontal seam, hem, row, abrupt value step, focal stain, hole, symbol,
> spotlight, full-height gradient, repeated 22-pixel rhythm, or periodic weave.
>
> After reduction to 32 by 192, the crop must read as one substantial old hand-painted
> material plane. It must not read as a striped ribbon, folded tape, golden cloth,
> modern toolbar, or repeated fabric tile.
>
> TARGET PALETTE — GREYED, DARKER, AND LESS SATURATED
>
> Repaint the complete Image 3 surface away from gold, mustard, ochre-yellow, orange,
> honey, caramel, and yellow suede. The first read must be greyed smoked old-bone umber:
> an old expedition-document brown with a noticeable neutral grey component. In the
> important central crop, aim for a dominant average near RGB [104,86,62], with most
> middle material between RGB [94,78,57] and [116,96,70]. Use broad compressed shadow
> masses around RGB [50,42,34] to [70,58,46], and very sparse faded old-bone accents no
> brighter than RGB [128,108,82]. Raise the blue component relative to the rejected
> Image 3 average [141,107,58], while lowering red and overall saturation. No large
> area may retain Image 3's yellow-gold or caramel identity.
>
> The material must remain visibly distinct from the brighter quest page behind it:
> darker, greyer, firmer, and more smoked. It must also remain lighter and more fibrous
> than near-black leather. Avoid clean beige, cream parchment, golden fabric, orange
> burlap, yellow canvas, polished wood, and black cloth.
>
> REPAINT MICROGRAIN INTO BROAD MANUAL PLANES
>
> Preserve Image 3's three to five large value regions, but paint over its dense,
> uniform, all-over micrograin. Replace repeated tiny curls, pores, nubs, and weave with
> broad matte brush masses that are legible at thumbnail size. Each main region should
> read as a manually painted plane spanning roughly one quarter to one half of the
> canvas, with soft broken transitions rather than straight edges.
>
> Retain only a few sparse, deliberately placed coarse fibre cues. Each fibre bundle
> must be wide enough for a 2004-era game bitmap, irregular, hand-painted, interrupted,
> and shorter than one quarter of the canvas height. Reserve large smooth quiet areas
> between them. Use at most two to four broad compression marks and a few blunt
> grey-brown handling stains. No fine threads, crosshatching, uniform nubs, all-over
> grain, repeated loops, regular warp and weft, photographic burlap, canvas scan, felt
> fuzz, leather pores, embossed texture, procedural noise, or wallpaper pattern.
>
> MATERIAL WEIGHT AND LIGHT
>
> Keep the material firmer and more compressed than soft cloth, felt, velvet, or a
> hanging celebration ribbon. Compressed vellum controls the surface; linen is only a
> sparse embedded secondary cue. It should plausibly become a narrow Azeroth guild oath
> strip beneath a heavy wax seal, while remaining an empty donor field with no outline.
>
> Preserve Image 3's broad orthographic illumination topology, but align its paint with
> Image 1's subdued upper-left warmth. Express light only as wide broken matte planes.
> No perspective, thickness, cast shadow, full-height highlight, full-height shadow,
> glossy streak, centred beam, rim light, bevel, metallic edge, relief, or 3D fold.
> Wear is old, sparse, blunt, and accumulated by handling; no fresh cuts, dramatic
> cracks, holes, burned edges, blood, or luminous magic.
>
> STRICT EXCLUSIONS
>
> No new composition, object silhouette, ribbon outline, torn outer edge, pointed tail,
> sawteeth, fishtail, fringe, tassel, wax, seal, medallion, emblem, motif, text, rune,
> prayer, icon, button, state, border, frame, card, book, page edge, leather, wood,
> metal, rivet, brass, transparent HUD glass, modern toolbar, Diablo III ornament,
> Skyrim minimalism, neon, glow, chrome, satin, velvet, clean canvas, macro photograph,
> photoreal material scan, procedural texture, repeating microtexture, tile, straight
> seam, stitched join, centre crease, vertical fold, horizontal fold, full-canvas
> stripe, perfect symmetry, or regular weave.
>
> Do not depict or copy Warhammer or any other franchise identity: no aquila,
> double-headed eagle, skull, armour, devotional wording, prayer text, purity-seal
> silhouette, science-fiction metal, heraldic badge, or religious insignia. The result
> remains an original Azeroth expedition-document material in the locked vanilla-era
> visual language.
>
> FINAL SELF-CHECK
>
> Confirm the result preserves Image 3's one edge-to-edge surface, broad asymmetric
> value topology, large quiet areas, and completely seamless central x 448..576,
> y 128..896 band. Confirm no seam, fold system, panel boundary, stripe, tile, object,
> silhouette, or transparency was added. Confirm the rejected gold-caramel family is
> replaced across the entire material by greyer darker smoked old-bone, with the central
> crop visually near RGB [104,86,62] rather than [141,107,58]. Confirm dense uniform
> micrograin has been painted over into three to five broad matte hand-painted planes
> with only sparse short coarse fibres. Confirm there is no wax, emblem, text, button,
> border, tail, cast shadow, bright cream, golden cloth, orange burlap, near-black
> leather, metallic shine, modern HUD language, or Warhammer iconography.

### QS-B1 V7-A attempt 3 execution and internal rejection

- 执行前 commit：8af3a60；完整正文：QS-B1 V7-A.r2；fixed Image 1／2，
  紧邻 attempt 2 raw 是唯一 Image 3。child prompt SHA-256
  745d6c67b0f6dba1ad5dbeb70f229aec72d88cbdbf469624833e3c5abd02bc00。
- fixed child：@openai/codex@0.143.0，session
  019fd6e0-06c5-76b0-a8fe-4c18b214fc49；内建 ImageGen 返回一张图片，计入
  V7-A 3/5；累计流程错误仍为 1。
- raw：generated/quests/QUEST-SEALS/QS-B1-V7-A/attempt-03/raw.png；
  1254×1254 RGB；SHA-256
  7c8d13573bbb2ec557cf60d8a3a2bd4cd45051677d1cc90dde6bf4767a08a1c5。
- 技术门禁 13/13 pass；真实排版几何 29/29 pass；display-region
  6/6 pass、violations 0。review JSON SHA-256
  be669804dc0c0ed87b9f4c334e9d17e72ea86f6e48c974f5a91eefba8998d32e；
  display report SHA-256
  dd2646a33eab5d77fdb78dcf06071d3db702b3a9c1e2eb650c1605775c928bf4；
  real-layout SHA-256
  89d16de51fb0d7ab6624da7198295cef4aae7c3b72621cbe7bbda9ed3f2ae6de。
- 配色门禁已通过：fixed crop 均值 [102.68,86.25,63.73]，与 r2 目标
  [104,86,62] 基本一致；最大相邻列／行跳变 0.80／2.14，无纵缝、横缝或
  条带分区；high-frequency residual 3.06，技术尺度继续下降。
- 第一失败门禁：non-repeating-hand-painted-surface-language。尽管数值残差
  低，raw、128×768 crop 和 4× runtime review 都能看到覆盖全图的规则小环／
  压花状重复单元；这是低对比但高规律的程序式壁纸纹，不是稀疏粗纤维或
  香草时代宽阔手绘面。
- 结论：internal-rejected / edit-repair-prepared / P3 / 3/5。attempt 4
  冻结 attempt 3 的 edge-to-edge 单面材质、宽阔明暗拓扑、无缝中央 crop、
  灰暗旧骨褐配色与污渍位置；唯一修复是用不重复的大笔触覆盖所有规则压纹。
  固定 Image 1／2 加紧邻 attempt 3 raw 作为唯一 Image 3。不得上传 attempt 2
  或更旧候选。没有写入 source、runtime、atlas 或 addon。

### QS-B1 V7-A.r3 complete production Prompt — attempt 4 local flattening edit

状态：repair-prepared / authorized-by-bounded-loop。这是 attempt 4 的完整、
自包含执行正文。它只修复 attempt 3 的规则重复压纹；已通过的综合色、宽阔
明暗结构、中央 crop 连续性和单材质身份全部冻结。

> Edit Image 3 into exactly one edge-to-edge square orthographic hand-painted material
> donor for the narrow oath carrier of an Azeroth expedition guild wax seal. Image 3
> is the immediately preceding V7-A attempt and the only edit target. Preserve its
> complete edge-to-edge composition, its three to five broad asymmetric value masses,
> its seamless calm central field, its current greyed smoked old-bone palette, its
> broad stains, and its overall light topology. Perform one narrowly bounded repair:
> completely paint over and remove every instance of the small repeating embossed-loop
> pattern and every other regular microtexture. Replace that pattern with large quiet
> matte hand-painted planes and only a few isolated, non-repeating coarse fibre marks.
>
> The output is still only one continuous flat material field. It is not a finished
> ribbon, purity-seal object, UI button, prop, photographed sample, or interface. Fill
> every exterior pixel with the same coherent aged compressed-vellum material. Linen
> is only implied by a handful of isolated embedded fibres; there is no woven surface.
> Do not create transparency, an object silhouette, torn outline, wax, contact shadow,
> stamp, emblem, icon, writing, rune, border, hole, button, or state.
>
> REFERENCE AUTHORITY AND FROZEN EDIT BOUNDARY
>
> Image 1 is the highest visual authority for circa-2004 vanilla World of Warcraft
> two-dimensional hand-painted bitmap language: broad readable light, middle, and
> shadow masses; imperfect manual brushwork; subdued upper-left warmth; muted
> expedition-document colour; substantial material weight; and coarse selective wear.
> Ignore its complete book, pages, leather, brass, wax, bookmarks, text, buttons,
> rewards, and all finished-object silhouettes.
>
> Image 2 is secondary and material-only. Inherit only its quiet field-document age,
> sparse broad grey-brown handling stains, a few broken fibre cues, and long-used
> expedition-record character. Ignore its tracker shape, strap, compass, quill, rivets,
> layered pages, text, buttons, and layout. Image 1 controls era, value grouping,
> palette discipline, light, weight, and non-photographic finish.
>
> Image 3 controls and freezes the spatial composition and colour: keep its edge-to-edge
> surface, the position and extent of its broad value masses, the current muted greyed
> smoked old-bone family around RGB [102.68,86.25,63.73] in the central crop, its dark
> broad corner regions, its large quiet centre, and the absence of seams or panels.
> Do not move, rotate, mirror, crop, reframe, relight, recolour, add a focal object, or
> redesign those broad regions. Image 3 does not control the rejected repeating small
> loop texture. Erase that pattern everywhere, including inside shadows and stains.
>
> COMPLETE REMOVAL OF REGULAR MICROTEXTURE
>
> Cover the full Image 3 surface with broad opaque manual brush passes that preserve
> its existing values and colours while destroying the repeated small pattern. At
> 1024 by 1024 viewing size, there must be no grid, loop, curl, hook, pore, pebble,
> stamped cell, embossed unit, crosshatch, warp, weft, stipple, repeated brush stamp,
> regular grain, tiled noise, or texture motif that appears twice. Do not merely lower
> the contrast of the pattern; remove its geometry completely.
>
> The surface should be mostly quiet. Use only three to five broad matte value planes,
> each spanning roughly one quarter to one half of the canvas. Their transitions may
> be soft and broken, never straight or periodic. Across the entire square, add no more
> than six to ten isolated coarse fibre or compression marks. Each mark must be unique,
> broad, blunt, hand-painted, irregularly oriented, separated by large empty material
> areas, and shorter than one sixth of the canvas height. Do not create an all-over
> texture to communicate material. Material must be conveyed through broad colour,
> weight, sparse wear, and a few unique fibres.
>
> Avoid photographic burlap, canvas scan, felt fuzz, leather pores, stucco, stone,
> carpet, knitted loops, lace, paper grain scan, procedural noise, sharpened grain,
> wallpaper, embossing, and any repeated mark. Avoid uniform blur or plastic airbrush;
> retain broad imperfect brush boundaries and a few deliberate coarse painted scars.
>
> FIXED CENTRAL CROP
>
> A deterministic process will isotropically normalize the returned square to 1024 by
> 1024 and use x 448 through 576 and y 128 through 896. Preserve Image 3's current
> seamless central 128 by 768 band and its broad value continuity. In that entire band
> there must be no micro-pattern, seam, centre crease, stitched join, panel boundary,
> gutter, cord, full-height fibre, dark column, bright column, horizontal band, abrupt
> step, focal stain, hole, symbol, spotlight, full-height gradient, 22-pixel rhythm, or
> periodic weave. Reduced to 32 by 192, it must read as one solid old hand-painted oath
> material, not a striped ribbon, folded tape, patterned textile, or toolbar.
>
> PALETTE AND MATERIAL WEIGHT — PRESERVE
>
> Preserve the accepted Image 3 central colour near RGB [102.68,86.25,63.73]. Most
> middle material should remain between RGB [94,78,57] and [116,96,70], broad shadows
> around RGB [50,42,34] to [70,58,46], and sparse old-bone accents no brighter than
> RGB [128,108,82]. Keep it greyed, smoked, dark, and low-saturation. Do not drift back
> to gold, mustard, yellow, orange, honey, caramel, clean beige, bright cream, or
> near-black leather.
>
> Keep the material firmer and more compressed than soft cloth, felt, velvet, or a
> hanging celebration ribbon. It is primarily aged compressed vellum, with only a few
> embedded fibre clues. Preserve orthographic broad upper-left warmth without
> perspective, thickness, cast shadow, full-height highlight, glossy streak, centred
> beam, rim light, bevel, metallic edge, relief, or 3D fold. Wear remains old, sparse,
> blunt, and accumulated by handling.
>
> STRICT EXCLUSIONS
>
> No repeating loop, hook, curl, cell, grain, weave, crosshatch, pore, stipple, grid,
> embossing, knitted texture, wallpaper, procedural noise, photographic scan, object
> silhouette, ribbon outline, torn outer edge, pointed tail, sawteeth, fishtail, fringe,
> tassel, wax, seal, medallion, emblem, text, rune, icon, button, border, frame, book,
> page edge, leather, wood, metal, rivet, brass, modern HUD glass, modern toolbar,
> Diablo III ornament, Skyrim minimalism, neon, glow, chrome, satin, velvet, straight
> seam, centre crease, vertical fold, horizontal fold, full-canvas stripe, or symmetry.
>
> Do not depict or copy Warhammer or any other franchise identity: no aquila,
> double-headed eagle, skull, armour, devotional wording, prayer text, purity-seal
> silhouette, science-fiction metal, heraldic badge, or religious insignia. The result
...
> FINAL SELF-CHECK
>
> Confirm Image 3's edge-to-edge composition, broad value masses, seamless central
> crop, greyed smoked old-bone palette, and broad stains remain in the same positions.
> Confirm every repeated embossed-loop and regular microtexture unit has been fully
> painted over, not merely dimmed. Confirm the surface is mostly quiet and consists of
> only three to five broad matte hand-painted planes plus at most six to ten isolated
> unique coarse marks separated by large calm areas. Confirm the central
> x 448..576, y 128..896 crop has no pattern, seam, stripe, fold, or periodic rhythm.
> Confirm no object, silhouette, transparency, wax, emblem, text, button, border, tail,
> golden cloth, orange burlap, near-black leather, metallic shine, modern HUD language,
> or Warhammer iconography was introduced.

### QS-B1 V7-A attempt 4 execution and internal rejection

- 执行前 commit：2f067d9；完整正文：QS-B1 V7-A.r3；fixed Image 1／2，
  紧邻 attempt 3 raw 是唯一 Image 3。child prompt SHA-256
  217aa7d029ff931f1b1c5334e287d7548a93c3732f6af106469a7512fe3961b7。
- fixed child：@openai/codex@0.143.0，session
  019fd6eb-0c6b-7b73-9b36-4e204d0b8a0c；内建 ImageGen 返回一张图片，计入
  V7-A 4/5；累计流程错误仍为 1。
- raw：generated/quests/QUEST-SEALS/QS-B1-V7-A/attempt-04/raw.png；
  1254×1254 RGB；SHA-256
  22e2aafb208c85d52d575a58fe3babdb93362fc7c082b135f1b5ee367d86c786。
- 技术门禁 13/13 pass；真实排版几何 29/29 pass；display-region
  6/6 pass、violations 0。review JSON SHA-256
  8cd623de68f2bb1f155ed69eb76ad7625c8f2ef7a17d32d987b1d63e7e614da7；
  display report SHA-256
  fb53cf495bb9962d44e41e0be709d886fe8e1a2decf16550423e37ffd26fef41；
  real-layout SHA-256
  a9a845f7052a9d016e31f7bf3c77c77520ae53e3676a99df46dea6360d89f64f。
- fixed crop 均值 [96.55,79.77,56.05]，仍在灰暗烟熏旧骨褐方向；最大相邻
  列／行跳变 0.77／1.03，high-frequency residual 1.82。运行时视觉已接近
  可用，但源图与 128×768 crop 仍能连续辨认和 attempt 3 同源的规则环状纹。
- 第一失败门禁：edit-did-not-remove-repeating-embossed-loop-geometry。r3
  要求“完全移除而非降低对比度”，但 attempt 4 只把压纹变暗变软，没有消除
  重复单元。连续两次 edit 都保留同一缺陷，继续 edit 会把它固化为生产源。
- 结论：internal-rejected / final-fresh-prepared / P3 / 4/5。attempt 4 是
  当前最佳运行时 fallback 证据，但不得作为 Image 3；最终 attempt 5 必须只
  上传固定 Image 1／2 fresh regenerate，不上传任何 V7-A 旧候选。没有写入
  source、runtime、atlas 或 addon。

### QS-B1 V7-A.r4 complete production Prompt — final attempt 5 fresh regenerate

状态：final-fresh-prepared / authorized-by-bounded-loop。这是本批第 5 次也是
最后一次实际 ImageGen 的完整自包含正文。只使用固定 Image 1／2；没有
Image 3。目标是生成“手绘综合色与大色块 donor”，不是任何种类的表面扫描或
纹理贴图。

> Freshly generate exactly one edge-to-edge square orthographic hand-painted colour-
> and-value donor for the narrow oath carrier beneath an Azeroth expedition guild wax
> seal. Use Image 1 and Image 2 only as the fixed references. Do not upload, inspect,
> edit, trace, imitate, or reuse any pixel, crop, palette identity, pattern, texture,
> or composition from V7-A attempts 1 through 4 or any earlier generated candidate.
>
> The output is one uninterrupted square painted field and nothing else. It is a
> circa-2004 fantasy-game bitmap painting made from a few large opaque brush masses.
> It is deliberately not a material scan, texture sheet, photograph, procedural
> texture, woven fabric, or finished object. The field may suggest an old compressed
> oath parchment only through broad muted colour, broad value, sparse handling wear,
> and substantial visual weight. Do not rely on any all-over grain or repeated surface
> detail to communicate material.
>
> Fill every pixel of the square with the same continuous field. No exterior
> background, transparency, isolated silhouette, ribbon outline, torn edge, tail, wax,
> contact shadow, stamp, emblem, icon, writing, rune, border, hole, button, state,
> book, page edge, or complete UI element.
>
> IMAGE AUTHORITY
>
> Image 1 is the highest visual authority. Inherit its vanilla World of Warcraft
> two-dimensional hand-painted bitmap language: broad readable light, middle, and
> shadow masses; imperfect manual brush boundaries; subdued upper-left warmth; muted
> expedition-document colour; heavy old-world material presence; and wear painted at
> a coarse scale that survives low-resolution display. Ignore its complete book,
> page geometry, leather, brass, wax, bookmarks, text, buttons, rewards, and every
> finished-object silhouette.
>
> Image 2 is secondary. Inherit only its quiet expedition-record age, restrained
> grey-brown handling stains, economical brush detail, and long-used field-document
> character. Ignore its tracker shape, leather strap, compass, quill, rivets, layered
> pages, text, buttons, and rectangular layout. If the references conflict, Image 1
> controls era, palette, broad value grouping, light, weight, and brush language.
>
> THIS IS A PAINTED FIELD, NOT A TEXTURE
>
> Use exactly three to five large matte opaque colour-and-value regions across the
> entire square. Every main region must be larger than 240 pixels in both width and
> height on a 1024 by 1024 canvas. Their boundaries are broad, softly broken manual
> brush transitions, never straight, tiled, repeated, or sharp. Leave most of each
> region calm and nearly detail-free.
>
> No visible detail smaller than about 48 pixels. No small pattern at all. Do not draw
> fibre, thread, grain, pores, loops, curls, hooks, cells, pebbles, stipple, dots,
> specks, crosshatching, warp, weft, weave, mesh, embossing, repeated brush stamps,
> paper grain, canvas grain, leather grain, sharpened noise, photographic noise, or
> procedural noise. Do not make a fabric or paper macro. Do not fill calm areas with
> decorative texture. At full 1024 viewing size the first, second, and third reads
> must all be the same few broad hand-painted masses, never a micro-pattern.
>
> Add at most four unique broad handling marks across the whole square. Each optional
> mark must be 60 to 180 pixels long, at least 18 pixels wide, blunt, irregular,
> low-contrast, widely separated, and painted as one non-repeating coarse brush event.
> Zero such marks is acceptable. Add at most three broad stains, each over 120 pixels
> across with very soft irregular boundaries. Large quiet areas are mandatory.
>
> CROP-SAFE COMPOSITION
>
> A deterministic process will isotropically normalize the returned square to 1024 by
> 1024, then use x 448 through 576 and y 128 through 896. Treat this central 128 by
> 768 band as a deliberately calm continuous painted area. Across the full height of
> that band, use only broad slowly changing colour-and-value planes. Broad region
> transitions may cross it diagonally or irregularly, but no boundary may run along
> its length.
>
> Place no vertical seam, centre crease, stitched join, panel boundary, gutter, cord,
> full-height mark, dark column, bright column, colour stripe, horizontal seam, hem,
> band, row, abrupt step, focal stain, hole, symbol, spotlight, centred gradient,
> full-height highlight, repeated 22-pixel rhythm, or periodic element in the crop.
> Do not divide the square into panels, strips, lanes, tiles, folds, or woven sections.
> Reduced to 32 by 192, the crop must read as one solid old hand-painted oath field,
> not a striped ribbon, folded tape, patterned textile, or modern toolbar.
>
> PALETTE — MUTED GREYED SMOKED OLD-BONE
>
> Make the central crop visually average near RGB [102,85,62]. Keep most middle planes
> between RGB [92,77,58] and [116,96,72]. Use broad compressed shadows around RGB
> [50,42,34] to [70,58,46] and very sparse broad old-bone light no brighter than RGB
> [126,107,82]. Keep red, green, and blue close enough that the brown feels greyed and
> low-saturation. The first read is smoked old-bone umber, not gold or orange.
>
> Exclude yellow, mustard, ochre-gold, orange, honey, caramel, clean tan, clean beige,
> bright cream, golden cloth, yellow canvas, orange burlap, polished wood, saturated
> sepia, and near-black leather. Keep the field darker and greyer than the quest page
> in Image 1, but not black.
>
> WEIGHT, LIGHT, AND ERA
>
> Suggest a firm compressed oath-document carrier through broad matte values only.
> There is no visible weave and no surface scan. Use subdued upper-left warmth as one
> or two wide broken painted light masses, not a gradient or spotlight. Render
> orthographically with no perspective, thickness, cast shadow, full-height highlight,
> glossy streak, rim light, bevel, metallic edge, relief, or 3D fold. The result must
> look intentionally painted for a 2004-era game UI at low resolution: substantial,
> imperfect, restrained, and readable, never modern, photographic, physically rendered,
> vector-clean, or procedurally generated.
>
> STRICT EXCLUSIONS
>
> No small-scale texture, fibre, thread, grain, pore, loop, curl, hook, cell, pebble,
> stipple, dot, speck, weave, mesh, crosshatch, grid, embossing, knitted pattern,
> wallpaper, procedural noise, photographic scan, macro surface, repeated brush mark,
> object silhouette, ribbon outline, torn outer edge, tail, sawteeth, fishtail, fringe,
> tassel, wax, seal, medallion, emblem, motif, text, rune, prayer, icon, button, border,
> frame, book, page edge, leather, wood, metal, rivet, brass, modern HUD glass, modern
> toolbar, Diablo III ornament, Skyrim minimalism, neon, glow, chrome, satin, velvet,
> straight seam, centre crease, vertical fold, horizontal fold, full-canvas stripe,
> perfect symmetry, or repeated geometry.
>
> Do not depict or copy Warhammer or any other franchise identity: no aquila,
> double-headed eagle, skull, armour, devotional wording, prayer text, purity-seal
> silhouette, science-fiction metal, heraldic badge, or religious insignia. The result
> remains an original Azeroth expedition-document painted field in the locked
> vanilla-era visual language.
>
> FINAL SELF-CHECK
>
> Confirm the output is exactly one square edge-to-edge painted colour-and-value field
> made from only three to five regions larger than 240 by 240 pixels. Confirm there is
> no visible detail smaller than 48 pixels and no fibre, grain, loop, curl, cell, pore,
> weave, embossing, repeated mark, photographic scan, or procedural texture anywhere.
> Confirm the central x 448..576, y 128..896 band is calm, continuous, unstriped, and
> seamless with broad slow variation only. Confirm its colour is muted greyed smoked
