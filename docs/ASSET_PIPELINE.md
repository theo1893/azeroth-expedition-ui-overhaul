...
7. 九宫格／三段式切线或图集 UV。
8. 允许上传的参考图及每张图的唯一职责。
9. 每张锁定视觉基准对应的原始 prototype／provenance prompt 路径。
10. 从基准提示词提取的视觉 DNA、组件级转译、明确排除项与冲突裁决。
11. 禁止烘焙的文字、图标和交互结构。
12. 验收截图与回退条件。
...
2. 沿每张 `assets/locked/` 基准追溯并完整读取产生它的版本化提示词
   provenance；锁定图与原始提示词共同构成视觉权威。
3. 建立“必须继承／组件级转译／明确不继承／冲突裁决”块。运行时组件合同
   决定对象与几何，锁定图及其提示词决定物件身份、轮廓语言、材料、配色、
   笔触、光照、磨损和年代感；派生 `assets/source/` 不得反向覆盖前者。
4. 把用户需求重写为可执行、可验收的专业提示词，保存到
   `prompts/<module>/`。
5. 向用户展示具体版本的执行正文；只有用户明确授权该版本后，才把正文原样
   传给 `$imagegen`。“继续”或“下一步”只允许走到展示提示词，不构成生图
   授权。
6. 所有 `-i` 输入使用绝对路径，并在执行说明中声明 Image 1、Image 2 等
   参考职责。
7. 原始结果只写入 `generated/<module>/<version>/`。
...
- 目标逻辑对象及精确数量。
- 参考图列表与单一职责。
- 锁定视觉基准对应的原始提示词 provenance 路径。
- “必须继承／组件级转译／明确不继承／冲突裁决”美术基准块。
- 风格基线的必要摘录，并在最终执行正文中实际展开，而不是只写“同风格”。
- 视觉权威顺序；接受 source 只能承担明确的次级结构或材料职责。
- 运行时尺寸、源画布和透明方式。
- 状态差异、文字安全区和可拉伸区域。
- 禁止项。
- 输出验收标准。
...
但它作为锁定基准的 provenance 时必须被生产工作流读取；“不得直接执行”不
等于“可以忽略其中已经由用户确认的美术语言”。
