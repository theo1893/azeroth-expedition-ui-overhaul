...
| Unit Frames | `api/unitframes.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `UF-A1 V2-A V2 P3 / candidate-rejected / repair-budget-exhausted`；A2／B1 `P3 / paused` | 旧 V2-A V1 与新 V2-A V2 的单图四端帽循环均已用满各自 `5/5`。新版本每次都只输出一张 `1536×1024` 四格 atlas；最终稿 SHA `315470ff…4d20`，四件为 `139–142×798–799px`、比例误差 `4.380476%–6.766917%`、上下隔离 `109–117px`。mass/contact、装配及 display `6/6` 通过，但等比 fit 仅 `7×39–40`，未满足 `7×42` source 几何。V2-B `0/5 / sequence-blocked`；无 source/runtime/addon 改动 | 用户决定是否建立新的 V2-A 版本与新授权合同；禁止第六次调用、失败稿晋级或启动 V2-B。若改变画布／格位、输入职责、确定性处理或方向，先重新模拟／审阅；继续暂停 A2／B1 |
