...
| Unit Frames | `api/unitframes.lua`、`modules/raid.lua`、主／紧凑单位入口 | A／B `P3 / 5/5 exhausted`；B1 `P5`；Raid A1 `P3 / 5/5 exhausted`，Raid A2 `P3 prompt-authorized`；紧凑 A2 paused | B1 exact Health／Power 只接入 Player／Target／TargetTarget／Focus。Raid A1 五次终止且无产物；`UF-RAID-A2-SIM-V1` 已确认，`UF-RAID-A2-DONOR V1` 已精确授权。模型只供四块材料，四张 `592×296 → 74×37` 外壳和 A-D 维修由 Python 确定性构造；当前 ImageGen `0/5`，无 source/runtime/addon | 以已提交原文执行 attempt 1 并完成 donor、Python 造壳、40 人真实排版和 display-region 内审；通过即停，最多 5 次。A1 不得第六次；B1 另待 Turtle WoW P6 |
