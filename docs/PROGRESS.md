...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`，QL-D V1 最终美术仍为本地模拟。QS-B1 V3-A／V4-A 各 `5/5` 失败终止，V3-B `0/5`。V14／V5-A 由生成模型提供全幅暗旧布面 donor，tracked 固定 crop／mask 独占 `128×696` 轮廓与 Alpha；用户已确认方向并授权最终 production。attempt 1 的 raw、composite、动态六态与 display-region 技术全过，但均匀细颗粒、圆斑和两道横贯中央 crop 的折面违反美术／安静带合同，当前 `internal-rejected / repair-prepared / P3 / 1/5`；符合紧邻前稿受限 edit 条件。无 source、runtime 或菜单接入。Tracker 与 NPC Quest／Gossip 状态不变 | 提交完整 V5-A.r1 后，以固定 Image 1／2＋attempt 1 raw Image 3 执行 attempt 2 edit 并重新完整内审。旧按钮继续 fail-open，不执行 V3-B、不导出、不接入；QL-D、Tracker 与整模块 P6-C 门禁保持 |
