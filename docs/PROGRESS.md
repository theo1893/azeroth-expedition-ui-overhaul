...
| Chat | `modules/chat.lua` + AEUI Chat adapter | 核心 `P5` Full V1 / r1.19；输入候选 `P3` | 主框九宫格、右框回收、V3 Tab／输入／未读及暖黑纸面 Vanilla 色板保持。`CHAT.INPUT.DARK.V1` 已计 `2/5`：attempt 2 的对象、页叠、几何、真实排版与展示区域改善并通过，但中心重复压纹仍像皮革，focus 连续橙金上缘未通过；正式 runtime 尚未改变 | 执行冻结边界内的完整 `.r2` Image 4 edit，复核低频纸纤维与局部 focus 反馈；内部通过后交用户接受。游戏设备可用时再 `/reload` 验证核心 r1.19 与输入焦点、IME、历史 |
