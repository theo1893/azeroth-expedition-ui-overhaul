...
| Unit Frames | `api/unitframes.lua`、`modules/raid.lua`、主／紧凑单位入口 | A／B `P3 / 5/5 exhausted`；B1 `P5`；Raid A1 `P3 / 5/5 exhausted`，Raid A2 `P3 bounded-repair-loop-active`；紧凑 A2 paused | B1 exact Health／Power 只接入 Player／Target／TargetTarget／Focus。Raid A1 五次终止且无产物；A2 已确认并精确授权。模型只供四块材料，四张 `592×296 → 74×37` 外壳和 A-D 维修由 Python 确定性构造；循环细节与精确预算只在 Raid work 维护，无 source/runtime/addon | 继续已授权的最多 5 次实际生图循环；每稿完成 donor、Python 造壳、40 人真实排版和 display-region 内审，通过即停。A1 不得第六次；B1 另待 Turtle WoW P6 |
