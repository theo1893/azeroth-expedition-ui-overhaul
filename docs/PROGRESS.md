...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`。QS-B1 V6 授权批次已封闭：ImageGen `28/35`、流程错误 `7`；A／B／C 未获用户接受，D／E／F／G 各自耗尽。用户保留七项独立代理结构，但否决火漆悬浮、近黑布质和双钝缺口方向。用户于 `2026-08-06` 确认 V17／V7-A 的较窄旧骨褐誓约条带、`24px` 漆章跨压及不等距尖锐破口；本地 `42/42`、展示区 `6/6`、ImageGen `0/0`。最终单 donor production 正文、固定 Image 1／2 和确定性 mask／接触／`32×192px` 合同已完整审计，当前 `simulation-confirmed / production-prepared / P2`；无新 source/runtime/addon，旧按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 不变 | 用户独立授权或否决 `QS-B1 V7-A` 最终 production 正文、固定输入、受限同循环 Image 3 edit 与最多 `5` 次实际 ImageGen 调用 |
