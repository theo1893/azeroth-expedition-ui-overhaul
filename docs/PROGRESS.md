...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P3 / repair-prepared` | Field Kit V2 的 AutoBar `4×6 / 24 类` 三段分组、原生 popup 与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempts 1–3 已计 `3/5`；attempt 3 raw `0c6f0bc7…8048` 经确定性传输得到 exact canonical `6a91a2b5…5e13`，绿残留、透明 RGB 与最终 margin pass，但 C 原始物件越过中线并污染 D cell，scope fatal fail。纯绿 raw→canonical 修订不重绘、不新增输入图、预算不重置；Trinket `r3` 已准备，Consumable 保持 `0/5` | 提交 attempt 3 记录、cell 隔离门禁与完整 `r3` 后执行 Trinket attempt 4；通过即停，否则最多最后一次 `5/5`，禁止 attempt 6。Trinket 循环终止后才准备 Consumable transport body／attempt 1；不启用 AutoBar、不改 TrinketMenu SavedVariables |
