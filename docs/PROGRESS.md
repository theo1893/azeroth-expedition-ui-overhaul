...
| Unit Frames | `api/unitframes.lua`、`modules/raid.lua`、主／紧凑单位入口 | A／B `P3 / 5/5 exhausted`；B1 `P5`；Raid `P2 simulation-reviewed`；A2 paused | B1 exact Health／Power source 已只接入 Player／Target／TargetTarget／Focus；display `9/9 pass`。Raid 已按 40 个 `70×33` Secure Button、`10×4` 真实密度完成 `UF-RAID-SIM-V1` 本地预演与 display `7/7 pass`；无共享外框，等待用户方向确认，尚无 source/runtime/addon 变更 | Raid 先确认或退回 `UF-RAID-SIM-V1` 的薄皮革点名名条、四变体、配色与状态密度；确认前不进入正式资产阶段。B1 另待 Turtle WoW P6；A／B 是否重开另定，A2 暂停 |
