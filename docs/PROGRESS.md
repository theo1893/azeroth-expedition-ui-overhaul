...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6`；QS-B1 V7-A `P5`；QL-D final art `P3` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复。用户于 `2026-08-07` 接受 QS-B1 V7-A attempt 5 并授权 P4/P5；exact `128×768` source SHA `168f527f…05b8` 已等比导出为 `32×192` TGA SHA `db620778…c615`。Quest runtime `1.26`／Theme `1.9` 在真实 ScrollChild 内以 `28px` 闭合根部承托 QS-A1 火漆；7／5／3 连续前缀＋`14px` tail 公式已实现并通过 Lua 冒烟，但七枚独立纹章尚未验收，菜单保持 inactive，旧按钮继续 fail-open。QL-D V2 奖励槽方向与完整 production 正文已分别确认／授权，当前 `prompt-authorized / production-active / 0/5`，候选未接受前不接入。Tracker 与 NPC Quest／Gossip 不变 | 执行 QL-D 最多五次生成与真实排版审查；另在 Turtle WoW 验证闭合态物理接触、滚动裁切及 TGA 方向；独立完成七纹章和代理 parity 后才启用菜单 |
