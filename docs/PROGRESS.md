...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P4 / dual-source-accepted` | 用户于 `2026-08-09` 接受 Trinket 第4稿与 Consumable 第1稿。两套 exact `1024² RGBA` canonical 已 byte-exact 晋级为 tracked source：Trinket SHA `82dd2260…c012`（`4/5`）、Consumable SHA `623f29c5…a2419`（`1/5`）；各自 SourceManifest 固定 Character V3 provenance、Alpha、四格映射、`80 px` margin 与 `16/16` display／violations `0`。尚无 runtime／adapter | 用户若继续，分别冻结四 cell 的 crop／九宫格／旋转／拉伸／UV 与 provider adapter 合同，再走 P4→P5 display-region、静态回归及 fresh-checkout package。不得启用 AutoBar、应用 profile、改 TrinketMenu SavedVariables 或接管 provider 动态层 |
