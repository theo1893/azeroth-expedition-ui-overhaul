...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.2 / pending-retest` | accepted source 与两张 TGA 像素不变；v1.1 图标层序已实机通过。用户发现 AutoBar 原生向左线性 popup 遮挡主格并授权修改，同时要求两袋吸附。AEUI `0.8.2`／`fieldkit-contract=1.2` 在 exact `24 / 4×6 / 推荐 profile` 使用外置抽屉：`1–6` 单列、`7–12` 双列，支持 AUTO／LEFT／RIGHT／NATIVE 与原生回退。消耗品默认软吸附主栏左侧 `48 UI`，饰品默认右侧 `16 UI`，两侧阈值 `32 UI`、可独立拖离／回吸附；只保存 AEUI 布尔状态，无维护循环。simulation `89/89`、display `19/19`；runtime display `9/9`＋`10/10`、package pass，本次外部生成 `0` | Turtle WoW `/reload` 验证 `fieldkit-contract=1.2`、外置抽屉候选 `1／6／7／12` 不遮主格、四种 popup 模式、非 exact 原生回退、双侧拖离／回吸附、apply／restore、饰品候选／Queue／换装及 `/aeui actionbars` fail-open；通过前保持 P5 |
