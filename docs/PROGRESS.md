...
| Action Bars | `P2–P6` | AEUI `0.9.0`；Slot／Rail `P6`；Field Kit `2.9`、Supply `1.3`、Focus `3.5`、Sidebar Group `1.0`、Target Markers `2.3` 为 `P5`；AEUI Supply 配置只接受从背包拖入物品，内部按每角色精确 itemID 扫描当前背包库存并让库存为 `0` 的槽常驻，AutoBar 退为无 Supply 配置时的旧路径；Player／Target 保持 `240×48 / 0.8`、底锚点 `480 UI`、`23 UI / 每排 8` Aura 与 `73 UI` 间距；Aura 语义策略为 Player 仅技能书 Buff／全部 Debuff，敌对单位全部真实 Buff／自己施加与固定 `12` 项关键 Debuff 的并集，友方单位全部真实 Buff／Debuff，TargetTarget 与 Focus 同步按敌友套用；下方施法／Swing、Combat Deck 与所有角色 DDPS `TOPLEFT (650,-615)` 原位；标记方阵为骷髅优先 `4×2`，八个透明命中位共用既有 Consumable Kit 的缝制皮革九宫格并直读 `mark1..mark8`；空态使用中央大标记，占用态改用左下满亮小标记、顶部两行名字和右下血量；死亡目标只从 AEUI 方阵本地清空；ArchiTotem 相对旧居中位左移 `128 UI`，方阵留在既有位置，向下候选不再覆盖皮革 icon list；左侧 DDPS 坦克 Button 固定在 Combination 的真实 Frame 边界内并始终显示，provider 或装饰刷新异常时也保留基础可点击盾牌；provider API 就绪时左键指定、右键清除，未就绪时显示不可用态；右侧一键 Button 仅在 HDLRaidTools／SuperWoW provider 完整就绪时显示 | 实机验证 Supply 背包拖入／即时数量／耗尽后 `0` 常驻、AutoBar 互斥回退、四框敌友 Aura 过滤、Action Bars 禁用回退及另一角色配置隔离；再验证标记、图腾、坦克／一键 Button、DDPS 中央视野和姿态 `25 UI` |
