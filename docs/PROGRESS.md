...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P5 / runtime-exported`；`AB.SLOT.BASE.V1` | V3 战斗信息链已确认。accepted source `6d4a4d16…7dc0` 已确定性导出为 `128²` TGA `5c49a1db…23ca`，像素与 attempt 5 review 完全一致；AEUI `0.7.0` scoped adapter 只挂接 pfUI Bar `1–10` 既有 backdrop，Bar `11／12` 与全部 provider 行为／几何不变。最终 display `5/5 pass`、package `pass`，目标设备无需构建，P4→P5 新增外部生成 `0` | Turtle WoW 验证 `actionbar-runtime=1.0`、TGA 方向、真实图标／状态叠层、20–48 UI 与合法行列、自由移动／缩放／显隐、Bar `11／12` 排除和 `/aeui actionbars` fallback；通过前保持 P5 |
