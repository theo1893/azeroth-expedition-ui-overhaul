...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 只接收显式一次性邻接位置／舒适缩放 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.5 / pending-retest`；Combat Focus 为 `P5 / layout-v1.1 / pending-game-validation` | 用户实机截图为 `2560×1440`，客户端仍输出 `1920×1080`；旧 pfUI tier 7 经 `4/3` 显示放大后有效尺寸约 `1.0836×`，玩家框偏大并压住左卷袋。AEUI `0.8.7`／`focus-layout-contract=1.1` 的显式 comfort preset 改用 tier 8 `0.711111`，显示有效尺寸约 `0.9481×`，不改分辨率；Player／Target 与施法条 local scale 收敛到 `1.0` 并重算当前角色全部锚点，约 `80 px` 内缘间距保持。关键战斗信息沿用 provider Alpha，装饰鼠标穿透；Field Kit art／行为不变，无维护循环、无重绘、无外部生成 | 启动游戏验证 `version 0.8.7`、`focus-layout-contract=1.1`、`focus-ui-scale-tier=8`，确认整体尺寸、中文文字、CD、玩家／目标、施法、Swing、DoiteDPS、左卷袋与右双槽均舒适且不遮视野；需同时恢复 scale／位置时执行 `/aeui focuslayout comfort`。同轮继续 Field Kit popup、Queue／换装及 fail-open 复测；通过前保持 P5 |
