...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.3 / pending-retest` | accepted source／TGA／抽屉可见布局不变。v1.2 已实现 exact `24 / 4×6` 外置 `1×1–6／2×4–6` drawer 与消耗品左 `48 UI`／饰品右 `16 UI` 双侧软吸附；实机发现从主格穿越到右抽屉会提前关闭。AEUI `0.8.3`／`fieldkit-contract=1.3` 在外置态加入全卷袋高度、直接隶属 `AutoBarPopupFrame` 的透明悬停通道，XML Frame `OnLeave` 仅在此态延后，关闭仍由 provider 计时器负责，所有原生回退恢复脚本。本次外部生成 `0`，原生产预算不变 | Turtle WoW `/reload` 从上／中／下主格缓慢移入左右候选 `1／6／7／12`，确认穿越不断开、完全离开后正常关闭；再验证四种 popup 模式、非 exact 原脚本回退、双侧拖离／回吸附、apply／restore、饰品候选／Queue／换装及 `/aeui actionbars` fail-open；通过前保持 P5 |
