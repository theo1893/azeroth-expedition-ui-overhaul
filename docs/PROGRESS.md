...
| Action Bars／Field Kit | `actionbar.lua`、`castbar.lua`、`swingtimer.lua`、pfUI Player／Target／TargetTarget + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 的 scoped 视觉与一次性游戏坐标布局 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v2.0 与 Combat Focus runtime-v2.2 为 `P5 / pending-game-validation`；v1.4／v1.5 `game-geometry-failed` | AEUI `0.8.19`／`ACTION-BARS-CORE-SIM-V11` 保留 V10 的单位框、Aura 与中央计时栈几何；Player／Target／TargetTarget 的局部字形改为客户端系统字体 `OUTLINE / 18 UI`，DoiteDPS 时间线与资源两排作为一个 union 上移 `32 UI` 退出 Player Buff 占位。消耗品／饰品仍共同下移 `20 UI`；AutoBar 仍只显示当前有物品类别、最大 `4×6`。未手调的 exact v7–v11 几何和仍匹配旧字体的 exact v12 profile 一次迁移为 v13。右侧四栏已有 `2×2`、每块 `3×4` 的 `6×8` proposal-only 预览，尚未接入 runtime；位图字节不变，无外部生成 | `/reload` 后确认三框均为系统字体 `18`、DoiteDPS 双排不压 Player Buff，并复测 V10 的 Player／卷袋、Aura 八枚、Boss 16 Debuff 与计时栈净空；开关 pfUI unlock 确认 Bar 6／TargetTarget 不跳位，再验证 AutoBar 动态格数、ArchiTotem、popup／Queue／换装。四栏组合待用户确认后再实现 group mover 与可逆 unbind；回退：`/aeui focuslayout restore` 后 `/reload` |
