...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 视觉与布局桥接；pfUI Player／Target 只接收显式一次性邻接位置／舒适缩放 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v1.6 为 `P5 / pending-retest`；Combat Focus runtime-v1.5 为 `P5 / pending-game-validation`，v1.4 `game-geometry-failed` | `ACTION-BARS-CORE-SIM-V5` 视觉方向仍获用户确认；新实机截图证明 AEUI `0.8.10`／v1.4 混用了 768-high `UIParent`、normalized UI root 与 provider scale。AEUI `0.8.11`／`focus-layout-contract=1.5` 保留 tier 8、Combat Deck、Field Kit、单位／施法 `0.75` 与 Swing／姿态／DoiteDPS `0.82`，显式 preset 改为逐 Frame UI-root anchor 校准；普通 refresh 不维护坐标。`fieldkit-contract=1.6` 与 ArchiTotem bridge 不变；无新位图或外部生成 | `/reload` 后执行一次 `/aeui focuslayout comfort`，确认 `focus-layout-coordinate-space=ui-root-calibrated-v1`，再验证攻击／施法不重叠、中央空白收紧、单位框不挡人物，以及 DoiteDPS、ArchiTotem 全交互和 Field Kit popup／Queue／换装回归 |
