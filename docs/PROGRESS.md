...
| Action Bars／Field Kit | `actionbar.lua`、`castbar.lua`、`swingtimer.lua`、pfUI Player／Target／TargetTarget + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 的 scoped 视觉与一次性游戏坐标布局 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v2.7、Combat Focus runtime-v2.5 与 Sidebar Group runtime-v1.0 为 `P5 / pending-game-validation`；v1.4／v1.5 `game-geometry-failed` | AEUI `0.8.25`／`ACTION-BARS-CORE-SIM-V11` 保留既有单位框、Aura、三条计时栈、系统字体、DoiteDPS 与四栏组合。新实机证据证明旧 provider-local 回锚仍会被 AutoBar 最终 `SetupVisual` 自由坐标覆盖，且姿态仅改外层 scale 不会改变保存的 `18 UI` 按钮。bridge-v2.7 现通过 AutoBar 原生 docking 把活动 display 强绑定到 `pfActionBarMain`，自由坐标只作可逆回退；logout／reload 前移除 AEUI 专用 token。focus runtime-v2.5／profile v16 把真实 `bar11.icon_size` 改为 `25 UI`、scale `1.0` 并触发 pfUI 重建；v14／v15 copied profile 仅升级姿态，不改其他手调坐标。全部位图字节不变，无外部生成 | `/reload` 后确认卷袋立即在主栏左侧，连续两次 apply、开关配置页和移动主栏均不跳位，状态为 `autobar-anchor-basis=provider-dock`／`autobar-provider-dock=bound`／`autobar-drag-handle=hidden-bound`；确认三枚战士姿态按钮 `25 UI / scale 1` 且交互正常。继续复测既有系统字体、Aura、Boss Debuff、计时栈、单 mover、四栏可逆 unbind、popup／Queue／换装；回退：`/aeui focuslayout restore` 后 `/reload` |
