...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题均已修复；QL-A2 V4、18 行左页字体／无描边／类型墨色，以及 Quests `1.25`／Theme `1.8` 的右页金额、动态 ScrollChild、奖励无循环锚点、`108×41px` 双列间隔／换行和原生 `NameFrame` 抑制记为 `P6 user-confirmed`。QL-D V1 最终奖励槽美术仍只有本地模拟。QS-B1 V12 冻结动态空白背景、七张独立透明纹章／Button 与滚动合同；V3-A `5/5` 后被用户否决，V3-B `0/5`。V13／V4-A 的暗色非周期双钝缺口方向已确认；V4-A 正式候选循环 `5/5`、流程错误 `1`。attempt 5 视觉最接近 V13，但 raw `176×892px` 相对目标比例误差 `7.287%`，超过授权 `≤1%`；自动检查 `5/9`，终态 `candidate-rejected / repair-budget-exhausted / P3`。六个真实排版与 display-region 场景均通过。无 source、runtime 或菜单接入。Tracker 与 NPC Quest／Gossip 状态不变 | 禁止 attempt 6；等待用户选择 V5、显式比例合同例外或暂停。旧按钮继续 fail-open，不执行 V3-B、不导出、不接入。QL-D 与 Tracker 门禁保持，整个模块不进入 P6-C |
