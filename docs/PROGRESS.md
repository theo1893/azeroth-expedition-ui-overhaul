...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A／B 均 `5/5 exhausted`；B1 `2/5 / final.r2 repair-prepared`；A2 paused | A／B 终态不变。B1 attempt 2 的 Health `2.108:1`、误差 `5.40%` 已通过；Power 收敛至 `5.355:1`，误差 `33.87%` 仍未过 `4:1` 门禁。其余八项继续通过，无 candidate/source/runtime。流程错误 `4` | 提交 B1 `final.r2` 后执行 attempt 3，只以上一 B1 `1024²` raw 为 Image 1；冻结 Health，把 Power 修至 `664×166 / 4:1`。通过即停，否则最多 5 次。A2 暂停 |
