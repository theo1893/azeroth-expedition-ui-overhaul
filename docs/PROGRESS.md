...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 视觉与布局桥接；pfUI Player／Target 只接收显式一次性邻接位置／舒适缩放 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.5 / pending-retest`；Combat Focus v1.2 为可回退 `P5 / revision-requested`，V4 为 `P2 / pending-user-confirmation` | 最新 `1443×1067` 实机截图确认 v1.2 已消除单位框与左卷袋相交，但用户判定战斗核心偏小。`ACTION-BARS-CORE-SIM-V4` 保持 tier 8、Combat Deck 和 Field Kit 不变，只提议把 Player／Target、双方施法、Swing、姿态与 DoiteDPS local scale 从 `0.75` 调至 `0.82`，并内收双框以保持外包络。实机 provider 已校正为 ArchiTotem `1.7`；真实四元素、Air 最多七候选、拖动球、AllTotems、Recall 与 PresetManager 均已入合同，卫星栏提议置于动作条下方、向下展开并在绑定态随 Bar 1。layout `54/54`、display `7/7`，无 runtime／SavedVariables／美术或外部生成改动 | 用户接受或修订 `ACTION-BARS-CORE-SIM-V4`；确认前不写提议坐标或 ArchiTotem 方向。接受后实现下一版 focus contract，再实机验证放大后的可读性、单位框／卷袋净空、施法／Swing／DoiteDPS，以及 ArchiTotem 施放、右键、七层候选、拖动／锁定、Recall、强绑定和 fail-open；同轮继续 Field Kit popup、Queue／换装复测 |
