...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`，QL-D V1 最终美术仍为本地模拟。QS-B1 V3-A／V4-A 各 `5/5` 失败终止，V3-B `0/5`。V14／V5-A 由生成模型提供全幅暗旧布面 donor，tracked 固定 crop／mask 独占 `128×696` 轮廓与 Alpha；用户已确认方向并授权最终 production。attempt 4 已擦除横贯 crop 的两个局部 plane 外溢，自动 `18/18`、动态六态 `29/29`、display-region `6/6` 与完整美术内审通过；当前 `candidate-reviewed / awaiting-user-acceptance / P3 / 4/5`，流程错误 `1`，attempt 5 未调用。无 source、runtime 或菜单接入。Tracker 与 NPC Quest／Gossip 状态不变 | 用户明确接受或拒绝 attempt 4 composite；接受前旧按钮继续 fail-open，不执行 V3-B、不导出、不接入；QL-D、Tracker 与整模块 P6-C 门禁保持 |
