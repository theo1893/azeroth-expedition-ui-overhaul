...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 为 `P6 / game-validated`；`AB.RAIL.V1` 为 `P5 / runtime-exported`；`AB.FIELDKIT.V1` 为 `P2 / simulation-reviewed` | V3 战斗信息链已确认。`AB.SLOT` accepted source `6d4a4d16…7dc0` 与 runtime TGA `5c49a1db…23ca` 已通过 scoped adapter、display、package 及用户实机冷却／距离红／按下反馈验证。Rail 固定生产在 `5/5` 停止后，accepted source `7c49995d…32e9` 已确定性导出为 `256²` TGA `1e5cca09…0a3d`（像素 `1b09b93b…9db5`）；Bar `1–12`＋Bar `1／6` merged scoped adapter、display `8/8`、Lua smoke、repository tests 与 fresh-checkout package 均 pass，P4→P5 外部生成调用 `0`，禁止 attempt 6，pfUI／SavedVariables 未改。Field Kit V1 的 `5×2` 已按用户容量／分组要求修订；当前 `AB-FIELDKIT-SIM-V2` 使用 AutoBar 完整 `4×6 / 24 类`，分为应急／增益／工具三段，分类内仍保留原生 `1–12` popup；TrinketMenu 方向不变。display `16/16`、布局 `72/72`、violations `0`、外部生成调用 `0` | Rail 在 Turtle WoW `/reload` 后验证横／竖／多行、Bar `1／6` 合并无中缝、拖动／缩放／显隐、姿态／宠物栏跟随与 fail-open。另等待用户接受或修订 `AB-FIELDKIT-SIM-V2`；Field Kit 确认前不启用 AutoBar、不改 TrinketMenu 保存值、不执行其 production；`AB.SLOT` 单组件收口仍另行待办 |
