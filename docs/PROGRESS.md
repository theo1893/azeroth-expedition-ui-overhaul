...
| Action Bars／Field Kit | `actionbar.lua`、`castbar.lua`、`swingtimer.lua`、pfUI Player／Target／TargetTarget + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 的 scoped 视觉与一次性游戏坐标布局 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v2.0 与 Combat Focus runtime-v2.1 为 `P5 / pending-game-validation`；v1.4／v1.5 `game-geometry-failed` | AEUI `0.8.18`／`ACTION-BARS-CORE-SIM-V10`：Player／Target 保持 `240×60 / 0.8` 并上移 `30 UI`，TargetTarget 保持 `240×60 / 0.68`、右侧中线依附；三框字体 `14`、Aura `23 UI`，按真实 `size+7` 步进八枚占 `233／240 UI`；Target 16 Debuff 双排不压未移动的 `260×12 / 1.0` 计时栈。消耗品／饰品保持原 x 并共同下移 `20 UI`；AutoBar 继续保留 24 个逻辑类别、只显示当前有物品类别、最大 `4×6`。exact v7–v11 profile 一次迁移为 v12；位图字节不变，无外部生成 | `/reload` 后开关 pfUI unlock，确认无 `drag=nil`、Bar 6／TargetTarget 不跳位；确认 Player 与两侧组件分离、Aura 八枚满行、Boss 16 Debuff 双排净空、三条计时与 DoiteDPS 不变；确认“大奶黑牛”AutoBar 当前 13 格且随库存动态收缩／扩展，再验证 ArchiTotem、popup／Queue／换装。回退：`/aeui focuslayout restore` 后 `/reload` |
