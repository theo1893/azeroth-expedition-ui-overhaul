...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 视觉与布局桥接；pfUI Player／Target 只接收显式一次性邻接位置／舒适缩放 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v1.6 为 `P5 / pending-retest`；Combat Focus v1.3 为 `P5 / pending-game-validation` | 用户已接受 `ACTION-BARS-CORE-SIM-V4`。AEUI `0.8.9`／`focus-layout-contract=1.3` 保持 tier 8、Combat Deck 与 Field Kit 尺寸不变，把 Player／Target、双方施法、Swing、姿态与 DoiteDPS local scale 从 `0.75` 调至 `0.82`：线性 `+9.33%`、面积约 `+19.54%`，并内收双框以保持外包络。`fieldkit-contract=1.6` 将已审计的 ArchiTotem `1.7` 真实根作为主栏下方职业卫星；绑定态随 Bar 1，显式 focus preset 请求向下，普通 refresh 不写方向。layout `54/54`、simulation／runtime display 均 `7/7`、Lua smoke pass；无新位图或外部生成 | `/reload` 后执行一次 `/aeui focuslayout comfort`，实机验证 0.82 可读性、单位框／卷袋净空、施法／Swing／DoiteDPS，以及 ArchiTotem 施放、右键、Air 七层、拖动／锁定、Recall、bind／unbind 与 fail-open；同轮继续 Field Kit popup、Queue／换装复测。若 0.82 仍不足，再以截图进入 0.85 对照修订 |
