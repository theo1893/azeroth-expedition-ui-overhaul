...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题均已修复；QL-A2 V4、18 行左页字体／无描边／类型墨色，以及 Quests `1.25`／Theme `1.8` 的右页金额、动态 ScrollChild、奖励无循环锚点、`108×41px` 双列间隔／换行和原生 `NameFrame` 抑制记为 `P6 user-confirmed`。QL-D V1 最终奖励槽美术仍只有本地模拟。QS-B1 V12／V3 已拆为动态空白旧布背景与七张独立透明纹章，真实排版 `35/35`、display-region `6/6`；用户已确认方向并授权 V3-A／V3-B。V3-A 已执行 `5/5`，布局 `26/26` 与 display-region `6/6` 通过，但五稿持续出现 source 级连续微纹，最终宽高比仍差 `9.86%`，故以 `internal-rejected / repair-budget-exhausted` 终止；V3-B 依顺序合同保持 `0/5`。无 source、runtime 或菜单接入。Tracker 与 NPC Quest／Gossip 状态不变 | 等待用户审查 V3-A 失败证据并决定新版本／资产策略；旧按钮继续 fail-open，不执行 V3-B、不导出、不接入。QL-D 与 Tracker 门禁保持，整个模块不进入 P6-C |
