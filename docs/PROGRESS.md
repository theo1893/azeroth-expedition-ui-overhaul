...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P1 / simulation-reviewed`；P2 待用户 | 用户否决 V1 的贴底动作条后，V2 已把约 `39px` 主栏上移到 `y=827` 并把玩家／目标框收拢为 `80px` 内缘；用户随后要求纳入施法、攻击与 DoiteDPS，V3 形成 DoiteDPS → 攻击计时 → Aura／双框 → 双施法条 → 动作栏的纵向焦点，保留 provider 自由移动和 V2 动作区。新增 display `9/9`、综合布局 `46/46 pass`、V2 像素回归一致；未写 profile，无 source/runtime | 用户确认或继续修订 V3 的 DoiteDPS、攻击条、Aura 外肩、双施法条与动作区总体层级；确认后才锁定 P2 并逐批授权 production Prompt |
