...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6`；QS-B1 V7-A `P5`；QL-D V3 final art `P3 repairing` | Quest runtime `1.26`／Theme `1.9` 保持闭合载体根、火漆与旧按钮 fail-open。QL-D V3 attempt 4 已通过语义、层序、broad-plane 美术、安全区、四态和 display `5/5`，但 technical `18/19`：aspect `2.76945` 超过 `2.69`。当前 `4/5`、流程错误 `2`，最终 V3.r4 固定 Image 1／2 fresh-regenerate，未写 source/runtime/addon。Tracker 与 NPC Quest／Gossip 不变 | 固定执行器执行 QL-D V3.r4 最终 attempt 5 并完成候选内审；另在 Turtle WoW 验证闭合态物理接触、滚动裁切及 TGA 方向 |
