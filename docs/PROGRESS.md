...
| Action Bars | `P2–P6` | AEUI `0.9.0`；Slot／Rail `P6`；Field Kit `2.9`、Focus `3.3`、Sidebar Group `1.0`、Target Markers `2.3` 为 `P5`；Player／Target 恢复为 `240×48 / 0.8`、底锚点 `480 UI`，保持 `23 UI` Aura 并按每排 `8` 枚贴合短框，上 Buff／下 Debuff，`32` 枚 Debuff 最多四排；两框之间留 `73 UI`，TargetTarget 保持每排 `8` 枚，下方施法／Swing 与 Combat Deck 保持原位；三框布局按角色版本应用并保存独立回退；所有角色 DDPS 统一 `TOPLEFT (650,-615)`；AutoBar 固定四列、自底向上；标记方阵为骷髅优先 `4×2`，八个透明命中位共用既有 Consumable Kit 的缝制皮革九宫格并直读 `mark1..mark8`；空态使用中央大标记，占用态改用左下满亮小标记、顶部两行名字和右下血量；死亡目标只从 AEUI 方阵本地清空；ArchiTotem 相对旧居中位左移 `128 UI`，方阵留在既有位置，向下候选不再覆盖皮革 icon list；左侧 DDPS 坦克 Button 固定在 Combination 的真实 Frame 边界内并始终显示，provider 或装饰刷新异常时也保留基础可点击盾牌；provider API 就绪时左键指定、右键清除，未就绪时显示不可用态；右侧一键 Button 仅在 HDLRaidTools／SuperWoW provider 完整就绪时显示 | 实机验证 Player／Target 恢复后的等长短框、`16–32` 个 Debuff 的两至四排净空及另一角色 v5 默认迁移；再验证标记、图腾、坦克／一键 Button、职业栏、popup、DDPS 中央视野、`13` 格 `4/4/4/1` 和姿态 `25 UI` |
