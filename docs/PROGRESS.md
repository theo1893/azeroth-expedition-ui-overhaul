...
|---|---|---:|---|---|
| Core／pfUI | `api/expedition.lua`、`pfUI.lua`、作用域接管路由 | `P5` | pfUI `8.1.0-aeui.5` 已恢复公共绘制、原始默认值、全部未接管模块／skins 与配置入口；只让渡 Chat 辅助模块、Quest Log skin 与 Unit Frame 两张 fill donor；旧全局回退 SavedVariables 一次迁移 | 实机覆盖 Game Menu／`/pfui`、全模块加载、旧 SavedVariables、单模块失败隔离与第三方兼容 |
...
| Unit Frames | `api/unitframes.lua`、`modules/raid.lua`、主／紧凑单位入口 | A／B `P3 / 5/5 exhausted`；B1 `P5`；Raid A1 `P3 / 5/5 exhausted`，Raid A2 `P2 simulation-reviewed`；紧凑 A2 paused | B1 exact Health／Power 只接入 Player／Target／TargetTarget／Focus。Raid A1 五次终止且无产物；用户已选择 `UF-RAID-A2` 的 material-only donor＋Python 精确造壳架构。A2 本地模拟以四张 `592×296 → 74×37` 完整外壳覆盖 40 个真实成员，A-D 维修由确定性 mask 构造；display `7/7 pass`，ImageGen `0/0`，无 source/runtime/addon | 等待用户确认 `UF-RAID-A2-SIM-V1`；确认后仍需单独授权 `UF-RAID-A2-DONOR V1` 才能生图。A1 不得第六次；B1 另待 Turtle WoW P6；主 A／B 是否重开另定，紧凑 A2 暂停 |
