...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A `5/5 exhausted`；B `4/5 / final.r4 final-repair-prepared`；B1 `0/5 wait`；A2 paused | A 终态不变。B attempt 4 保持单开口与单一物理外壳，但 bbox 回归到 `1454×247`，ratio `15.532181%`、anisotropy `13.444030%`、hard safe `7979 px`、isolation `43/39`，且连续压纹与规则系带偏工业；无 candidate/source/runtime。`final.r4` 用绝对坐标锁定 bbox x `126..1410/y386..638` 与开口 x `168..1368/y422..602`，并恢复烟熏粗旧 Target 材料。流程错误仍 `2` | 提交 B `final.r4 / final-repair-prepared` 后执行 attempt 5，只以上一 B raw 为 Image 3；通过即停，失败则 B 耗尽且不得第六次。随后进入 B1，三段终态统一审查。A2 暂停 |
