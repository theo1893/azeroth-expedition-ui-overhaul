...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 只接收显式一次性邻接位置／舒适缩放 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.5 / pending-retest`；Combat Focus 为 `P5 / layout-v1.2 / pending-game-validation` | 第二张 `2560×1440` 实机截图证明 v1.1 未正确处理 `1920×1080 → 2560×1440` 的 `4/3` 显示链：tier 8 已让聊天、小地图和动作区达到舒适尺寸，但 `280 UI` 玩家框仍显示为 `374×114 px` 并覆盖左卷袋。AEUI `0.8.8`／`focus-layout-contract=1.2` 保留 tier 8，只对 Player／Target、双方施法、Swing、姿态与 DoiteDPS 使用 local scale `0.75`，并以目标显示固定坐标取代 scale-dependent UIParent 推导；预计玩家框约 `280 px`、两框内缘约 `80 px`、左卷袋净空约 `3 px`。Combat Deck／Field Kit 尺寸、位置、美术和行为不变；关键战斗信息沿用 provider Alpha，装饰鼠标穿透，无维护循环、无重绘、无外部生成 | 启动游戏验证 `version 0.8.8`、`focus-layout-contract=1.2`、`focus-ui-scale-tier=8`、`focus-layout-display-scale=0.75`，确认玩家／目标不再覆盖卷袋且与动作条保持垂直层次，再验证施法、Swing、DoiteDPS、中文文字、CD、右双槽和视野；需恢复完整 preset 时执行 `/aeui focuslayout comfort`。同轮继续 Field Kit popup、Queue／换装及 fail-open 复测；通过前保持 P5 |
