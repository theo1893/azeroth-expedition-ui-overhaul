...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `UF-A1 V3-A/B final` 与 `UF-B1 V2 final`：`P3 / prompt-authorized / 0/5`；A2 paused | 用户于 `2026-08-11` 接受 V3 模拟并另行授权三段 final，冻结 A→B→B1 顺序、A／B 固定锁定图、同段紧邻前稿有界 edit、B1 首次无图片、每段五次及最坏十五次实际生成预算。流程错误不占额度；跨段和旧失败像素禁止复用。展示区域仍为 `7/7 pass`，无 source/runtime/addon 改动 | 提交授权状态后启动 A attempt 1，逐段完成内部循环；通过即停，第五次仍失败则该段耗尽。A2 继续暂停 |
