...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 视觉与布局桥接；pfUI Player／Target 只接收显式一次性邻接位置／舒适缩放 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v1.7 为 `P5 / pending-retest`；Combat Focus runtime-v1.6 为 `P5 / pending-game-validation`，v1.4／v1.5 `game-geometry-failed` | AEUI `0.8.12`／`focus-layout-contract=1.6` 保留 tier 8、Combat Deck、单位／施法 `0.75` 与 Swing／姿态／DoiteDPS `0.82`，删除投影并直接写游戏原生坐标。用户随后实机报告 pfUI unlock 退出在 `unlock.lua:527` 访问空 `drag`；AEUI `0.8.13`／`fieldkit-contract=1.7` 保持 Bar 6 movable 登记稳定，只在 pfUI 创建 drag 后隐藏绑定态 Bar 6 mover，并在 `pfUI.bars:UpdateConfig()` 后恢复 `12×2` 相对锚。ArchiTotem、Field Kit atlas／provider 行为与位图均不变；无外部生成 | `/reload` 后先开关一次 pfUI 解锁，确认无报错、绑定态只有 Bar 1 mover 且 Bar 6 不跳位；再执行 `/aeui focuslayout comfort` 并验证攻击／施法不重叠、中央空白收紧、单位框不挡人物、DoiteDPS、ArchiTotem、Field Kit popup／Queue／换装；需要回退时 `/aeui focuslayout restore` 后 `/reload` |
