...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题均已修复；QL-A2 V4、18 行左页字体／无描边／类型墨色，以及 Quests `1.25`／Theme `1.8` 的右页金额、动态 ScrollChild、奖励无循环锚点、`108×41px` 双列间隔／换行和原生 `NameFrame` 抑制记为 `P6 user-confirmed`。QL-D V1 最终奖励槽美术仍只有 0／1／2／4／6 display-region `5/5 pass` 的本地模拟，正式生成尚未开始。QS-B1 V1 已在 `5/5` 后由用户明确判定“不可接受”，终态为 `P3 candidate-rejected / user-rejected / repair-budget-exhausted`，没有 source、runtime 或菜单接入；Tracker 与 NPC Quest／Gossip 状态不变 | 确认 QL-D 正式容器方向；处理 Tracker `display-region-blocked`／QT-GEO V2。若继续 QS-B1，先做 V2 本地确定性模拟，禁止 attempt 6、失败候选复用和几何例外晋级。整个模块仍有 P1–P5 范围，不进入 P6-C，正式生图继续等待独立授权 |
