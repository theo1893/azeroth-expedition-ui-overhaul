...
| Action Bars／Field Kit | `actionbar.lua`、`castbar.lua`、`swingtimer.lua`、pfUI Player／Target／TargetTarget + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 的 scoped 视觉与一次性游戏坐标布局 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v2.6、Combat Focus runtime-v2.4 与 Sidebar Group runtime-v1.0 为 `P5 / pending-game-validation`；v1.4／v1.5 `game-geometry-failed` | AEUI `0.8.24`／`ACTION-BARS-CORE-SIM-V11` 保留 V10 的单位框、Aura 与中央计时栈几何；Player／Target／TargetTarget 继续使用客户端系统字体 `OUTLINE / 18 UI`，DoiteDPS 与四栏 `2×2 / 3×4` 组合不变。战士实机反馈后，姿态栏保持 `BOTTOM (0,255)` 并从 local scale `0.72` 放大到 `1.0`；未手调的 exact v7–v14 profile 一次迁移为 v15。Field Kit v2.6 延续职业槽、同事件回锚与 provider-local 停靠，并在绑定态的 Apply、`SetupVisual`、完整配置 `OnShow` 边界同步隐藏真实 `AutoBarAnchorFrameHandle`；显式 `unbind`／AEUI 关闭时恢复 provider 偏好。全部位图字节不变，无外部生成 | `/reload` 后确认红色 AutoBar 拖拽点默认不可见、`autobar-drag-handle=hidden-bound`，三枚战士姿态按钮清晰放大且点击／状态正常；连续执行两次 `/aeui autobar apply` 并开关配置页，确认卷袋位置不变、`autobar-anchor-basis=provider-local`。继续复测系统字体、Player／卷袋、Aura 八枚、Boss 16 Debuff、计时栈、单 mover、四栏可逆 unbind，以及 AutoBar／ArchiTotem／popup／Queue／换装；回退：`/aeui focuslayout restore` 后 `/reload` |
