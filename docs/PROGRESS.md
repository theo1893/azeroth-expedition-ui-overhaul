...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6`；QS-B1 V7-A `P5`；QL-D V3 final art `P3 repairing` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复。用户于 `2026-08-07` 接受 QS-B1 V7-A attempt 5 并授权 P4/P5；Quest runtime `1.26`／Theme `1.9` 保持闭合载体根、火漆与旧按钮 fail-open。QL-D V2 五次正式生成均未晋级且已被用户否决。用户已确认并授权 QL-D V3；attempt 1 的 0／1／2／4／6 display `5/5`，但技术 `17/19`，因错误 aspect、微型摊书身份、等距缝圈、交叉纹理与绿边拒绝。当前 `1/5`、流程错误 `1`，V3.r1 为固定 Image 1／2 fresh-regenerate，未写 source/runtime/addon。Tracker 与 NPC Quest／Gossip 不变 | 固定执行器执行 QL-D V3.r1 attempt 2 并完成候选内审；另在 Turtle WoW 验证闭合态物理接触、滚动裁切及 TGA 方向 |
