...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.5 / pending-retest` | accepted source／TGA／v1.4 外置 drawer 与 `0.30s` 跨格意图保持不变。AEUI `0.8.5`／`fieldkit-contract=1.5` 按用户最新截图把旧双侧软吸附收敛为强绑定 Combat Deck：Bar 6、左 `4×6` 卷袋与右水平饰品双槽直接锚到 Bar 1，绑定态只移动主栏；`unbind` 恢复各自位置，`home` 恢复中心中下基线。当前“大奶黑牛”已写入主栏 `x=0／y=258`、上栏 `x=0／y=291` 与水平双槽。display `9/9＋10/10`、package pass；外部生成 `0`，原生产预算不变 | Turtle WoW 启动或 `/reload` 验证 `fieldkit-contract=1.5`、左卷袋—中央 `12×2`—右双槽、唯一主栏 mover、bind／unbind／home；再复测候选 `1／6／7／12` 跨格保持与 `0.30s` 切换、四种 popup、非 exact 回退、apply／restore、Queue／换装及 `/aeui actionbars` fail-open；通过前保持 P5 |
