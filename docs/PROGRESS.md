...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua` + AEUI Quests adapter | `P1–P5` | QL-A2 V4 固定卷宗背景已静态接入；用户接受 QL-B1 V1.r3 与 QL-B2 V1.r4 bbox-fit 运行时视觉，QL-B0 23 行、QL-B1 四态 atlas 和 QL-B2 同 Alpha 三态书签均已确定性导出、接入并通过静态测试；QL-B3 三槽几何和 A／B／C 三段 V1 完整生产正文已到 `prompt-draft / P2`，当前调用 `0`；QL-C/D 未完成；外部 tracker 暂停 | QL-A2／B0／B1／B2 等待实机；下一非实机门禁是授权 QL-B3-A／B／C V1 |
