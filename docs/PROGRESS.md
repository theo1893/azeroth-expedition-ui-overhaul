...
| Action Bars／Field Kit | `actionbar.lua`、`castbar.lua`、`swingtimer.lua`、pfUI Player／Target／TargetTarget + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 的 scoped 视觉与一次性游戏坐标布局 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v1.8 与 Combat Focus runtime-v1.7 为 `P5 / pending-game-validation`；v1.4／v1.5 `game-geometry-failed` | AEUI `0.8.14`／`ACTION-BARS-CORE-SIM-V6`：Player／Target `240×60 / 0.68`，TargetTarget `132×30 / 0.62` 依附 Target；玩家 Aura 左起、目标 Aura 右起，上 Buff／下 Debuff；玩家施法、Swing、目标施法统一 `180×16 / 0.72` 并排。Field Kit 三段文字移除，停靠间距收紧；仅未被手动调整的 exact v7 原生坐标 profile 在 `/reload` 一次性迁移为 v8，Bar 6／TargetTarget mover 均只在 pfUI 创建 drag 后隐藏并在 unlock 退出时重施相对锚。位图字节不变，无外部生成 | `/reload` 后开关 pfUI unlock，确认无 `drag=nil`、Bar 6／TargetTarget 不跳位；确认单位框不遮人物或卷袋、TargetTarget 依附、Aura 方向、无三段文字、三条等尺寸读数并排和整体紧凑度；再验证 DoiteDPS、ArchiTotem、Field Kit popup／Queue／换装。回退：`/aeui focuslayout restore` 后 `/reload` |
