...
| Unit Frames | `api/unitframes.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | A1 V2 `P2 / simulation-confirmed`；A2／B1 `P3 / paused` | `UF-A1 V1` 已用满 `5/5` 且用户拒绝例外。用户于 `2026-08-11` 确认 `UF-A1-V2-SIM-V2`：每角色四个独立 source，标准 `W=200` 预合成为单 `214×42` shell；可变宽度才用端帽＋中央带三切片；高度固定 `42`。标准 `0.64–1.15×` 与可变 `W=160/200/240` 校验均无接缝／内容侵入，display-region `6/6 pass`、双次重建 SHA 一致。V2-A／V2-B 最终正文及 builder 合同已冻结，尚无正式候选或 runtime | 等待用户分别授权 `UF-A1 V2-A V1` 四端帽与 `UF-A1 V2-B V1` 四横轨：A→B、每段最多 `5` 次、最坏 `10` 次及固定修复边界。继续暂停 A2／B1，不修改 addon |
