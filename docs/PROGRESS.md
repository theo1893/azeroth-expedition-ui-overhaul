...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P3 / prompt-authorized`；`AB.SLOT.BASE.V1` | 用户于 `2026-08-08` 确认 V3 的战斗信息链，并授权 `AB.SLOT.BASE.V1` production Prompt 与最多五次实际生成／修复。固定子进程在启动前被外部数据出境审查阻止：指定 Character V3 锁定图尚无明确的外部 ImageGen 上传授权；已记流程错误 `E1`，无图、无 provider 结果，ImageGen 仍为 `0/5`。首批仍只生产一个普通／空槽基底；未写 profile，无 source/runtime | 用户明确授权将指定 Character V3 锁定图上传至外部 ImageGen 作为 Image 1；之后用已提交正文重试 attempt 1，并执行全量候选内审 |
