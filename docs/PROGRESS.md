...
| Unit Frames | `api/unitframes.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `UF-A1 V2-A V2 P2 / prompt-draft`；A2／B1 `P3 / paused` | 旧 V2-A V1 的单图四端帽循环已用满 `5/5` 并退回。用户明确否决逐端帽独立生成；新 V2-A V2 仍由一次调用输出一张 `1536×1024` 四格 atlas，只通过更详尽的占位、比例、接触与反过度修饰正文约束结果。已确认的八件 source、标准单 shell、可变三切片和 `42px` 高度均不变，因此既有模拟确认继续有效。新版本当前 `0/5`、未授权；V2-B `0/5 / sequence-blocked`。无 source/runtime/addon 改动 | 用户审阅并授权 V2-A V2 完整正文与独立 `5` 次预算；不得计作旧版第六次。V2-A 内部通过后才恢复 V2-B；继续暂停 A2／B1 |
