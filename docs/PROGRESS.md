...
| Unit Frames | `api/unitframes.lua`、`modules/raid.lua`、主／紧凑单位入口 | A／B `P3 / 5/5 exhausted`；B1 `P5`；Raid `P2 simulation-confirmed`；A2 paused | B1 exact Health／Power source 已只接入 Player／Target／TargetTarget／Focus；display `9/9 pass`。用户已确认 Raid `UF-RAID-SIM-V1` 的 40 个真实成员、无共享外框、薄皮革点名名条、四变体与状态密度；`UF-RAID-A1 V1 final` 四格完整背景板 production draft 已通过完整性预检，正式生产尚未执行，且无 source/runtime/addon 变更 | 用户独立授权 `UF-RAID-A1 V1 final`、固定 Image 1／2、同循环紧邻前稿 edit 边界和五次正式资产调用上限；未授权前不上传或生图。B1 另待 Turtle WoW P6；A／B 是否重开另定，A2 暂停 |
