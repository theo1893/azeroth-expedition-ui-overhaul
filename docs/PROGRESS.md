...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；QL-A2 V4、18 行左页字体／无描边／类型墨色，以及 Quests `1.25`／Theme `1.8` 的奖励无循环锚点、双列间隔／换行与原生 `NameFrame` 抑制记为 `P6 user-confirmed`。QL-D V1 最终美术仍为本地模拟。QS-B1 V3-A／V4-A 各 `5/5` 失败终止，V3-B `0/5`。V14／V5-A 将生成模型限为全幅连续暗旧布面 donor，由 tracked 固定 crop／`4×` polygon mask 独占 `128×696` 轮廓、Alpha 与双钝缺口；真实六态 `46/46`、display-region `6/6`、模拟生图 `0/0`。用户已确认 V14，并独立授权 V5-A 最终正文、固定 Image 1／2、确定性 crop/mask、受限同循环 Image 3 edit 和最多五次实际调用；当前 `prompt-authorized / P3 / production 0/5`。无候选、source、runtime 或菜单接入。Tracker 与 NPC Quest／Gossip 状态不变 | 提交授权正文与 deterministic reviewer 后执行 V5-A attempt 1 和完整 composite／真实排版／display-region 内审；内部通过只交用户复审。旧按钮继续 fail-open，不执行 V3-B、不导出、不接入；QL-D、Tracker 与整模块 P6-C 门禁保持 |
