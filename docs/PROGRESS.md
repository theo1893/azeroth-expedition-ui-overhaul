...
| Chat | `modules/chat.lua` + AEUI Chat adapter | 核心 `P5` / r1.21；Tab 替换 `P3` rejected | Full V1 主框九宫格、右框回收、V3 Tab／未读保持；`CHAT.INPUT.DARK.V1.r3 attempt 4` 已导出为 `ChatInputDarkV1.tga`，normal／focus 共同 Alpha、三段 UV、真实排版和 display-region 均通过。`CHAT.TABS.DARK.V2` 固定 0.143.0 循环已耗尽 `5/5`、流程错误 `0`；attempt 5 仍因 source cell 外 `98555` 像素、五对象 4px 隔离失败和轮廓偏规整被内部退回，未改 runtime。v1.21 保持客户端与 provider 的经典配色 | 禁止 V2 attempt 6；用户另行授权新批次或确定性重排合同例外；游戏设备可用时 `/reload` 验证核心 runtime |
