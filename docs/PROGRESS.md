...
| Chat | `modules/chat.lua` + AEUI Chat adapter | 核心 `P5` Full V1 / r1.19；输入候选 `P3` | 主框九宫格、右框回收、V3 Tab／输入／未读及暖黑纸面 Vanilla 色板保持。用户已接受 `CHAT.INPUT.DARK.V1-SIM` 暖烟草抄写纸条方向并独立授权精确生产版本、固定三图与有界修复循环；正式 runtime 尚未改变 | 执行输入候选的固定生产与完整审查；内部通过后交用户接受。游戏设备可用时再 `/reload` 验证核心 r1.19 与输入焦点、IME、历史 |
