...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A `2/5 / final.r2 repair-prepared`；B/B1 `0/5 sequence-wait`；A2 paused | A attempt 2 已把双槽修为单开口，物理连通、ratio `4.518966%`、anisotropy `4.323585%` 和 isolation 均通过；但左右端板／上下内唇仍造成 hard safe core `57077 px` 侵入，且连续缝线偏工业，故仍无 candidate/source/runtime。`final.r2` 保持单开口和粗旧皮革，要求开口至少 `1200×180`、端带／轨道压入 `42/36 source px` 并打断规则缝线。流程错误仍 `1` 且不占额度 | 提交 `final.r2 / repair-prepared` 后执行 A attempt 3，只以 attempt 2 raw 为 Image 3；A 终态后依次 B、B1。通过即停，第五次仍失败则耗尽；A2 继续暂停 |
