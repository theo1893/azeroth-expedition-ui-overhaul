...
| Map | `P2–P5` | runtime `1.1`；World／Mini 外壳已在实机加载；当前大地图外框越界／角连接和小地图屏幕边距／铭牌避让待修；动态地图、pfQuest、真实控件及 Farm mode 保持 provider 所有权 | 修正外壳布局，再验证缩放、层级、动态文字、相邻控件、Farm mode 与 `/aeui map` 回退 |
...
| Unit Frames | `P5` | runtime `1.6`；外壳已在实机加载；当前放大布局下端部身份不清，次级框名称／生命文字挤压待查 | 检查 base／rim 层序、九切片和文字净空，再验证状态层序、Aura／施法条及禁用回退 |
