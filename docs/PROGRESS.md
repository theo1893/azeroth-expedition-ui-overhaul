...
| Chat | `modules/chat.lua` + AEUI Chat adapter | 核心 `P5` / r1.20 | Full V1 主框九宫格、右框回收、V3 Tab／未读及暖黑纸面 Vanilla 色板保持；`CHAT.INPUT.DARK.V1.r3 attempt 4` 已从固定 source 确定性导出为 `ChatInputDarkV1.tga`，normal／focus 共同 Alpha、三段 UV、`380/480 × 25px` 真实排版和五场景 display-region 均通过。adapter 只替换可见输入贴图，真实 EditBox 行为保持；旧 V3 输入保留为回退 | 游戏设备可用时 `/reload` 验证 `chat-runtime=1.20`、书框、输入 normal/focus、文字、光标、IME、频道头、历史、键盘行为、Tab、缩放与右框消息回收 |
