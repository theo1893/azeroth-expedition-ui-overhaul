...
| Chat | `modules/chat.lua` + AEUI Chat adapter | 核心 `P5` Full V1 / r1.19；输入候选 `P3` | 主框九宫格、右框回收、V3 Tab／输入／未读及暖黑纸面 Vanilla 色板保持。`CHAT.INPUT.DARK.V1` 已计 `3/5`：attempt 3 仍未移除重复压纹与连续 focus 上缘；正式 runtime 尚未改变 | 执行冻结边界内的完整 `.r3`，只用固定 Image 1／2／3 regenerate，复核平静烟熏纸面；内部通过后交用户接受。游戏设备可用时再 `/reload` 验证核心 r1.19 与输入焦点、IME、历史 |
