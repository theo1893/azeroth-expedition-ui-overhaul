...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P3 / dual-candidate-reviewed` | Field Kit V2 的 AutoBar `4×6 / 24 类` 三段分组、原生 popup 与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempt 4 canonical `82dd2260…c012` 在 `4/5` 全门禁通过；Consumable attempt 1 raw `de25567f…b8ba`／canonical `623f29c5…a2419` 在 `1/5` 同样通过 Prompt、语义、美术、技术与 `16/16` display。两者均为 `candidate-reviewed / pending-user-acceptance`，按 pass 即停；纯绿 raw→canonical 修订不重绘、不新增输入图、预算不重置 | 提交 Consumable attempt 1 记录后，请用户分别接受或拒绝 Trinket 第4稿与 Consumable 第1稿；接受前不得晋级。若拒绝，Trinket 余 `1` 次、Consumable 余 `4` 次；均禁止 attempt 6，不启用 AutoBar、不改 TrinketMenu SavedVariables |
