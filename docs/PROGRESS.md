...
| Unit Frames | `api/unitframes.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | A1 V2 `P2 / simulation-reviewed`；A2／B1 `P3 / paused` | `UF-A1 V1` 已用满 `5/5` 且用户拒绝例外。`UF-A1-V2-SIM-V2` 保留 Player／Target 各四个独立 source，但标准 `W=200` 预合成为每角色一张 `214×42` shell；`0.64–1.15×` 内部 Texture 接缝 `0`、内容侵入 `0`。可变 `W=160/200/240` 才用端帽＋中央带三切片，在 `0.71/1.00×` 接头空洞／内容侵入均 `0px`；display-region `6/6 pass`，双次重建 SHA 一致，ImageGen `0/0`。无 production source/runtime；A2／B1 均 `0/5` | 等待用户确认单 shell／三切片运行时合同；确认后冻结 V2-A 四端帽、V2-B 四横轨及 builder，再单独请求正式生产授权。继续暂停 A2／B1，不修改 addon |
