...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6`；QS-B1 V7-A `P5`；QL-D final art `P3` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复。用户于 `2026-08-07` 接受 QS-B1 V7-A attempt 5 并授权 P4/P5；Quest runtime `1.26`／Theme `1.9` 保持闭合载体根、火漆与旧按钮 fail-open。QL-D V2 奖励槽方向与 production 正文已分别确认／授权；attempt 1／2／3 分别因 non-square／aspect `3.3484`、aspect `3.0576` 与 aspect `2.56085` 内部退回。attempt 3 technical `18/19`、display `5/5`，是当前视觉最接近稿但仍越过硬门槛；V2.r4 已收紧为 `880×332`／aspect `2.62..2.67`，准备 fixed Image 1／2 fresh regenerate，当前 `3/5`，候选未接受前不接入。Tracker 与 NPC Quest／Gossip 不变 | 执行 QL-D attempt 4；另在 Turtle WoW 验证闭合态物理接触、滚动裁切及 TGA 方向；独立完成七纹章和代理 parity 后才启用菜单 |
