...
| Unit Frames | `api/unitframes.lua`、`modules/raid.lua`、主／紧凑单位入口 | Player／Target V4 `P3 candidate-reviewed`；V3 A／B `P3 / 5/5 exhausted` 历史；B1 `P5`；Raid A2 `P5 source/runtime/addon / 5/5`；紧凑 A2 paused | 用户已确认 `UF-PRIMARY-V4-SIM-V1` 并授权 Raid A2 sample 只读输入。`UF-PRIMARY-V4-CANDIDATE-V1` 已确定性形成两张独立 `1284×252 → 214×42` 透明候选；安全区／透明清理／非镜像维修均通过，展示区域 `10/10 pass`，没有新增位图生成。B1 与 Raid A2 既有 P5 不变，V3 rejected pixels 禁止复用 | 用户审查 Player／Target exact candidate pixels；接受前不晋级 source、不导出 runtime、不接入 addon。Raid／B1 继续等待 Turtle WoW P6 |
