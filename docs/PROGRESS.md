...
| Chat | `modules/chat.lua` + AEUI Chat adapter | 核心 `P5` Full V1 / r1.19；输入候选 `P3` | 主框九宫格、右框回收、V3 Tab／输入／未读及暖黑纸面 Vanilla 色板保持。`CHAT.INPUT.DARK.V1` attempt 1 已计 `1/5`：几何、真实排版与展示区域通过，但缝线皮革槽物件身份、长导线及橙金 focus 未通过；正式 runtime 尚未改变 | 执行冻结边界内的完整 `.r1` Image 4 edit，按同一真实排版审查；内部通过后交用户接受。游戏设备可用时再 `/reload` 验证核心 r1.19 与输入焦点、IME、历史 |
