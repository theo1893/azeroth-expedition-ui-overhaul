...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A `1/5 / final.r1 repair-prepared`；B/B1 `0/5 sequence-wait`；A2 paused | A attempt 1 材料方向成立，但 deterministic review 检出双开口、bbox `1425×224`、比例误差 `24.853972%`、各向异性 `19.906433%` 与 hard safe core 侵入，故未产出 candidate/source/runtime。自包含 `final.r1` 只保留粗旧皮革及左右身份，明确改单开口和约 `1284×252`。一次无生成证据的 reviewer 性能错误已线性化修复，不占额度；展示区域模拟仍 `7/7 pass` | 提交 `final.r1 / repair-prepared` 与 reviewer 后执行 A attempt 2，只以上一 raw 为 Image 3；A 终态后依次 B、B1。通过即停，第五次仍失败则耗尽；A2 继续暂停 |
