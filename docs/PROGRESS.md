...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6`；QS-B1 V7-A `P5`；QL-D V3 final art `P2 simulation-proposed` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复。用户于 `2026-08-07` 接受 QS-B1 V7-A attempt 5 并授权 P4/P5；Quest runtime `1.26`／Theme `1.9` 保持闭合载体根、火漆与旧按钮 fail-open。QL-D V2 五次 actual ImageGen 均未晋级，且用户随后否决其规则现代感。V3 本地稿改为粗裁皮革补片＋撕边羊皮纸装备签＋不等手扎／错位暗铜痕，保持真实 `108×41px` 几何；0／1／2／4／6 display `5/5`，ImageGen `0/0`。Tracker 与 NPC Quest／Gossip 不变 | 用户审查 `QL-D-SIM-V3`；确认前不生成正式资产；另在 Turtle WoW 验证闭合态物理接触、滚动裁切及 TGA 方向 |
