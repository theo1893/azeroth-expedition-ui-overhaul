...
| Chat | `modules/chat.lua` + AEUI Chat adapter | 核心 `P5` Full V1 / r1.19；输入候选 `P2` | 主框九宫格、右框回收、V3 Tab／输入／未读及暖黑纸面 Vanilla 色板保持。用户已于 `2026-08-03` 接受 `CHAT.INPUT.DARK.V1-SIM` 暖烟草抄写纸条方向；普通／聚焦同几何，真实 `380 × 25`、扩展 `480 × 25` 与五场景 display-region 均通过。稳定 Prompt 和精确生产合同已就绪；正式 runtime 尚未改变，生产预算未开启 | 用户独立授权 `CHAT.INPUT.DARK.V1` 完整正文、固定三图上传、最多五次实际生图与冻结 edit 边界。游戏设备可用时再 `/reload` 验证核心 r1.19 与输入焦点、IME、历史 |
