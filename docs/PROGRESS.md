...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`，QL-D V1 最终美术仍为本地模拟。QS-B1 V3-A／V4-A 各 `5/5` 失败终止，V3-B `0/5`。用户于 `2026-08-06` 明确接受 V5-A attempt 4 deterministic composite；精确 `128×696 RGBA` 暗旧布底已固化为 tracked source 与 manifest，状态 `source-accepted / P4 / 4/5`，流程错误 `1`，attempt 5 未调用。该验收不覆盖七枚独立纹章、runtime 或菜单代理，旧按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 状态不变 | 确定性导出已接受布底；另行解除七纹章生产门禁并完成七项代理 parity 后再接入菜单、隐藏旧按钮。QL-D、Tracker 与整模块 P6-C 门禁保持 |
