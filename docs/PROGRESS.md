...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P3 / candidate-reviewed`；`AB.SLOT.BASE.V1` | V3 战斗信息链已确认。`AB.SLOT.BASE.V1` 固定执行器有界循环在 ImageGen `5/5` 停止；attempt 5 exact canonical RGBA `6d4a4d16…7dc0` 以深褐主导、断续暗黄铜和低频安静中心通过全量内审，五种真实排版 `5/5 pass`、violations `0`。provider raw `1254²`／近绿色背景作为 provenance 保留，确定性 review normalization 不重绘。未写 profile，无 accepted source/runtime | 用户复审 attempt 5 exact canonical；接受后才建立 source／manifest／export，拒绝则记录并停止，当前授权不得 attempt 6 |
