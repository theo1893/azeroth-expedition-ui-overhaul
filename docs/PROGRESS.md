...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P3 / prompt-authorized`；`AB.SLOT.BASE.V1` | 用户于 `2026-08-08` 确认 V3 的 DoiteDPS → 攻击计时 → Aura／双框 → 双施法条 → 动作栏纵向焦点，并明确授权固定 Character V3 输入下的 `AB.SLOT.BASE.V1` 完整 production Prompt 与最多五次实际生成／修复。首批仍只生产一个普通／空槽基底，不含 Rail 或假 disabled 状态；当前 ImageGen `0/5`，未写 profile，无 source/runtime | 使用固定 `@openai/codex@0.143.0` 执行 attempt 1；每次完成语义、组件、技术像素、真实排版与展示区域内审，通过即停止 |
