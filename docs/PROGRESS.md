...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`。QS-B1 V5-A 布底已 source-accepted。V5-B 已完成固定 0.143.0 五次实际 generation，终态 `candidate-rejected / repair-budget-exhausted / P3 / 5/5`：终稿第八格与 ABANDON 通过，但其余六项仍越出 safe box，RESET 有 `44` 绿残留。无纹章 source/runtime，旧按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 不变 | 用户选择全画布连通域裁切合同例外、七张单对象 source 新版本／新预算，或放弃；禁止 attempt 6 |
