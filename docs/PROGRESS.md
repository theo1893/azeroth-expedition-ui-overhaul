...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua` + 可选 AutoBar／TrinketMenu 视觉桥接 | `P1 / simulation-reviewed`；P2 待用户 | 十二条 pfUI Bar、真实状态、布局公式、移动／缩放和目标设备 `1920×1080 / UI Scale 0.812698` 已审计。V1 采用可适配任意合法行列的窄 Rail、底部 `12×1` 主／副层、成对狮鹫、`5×2` 炼金卷袋与装备槽 `13／14` 双护套；预设不强写现有 profile，当前无 source/runtime | 用户确认或否决 V1 的布局、隐喻、材质层级与综合色重；确认后才锁定 P2 并逐批授权 production Prompt |
