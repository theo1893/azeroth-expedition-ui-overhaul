...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.1 / pending-retest` | Trinket 第4稿与 Consumable 第1稿的 accepted source、TGA 文件／像素 SHA 均不变。首次实机检查发现 AutoBar Count 可见但多枚 Icon 被口袋遮挡，TrinketMenu 双槽也只剩护套；根因是后创建的 A／B 与 ActionButtonTemplate Icon 共用 `BACKGROUND`。`fieldkit-contract=1.1` 已把 AutoBar `24+12`、TrinketMenu `2+30` 的 A／B 移入低于真实 Button `1` 级的非交互子 Frame。新增 `/aeui autobar open／apply／restore`：只有用户主动 apply 才备份并写当前角色 `4×6` profile，可逆且不启用 provider、不改其他角色／TrinketMenu。display `9/9`＋`7/7`、package pass，本次外部生成 `0` | Turtle WoW `/reload` 后先复测 AutoBar／TrinketMenu 的 Icon、Count、Cooldown、Queue 层序；再执行 apply／restore 验证 `4×6` 三组与可逆配置，并完成四向 popup、候选／换装／拖动／缩放／停靠及 `/aeui actionbars` 回退；通过前保持 P5 |
