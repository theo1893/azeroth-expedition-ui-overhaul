...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`。QS-B1 V5-A 布底已 source-accepted；V5-B worksheet 在 `5/5` 后失败并冻结。用户于 `2026-08-06` 接受 V16 七张单对象拓扑；accepted UI 本地预演 `59/59`、展示区 `6/6`，当前 `simulation-confirmed / production-draft / P2 / local simulation 0/0 / production 0/35 not-authorized`。V6-A..G 七份完整正文与修复边界已准备；无纹章 source/runtime，旧按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 不变 | 用户明确授权 V6-A..G 七段生产正文与逐段 `5` 次／最坏 `35` 次预算；授权前不得生成资产 |
