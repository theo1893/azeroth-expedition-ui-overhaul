...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`。QS-B1 V5-A 布底已 source-accepted。用户于 `2026-08-06` 接受 V15 综合色并独立授权 V5-B，当前 `prompt-authorized / execution-blocked / P3 / 0/5`：固定 0.143.0 child 的 remote plugin catalog／内置工具发现经 resume 仍失败，六次流程错误均无生成证据。无纹章 source/runtime，旧按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 不变 | 等待固定 0.143.0 内置 `image_gen` 路径恢复，或用户另行授权改变执行路径；预算仍完整。恢复后执行最多五次循环并进行候选验收 |
