...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P3 / trinket-candidate-reviewed` | Field Kit V2 的 AutoBar `4×6 / 24 类` 三段分组、原生 popup 与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempt 4 raw `2e4efc1a…19e3a`／canonical `82dd2260…c012` 在 `4/5` 通过 Prompt、语义、美术、技术与 `16/16` display 全门禁，现为 `candidate-reviewed / pending-user-acceptance`，按 pass 即停且不执行第 5 次。纯绿 raw→canonical 修订不重绘、不新增输入图、预算不重置；Consumable 完整 transport-amended 正文已准备，仍为 `0/5` | 提交 attempt 4 记录与 Consumable 执行正文后执行 Consumable attempt 1；通过即停，否则最多 `5/5`，禁止 attempt 6。两套候选均须用户接受后才可晋级；不启用 AutoBar、不改 TrinketMenu SavedVariables |
