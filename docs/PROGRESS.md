...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P3 / generate` | V3 战斗信息链已确认。`AB.SLOT` accepted source `6d4a4d16…7dc0` 与 runtime TGA `5c49a1db…23ca` 已通过 scoped adapter、display、package 及用户实机验证。Rail accepted source `7c49995d…32e9` 已确定性导出为 TGA `1e5cca09…0a3d`，Bar `1–12`＋Bar `1／6` merged adapter、display／package／P6 六项均 pass。Field Kit V2 的 AutoBar 完整 `4×6 / 24 类`、三段分组、原生 `1–12` popup 与 TrinketMenu 双槽方向已获用户确认；两个独立 production body、各 `5` 次预算和各自 Character V3 Image 1 上传均已授权。Trinket attempt 1 raw `fe4b854e…c9e8d` 已计 `1/5`：四对象语义与 display `16/16` 成立，但 `1254² RGB`、烘焙棋盘、无 Alpha、A／B／C margin 不足，故失败；自包含 `r1` 已就绪。Consumable 保持 `0/5` | 提交 attempt 1 失败证据、reviewer／测试和 `r1` 后执行 Trinket attempt 2；通过即停，最迟 `5/5`。之后再独立执行 Consumable。全过程不启用 AutoBar、不改 TrinketMenu 保存值；内部通过候选仍须用户验收后才能晋级 source／runtime |
