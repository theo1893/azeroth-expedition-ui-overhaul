...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua` + 可选 AutoBar／TrinketMenu 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P1 / simulation-reviewed`；P2 待用户 | 十二条 pfUI Bar、真实状态、布局公式、移动／缩放和目标设备 `1920×1080 / UI Scale 0.812698` 已审计。用户否决 V1 的贴底动作条与分散单位框；V2 把 `scale=1.2`、约 `39px` 的主栏上移到 `y=827`，玩家／目标框同基线收拢为 `80px` 内缘间距，消耗品与饰品同步上移，默认不显示狮鹫。display `9/9`、布局 `20/20 pass`；未写 profile，无 source/runtime | 用户确认或继续修订 V2 的战斗焦点高度、单位框距离、随身栏位置与综合色重；确认后才锁定 P2 并逐批授权 production Prompt |
