...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题均已修复；QL-A2 V4、18 行左页字体／无描边／类型墨色，以及 Quests `1.25`／Theme `1.8` 的右页金额、动态 ScrollChild、奖励无循环锚点、`108×41px` 双列间隔／换行和原生 `NameFrame` 抑制记为 `P6 user-confirmed`。QL-D V1 最终奖励槽美术仍只有本地模拟。QS-B1 V1 保持 `P3 candidate-rejected / user-rejected / repair-budget-exhausted / 5/5`；V10 页外索引签已被用户改向。V11 的 ScrollChild 内火漆＋七段公会授印绶带方向已确认；完整 V2 生产正文、固定 Image 1／2、紧邻 Image 3 edit 与最多五次实际生成已获明确授权，当前 `P3 prompt-authorized / 0/5`，无 source、runtime 或菜单接入。Tracker 与 NPC Quest／Gossip 状态不变 | 用固定执行器执行 QS-B1 V2 最多五次的自主 review／repair；内部通过即停并等待用户接受候选。QL-D 与 Tracker 门禁保持；禁止 V1 attempt 6、失败候选复用、自动晋级 source 或提前接入。整个模块不进入 P6-C |
