...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.4 / pending-retest` | accepted source／TGA／抽屉可见布局不变。v1.2 的 exact `24 / 4×6` 外置 drawer 与双侧软吸附保留；v1.3 外缘通道仍因穿越其他主格触发原生 `SetPopupButton` 而实机失败。AEUI `0.8.4`／`fieldkit-contract=1.4` 仅在 exact 外置态对不同主格的 popup 切换加入 `0.30s` 意图停留：跨格前往抽屉时保持原分类，明确停留时调用捕获的 provider 原方法；NATIVE、非 exact、关闭态与调度 API 缺失都立即回退。外部生成 `0`，原生产预算不变 | Turtle WoW `/reload` 从内侧主格横穿相邻主格进入左右候选 `1／6／7／12`，确认原抽屉保持；在另一主格停留约 `0.30s` 确认正常切换，完全离开后正常关闭。再验证四种 popup 模式、非 exact 立即回退、双侧拖离／回吸附、apply／restore、饰品候选／Queue／换装及 `/aeui actionbars` fail-open；通过前保持 P5 |
