...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题均已修复；QL-A2 V4、18 行左页字体／无描边／类型墨色，以及 Quests `1.25`／Theme `1.8` 的右页金额、动态 ScrollChild、奖励无循环锚点、`108×41px` 双列间隔／换行和原生 `NameFrame` 抑制记为 `P6 user-confirmed`。QL-D V1 最终奖励槽美术仍只有本地模拟。QS-B1 V2 在 `4/5` 后被用户以质感差、过度工整和单图无法独立隐藏功能取代；attempt 5 未调用。V12／V3 已拆为动态空白旧布背景与七张独立透明纹章，真实排版 `35/35`、display-region `6/6`；用户已回复“可以”确认方向。V3-A 连续空白 max master 与 V3-B 七格独立纹章完整生产正文已准备，各 `0/5`，无 source、runtime 或菜单接入。Tracker 与 NPC Quest／Gossip 状态不变 | 独立授权 V3-A／V3-B 固定输入、同段 edit 边界和各 `5` 次／最坏 `10` 次实际 ImageGen；候选接受前旧按钮继续 fail-open。QL-D 与 Tracker 门禁保持，整个模块不进入 P6-C |
