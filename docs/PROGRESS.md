...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 为 `P6 / game-validated`；`AB.RAIL.V1` 为 `P3 / candidate-reviewed`；`AB.FIELDKIT.V1` 为 `P2 / simulation-reviewed` | V3 战斗信息链已确认。`AB.SLOT` accepted source `6d4a4d16…7dc0` 与 runtime TGA `5c49a1db…23ca` 已通过 scoped adapter、display、package 及用户实机冷却／距离红／按下反馈验证。Rail 固定生产已在 `5/5` 停止，attempt 5 exact canonical RGBA `7c49995d…32e9` 通过 complete-object normalization、technical `4/4`、真实九宫格布局 `8/8`、violations `0` 与内部视觉审查；raw `3f92fb61…ac42` 只作 provenance，禁止 attempt 6，尚未创建 source／runtime。Field Kit V1 的 `5×2` 已按用户容量／分组要求修订；当前 `AB-FIELDKIT-SIM-V2` 使用 AutoBar 完整 `4×6 / 24 类`，分为应急／增益／工具三段，分类内仍保留原生 `1–12` popup；TrinketMenu 方向不变。display `16/16`、布局 `72/72`、violations `0`、外部生成调用 `0` | 用户接受或拒绝 Rail attempt 5 exact canonical candidate；接受前不晋级 source／runtime。另等待用户接受或修订 `AB-FIELDKIT-SIM-V2`；Field Kit 确认前不启用 AutoBar、不改 TrinketMenu 保存值、不执行其 production；`AB.SLOT` 单组件收口仍另行待办 |
