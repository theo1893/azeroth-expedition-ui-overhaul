...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua` | `P2–P4` | 当前首要大面积 UI；QL-A1 source 已确认；QL-A2 V3.3 的 B1 已在 `5/5` 后退回，B2 attempt 3 仍失败且 attempt 4 已在调用前计数，B3 仍获授权，当前 `9/15`；外部 tracker 暂停 | 只用固定 Image 1 执行 B2 attempt 4；整批装配仍被 B1 阻塞 |
