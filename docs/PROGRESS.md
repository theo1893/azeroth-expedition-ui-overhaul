...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A `5/5 exhausted`；B `2/5 / final.r2 repair-prepared`；B1 `0/5 wait`；A2 paused | A 终态不变。B attempt 2 的单开口／物理连通／ratio `6.366135%`／anisotropy `5.985115%` 通过；残余 hard safe `27311 px`，归一化四边多 `46/42/11/13 source px`，右 isolation `73` 次级失败。无 candidate/source/runtime。`final.r2` 仅移动四边内脸、收至约 `1284×252` 并保持 Target 左磨损／右暗黄铜身份。流程错误仍 `2` | 提交 B `final.r2 / repair-prepared` 后执行 attempt 3，只以上一 B raw 为 Image 3，禁止复用 A；B 终态后 B1。三段终态后统一审查；A2 暂停 |
