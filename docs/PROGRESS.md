...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`，QL-D V1 最终美术仍为本地模拟。QS-B1 V5-A 深色布底已于 `2026-08-06` 固化为 `128×696 RGBA` tracked source，`source-accepted / P4 / 4/5`。旧 V3-B 仍为 `0/5 gated`；同日用户已接受 V15／V5-B 的六项哑光低饱和旧赭金矿物颜料与放弃项灰暗酒红综合色，并独立授权 V5-B 完整生产正文、固定输入、受限同循环 edit 与五次上限；当前 `prompt-authorized / P3 / 0/5`。无纹章 source/runtime，旧按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 不变 | 执行 V5-B 最多五次自主生成—审查—修复循环；内部通过后等待用户接受具体候选，再完成确定性布底导出、代理 parity 与菜单接入。QL-D、Tracker、P6-C 门禁保持 |
