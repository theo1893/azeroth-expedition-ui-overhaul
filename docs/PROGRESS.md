...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题均已修复；QL-A2 V4、18 行左页字体／无描边／类型墨色，以及 Quests `1.25`／Theme `1.8` 的右页金额、动态 ScrollChild、奖励无循环锚点、`108×41px` 双列间隔／换行和原生 `NameFrame` 抑制记为 `P6 user-confirmed`。QL-D V1 最终奖励槽美术仍只有本地模拟，正式生成尚未开始。QS-B1 V1 保持 `P3 candidate-rejected / user-rejected / repair-budget-exhausted / 5/5`；V10 页外索引签已被用户改向。V11 的 ScrollChild 内火漆＋七段公会授印绶带可见方向已获用户确认，模拟 `21/21 pass`、display-region `4/4 pass`；完整 V2 生产正文已准备，新一轮正式生成仍为 `0/5`，无 source、runtime 或菜单接入。Tracker 与 NPC Quest／Gossip 状态不变 | 等待用户独立授权 QS-B1 V2 固定输入、紧邻 edit 边界与最多五次实际生成；授权前不生成、不上传、不导出、不接入。QL-D 与 Tracker 门禁保持；禁止 V1 attempt 6、失败候选复用和几何例外晋级。整个模块不进入 P6-C |
