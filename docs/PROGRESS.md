...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `UF-A1 V3-A/B final` 与 `UF-B1 V2 final`：`P2 / simulation-confirmed / awaiting-production-authorization / 0/5`；A2 paused | 用户于 `2026-08-11` 接受 `UF-PRIMARY-V3-SIM-V1` 的可见方向；完整外壳、粗犷旧皮革、非镜像身份、Health／Power 层级和 Mana／Rage／Focus／Energy 经典乘色已写入三段 final。本地模拟未调用远程生图，Player／Target、四 Power mode 与 `W=160/240` 共 `7/7 display pass`；不接受模拟像素，旧失败像素禁止复用，无 source/runtime/addon 改动 | 用户另行授权三段 final 的正文、输入职责、修复边界与预算后才按 A→B→B1 执行；每段最多 `5` 次实际生图、最坏 `15` 次，流程错误不占额度；A2 继续暂停 |
