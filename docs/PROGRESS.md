...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2 / simulation-confirmed`；`AB.SLOT` prompt-draft | 用户于 `2026-08-08` 确认 V3 的 DoiteDPS → 攻击计时 → Aura／双框 → 双施法条 → 动作栏纵向焦点，保留 provider 自由移动和 V2 动作区；display `9/9`、综合布局 `46/46 pass`、V2 像素回归一致。首批已拆为单一 `AB.SLOT.BASE.V1` 基底，不含 Rail 或假 disabled 状态；ImageGen `0/5`，未写 profile，无 source/runtime | 用户明确授权 `AB.SLOT.BASE.V1` 完整 production Prompt 与最多五次修复预算；“继续”不等于生图授权 |
