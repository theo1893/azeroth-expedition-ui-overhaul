...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A `5/5 exhausted`；B `1/5 / final.r1 repair-prepared`；B1 `0/5 wait`；A2 paused | A 终态不变。B attempt 1 独立生成，单开口／物理连通／isolation 通过；bbox `1354×305` 的 ratio/aniso 均 `12.872683%`，hard safe core `66780 px`，无 candidate/source/runtime。Target 左磨损折痕／右黄铜损伤可保留；`final.r1` 要求约 `1284×252` 薄皮环、开口至少 `1200×180`、右损伤压进极端 `42 px`。流程错误仍 `2` | 提交 B `final.r1 / repair-prepared` 后执行 attempt 2，只以上一 B raw 为 Image 3，禁止复用 A；B 终态后执行 B1。三段终态后统一用户审查；A2 暂停 |
