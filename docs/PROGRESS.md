...
| Group Finder | `P5 / standalone MVP` | `0.1.0` 使用 `AEGF1` 自定义频道同步完整招募目录，面板直接显示团长、T/N/D、当前／目标职业、最低装等与装备竞争摘要；申请通过 Turtle 定向 addon whisper 传输职业、职责和客户端自报装等，不依赖 LFT 或外部服务器 | 双客户端验证频道是否全服／跨阵营、频道隐藏、发布／同步／过期、最低装等拒绝、邀请／拒绝／停招及频道限流 |
