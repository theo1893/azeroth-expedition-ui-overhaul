...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 视觉与布局桥接；pfUI Player／Target 只接收显式一次性邻接位置／舒适缩放 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v1.6 为 `P5 / pending-retest`；Combat Focus v1.4 为 `P5 / pending-game-validation` | 用户以新实机截图否决 V4 后明确“确认接入” `ACTION-BARS-CORE-SIM-V5`。AEUI `0.8.10`／`focus-layout-contract=1.4` 保持 tier 8、Combat Deck 与 Field Kit 尺寸不变：Player／Target 与双方施法使用 `0.75`，Swing／姿态／DoiteDPS 保持 `0.82`；显式 preset 从重置后的 live Bar 1 中心／顶边一次性投影，普通 refresh 不维护坐标。`fieldkit-contract=1.6` 继续将 ArchiTotem `1.7` 真实根作为主栏下方职业卫星。layout `59/59`、simulation／runtime display 均 `8/8`、Lua smoke 与 fresh-checkout package pass；无新位图或外部生成 | `/reload` 后执行一次 `/aeui focuslayout comfort`，确认攻击／施法不重叠、中央空白收紧、单位框不挡人物，并实机验证双方状态／施法、近战／远程 Swing、DoiteDPS、ArchiTotem 全交互及 Field Kit popup、Queue／换装回归 |
