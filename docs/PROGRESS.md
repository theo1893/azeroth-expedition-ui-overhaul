...
| 模块 | 阶段 | 当前结论 | 下一门禁 |
|---|---:|---|---|
| Core／pfUI | `P5` | AEUI `0.8.30`；pfUI 公共功能保留，Chat、Quest、Unit Frames 等只通过精确 route 接管 | 实机验证模块隔离、旧 SavedVariables、配置页和禁用回退 |
...
| Map | `P2` | 羊皮大地图和黄铜罗盘方向已确认，尚未接入 | 实机对象／尺寸／状态审计 |
| Character | `P2 / paused` | 香草同构角色面板方向保留，当前使用 pfUI 默认 skin | 等用户明确恢复 |
| Unit Frames | `P4–P5` | runtime `1.2`；头像关闭、Health／Power、Raid A2 已接入；Player／Target V4 source 已接受但未导出 | 实机验证现有 P5；解决 V4 `214×42` 与现有 `240×60` 几何兼容 |

## 尚未启动

Combat HUD、DPS／Threat、Bags／Loot／Roll、Spell／Talent／Profession、Economy、
Social 与 System 继续使用 pfUI 默认实现。启动时先完成真实对象对齐，不为规划
中的假组件提前生成资产或增加文档。
