...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P3 / contract-reauthorization-required` | Field Kit V2 的 AutoBar `4×6 / 24 类` 三段分组、原生 popup 与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempts 1／2 raw `fe4b854e…c9e8d`／`85f3f6f0…50b7` 已计 `2/5`，两次均因固定服务只返回 `1254² RGB` 烘焙棋盘而技术失败；attempt 2 降噪与 B／D margin 改善，但 A／C margin 不足且 C 为空心。两稿真实排版均 `16/16 pass`。Consumable 保持 `0/5` | 需用户授权把两套剩余外部 raw 改为纯 `#00FF00` RGB，并本地只做确定性归一／逐 cell fit／边缘连通色键转 final `1024² RGBA`；不重绘、不新增输入图、预算不重置。授权前不执行 attempt 3 或 Consumable attempt 1 |
