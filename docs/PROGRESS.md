...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 只接收显式一次性邻接位置 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-v1.5 / pending-retest`；Combat Focus 位置层为 `P5 / layout-v1.0 / pending-game-validation` | accepted Field Kit source／TGA、v1.4 drawer 与 v1.5 强绑定不变。AEUI `0.8.6` 新增 `focus-layout-contract=1.0`：显式一次性 preset 将 DoiteDPS、攻击计时、双方外肩 Aura、Player／Target、双施法条、姿态与已绑定 Combat Deck 排成用户确认的 V3 纵栈；当前“大奶黑牛”pfUI／DoiteDPS SavedVariables 已写入。关键战斗信息保持 provider 原生 Alpha，装饰层鼠标穿透，不创建中央大型透明命中层；无维护循环、无重绘、无外部生成。fresh-checkout package pass | 启动或 `/reload` 先验证 `focus-layout-contract=1.0`、整组物理纵栈、Aura 六枚换行、双方施法、近／远程计时、DoiteDPS 锁定态鼠标穿透及中央视野；需要恢复时只执行 `/aeui focuslayout apply`。同轮继续验证 `fieldkit-contract=1.5`、左卷袋—中央 `12×2`—右双槽、唯一 mover、跨格意图、popup、apply／restore、Queue／换装及 fail-open；通过前保持 P5 |
