...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua` | `P2–P4` | 当前首要大面积 UI；QL-A1 source 已确认；QL-A2 V3.3 的 B1 已在 `5/5` 后退回，B2 attempt 4 几何通过但材料失败，attempt 5 已在调用前计数，B3 仍获授权，当前 `10/15`；外部 tracker 暂停 | 用同段底稿执行 B2 attempt 5 material edit；整批装配仍被 B1 阻塞 |
