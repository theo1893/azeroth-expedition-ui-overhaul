...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 为 `P6 / game-validated`；`AB.RAIL.V1` 为 `P3 / prompt-authorized`；`AB.FIELDKIT.V1` 为 `P2 / simulation-reviewed` | V3 战斗信息链已确认。`AB.SLOT` accepted source `6d4a4d16…7dc0` 与 runtime TGA `5c49a1db…23ca` 已通过 scoped adapter、display、package 及用户实机冷却／距离红／按下反馈验证。Rail 的 8 个物理实例均 pass，用户已接受 `AB-RAIL-SIM-V1` 的文字化方向，并于 `2026-08-09` 授权最终正文、最多 `5` 次实际生成／修复及指定 Character V3 Image 1 外部上传；正式生产仍为 `0/5`。Field Kit V1 的 `5×2` 已按用户容量／分组要求修订；当前 `AB-FIELDKIT-SIM-V2` 使用 AutoBar 完整 `4×6 / 24 类`，分为应急／增益／工具三段，分类内仍保留原生 `1–12` popup；TrinketMenu 方向不变。display `16/16`、布局 `72/72`、violations `0`、外部生成调用 `0` | 提交 Rail 授权正文并由固定执行器开始 attempt 1；同时等待用户接受或修订 `AB-FIELDKIT-SIM-V2`。Field Kit 确认前不启用 AutoBar、不改 TrinketMenu 保存值、不执行其 production；`AB.SLOT` 单组件收口仍另行待办 |
