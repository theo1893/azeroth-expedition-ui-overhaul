...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A `4/5 / final.r4 repair-prepared`；B/B1 `0/5 sequence-wait`；A2 paused | A attempt 4 保持单开口／物理连通／ratio／anisotropy 通过，hard safe core 降至 `9061 px`；但归一化开口仍约 x `71..1206`，bbox `1392×281` 与 isolation `70/74` 仍失败，故无 candidate/source/runtime。最终 `r4` 只要求整体等比缩至 `1284×252`，把两竖边减至与横轨同厚并移内脸至 x `42/1242`。流程错误 `2` 均无额外 provider 图、不占额度 | 提交 `final.r4 / repair-prepared` 后执行 A attempt 5，只以 attempt 4 raw 为 Image 3；通过则停，仍失败则 A 段耗尽并等待用户审查。之后依序处理已授权 B、B1；A2 暂停 |
