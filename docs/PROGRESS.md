...
| Unit Frames | `api/unitframes.lua`、`modules/raid.lua`、主／紧凑单位入口 | A／B `P3 / 5/5 exhausted`；B1 `P5`；Raid A1 `P3 / 5/5 exhausted`，Raid A2 `P2 simulation-confirmed`；紧凑 A2 paused | B1 exact Health／Power 只接入 Player／Target／TargetTarget／Focus。Raid A1 五次终止且无产物；`UF-RAID-A2-SIM-V1` 已确认 material-only donor＋Python 精确造壳方向：四张 `592×296 → 74×37` 外壳覆盖 40 个真实成员，A-D 维修由确定性 mask 构造；display `7/7 pass`，ImageGen `0/0`，无 source/runtime/addon | 等待用户单独授权已完成 `pass-final` 审计的 `UF-RAID-A2-DONOR V1`；模拟确认不授权生图。A1 不得第六次；B1 另待 Turtle WoW P6；主 A／B 是否重开另定，紧凑 A2 暂停 |
