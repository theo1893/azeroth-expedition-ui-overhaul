...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A `5/5 exhausted`；B `3/5 / final.r3 repair-prepared`；B1 `0/5 wait`；A2 paused | A 终态不变。B attempt 3 把 hard safe 降至 `10860 px`，但 bbox `1380×246` 过宽矮，ratio `10.098017%`、anisotropy `9.171843%` 和 isolation `79/77` 失败。开口约 x `80..1208/y37..212`，上下轨接近正确。`final.r3` 只做宽减 `96`、高增 `6`、左右内脸让 `38/34 px` 与底边扩 `4px`；无 candidate/source/runtime。流程错误仍 `2` | 提交 B `final.r3 / repair-prepared` 后执行 attempt 4，只以上一 B raw 为 Image 3；B 后 B1，三段终态统一审查。A2 暂停 |
