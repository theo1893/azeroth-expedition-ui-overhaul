...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；`AB.FIELDKIT.V1` 为 `P5 / runtime-exported` | 已接受的 Trinket 第4稿与 Consumable 第1稿分别确定性导出为 `512² RGBA` TGA：文件 SHA `3614d9a8…f455`／`c48f6292…320e`，像素 SHA `0961d750…aef`／`658f826f…e30d`。`ActionBars` 只在真实 AutoBar `24+12` 与 TrinketMenu `2+30` Button／Frame 下添加非交互视觉层和 provider 完成后的刷新钩子；自适应外壳、精确 `4×6` 三组签名、横竖双槽、动态候选与 fail-open 已入静态合同。display `9/9`＋`7/7`、violations `0`，fresh-checkout package `pass`，目标设备无需构建。AutoBar 未启用、profile 与双方 SavedVariables 未写入，本阶段无外部生成 | Turtle WoW `/reload` 后验证 TGA 方向、AutoBar 自定义／`4×6`／四向 popup、TrinketMenu 横竖／候选／Queue／换装／拖动／缩放／停靠及 `/aeui actionbars` 回退；通过前保持 P5 |
