...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P3 / prompt-authorized`；`AB.SLOT.BASE.V1` | 用户于 `2026-08-08` 确认 V3 的战斗信息链，并授权 `AB.SLOT.BASE.V1` production Prompt、最多五次实际生成／修复，以及指定 Character V3 锁定图向外部 ImageGen 的上传。pre-launch `E1` 无图、无 provider 结果，不计预算；ImageGen 仍为 `0/5`。首批仍只生产一个普通／空槽基底；未写 profile，无 source/runtime | 使用固定 `@openai/codex@0.143.0` 和已提交正文重试 attempt 1，并执行语义、组件、技术像素、真实排版与展示区域内审 |
