...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 为 `P6 / game-validated` | V3 战斗信息链已确认。accepted source `6d4a4d16…7dc0` 已确定性导出为 `128²` TGA `5c49a1db…23ca`；AEUI `0.7.0` scoped adapter 只挂接 pfUI Bar `1–10` 既有 backdrop，Bar `11／12` 与全部 provider 行为／几何不变。display `5/5`、package 均 `pass`。用户已以实机截图 `dc9615ac…4d5d` 及“动作条功能验证通过”确认冷却、距离红、按下反馈和完整 P6 清单；P5→P6 新增外部生成 `0` | 下一独立门禁为 `AB.RAIL.V1` 本地确定性模拟；`AB.SLOT` 进入单组件 `P6-C` 前须展示精确保留／删除清单并取得明确批准 |
