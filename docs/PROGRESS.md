...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS 视觉桥接；pfUI Player／Target 仅作邻接布局提案 | `P2–P6`；`AB.SLOT.BASE.V1` 为 `P6 / game-validated`；`AB.RAIL.V1` 为 `P2 / simulation-confirmed` | V3 战斗信息链已确认。accepted source `6d4a4d16…7dc0` 已确定性导出为 `128²` TGA `5c49a1db…23ca`；AEUI `0.7.0` scoped adapter 只挂接 pfUI Bar `1–10` 既有 backdrop，Bar `11／12` 与全部 provider 行为／几何不变。display `5/5`、package 均 `pass`。用户已以实机截图 `dc9615ac…4d5d` 及“动作条功能验证通过”确认冷却、距离红、按下反馈和完整 P6 清单；P5→P6 新增外部生成 `0`。Rail 模拟已按真实 backdrop 外扩与 Bar 1／6 合并规则覆盖 8 个物理实例，display `8/8 pass`、violations `0`、外部生成调用 `0`；用户于 `2026-08-08` 接受 `AB-RAIL-SIM-V1`，确认的可见方向已冻结进 `AB.RAIL.V1` 最终生产正文，模拟像素未被接受 | 另行取得 `AB.RAIL.V1` 最终正文与最多 `5` 次实际生成／修复授权，以及指定 Character V3 锁定图作为 Image 1 的外部上传授权；两项齐全前不执行。`AB.SLOT` 进入单组件 `P6-C` 前仍须展示精确保留／删除清单并取得明确批准 |
