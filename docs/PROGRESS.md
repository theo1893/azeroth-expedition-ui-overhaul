...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `UF-A1 V3-A/B` 与 `UF-B1 V2`：`P2 / simulation-reviewed / 0/5`；A2 paused | 用户接受每角色完整外壳与 Python 精确工程化，并新增 Health／Power 材质；Power 由同一灰阶 donor 承担 Mana／Rage／Focus／Energy，pfUI 继续乘经典语义色。`UF-PRIMARY-V3-SIM-V1` 本地生成前模拟未调用远程生图，真实 Player／Target、四 Power mode 与 `W=160/240` 共 `7/7 display pass`。旧 V1／V2 失败像素禁止复用，无 source/runtime/addon 改动 | 用户确认或否决 V3 模拟的粗犷完整外壳、Health／Power 材质层级和四资源乘色；确认后才完成三段 final production 正文并请求各自最多 `5` 次授权；A2 继续暂停 |
