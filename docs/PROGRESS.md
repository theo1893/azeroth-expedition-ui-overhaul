...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A／B 均 `5/5 exhausted`；B1 `0/5 queued`；A2 paused | B attempt 5 的单开口、单物理外壳、ratio `4.973681%` 与 anisotropy `4.738027%` 通过；但 hard safe `22649 px`、isolation `68/72` 失败，工业式长边和整高右黄铜仍在。A／B 均无 candidate/source/runtime，禁止第六次。流程错误 `3`，均无额外 provider 图 | 提交 B1 专用 reviewer 后执行 `UF-B1 V2 final` attempt 1，首次无图片；最多 5 次同段循环。三段终态后统一审查。A2 暂停 |
