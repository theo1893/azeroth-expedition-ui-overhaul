...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A／B 均 `5/5 exhausted`；B1 `1/5 / final.r1 repair-prepared`；A2 paused | A／B 终态不变。B1 attempt 1 除 source ratio 外 `8/9` 通过：两对象、上下顺序、隔离、中性、明度、中心与层级成立；Health `2.81:1`、Power `9.07:1` 未过 `2:1/4:1` 门禁。无 candidate/source/runtime。流程错误 `4`，均无额外 provider 图 | 提交 B1 `final.r1` 后执行 attempt 2，只以上一 B1 `1024²` raw 为 Image 1；目标 `512×256`／`512×128`。通过即停，否则最多 5 次同段循环。A2 暂停 |
