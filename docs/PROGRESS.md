...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P5` | Quest Log 主体保持 QL-A2 V4；pfQuest tracker 临时纸面仍为 `display-region-blocked`。`QS-A1` attempt 1 已完成（`P3 / 1/5`）：单物件和真实展示区域通过，源尺寸／安全边／纯绿和香草二维手绘视觉失败，已冻结受限 r2 修复。七个旧 icon 只有在 hub menu 七项功能等价后才可隐藏 | 以固定双输入和紧邻 r1 执行 attempt 2；首个内部完整通过的候选停止并提交用户审核 |
