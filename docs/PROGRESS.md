...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua` | `P2–P4` | 当前首要大面积 UI；QL-A1 source 已确认；QL-A2 V3.3 的 B1／B2 均在 `5/5` 后退回，B3 attempt 3 仅尺寸失败且 attempt 4 已在调用前计数，当前 `14/15`；外部 tracker 暂停 | 用同段底稿执行 B3 attempt 4 bounded scale edit；整批装配仍被 B1／B2 阻塞 |
