...
| Quests | `questlog.lua`、`gossipquest.lua`、`questitem.lua`、`pfQuest`／`pfQuest-turtle` + AEUI Quests adapter | `P1–P6` | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`，QL-D V1 最终美术仍为本地模拟。QS-B1 V5-A 深色布底已于 `2026-08-06` 固化为 `128×696 RGBA` tracked source，`source-accepted / P4 / 4/5`。旧 V3-B 因前置 V3-A 失败和近黑墨对深布对比不足仍为 `0/5 gated`；新的 V15／V5-B 已直接使用 accepted source 完成六态真实排版，提议六项克制旧赭金、放弃项灰暗酒红，`51/51`、display `6/6`，等待用户确认。无纹章 source/runtime，旧按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 不变 | 用户确认／否决 V15 综合色；确认后才准备 V5-B production Prompt 和独立授权。随后完成确定性布底导出、七纹章、代理 parity 与菜单接入；QL-D、Tracker、P6-C 门禁保持 |
