...
| Unit Frames | `api/unitframes.lua`、`api/ui-widgets.lua`、`player.lua`、`target.lua`、`targettarget.lua`、`focus.lua` | `P3`；A `3/5 / final.r3 repair-prepared`；B/B1 `0/5 sequence-wait`；A2 paused | A attempt 3 保持单开口／物理连通／ratio／anisotropy 通过，并把 hard safe core 从 `57077` 降至 `11360 px`；残余侵入集中在左右内脸。bbox `1415×282` 的左右隔离 `62/59` 亦未过门禁，连续绳边仍偏工整，故无 candidate/source/runtime。`final.r3` 要求整体等比缩至约 `1284×252`、端部内脸再让 `32/46 source px` 并打断边缘节奏。流程错误仍 `1` 且不占额度 | 提交 `final.r3 / repair-prepared` 后执行 A attempt 4，只以 attempt 3 raw 为 Image 3；A 终态后依次 B、B1。通过即停，第五次仍失败则耗尽；A2 继续暂停 |
