...
| Action Bars／Field Kit | `actionbar.lua`、`gryphons.lua`、`hunterbar.lua`、`castbar.lua`、`swingtimer.lua` + 可选 AutoBar／TrinketMenu／DoiteDPS／ArchiTotem 视觉与布局桥接；pfUI Player／Target 只接收显式一次性邻接位置／舒适缩放 | `P2–P6`；`AB.SLOT.BASE.V1` 与 `AB.RAIL.V1` 均为 `P6 / game-validated`；Field Kit 视觉 runtime-v1.5／bridge-v1.6 为 `P5 / pending-retest`；Combat Focus runtime-v1.6 为 `P5 / pending-game-validation`，v1.4／v1.5 `game-geometry-failed` | `ACTION-BARS-CORE-SIM-V5` 视觉方向不变。第三张实机截图与落盘 SavedVariables 证明 v1.5 把物理 `1920×1080`、1.12 SetPoint 坐标、provider scale、探针和回读混为一体。AEUI `0.8.12`／`focus-layout-contract=1.6` 保留 tier 8、Combat Deck、Field Kit、单位／施法 `0.75` 与 Swing／姿态／DoiteDPS `0.82`，删除全部投影并直接写游戏原生坐标；提供 pre-apply profile restore，普通 refresh 不维护坐标。`fieldkit-contract=1.6` 与 ArchiTotem bridge 不变；无新位图或外部生成 | `/reload` 后执行一次 `/aeui focuslayout comfort`，确认 `focus-layout-coordinate-space=game-native-v1`，再验证攻击／施法不重叠、中央空白收紧、单位框不挡人物，以及 DoiteDPS、ArchiTotem 全交互和 Field Kit popup／Queue／换装回归；需要回退时 `/aeui focuslayout restore` 后 `/reload` |
