...
| Unit Frames | `api/unitframes.lua`、`modules/raid.lua`、主／紧凑单位入口 | Player／Target V4 `P2 simulation-reviewed`；V3 A／B `P3 / 5/5 exhausted` 历史；B1 `P5`；Raid A2 `P5 source/runtime/addon / 5/5`；紧凑 A2 paused | `UF-PRIMARY-V4-SIM-V1` 以“已接受 Unit Frames 材料 sample＋Python 完整壳”重开 Player／Target：最终仍一角色一张完整 `214×42`，几何、Alpha、安全区、非镜像维修和 `32/150/32` 全由 builder 持有，首选路径不调用新的位图生成；展示区域 `7/7 pass`。B1 与 Raid A2 既有 P5 不变，V3 rejected pixels 禁止复用 | 用户先确认 V4 架构模拟；之后确定性构造两张透明 candidate 并审查 exact pixels，不直接 P5。Raid／B1 继续等待 Turtle WoW P6 |
