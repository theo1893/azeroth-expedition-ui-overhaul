...
| 模块 | 阶段 | 当前结论 | 下一门禁 |
|---|---:|---|---|
| Core／pfUI | `P5` | AEUI `0.9.0`；pfUI 公共功能保留，Chat、Quest、Unit Frames、Map、Character 等只通过精确 route 接管；Gear Planner `1.1-zhCN` 接入 Character 与 Inspect 独立伴随会话并保留独立回退 | 实机验证模块隔离、旧 SavedVariables、两套伴随栏、配置页和禁用回退 |
...
| Character | `P2–P5 / active` | runtime `2.0`；外壳、PaperDoll 组件、装备槽、Tabs、Ammo 与分页档案页保持 2× runtime；Gear Planner `1.1-zhCN` 提供 `40 UI` 完整文本角色栏、默认双栏、`996 UI` 配装合同、当前／配装／变化属性对比与 Inspect“装／属／比／存”独立会话，不改 Provider Parent、尺寸或数据 | 实机复核既有 PaperDoll／分页组件，再验证默认双栏、配装净空、即时属性对比、角色与观察伴随栏及禁用回退 |
| Gear Planner | `P5` | runtime `1.1-zhCN`；accepted `GEAR-MAIN-V1` 军需官案板与 `GEAR-SLOT-STATES-V1` 强／弱黄铜夹签、冷灰蓝修订缝带均已导出为 2× runtime；状态文字保持动态且两类 sprite 可叠加；保存 Button 与方案选中反馈已修复，AtlasLoot 仍持有查询、分页、来源和 Wishlist | 完整重启后实机验证默认双栏、三种槽位 sprite、保存状态、方案选中反馈，再回归案板层序、武器差值、AtlasLoot、方案 CRUD 与观察栏 |
