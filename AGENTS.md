...
| 任务 | 当前首要大面积 UI；QL-A1 `P4`；QL-A2 V4 固定卷宗背景 `P5`；用户接受 QL-B1 V1.r3 运行时视觉，QL-B0 23 行与 QL-B1 四态 atlas 已确定性导出、接入并静态通过，`P5`；QL-B2 V1 已完成实际生图 `5/5` 与三态真实排版，因 source 安全盒／原生色键不合格停在 `candidate-rejected / P3`，第五张运行时视觉可审视；V3.3 `15/15` 与 QL-B1 `5/5` 内部失败事实保留 | QL-A2／B0／B1 等待 Turtle WoW 实机；用户决定 QL-B2 的确定性 bbox-fit 合同例外、V2 或暂停 |
