...
| 聊天 | Full V1 runtime `1.19 / P5` 保持；正式仍复用 V3 Tab／输入／未读。`CHAT.INPUT.DARK.V1` 已实际生成 `3/5`，流程错误 `4`；三次技术归一化、真实 `380×25/480×25` 排版和五场景 display-region 均可通过。attempt 3 仍保留重复卷曲压纹并加重连续 focus 上缘，证明连续 edit 被错误 Image 4 表面锚定。完整 `.r3` 改为只上传固定 Image 1／2／3 从零 regenerate，同一组件、权威、画布和 runtime 合同不变；runtime 改动仍为 `0`。Chat Copy／URL Copy 暂缓；右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 提交 `.r3` 后执行 attempt 4，重点复核从零生成的平静烟熏 rag paper 和局部 focus 反馈；内部通过后交用户接受。游戏设备可用时再 `/reload` 验证核心 r1.19 |
