...
| 聊天 | 核心 runtime `1.21 / P5`。Full V1 主框、V3 Tab／未读保持；`CHAT.INPUT.DARK.V1.r3 attempt 4` 已导出为 `ChatInputDarkV1.tga`。V1 深色 Tab attempt 3 已由用户否决。用户已确认 `CHAT.TABS.DARK.SIM.V2` 的旧式斜梯形皮签、外撇端部、手工缝线、压暗夹页层与深色低亮 shelf 方向，并于 `2026-08-04` 精确授权 `CHAT.TABS.DARK.V2` 正文、固定 Image 1–4、受限同循环紧邻 Image 5 edit 与最多五次实际 ImageGen；当前 `prompt-authorized / attempt-01-ready / P3 / 0/5`，未改 runtime。v1.21 保持客户端／pfUI／ChatMOD 经典颜色透传；Chat Copy／URL Copy 暂缓，右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 提交授权记录后使用固定 `imagegen-0-143-0` 执行 V2 attempt 1，并完成逐候选真实排版与展示区域审查；游戏设备可用时再 `/reload` 验证核心 runtime |
