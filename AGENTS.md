...
| 任务 | 当前首要大面积 UI；QL-A2 V4、QL-B0 23 行 V1、QL-B1 与 QL-B2 当前为 `P5 fallback`，但实机确认左页大面积视觉未通过；QL-B0 V2 `18 × 18` 已获授权，A attempt 1／2／3 因设计网格／bbox／开口／色键退回，attempt 3 的单层暗皮与两枚非对称黄铜改善保留，现为 `repair-prepared / P3`、A `3/5`、B `0/5`；QL-B3-A `5/5` exhausted、B/C `0/5` 并暂停 | 提交 `QL-B0-A V2.r3` 后执行 attempt 4；内部通过只到 P3 |
