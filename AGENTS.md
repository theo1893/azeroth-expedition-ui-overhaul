...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5`。Raid A1 仍为 `P3 / 5/5 exhausted / rejected`；Raid A2 attempt 5 已按只豁免未消费外围 field bbox 最大 `19px` 的 `sample-window-only` 例外进入 `P5 source/runtime/addon`；四张独立 `74×37` TGA、`6/62/6` 与 40 人排版已固定，生成仍为 `5/5` | Turtle WoW P6 验证 `pfRaid1..40` A-D 分配、TGA 方向、完整纹理／变宽三切片、动态状态层与禁用／高度失配回退；禁止第六次。B1 同步实机验证 |
