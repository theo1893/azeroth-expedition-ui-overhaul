...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v1.6 为 `P5 / pending-retest`。用户确认的 `ACTION-BARS-CORE-SIM-V5` 视觉方向不变，但 AEUI `0.8.10`／focus runtime-v1.4 已被新实机截图判定 `game-geometry-failed`：它把 1.12 的 768-high `UIParent`、normalized UI root 与 provider scale 混用，导致整组错位。AEUI `0.8.11`／`focus-layout-contract=1.5` 保留 tier 8、Combat Deck、Field Kit、单位／施法 `0.75` 与 Swing／姿态／DoiteDPS `0.82`，显式 preset 改为逐 Frame 测量 UI-root scaled anchor；普通 refresh 不维护坐标。`fieldkit-contract=1.6` 与 ArchiTotem bridge 不变；无新位图，ImageGen `0/0`，当前 `P5 / pending-game-validation` | `/reload` 后执行一次 `/aeui focuslayout comfort`，确认状态含 `focus-layout-coordinate-space=ui-root-calibrated-v1`，再确认攻击／施法不重叠、中央空白收紧、单位框不挡人物，并验证双方状态／施法、近战／远程 Swing、DoiteDPS、ArchiTotem 全交互、强绑定与 Field Kit popup／Queue／换装回归 |
