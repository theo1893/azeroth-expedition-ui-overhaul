...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 为 `P6 / game-validated`：attempt `5/5` accepted source `6d4a4d16…7dc0`、runtime TGA `5c49a1db…23ca`，Bar `1–10` scoped adapter、display／package／冷却／距离红／按下反馈均 pass，不得 attempt 6。`AB.RAIL` 同为 `P6 / game-validated`：accepted source `7c49995d…32e9` 已确定性导出为 `256²` TGA `1e5cca09…0a3d`（像素 `1b09b93b…9db5`）；Bar `1–12`＋Bar `1／6` merged scoped adapter、display `8/8`、package 与用户六项实机清单均 pass，截图 `5e89c6e5…12942`，未修改 pfUI／SavedVariables，P4→P6 ImageGen `0`，固定生产仍为 `5/5` 且禁止 attempt 6。`AB.FIELDKIT.V1` 的 `5×2` 已按用户容量／分组要求修订；`AB-FIELDKIT-SIM-V2` 使用 AutoBar 完整 `4×6 / 24 类`，连续八格分为应急／增益／工具，分类内仍由 `1–12` popup 展开真实物品；TrinketMenu 双槽／候选／Queue 方向不变。display `16/16`、布局 `72/72`、violations `0`。用户于 `2026-08-09` 接受八项文字化方向，并分别授权两个冻结 production body、各自最多 `5` 次实际生成／修复及各自 Character V3 Image 1 上传；当前为 `prompt-authorized / P3`，固定执行器账本 `0/5 + 0/5` | 先提交授权检查点，再依次执行 Trinket Kit 与 Consumable Kit 两个独立有界循环；每稿完成真实排版、display-region 与像素门禁，通过即停，最迟 `5/5` 停止。全过程不启用 AutoBar、不改 TrinketMenu SavedVariables；候选须用户验收后才能晋级 source／runtime。`AB.SLOT`／`AB.RAIL` 单组件收口均须另批组件专属精确清单 |
