...
| 任务 | 当前首要大面积 UI；QL-A1 `P4`；QL-A2 V4 固定卷宗背景 `P5`；用户接受 QL-B1 V1.r3 运行时视觉，QL-B0 23 行与 QL-B1 四态 atlas 已确定性导出、接入并静态通过，`P5`；QL-B2 V1 已完成实际生图 `5/5`，不调用 ImageGen 的 V1.r4 bbox-fit 合同例外候选已通过安全盒、透明像素与三态真实排版内审，但冻结合同尚未获用户修改，规范状态仍为 `candidate-rejected / P3`；V3.3 `15/15` 与 QL-B1 `5/5` 内部失败事实保留 | QL-A2／B0／B1 等待 Turtle WoW 实机；用户明确接受／拒绝 `QL-B2 V1.r4 bbox-fit` 合同例外 |
