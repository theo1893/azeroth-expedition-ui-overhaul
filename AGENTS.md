...
| 单位框 | `P3`；`UF-A1 V1` 五次循环已耗尽，当前 `candidate-rejected / repair-budget-exhausted`。attempt 5 比例合同通过，但端柱侵入真实动态走廊 `872/818` Alpha 像素且横向隔离仅 `68–78px`，无 source/runtime。正式生产累计 `5/15`；A2／B1 均 `0/5`、保持 `prompt-authorized / paused` | 等待用户审核 A1 attempt 5：明确接受显示区域合同例外后才可 P4，或授权新的 A1 版本；禁止 A1 attempt 6，决策前不启动 A2／B1 |
