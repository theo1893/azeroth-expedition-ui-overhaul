...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5` 且只接入 Player／Target／TargetTarget／Focus。Raid A1 仍为 `P3 / 5/5 exhausted / rejected`；`UF-RAID-A2-SIM-V1` 已于 `2026-08-12` 确认 material-only donor＋Python 精确造壳方向，当前 `P2 / simulation-confirmed / production-authorization-pending`，display `7/7 pass`、ImageGen `0/0`，无 source/runtime/addon | 等待用户单独授权已完成 `pass-final` 审计的 `UF-RAID-A2-DONOR V1`；模拟确认不授权生图。A1 禁止第六次；B1 另待 Turtle WoW P6；主 A／B 是否重开另定，紧凑 A2 暂停 |
