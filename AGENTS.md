...
| 单位框 | `P2 / simulation-confirmed`；用户已接受 `UF-PRIMARY-SIM-V1` 的“粗犷行军身份牌”方向；四框真实资源尺寸、非镜像／减法结构、Focus 靛蓝布结及两条灰阶填充纹已形成三段完整生产正文，ImageGen `0/15`；位置不进入本批合同 | 用户看过并明确授权 `UF-A1 V1`、`UF-A2 V1`、`UF-B1 V1` 的固定输入、修复边界与每段五次实际生图预算 |
