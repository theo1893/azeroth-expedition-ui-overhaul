...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit art／v1.4 drawer／v1.5 强绑定不变，仍为 `P5 / pending-retest`。最新 `1443×1067` 实机截图确认 AEUI `0.8.8`／`focus-layout-contract=1.2` 已消除单位框与左卷袋相交，但用户判定 `0.75` 战斗核心偏小；v1.2 留作可回退 P5。`ACTION-BARS-CORE-SIM-V4` 保持 tier 8、Combat Deck 与 Field Kit 不变，提议只把 Player／Target、双方施法、Swing、姿态和 DoiteDPS local scale 调到 `0.82`，并内收双框以保持外包络。口述 atomchi 已按实机校正为 ArchiTotem `1.7`；四元素、Air 七候选、拖动球、AllTotems、Recall 与 PresetManager 已入合同，提议作为动作条下方、向下展开、绑定态随 Bar 1 的职业卫星。layout `54/54`、display `7/7`；无 runtime／SavedVariables／重绘，ImageGen `0/0`，当前 `P2 / pending-user-confirmation` | 用户接受或修订 `ACTION-BARS-CORE-SIM-V4`；确认前不得写提议坐标或 ArchiTotem `direction=down`。接受后实现下一版 focus contract，再实机验证战斗核心可读性、单位框净空、施法／Swing／DoiteDPS，以及 ArchiTotem 施放、右键、七层候选、拖动／锁定、Recall、强绑定和 fail-open；同轮继续 Field Kit popup、Queue／换装复测 |
