...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5` 且只接入 Player／Target／TargetTarget／Focus。Raid A1 仍为 `P3 / 5/5 exhausted / rejected`；`UF-RAID-A2-SIM-V1` 已确认，`UF-RAID-A2-DONOR V1` 已精确授权，当前 `P3 / bounded-repair-loop-active`，无 source/runtime/addon | 依据 Raid work 中已提交的当前完整正文继续固定 `imagegen-0-143-0` 循环；每稿做 donor 技术审查、Python 精确造壳和 40 人真实排版，通过即停，最多 5 次。A1 禁止第六次；B1 另待 Turtle WoW P6 |
