...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。`AB.FIELDKIT.V1` 的 AutoBar `4×6 / 24 类` 三段分组与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempts 1–3 已计 `3/5`：attempt 3 raw `0c6f0bc7…8048` 经确定性传输得到 exact canonical `6a91a2b5…5e13`，绿残留、透明 RGB 与最终 margin pass，但 C 原始物件越过中线并污染 D cell，因 scope fatal fail 未进入 reviewer／display。用户授权的纯 `#00FF00` RGB raw→canonical 修订不重绘、不新增输入图、预算不重置；完整 `r3` 已准备，Consumable 仍为 `0/5` | 提交 attempt 3 记录、cell 隔离门禁与完整 `r3` 后执行 Trinket attempt 4；通过即停，否则最多最后一次 `5/5`，禁止 attempt 6。Trinket 循环终止后才准备 Consumable transport body／attempt 1；全过程不启用 AutoBar、不改 TrinketMenu SavedVariables |
