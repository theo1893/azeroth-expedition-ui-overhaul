...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v2.6、Combat Focus runtime-v2.4 与 Sidebar Group runtime-v1.0 为 `P5 / pending-game-validation`。AEUI `0.8.24` 的 `ACTION-BARS-CORE-SIM-V11` 保留 V10 的 Turtle WoW 游戏原生几何：Player／Target `240×60 / 0.8`，TargetTarget `240×60 / 0.68` 并以 `8 UI` 间距中线依附 Target 右侧；Aura `23 UI`、真实 `size+7` 步进每行八枚；玩家 Cast／目标 Cast／Swing 同轴 `260×12 / 1.0`；三框系统字体 `OUTLINE / 18 UI`、DoiteDPS 安全带与四栏 `2×2 / 3×4` 组合不变。战士实机反馈后，姿态栏保持 `BOTTOM (0,255)` 并从 local scale `0.72` 放大到 `1.0`；exact v7–v14 profile 完整匹配时一次迁移为 v15，手调字体／姿态 scale／坐标受保护。v2.6 延续 AutoBar 分类说明、职业槽、同步回锚与 provider-local 停靠，并在绑定态的 Apply、`SetupVisual`、完整配置 `OnShow` 边界同步隐藏真实 `AutoBarAnchorFrameHandle`；显式 `unbind`／AEUI 关闭时恢复 provider 偏好。全部位图字节不变，ImageGen `0/0` | `/reload` 后确认红色拖拽点默认不可见、`autobar-drag-handle=hidden-bound`，三枚战士姿态按钮清晰放大且点击／状态正常；连续执行两次 `/aeui autobar apply` 并开关配置页，确认卷袋位置不变、`autobar-anchor-basis=provider-local`。继续复测系统字体、Player／卷袋、Aura 八枚、Boss 16 Debuff、三条计时栈、单 mover、四栏可逆 unbind，以及 AutoBar／ArchiTotem／popup／Queue／换装。需要回退时 `/aeui focuslayout restore` 后 `/reload` |
