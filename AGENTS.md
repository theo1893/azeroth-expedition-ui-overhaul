...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5` 且只接入 Player／Target／TargetTarget／Focus。Raid 的 `UF-RAID-SIM-V1` 已于 `2026-08-12` 确认方向，仍为 `P2 / simulation-confirmed`；`UF-RAID-A1 V1 final` 四变体完整背景板正文与 `1536×1024` 四格合同已固定，ImageGen `0/5`，无 source/runtime/addon 变更 | 用户看过后独立授权 `UF-RAID-A1 V1 final`、固定 Image 1／2、同循环紧邻前稿 edit 边界与最多 5 次实际 ImageGen；未授权前不上传或生图。B1 另待 Turtle WoW P6；A／B 是否重开另定，A2 暂停 |
