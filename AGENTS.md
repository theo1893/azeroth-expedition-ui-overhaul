...
| 模块 | 当前状态 | 下一步 |
|---|---|---|
| Bagshui | runtime `1.3 / P5`；皮革外缘与布料内衬已接入背包／银行／钥匙链，所有角色默认启用，`/aeui bags` 回退；工具栏、物品槽与袋位材质已接入，标题回退原生，品质与交互状态仍由 provider 更新 | 完整重启后验证主框、搜索、编辑模式、离线库存和回退 |
| Tooltips | runtime `1.0 / P5`；六个明确提示框复用 Readout V1 的 1 UI 细边，单位生命条使用现有 2× 填充；`/aeui tooltips` 独立回退 | `/reload` 验证物品／技能／Aura、单位颜色与生命条、双比较、地图、链接关闭按钮和 AtlasLoot |
...
| Quests | runtime `1.30`、Theme `1.13`；既有 2× 书体与媒体保留，底部八按钮接入三尺寸四态 2× 皮革签；长任务滚动、奖励显示和原按钮操作已实机通过；事务菜单未启用 | 完整重启验证底部皮革签、文字净空、四态与原操作；Tracker 重新确认真实区域 |
...
| BigWigs | `2.16.2 / P5`；已合入 Golden 竖向时间轴、点击／资源监控条、抗性检查、每角色世界 Boss 冷却与副本机制，保留中文误报、木喉及光环目标修复；`/bwv off` 恢复横向，`/bwcd` 查看冷却 | 完整重启后验证新增模块、竖向／横向切换、Anomalus 监控、抗性双端同步与角色隔离，再验证单模块关闭及缺失扩展接口回退 |
