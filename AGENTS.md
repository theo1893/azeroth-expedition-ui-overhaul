...
| 单位框 | Player／Target 外壳已重开为 `UF-PRIMARY-V4-SIM-V1 / P2 / simulation-reviewed / user-confirmation-pending`；首选复用已接受 Raid A2 四块材料 sample，Python 独占完整外壳几何、Alpha、安全区、维修 mask 与 `32/150/32`，ImageGen `0/0`。V3 A／B 保持 `P3 / 5/5 exhausted / rejected` 历史终态；B1 Health／Power 与 Raid A2 均为 `P5` | 先确认 V4 架构模拟；确认后从已接受材料确定性构造 Player／Target 透明 candidate 并审查 exact pixels，不直接接入 addon。Raid／B1 仍等待 Turtle WoW P6 |
