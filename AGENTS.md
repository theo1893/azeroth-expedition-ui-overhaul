...
| Action Bars | AEUI `0.9.0`；Slot／Rail `P6`；Field Kit `2.9`、Focus `3.3`、Sidebar Group `1.0`、Target Markers `2.3` 均 `P5`；Player／Target 恢复为 `240×48 / 0.8`、底锚点 `480 UI`，保持 `23 UI` Aura 并按每排 `8` 枚贴合短框，上 Buff／下 Debuff，`32` 枚 Debuff 最多四排；两框间留 `73 UI`，TargetTarget 保持每排 `8` 枚，下方读条与 Combat Deck 原位；三框布局按角色版本应用一次；所有角色 DDPS 统一 `(650,-615)` | 实机验证 Player／Target 恢复后的等长短框、`16–32` 个 Debuff 的两至四排净空及另一角色 v5 默认迁移，再验证皮革标记方阵、坦克／一键 Button、ArchiTotem 分离、AutoBar popup／网格、DDPS 中央视野、姿态尺寸及相邻 provider |
