...
| 动作条／随身栏 | pfUI 十二条 Bar、真实状态、自由移动／缩放／行列与目标设备 profile 已完成 `P1` 审计；`ACTION-BARS-CORE-SIM-V1` 为 `simulation-reviewed`，提出自适应战斗甲板、炼金卷袋和饰品双护套。TrinketMenu／AutoBar 只做可选 feature-detect 桥接；ImageGen `0/5`，尚无 runtime 接管 | 用户确认 V1 的中心双层布局、左右随身栏、成对狮鹫重量和卷袋隐喻；确认前不锁定 `P2`、不生成正式资产 |
...
- [当前核心 V1 工作文件](docs/modules/actionbars/work/ACTION.BARS.CORE.V1.md)
