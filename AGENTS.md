...
| 任务 | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；活动 runtime 范围保持 `P6 user-confirmed`。QS-B1 V5-A 布底已 source-accepted；V5-B worksheet 在 `5/5` 后失败并冻结。用户已选择 V6 七张单对象 source；V16 用 accepted UI 完成 `59/59`、展示区 `6/6` 本地预演，当前 `simulation-reviewed / P2 / ImageGen 0/0 / production 0/35 not-authorized`。无纹章 source/runtime，旧按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 不变 | 用户确认具体 V16；确认后才准备 V6-A..G 七份完整 production 正文与独立授权 |
