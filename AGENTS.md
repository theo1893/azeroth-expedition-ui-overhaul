...
| 单位框 | `P3`。A `5/5 exhausted` 终态不变。B attempt 1 已计 `1/5` 且未复用 A：单开口／物理／isolation 通过；bbox `1354×305` 的 ratio/aniso `12.872683%`、hard safe core `66780 px` 失败，无 candidate/source/runtime。Target 左磨损折痕／右黄铜损伤可保留；`UF-A1 V3-B final.r1` 要求约 `1284×252` 薄皮环、开口至少 `1200×180`、右损伤压进极端 `42 px`。B1 `0/5`；流程错误仍 `2` | 提交 B `final.r1 / repair-prepared` 后执行 attempt 2，只以上一 B raw 为 Image 3，禁止复用 A；B 后执行 B1。三段终态后统一审查。A2 暂停 |
