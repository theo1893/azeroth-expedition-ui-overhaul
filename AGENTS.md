...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v2.0 与 Combat Focus runtime-v2.1 为 `P5 / pending-game-validation`。AEUI `0.8.18` 的 `ACTION-BARS-CORE-SIM-V10` 使用 Turtle WoW 游戏原生坐标：Player／Target 保持 `240×60 / 0.8` 并只上移至 `BOTTOM (-160,485)／(105,485)`；TargetTarget 保持 `240×60 / 0.68`、fallback `BOTTOM (393,576)` 并以 `8 UI` 间距中线依附 Target 右侧；三框局部字体 `14`，Aura 改为 `23 UI`，按当前 pfUI 真实 `size+7` 步进每行八枚占 `233／240 UI`；Target 16 Debuff 双排到未移动施法条仍有净空。玩家 Cast／目标 Cast／Swing 保持同轴 `260×12 / 1.0`，姿态与 DoiteDPS 不动；消耗品／饰品保持原 x 并共用低 `20 UI` 的底线。AutoBar 默认保留 24 个逻辑类别但只显示当前有物品类别，最大 `4×6`；仅 exact v7–v11 profile 在 `/reload` 一次迁移为 v12。全部位图字节不变，ImageGen `0/0`；当前 `P5 / pending-game-validation` | `/reload` 后开关一次 pfUI 解锁，确认无 `drag=nil`、绑定态只显示 Bar 1 mover、Bar 6 与 TargetTarget 均不跳位；确认 Player 与两侧下移组件不再重叠、Aura 每行完整八枚、Boss 16 Debuff 双排不压施法条、三条计时条与 DoiteDPS 不变；确认“大奶黑牛”AutoBar 当前仅显示 13 个有物品类别且库存变化时动态收缩／扩展，再验证双方状态／施法、近战／远程 Swing、ArchiTotem、Field Kit popup／Queue／换装；需要回退时 `/aeui focuslayout restore` 后 `/reload` |
