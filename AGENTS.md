...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已于 `2026-08-08` 获用户确认，锁定 DoiteDPS → 攻击计时 → Aura／双框 → 双施法条 → 动作栏纵向焦点。首批 `AB.SLOT.BASE.V1` 在 `5/5` 停止，accepted source `6d4a4d16…7dc0` 已确定性导出为 `128²` TGA `5c49a1db…23ca`，像素与 attempt 5 review 完全一致。AEUI `0.7.0` runtime `1.0` scoped adapter 仅挂接 pfUI Bar `1–10` 既有 backdrop；Bar `11／12`、状态、几何、交互和 SavedVariables 不变。最终真实排版 `5/5 pass`、package `pass`。用户已以截图 `dc9615ac…4d5d` 和“动作条功能验证通过”确认冷却、距离红、按下反馈及完整 P6 清单；当前 `AB.SLOT P6 / game-validated`，P5→P6 新增外部生成 `0`，不得 attempt 6。`AB-RAIL-SIM-V1` 已完成本地确定性模拟，覆盖 8 个独立／合并布局，display `8/8 pass`、violations `0`、ImageGen `0/0`；用户于 `2026-08-08` 明确接受该模拟版本，确认条款已冻结进 `AB.RAIL.V1` 最终生产正文，模拟像素未被接受；当前 `AB.RAIL P2 / simulation-confirmed` | 另行取得 `AB.RAIL.V1` 最终正文与最多 `5` 次实际生成／修复授权，以及指定 Character V3 锁定图作为 Image 1 的外部上传授权；两项齐全前不得执行。`AB.SLOT` 单组件收口仍须先展示精确保留／删除清单并取得明确批准 |
