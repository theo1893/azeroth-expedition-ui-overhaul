...
| 任务 | 用户于 `2026-08-05` 在 Turtle WoW 中确认当前 Quest Log 左页与右页的既有 bug 和显示问题均已修复。该确认关闭 QL-A2 V4 书体、18 行左页字体／无描边／类型墨色，以及 Quests `1.25`／Theme `1.8` 的右页金额、动态 ScrollChild、奖励无循环锚点、间隔、换行和原生 `NameFrame` 抑制门禁；相关活动 runtime 范围记为 `P6 user-confirmed`。仓库没有新增或虚构本轮截图路径，当前 work 继续保留到组件／模块收口。`QL-D V1` 最终奖励槽美术仍为 `P2 simulation-proposed`；QS-B1、Tracker 与 NPC Quest／Gossip 状态不变 | 先确认 QL-D 正式容器方向；随后处理 Tracker `display-region-blocked`／QT-GEO V2、QS-B1 事务菜单及其余未完成范围。未经独立授权不生图，不把本次 Quest Log 验收扩展为整个 Quests 模块 P6-C |
