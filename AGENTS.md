...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已于 `2026-08-08` 获用户确认，锁定 DoiteDPS → 攻击计时 → Aura／双框 → 双施法条 → 动作栏纵向焦点。首批 `AB.SLOT.BASE.V1` ImageGen 在 `5/5` 停止，accepted source `6d4a4d16…7dc0` 已确定性导出为 `128²` TGA `5c49a1db…23ca`，像素与 attempt 5 review 完全一致。AEUI `0.7.0` runtime `1.0` scoped adapter 仅挂接 pfUI Bar `1–10` 既有 backdrop；Bar `11／12`、状态、几何、交互和 SavedVariables 不变。最终真实排版 `5/5 pass`、package `pass`、目标设备无需构建；当前 `P5 / runtime-exported`，P4→P5 ImageGen `0`，不得 attempt 6 | Turtle WoW `/reload` 验证 `actionbar-runtime=1.0`、TGA 方向、真实图标／状态、20–48 UI、合法行列、自由移动／缩放／显隐、Bar `11／12` 排除及 `/aeui actionbars` fallback；通过前保持 P5 |
