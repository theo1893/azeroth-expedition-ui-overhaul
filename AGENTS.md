...
| 聊天 | 核心 runtime `1.21 / P5`。Full V1 主框、V3 Tab／未读保持；`CHAT.INPUT.DARK.V1.r3 attempt 4` 已导出为 `ChatInputDarkV1.tga`。`CHAT.TABS.DARK.V2` 原固定 0.143.0 循环终态 `candidate-rejected / 5/5`、流程错误 `0`；其后以 ImageGen `0` 对固定 attempt 1／5 keyed 候选完成确定性等比重排可行性预演，两案 source cell 外 `0`、4px 隔离和三场景 display-region 均通过。A=attempt 1 的旧式手切轮廓更好，B=attempt 5 仍偏规整；当前 `exception-feasibility-reviewed / P3`，没有 source／runtime／addon 变更，现行 V3 保持。v1.21 继续透传客户端／pfUI／ChatMOD 经典颜色；Chat Copy／URL Copy 暂缓，右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 禁止 V2 attempt 6；用户选择并明确授权 A／B 的固定 donor＋transform＋输出 SHA 合同例外，或另行授权新批次；游戏设备可用时再 `/reload` 验证核心 runtime |
