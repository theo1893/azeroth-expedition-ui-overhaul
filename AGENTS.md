...
| 聊天 | 核心 runtime `1.21 / P5`。Full V1 主框、V3 Tab／未读保持；Dark V1 输入已导出。用户于 `2026-08-04` 接受 `CHAT.TABS.DARK.V2` 方案 A 的固定 donor＋等比 transform＋输出 SHA 合同例外进入 `source-accepted / P4`：tracked source `ChatTabs_Dark_V2_A.png` SHA `616f965b…a1e3c`，五对象 cell 外 `0`、最小边距 `6px`、三场景 display-region `0` violations；原生产 `5/5`、流程错误 `0`，本次接受没有 TGA／Lua／addon 变更，现行 V3 保持。v1.21 继续透传客户端／pfUI／ChatMOD 经典颜色；Chat Copy／URL Copy 暂缓，右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 确定性 P4→P5 导出四态 atlas／shelf、RGB-only 清理低 Alpha 绿边、接入 adapter，并通过最终 display-region 与 package gate；禁止 attempt 6；游戏设备可用时再 `/reload` 验证核心 runtime |
