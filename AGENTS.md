...
| 任务 | QL-A2 V4 书本主体保持；Quests `1.17`／Quest Visual Theme `1.6` 的 18 个 `246 × 18px` 活动行恢复 `pfUI.font_default` 的 `12px OUTLINE`，额外 shadow 清零。右页按最底动态对象重算 ScrollChild，奖励双列槽为 `108px`／名称 `64px`；滚轮和真实 ScrollFrame 保留。Quest Log 漆章 V1–V8 已否决；当前 `QUEST-LOG-SEAL-ACTIONS-SIM-V9` 保留详情页右上 `40×40px` 火漆入口，展开态以七个 `112×20px` 短书口事务签从 detail 右边界向书外滑出，真实页边 mask 遮根，正文／奖励零占用，只声明 `48px` 右侧 outset；取消尖头、逐项铆钉、亮黄铜高光与整条危险色。当前只到 `simulation-reviewed`，未改 runtime、未隐藏 fallback。Tracker 继续批次提交与 `16px` 底部安全区，QT-A1 仍 `display-region-blocked` | 先由用户确认／退回 V9 克制型书口事务签；游戏设备可用时验证统一字体字重、长正文与 0／1／2／4／6 奖励完整滚动、pfQuest 后加载，再验证菜单代理与放弃确认 |
