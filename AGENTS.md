...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 为 `P6 / game-validated`：attempt `5/5` accepted source `6d4a4d16…7dc0`、runtime TGA `5c49a1db…23ca`，Bar `1–10` scoped adapter、display／package／冷却／距离红／按下反馈均 pass，不得 attempt 6。`AB.RAIL` 为 `P3 / prompt-authorized`：`AB-RAIL-SIM-V1` 的 8 个布局全部 pass，确认方向已冻结进正文；用户于 `2026-08-09` 授权正文、最多 `5` 次实际生成／修复及指定 Character V3 Image 1 外部上传，正式生产 `0/5`。`AB.FIELDKIT.V1` 的 `5×2` 已按用户容量／分组要求修订；当前 `AB-FIELDKIT-SIM-V2` 使用 AutoBar 完整 `4×6 / 24 类`，连续八格分为应急／增益／工具，分类内仍由 `1–12` popup 展开真实物品；TrinketMenu 双槽／候选／Queue 方向不变。display `16/16`、布局 `72/72`、violations `0`、ImageGen `0/0`，现为 `simulation-reviewed / P2` | 提交 Rail 授权正文并由固定执行器开始 attempt 1；用户接受或修订 `AB-FIELDKIT-SIM-V2` 前不启用 AutoBar、不改 TrinketMenu SavedVariables、不执行其生产。Field Kit 两个 kit 后续各需独立最多 `5` 次授权及 Character V3 Image 1 上传授权；`AB.SLOT` 收口须另批精确清单 |
