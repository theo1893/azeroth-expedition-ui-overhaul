...
| 任务 | QL-A2 V4 书本主体保持；Quests `1.17`／Quest Visual Theme `1.6` 的 18 个 `246 × 18px` 活动行恢复 `pfUI.font_default` 的 `12px OUTLINE`，额外 shadow 清零。右页按最底动态对象重算 ScrollChild，奖励双列槽为 `108px`／名称 `64px`；滚轮和真实 ScrollFrame 保留。旧悬空锚点与 V1 书框皮签均不再作为方向；`QUEST-LOG-SEAL-ACTIONS-SIM-V2` 改为从右页层间伸出的羊皮纸火漆封签，漆章压在书外折头并展开七项原 Button 代理，当前只到 `simulation-reviewed`，未改 runtime、未隐藏 fallback。Tracker 继续批次提交与 `16px` 底部安全区，QT-A1 仍 `display-region-blocked` | 先由用户确认／退回 V2 火漆封签与事务菜单模拟；游戏设备可用时验证统一字体字重、长正文与 0／1／2／4／6 奖励完整滚动、pfQuest 后加载，再验证菜单代理与放弃确认 |
