...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5` 且只接入 Player／Target／TargetTarget／Focus。Raid `UF-RAID-SIM-V1` 方向已确认；`UF-RAID-A1 V1 final` 四变体完整背景板正文、固定 Image 1／2、同循环紧邻前稿 edit 边界和最多 5 次实际 ImageGen 已获授权，当前 `P3 / prompt-authorized / 0/5`，无 source/runtime/addon 变更 | 提交授权状态后执行 attempt 1，并在最多五次内自行完成生成、完整审查与有界修复；通过后只交用户复审，不自动进入 P4。B1 另待 Turtle WoW P6；A／B 是否重开另定，A2 暂停 |
