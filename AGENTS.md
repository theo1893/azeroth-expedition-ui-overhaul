...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 为 `P6 / game-validated`：attempt `5/5` accepted source `6d4a4d16…7dc0`、runtime TGA `5c49a1db…23ca`，Bar `1–10` scoped adapter、display／package／实机交互均 pass，不得 attempt 6。`AB.RAIL` 同为 `P6 / game-validated`：accepted source `7c49995d…32e9`、runtime TGA `1e5cca09…0a3d`，Bar `1–12`＋Bar `1／6` merged adapter、display／package／实机六项均 pass，固定生产 `5/5` 且禁止 attempt 6。`AB.FIELDKIT.V1` 的 AutoBar `4×6 / 24 类` 三段分组与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempt 1 raw `fe4b854e…c9e8d` 已计 `1/5`：语义与 display `16/16` pass，但 `1254² RGB`、无 Alpha、棋盘烘焙与 margin 不足，内部 fail；自包含 `r1` 已就绪。Consumable 仍为 `0/5` | 提交 attempt 1 证据、reviewer／测试与 `r1` 后执行 Trinket attempt 2，通过即停、最迟 `5/5`；随后独立执行 Consumable。全过程不启用 AutoBar、不改 TrinketMenu SavedVariables；候选须用户验收后才能晋级 source／runtime。`AB.SLOT`／`AB.RAIL` 收口另批精确清单 |
