...
| 聊天 | Full V1 runtime `1.19 / P5` 保持；正式仍复用 V3 Tab／输入／未读。`CHAT.INPUT.DARK.V1.r3 attempt 4` 已实际生成 `4/5`、流程错误 `4`，当前为 `candidate-reviewed / P3`。仅用固定 Image 1／2／3 regenerate 后，候选成为两层薄烟熏 rag-paper 纸条，不再有卷曲压花、缝线、长导线或连续 focus 金边；共享 Alpha、确定性色键、真实 `380×25/480×25` 排版及五场景 display-region 均通过。自主循环已在首个内部通过候选处停止，正式 source／TGA／Lua 改动仍为 `0`。Chat Copy／URL Copy 暂缓；右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 等待用户接受或退回精确 attempt 4；接受后才进入 P4 source／manifest，退回时剩余 `1/5` 仍受冻结边界约束。游戏设备可用时再 `/reload` 验证核心 r1.19 |
