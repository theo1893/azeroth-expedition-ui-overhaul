...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 为 `P6 / game-validated`：attempt `5/5` accepted source `6d4a4d16…7dc0`、runtime TGA `5c49a1db…23ca`，Bar `1–10` scoped adapter、display／package／冷却／距离红／按下反馈均 pass，不得 attempt 6。`AB.RAIL` 为 `P2 / simulation-confirmed`：`AB-RAIL-SIM-V1` 的 8 个布局全部 pass，确认方向已冻结进正文。当前优先的 `AB.FIELDKIT.V1` 已按现用 TrinketMenu `3.3` 与已安装但当前禁用的 AutoBar `1.31` 完成真实对象审计；双槽 `92×52／52×92 UI`、候选 `0–30`、Queue、AutoBar `1–24` 主 Button 与 `1–12` popup 均进入 `AB-FIELDKIT-SIM-V1`，display `15/15`、布局 `60/60`、violations `0`、ImageGen `0/0`，现为 `simulation-reviewed / P2` | 用户接受或修订 `AB-FIELDKIT-SIM-V1`；确认前不启用 AutoBar、不改 TrinketMenu SavedVariables、不执行生产。之后两个 kit 各需独立最多 `5` 次授权及 Character V3 Image 1 上传授权。Rail 的正文／五次预算／Image 1 上传也仍待独立授权；`AB.SLOT` 收口须另批精确清单 |
