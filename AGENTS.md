...
| 单位框 | `UF-A1 V3-A/B final` 完整 Player／Target 外壳与 `UF-B1 V2 final` Health／Power 灰阶填充纹均为 `P2 / simulation-confirmed / awaiting-production-authorization / 0/5`。用户于 `2026-08-11` 明确接受 `UF-PRIMARY-V3-SIM-V1` 的完整外壳、旧马鞍／盾带式粗犷皮革、非镜像身份、bar 层级与四资源经典乘色方向；模拟 ImageGen `0/0`、展示区域 `7/7 pass`，不接受模拟像素。三段 final 已通过完整性预检，无 source/runtime/addon 改动 | 用户明确授权三段 final 正文、输入职责、修复边界与预算后，才按 A→B→B1 执行；每段最多 `5` 次实际 ImageGen、最坏 `15` 次，流程错误不占额度。A2 紧凑框继续暂停 |
