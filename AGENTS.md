...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit art／v1.4 drawer／v1.5 强绑定不变，仍为 `P5 / pending-retest`。用户最新 `2560×1440` 实机截图证明客户端 `1920×1080` 输出被 `4/3` 显示放大，旧 pfUI tier 7 有效尺寸约 `1.0836×`，玩家框偏大并压住左卷袋。AEUI `0.8.7`／`focus-layout-contract=1.1` 新增 `/aeui focuslayout comfort`：显式切换 tier 8 `0.711111`、不改分辨率，Player／Target 与施法条 local scale 改为 `1.0`，按新 UIParent 重算当前角色单位框、施法、Swing、姿态、DoiteDPS 与 Combat Deck；玩家／目标约 `80 px` 内缘间距保持，TrinketMenu MainScale 归一 `1.0`。无维护循环、无重绘、ImageGen `0` | 启动验证 `version 0.8.7`、`focus-layout-contract=1.1`、`focus-ui-scale=saved`、`focus-ui-scale-tier=8`，检查整体尺寸、中文文字、CD、玩家／目标、施法、Swing、DoiteDPS、左卷袋／右双槽与世界视野；需同时恢复 scale／位置时执行 `/aeui focuslayout comfort`。同轮继续 Field Kit popup、Queue／换装及 fail-open 复测；通过前不得标记 P6 |
