...
| 聊天 | Full V1 runtime `1.19 / P5` 保持；正式仍复用 V3 Tab／输入／未读。用户已接受 `CHAT.INPUT.DARK.V1-SIM` 暖烟草抄写纸条方向，并于 `2026-08-03` 独立授权 `CHAT.INPUT.DARK.V1 / prompt-authorized / P3`、固定 SHA 的 Image 1／2／3、最多五次实际 ImageGen 与冻结的 Image 4 edit 边界；授权正文已由 `aa7a374` 固定。四个流程错误均无 provider 生成证据；当前生产 `0/5`、流程错误 `4`，runtime 改动仍为 `0`。Chat Copy／URL Copy 暂缓；右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 保持空临时目录方案，只把执行指令中的 `$imagegen` 转义为 shell 字面量后重试同一 attempt 1；逐候选完成全量审查、真实排版与 display-region，内部通过后交用户接受。游戏设备可用时再 `/reload` 验证核心 r1.19 |
