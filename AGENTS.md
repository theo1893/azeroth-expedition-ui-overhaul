...
| 单位框 | A1 当前进入 `UF-A1 V2-A V2 / P3 / prompt-authorized`。旧 V2-A V1 的单图四端帽循环已用满 ImageGen `5/5` 并退回；用户于 `2026-08-11` 明确否决逐端帽独立生成，要求继续以一次调用输出一张四端帽 atlas，并通过更详尽正文约束。`UF-A1-V2-SIM-V2` 的八件 source、标准单 shell、可变三切片、`42px` 固定高度与可见方向均未改变，确认继续有效。新 V2-A V2 已按固定 Image 1／2、attempt 1 无 Image 3、同循环紧邻前稿仅限整图有界 edit、独立 `5` 次预算及确定性审查合同授权，当前 `0/5`；V2-B 保持 `0/5 / sequence-blocked`；无 source/runtime/addon 改动；A2／B1 继续暂停 | 提交授权基线后执行 V2-A V2 attempt 1，完成四列拆分、色键、等比 fit、真实排版与缩放审查；内部通过即停，否则冻结边界内最多五次。不得把它当成旧 V1 的第六次调用；通过后才恢复 V2-B |
