...
| 单位框 | `UF-A1 V3-A/B final` 与 `UF-B1 V2 final` 均为 `P3 / prompt-authorized / 0/5`。用户于 `2026-08-11` 接受 `UF-PRIMARY-V3-SIM-V1`，并另行授权三段 final 按 A→B→B1 执行；每段最多 `5` 次实际 ImageGen、最坏 `15` 次，流程错误不占额度。A／B 固定两张锁定图且只允许同段紧邻前稿有界 edit；B1 首次无图片；禁止跨段和旧失败像素复用。尚无 source/runtime/addon 改动 | 提交授权正文后启动 A attempt 1，逐段完成生成—审查—有界修复；内部通过即停，第五次仍失败则该段耗尽。A2 紧凑框继续暂停 |
