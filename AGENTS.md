...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。Field Kit accepted source／TGA 像素不变；v1.1 Icon 层序已实机通过，随后发现 AutoBar 原生左向 popup 遮挡主格。AEUI `0.8.2`／`fieldkit-contract=1.2` 在 exact `24 / 4×6 / 推荐 profile` 使用外置 `1×1–6／2×4–6` drawer，支持 AUTO／LEFT／RIGHT／NATIVE 与非 exact 原生回退；消耗品／饰品分别默认软吸附主栏左 `48 UI`／右 `16 UI`，阈值 `32 UI`，可独立拖离／回吸附，只保存 AEUI 布尔状态且无维护循环。simulation `89/89`、display `19/19`，runtime display `9/9`＋`10/10`、package pass。Field Kit 为 `P5 / runtime-v1.2 / pending-retest`，ImageGen `0`，原循环仍止于 `4/5` 与 `1/5` | Turtle WoW `/reload` 验证 `fieldkit-contract=1.2`、候选 `1／6／7／12` 外置抽屉不遮主格、四种 popup 模式、非 exact 回退、两袋独立拖离／回吸附、apply／restore、TrinketMenu 候选／Queue／换装及 `/aeui actionbars` fail-open；通过前不得标记 P6 |
