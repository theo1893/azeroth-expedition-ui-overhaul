...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。`AB.FIELDKIT.V1` 的 AutoBar `4×6 / 24 类` 三段分组与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempts 1／2 raw `fe4b854e…c9e8d`／`85f3f6f0…50b7` 已计 `2/5`：attempt 2 降噪和 B／D margin 改善，但 A／C margin 仍不足且 C 为空心。两稿 display 均 `16/16 pass`。用户已授权两套纯 `#00FF00` RGB raw→本地确定性 canonical `1024² RGBA` 传输修订；不重绘、不新增输入图、预算不重置。Trinket 完整 `r2` 已准备，Consumable 仍为 `0/5` | 提交 `r2`、canonicalizer／reviewer 与测试后执行 Trinket attempt 3；通过即停，否则最迟 `5/5`。Trinket 循环结束后才准备 Consumable transport body／attempt 1；全过程不启用 AutoBar、不改 TrinketMenu SavedVariables |
