...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。`AB.FIELDKIT.V1` 的 AutoBar `4×6 / 24 类` 三段分组与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempt 4 raw `2e4efc1a…19e3a`／canonical `82dd2260…c012` 在 `4/5` 通过 Prompt、语义、美术、技术与 `16/16` display 全门禁，现为 `candidate-reviewed / pending-user-acceptance`，按 pass 即停且不执行第 5 次。用户授权的纯 `#00FF00` RGB raw→canonical 修订不重绘、不新增输入图、预算不重置。Consumable 完整 transport-amended 自包含正文已准备，仍为 `0/5` | 提交 Trinket attempt 4 记录与 Consumable 执行正文后，以 Character V3 唯一 Image 1 执行 Consumable attempt 1；通过即停，否则最多 `5/5`，禁止 attempt 6。两套候选均须用户明确接受才可晋级 source／runtime；全过程不启用 AutoBar、不改 TrinketMenu SavedVariables |
