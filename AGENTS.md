...
| 任务 | 当前首要大面积 UI；QL-A1 `P4`；QL-A2 V4 固定卷宗背景 `P5`；用户接受 QL-B1 V1.r3 运行时视觉，QL-B0 23 行与 QL-B1 四态 atlas 已确定性导出、接入并静态通过，`P5`；QL-B2 V1.r4 bbox-fit 候选及一次性确定性合同例外已由用户接受，同 SHA source 与 manifest 已晋级 `P4`，实际生图仍为 `5/5`、接受后调用 `0`；V3.3 `15/15` 与 QL-B1 `5/5` 内部失败事实保留 | QL-A2／B0／B1 等待 Turtle WoW 实机；QL-B2 按固定三态算法导出、接入并静态验证到 P5 |
