...
| 任务 | QL-A2 V4 书本主体保持；Quests `1.17`／Quest Visual Theme `1.6` 的 18 个 `246 × 18px` 活动行恢复 `pfUI.font_default` 的 `12px OUTLINE`，额外 shadow 清零。右页按最底动态对象重算 ScrollChild，奖励双列槽为 `108px`／名称 `64px`；滚轮和真实 ScrollFrame 保留。旧悬空锚点、V1 书框皮签和 V2 硬质二段封签均不再作为方向；`QUEST-LOG-SEAL-ACTIONS-SIM-V3` 改为由真实右页像素压住根部的一整张柔性羊皮纸，漆章压在书外末端，七项事务从同一纸面向书内连续展开，当前只到 `simulation-reviewed`，未改 runtime、未隐藏 fallback。Tracker 继续批次提交与 `16px` 底部安全区，QT-A1 仍 `display-region-blocked` | 先由用户确认／退回 V3 火漆封签与事务签模拟；游戏设备可用时验证统一字体字重、长正文与 0／1／2／4／6 奖励完整滚动、pfQuest 后加载，再验证菜单代理与放弃确认 |
