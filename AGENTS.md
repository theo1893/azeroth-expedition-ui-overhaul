...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已于 `2026-08-08` 获用户确认，锁定 DoiteDPS → 攻击计时 → Aura／双框 → 双施法条 → 动作栏纵向焦点。首批 `AB.SLOT.BASE.V1` 固定 `@openai/codex@0.143.0` 循环在 ImageGen `5/5` 停止；用户于 `2026-08-08` 接受第 5 稿，exact canonical RGBA `6d4a4d16…7dc0` 已字节一致晋级 tracked source／manifest。五种真实排版 `5/5 pass`、violations `0`；当前 `P4 / source-accepted`，无 runtime、addon 接管或 SavedVariables 写入，TrinketMenu／AutoBar／DoiteDPS 仍只做可选 feature-detect | 另行执行确定性 `128×128` runtime export、Bar `1–10` scoped adapter、最终 display-region 与 fresh-checkout addon package 门禁；不得 attempt 6 |
