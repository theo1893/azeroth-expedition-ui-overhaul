...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v1.6 为 `P5 / pending-retest`。用户已接受 `ACTION-BARS-CORE-SIM-V4`：AEUI `0.8.9`／`focus-layout-contract=1.3` 保持 tier 8、Combat Deck 与 Field Kit 不变，将 Player／Target、双方施法、Swing、姿态和 DoiteDPS local scale 从 `0.75` 调到 `0.82`，线性 `+9.33%`、面积约 `+19.54%`，并内收双框保持外包络。`fieldkit-contract=1.6` 把已审计的 ArchiTotem `1.7` 根作为动作条下方职业卫星；绑定态随 Bar 1，显式 focus preset 请求 `down`，普通 refresh 不写方向。layout `54/54`、simulation／runtime display 均 `7/7`、Lua smoke pass；无新位图，ImageGen `0/0`，当前 `P5 / pending-game-validation` | `/reload` 后执行一次 `/aeui focuslayout comfort`，验证 0.82 可读性、单位框净空、施法／Swing／DoiteDPS，以及 ArchiTotem 施放、右键、Air 七层、拖动／锁定、Recall、bind／unbind、强绑定和 fail-open；同轮继续 Field Kit popup、Queue／换装复测。若 0.82 仍不足，再以截图进入 0.85 对照修订 |
