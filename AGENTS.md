...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5` 且只接入 Player／Target／TargetTarget／Focus。Raid A1 仍为 `P3 / 5/5 exhausted / rejected`；`UF-RAID-A2-SIM-V1` 已确认，`UF-RAID-A2-DONOR V1` 已于 `2026-08-12` 精确授权，当前 `P3 / prompt-authorized / attempt-01-ready`，ImageGen `0/5`，无 source/runtime/addon | 以已提交原文和固定 SHA Image 1／2 调用 `imagegen-0-143-0` attempt 1，随后执行 donor 技术审查、Python 精确造壳与 40 人真实排版；通过即停，最多 5 次。A1 禁止第六次；B1 另待 Turtle WoW P6 |
