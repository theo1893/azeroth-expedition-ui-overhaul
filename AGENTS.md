...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v1.8 与 Combat Focus runtime-v1.7 为 `P5 / pending-game-validation`。AEUI `0.8.14` 的 `ACTION-BARS-CORE-SIM-V6` 继续使用 Turtle WoW 游戏原生坐标：Player／Target 缩为 `240×60 / 0.68`，TargetTarget 以 `132×30 / 0.62` 依附 Target；双方 Aura 按玩家左起、目标右起分列上下；三条施法／Swing 统一为 `180×16 / 0.72` 并排；消耗品三段文字移除但语义分隔保留，Field Kit／饰品／ArchiTotem 间距收紧。仅未被手动调整的 exact v7 profile 在 `/reload` 一次性迁移为 v8；pfUI mover 登记保持稳定，TargetTarget 独立 mover 只在 unlock 创建后隐藏，退出 unlock 重施依附锚。全部位图字节不变，ImageGen `0/0`；当前 `P5 / pending-game-validation` | `/reload` 后开关一次 pfUI 解锁，确认无 `drag=nil`、绑定态只显示 Bar 1 mover、Bar 6 与 TargetTarget 均不跳位；确认 Player／Target 不遮人物或卷袋、TargetTarget 紧贴目标、Aura 方向正确、无“工具／增益／应急”文字、三条 `180×16` 读数并排且组件间距紧凑；再验证双方状态／施法、近战／远程 Swing、DoiteDPS、ArchiTotem、Field Kit popup／Queue／换装；需要回退时 `/aeui focuslayout restore` 后 `/reload` |
