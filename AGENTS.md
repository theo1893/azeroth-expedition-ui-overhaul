...
| 聊天 | Full V1 runtime `1.19 / P5` 保持；正式仍复用 V3 Tab／输入／未读。`CHAT.INPUT.DARK.V1` attempt 1 已由固定 `0.143.0` session `019fc5dd-d5a9-7ba0-912d-7d08fe738824` 实际生成并计为 `1/5`；技术归一化、共享 Alpha、真实 `380×25/480×25` 排版与五场景 display-region 通过，但首个美术门禁因“整块缝线皮革槽而非多层烟草纸”、近全宽导线与过亮橙金 focus 失败。四个更早流程错误均无 provider 结果；runtime 改动仍为 `0`。冻结边界内的完整 `.r1` 使用紧邻 attempt 1 作为 Image 4 edit，只保留两对象、长比例与暗色顺序。Chat Copy／URL Copy 暂缓；右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 提交 `.r1` 后执行 attempt 2；逐候选完成同一全量审查，内部通过后交用户接受。游戏设备可用时再 `/reload` 验证核心 r1.19 |
