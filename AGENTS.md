...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5` 且只接入 Player／Target／TargetTarget／Focus。Raid A1 仍为 `P3 / 5/5 exhausted / rejected`；用户已选择 A2 的 material-only donor＋Python 精确造壳，`UF-RAID-A2-SIM-V1` 当前 `P2 / simulation-reviewed / user-confirmation-pending`，display `7/7 pass`、ImageGen `0/0`，无 source/runtime/addon | 等待用户确认 A2 模拟的精确轮廓、四维修差异、40 人密度与动态层序；之后仍需单独授权 `UF-RAID-A2-DONOR V1`。A1 禁止第六次；B1 另待 Turtle WoW P6；主 A／B 是否重开另定，紧凑 A2 暂停 |
