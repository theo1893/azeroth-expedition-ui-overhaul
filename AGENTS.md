...
| 单位框 | A1 V2 `P3 / prompt-authorized`；`UF-A1 V1` 已用满 `5/5` 且用户拒绝例外。`UF-A1-V2-SIM-V2` 已确认：每角色四个 source，标准 `W=200` 单 `214×42` shell，可变宽度三切片，固定 `42px` 高度；缩放／宽度矩阵无接缝与内容侵入，display-region `6/6 pass`。用户于 `2026-08-11` 授权 V2-A 四端帽与 V2-B 四横轨按 A→B 执行，每段最多 `5` 次、最坏 `10` 次，当前均 `0/5`；无 production source/runtime；A2／B1 继续暂停 | 先执行 V2-A 自主修复循环，通过后执行 V2-B；合成候选通过语义、美术、装配、真实排版与缩放门禁后交用户复审。用户接受前不接入 addon |
