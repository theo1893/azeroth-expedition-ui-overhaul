...
| 动作条／随身栏 | pfUI 十二条 Bar、真实状态、自由移动／缩放／行列与目标设备 profile 已完成 `P1` 审计。用户否决贴底且单位框分散的 V1 后，`ACTION-BARS-CORE-SIM-V2` 已把约 `39px` 主技能格上移至中下战斗焦点，并把玩家／目标框同基线收拢为 `80px` 内缘间距；display `9/9`、布局 `20/20 pass`。TrinketMenu／AutoBar 仍只做可选 feature-detect；ImageGen `0/5`，无 runtime 接管或 SavedVariables 写入 | 用户确认 V2 的战斗焦点高度、双方状态框距离、左右随身栏和默认无狮鹫重量；确认前不锁定 `P2`、不生成正式资产 |
...
- [当前核心 V2 工作文件](docs/modules/actionbars/work/ACTION.BARS.CORE.V2.md)
