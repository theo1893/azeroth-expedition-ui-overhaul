...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 为 `P6 / game-validated`：attempt `5/5` accepted source `6d4a4d16…7dc0`、runtime TGA `5c49a1db…23ca`，Bar `1–10` scoped adapter、display／package／冷却／距离红／按下反馈均 pass，不得 attempt 6。`AB.RAIL` 为 `P5 / runtime-exported`：accepted source `7c49995d…32e9` 已确定性导出为 `256²` TGA `1e5cca09…0a3d`（像素 `1b09b93b…9db5`）；Bar `1–12`＋Bar `1／6` merged scoped adapter、display `8/8`、Lua smoke、repository tests 与 fresh-checkout package 均 pass，未修改 pfUI／SavedVariables，P4→P5 ImageGen `0`，固定生产仍为 `5/5` 且禁止 attempt 6。`AB.FIELDKIT.V1` 的 `5×2` 已按用户容量／分组要求修订；当前 `AB-FIELDKIT-SIM-V2` 使用 AutoBar 完整 `4×6 / 24 类`，连续八格分为应急／增益／工具，分类内仍由 `1–12` popup 展开真实物品；TrinketMenu 双槽／候选／Queue 方向不变。display `16/16`、布局 `72/72`、violations `0`、ImageGen `0/0`，现为 `simulation-reviewed / P2` | Rail 在 Turtle WoW `/reload` 后验证横／竖／多行、Bar `1／6` 合并无中缝、拖动／缩放／显隐、姿态／宠物栏跟随与 fail-open。用户接受或修订 `AB-FIELDKIT-SIM-V2` 前不启用 AutoBar、不改 TrinketMenu SavedVariables、不执行其生产。Field Kit 两个 kit 后续各需独立最多 `5` 次授权及 Character V3 Image 1 上传授权；`AB.SLOT` 收口须另批精确清单 |
