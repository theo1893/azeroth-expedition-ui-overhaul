...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。Field Kit accepted source／TGA 像素不变；v1.1 Icon 层序已实机通过，v1.2 完成外置 drawer 与双侧软吸附。v1.3 的外缘透明通道实机失败：从内侧主格向右抽屉移动仍会经过其他 AutoBar 主格，而这些格子的原生 `OnEnter` 会立即调用 `SetPopupButton` 切换或关闭当前 popup。AEUI `0.8.4`／`fieldkit-contract=1.4` 保留联合悬停区，并仅在 exact 外置态对主格切换加入 `0.30s` 意图停留；穿越不切换，停留才把操作交回捕获的 AutoBar 原方法。NATIVE、非 exact、AEUI 关闭或调度 API 缺失均立即走 provider。simulation `89/89`、display `19/19`，runtime display `9/9`＋`10/10`；Field Kit 为 `P5 / runtime-v1.4 / pending-retest`，ImageGen `0`，原循环仍止于 `4/5` 与 `1/5` | Turtle WoW `/reload` 验证 `fieldkit-contract=1.4`：打开内侧分类后横穿相邻主格进入左右抽屉，原抽屉必须保持；在另一分类主格停留约 `0.30s` 应正常切换，完全离开联合区域后正常关闭。再验证四种 popup 模式、非 exact 立即回退、两袋独立拖离／回吸附、apply／restore、TrinketMenu 候选／Queue／换装及 `/aeui actionbars` fail-open；通过前不得标记 P6 |
