...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v1.6 为 `P5 / pending-retest`。用户以新实机截图否决 V4 后已明确“确认接入” `ACTION-BARS-CORE-SIM-V5`：AEUI `0.8.10`／`focus-layout-contract=1.4` 保持 tier 8、Combat Deck 与 Field Kit 不变，Player／Target 和双方施法使用 `0.75`，Swing／姿态／DoiteDPS 保持 `0.82`；显式 preset 从重置后的 live Bar 1 中心／顶边一次性投影，普通 refresh 不维护坐标。`fieldkit-contract=1.6` 继续把 ArchiTotem `1.7` 根作为动作条下方职业卫星；绑定态随 Bar 1，显式 preset 请求 `down`，普通 refresh 不写方向。layout `59/59`、simulation／runtime display 均 `8/8`、Lua smoke 与 fresh-checkout package pass；无新位图，ImageGen `0/0`，当前 `P5 / pending-game-validation` | `/reload` 后执行一次 `/aeui focuslayout comfort`，确认攻击／施法不重叠、中央空白收紧、单位框不挡人物，并验证双方状态／施法、近战／远程 Swing、DoiteDPS，以及 ArchiTotem 施放、右键、Air 七层、拖动／锁定、Recall、bind／unbind、强绑定和 fail-open；同轮继续 Field Kit popup、Queue／换装复测 |
