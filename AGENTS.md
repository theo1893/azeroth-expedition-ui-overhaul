...
| 单位框 | `P3`。A attempt 4 已计 `4/5`：单开口／物理连通／ratio／anisotropy 通过，hard safe core 降至 `9061 px`；但开口仍约 x `71..1206`、bbox `1392×281` 与 isolation `70/74` 失败，故无 candidate/source/runtime。最终 `UF-A1 V3-A final.r4` 只要求整体等比缩至 `1284×252`，把两竖边做成与横轨同厚并移内脸至 x `42/1242`。B/B1 `0/5`；两次流程错误均无额外 provider 图、不占额度 | 提交 `final.r4 / repair-prepared` 后执行 A attempt 5，只以 attempt 4 raw 为 Image 3；通过即停，仍失败则 A 耗尽等待用户审查。之后按授权顺序继续 B、B1；A2 暂停 |
