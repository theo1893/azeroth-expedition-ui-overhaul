...
| 聊天 | 核心 runtime `1.21 / P5`。Full V1 主框、V3 Tab／未读保持；`CHAT.INPUT.DARK.V1.r3 attempt 4` 已导出为 `ChatInputDarkV1.tga`。`CHAT.TABS.DARK.V1` 固定 0.143.0 循环已耗尽 `5/5`，流程错误 `2`；未改 runtime。attempt 5 因 `125,257` 个 cell 外像素退回；attempt 3 是唯一原冻结 cell 合同、真实排版和 display-region 全通过的候选，但违反 `.r2` 临时 hard-mask `3,460px`，当前仅为待用户明确接受的确定性合同例外候选。v1.21 保持客户端／pfUI／ChatMOD 经典颜色透传；Chat Copy／URL Copy 暂缓，右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 用户审视 attempt 3：明确接受例外后才进入 P4；否则另开新版本。游戏设备可用时再 `/reload` 验证核心 runtime |
