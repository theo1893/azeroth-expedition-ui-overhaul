...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5` 且只接入 Player／Target／TargetTarget／Focus。Raid `UF-RAID-A1 V1 final` 也已 `P3 / 5/5 exhausted / candidate-rejected`：attempt 3 为最佳内部视觉参考但 padding／亮边失败，attempt 5 padding 通过但误成约 `3.19:1`；无 candidate/source/runtime/addon 变更 | Raid 禁止第六次；等待用户明确重开新合同、改用材质 donor＋确定性几何，或授权具体确定性例外。B1 另待 Turtle WoW P6；A／B 是否重开另定，A2 暂停 |
