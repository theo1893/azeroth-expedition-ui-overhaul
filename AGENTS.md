...
| 聊天 | 核心 runtime `1.21 / P5`。Full V1 主框、V3 Tab／未读保持；`CHAT.INPUT.DARK.V1.r3 attempt 4` 已导出为 `ChatInputDarkV1.tga`。用户已确认并授权 `CHAT.TABS.DARK.V1`：深胡桃皮革四态、无浅纸 selected、无连续亮带 shelf，四张固定 SHA 输入、受限 Image 5 edit、最多五次实际 ImageGen；当前 `prompt-authorized / P3 / 0/5`，runtime 未改。v1.21 保持客户端／pfUI／ChatMOD 经典颜色透传；Chat Copy／URL Copy 暂缓，右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 提交精确正文后执行固定 ImageGen 0.143.0 attempt 1；游戏设备可用时再 `/reload` 验证核心 runtime |
