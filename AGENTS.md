...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。Field Kit accepted source／TGA 像素不变；v1.4 已保留 external drawer、联合悬停与 `0.30s` 主格切换意图。用户最新完整 UI 截图要求把动作条、消耗品栏和饰品栏强绑定并按最初构图重排。AEUI `0.8.5`／`fieldkit-contract=1.5` 使用唯一 `fieldKitBound`：Bar 6、左 `4×6` 卷袋、右水平双槽直接相对锚到 Bar 1；绑定态只保留主栏 mover，侧 provider 误拖松手回位；`unbind` 恢复自由位置，`home` 恢复中心中下基线。当前“大奶黑牛”已写入主栏 `x=0／y=258`、上栏 `x=0／y=291` 与水平 TrinketMenu。runtime display `9/9`＋`10/10`、package pass；Field Kit 为 `P5 / runtime-v1.5 / pending-retest`，ImageGen `0`，原循环仍止于 `4/5` 与 `1/5` | Turtle WoW 启动或 `/reload` 验证 `fieldkit-contract=1.5`、`fieldkit-binding=bound`、`actionbar-stack=12x2-bound` 与左卷袋—中央动作条—右双槽；拖动 Bar 1 应整体跟随，拖动两侧松手应回位，bind／unbind／home 可逆。再复测候选跨格保持、`0.30s` 切换、四种 popup、非 exact 回退、apply／restore、Queue／换装及 `/aeui actionbars` fail-open；通过前不得标记 P6 |
