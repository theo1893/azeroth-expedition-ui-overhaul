...
| 单位框 | `P3`。A `5/5 repair-budget-exhausted`：attempt 5 的单开口／物理连通／bbox `1298×249`／ratio／anisotropy／isolation 通过，但 hard safe core 仍 `6355 px`，竖边多 `23/26 source px`，不能由 Python 清除；无 candidate/source/runtime，禁止第六次。B `0/5 attempt-1 queued`，B1 `0/5 wait`；两次流程错误无额外 provider 图、不占额度。按授权 A 耗尽不阻止独立段 | 提交 A 终态后执行 B attempt 1，只上传固定 Image 1／2、无 Image 3，禁止复用 A；B 后执行 B1。三段终态后统一用户审查。A2 暂停 |
