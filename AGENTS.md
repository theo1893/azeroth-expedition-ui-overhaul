...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。`AB.TRINKET.KIT.V1` 第4稿与 `AB.CONSUMABLE.KIT.V1` 第1稿已确定性导出并由 `ActionBars` 可选 provider bridge 接入：两张 `512² RGBA` TGA SHA 分别为 `3614d9a8…f455`、`c48f6292…320e`，像素 SHA 为 `0961d750…aef`、`658f826f…e30d`；最终 display `9/9` 与 `7/7`、violations `0`，fresh-checkout package `pass`、目标设备无需构建。AutoBar 仍未启用且未应用 profile；TrinketMenu 的 Button、Queue、换装、拖动、方向、停靠与两方 SavedVariables 均未接管。Field Kit 当前为 `P5 / runtime-exported`，P4→P5 ImageGen `0`，原循环仍终止于 `4/5` 与 `1/5` | 在 Turtle WoW `/reload` 后验证两套 TGA 方向、AutoBar 自定义布局／精确 `4×6` 三组／四向 popup、TrinketMenu 横竖双槽／候选／Queue／换装／拖动／缩放／停靠及 `/aeui actionbars` fail-open；通过前不得标记 P6 |
