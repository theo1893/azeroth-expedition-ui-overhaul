...
| 聊天 | 核心 runtime `1.21 / P5`。Full V1 主框、V3 Tab／未读保持；`CHAT.INPUT.DARK.V1.r3 attempt 4` 已导出为 `ChatInputDarkV1.tga`。V1 深色 Tab attempt 3 已因轮廓过于工整由用户否决，不得晋级或作为 edit 输入。当前 `CHAT.TABS.DARK.V2 / simulation-reviewed / P2` 用旧 V3 的斜梯形皮签、外撇端部、手工缝线与夹页物理关系收敛轮廓，仍保持深色、`92×30` 和原命中区；本地真实排版三场景通过，ImageGen `0/0`，未改 runtime。v1.21 保持客户端／pfUI／ChatMOD 经典颜色透传；Chat Copy／URL Copy 暂缓，右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 用户确认／否决 `CHAT.TABS.DARK.SIM.V2`；确认后才建立 V2 生产 Prompt、scaffold、固定输入和新授权。游戏设备可用时再 `/reload` 验证核心 runtime |
