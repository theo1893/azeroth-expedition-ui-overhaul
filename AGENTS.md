...
| 聊天 | 核心 runtime `1.21 / P5`。Full V1 主框、V3 Tab／未读保持；`CHAT.INPUT.DARK.V1.r3 attempt 4` 已导出为 `ChatInputDarkV1.tga`。`CHAT.TABS.DARK.V2` 已按用户精确授权完成固定 0.143.0 循环，终态 `candidate-rejected / repair-budget-exhausted / P3 / 5/5`、流程错误 `0`：attempt 5 的对象身份、材料和真实排版可读，但 source cell 外仍有 `98555` 个可见像素，五对象均未满足 4px 隔离且轮廓仍偏规整；没有 source／runtime／addon 变更，现行 V3 保持。v1.21 继续透传客户端／pfUI／ChatMOD 经典颜色；Chat Copy／URL Copy 暂缓，右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 禁止 V2 attempt 6；用户另行授权新批次或明确授权确定性重排合同例外；游戏设备可用时再 `/reload` 验证核心 runtime |
