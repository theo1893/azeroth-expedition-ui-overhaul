...
| 单位框 | `P3`。A `5/5 exhausted` 不变。B attempt 4 已计 `4/5`：单开口／物理连通通过，但 bbox `1454×247`，ratio `15.532181%`、anisotropy `13.444030%`、hard safe `7979 px`、isolation `43/39` 失败；连续压纹、规则系带和偏亮红皮也偏工业。无 candidate/source/runtime。`UF-A1 V3-B final.r4` 用绝对坐标锁定外 bbox x `126..1410/y386..638` 与开口 x `168..1368/y422..602`，恢复烟熏粗旧 Target 身份。B1 `0/5`；流程错误 `2` | 提交 B `final.r4 / final-repair-prepared` 后执行最后 attempt 5，只以上一 B raw 为 Image 3；失败则 B 耗尽且不得第六次，随后 B1。A2 暂停 |
