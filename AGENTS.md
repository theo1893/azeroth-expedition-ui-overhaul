...
| 聊天 | 核心 V3 runtime `1.18 / P5`；新 `CHAT.FRAME.FULL.V1.r1 attempt 2` 暖黑完整旧书 source 已于 `2026-08-03` 接受进入 `P4`，尚未导出或替换 V3 TGA。受管正文沿用 pfUI 配置字体，移除描边、文字阴影与矩形压光层，保留 `3px` 行距；v1.18 使用 Vanilla 原色相的等比例深墨，团队焦橙／小队蓝紫独立。Chat Copy／URL Copy 暂缓；单一左框、右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 为新 source 建立确定性九宫格／atlas 导出与最终展示区域复核；现行 `chat-runtime=1.18` 仍待 Turtle WoW 重载验证团队／小队、九职业、DPSMate 红绿报告及书外 Frame |
