...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v1.7 为 `P5 / pending-retest`。用户确认的 `ACTION-BARS-CORE-SIM-V5` 视觉方向不变；focus runtime-v1.4／v1.5 均已被实机判定 `game-geometry-failed`，AEUI `0.8.12`／`focus-layout-contract=1.6` 已删除投影并改用游戏原生坐标。用户随后报告 pfUI 解锁退出于 `unlock.lua:527` 访问空 `drag`；AEUI `0.8.13`／`fieldkit-contract=1.7` 不再动态删回 Bar 6 的 movable 登记，在 pfUI 创建 drag 后只隐藏绑定态 Bar 6 mover，并在 actionbar 配置刷新后重施相对锚点。tier 8、Combat Deck、单位／施法 `0.75`、Swing／姿态／DoiteDPS `0.82`、ArchiTotem bridge 与全部位图均不变；ImageGen `0/0`，当前 `P5 / pending-game-validation` | `/reload` 后先开关一次 pfUI 解锁，确认无 Lua 错误、绑定态只显示 Bar 1 mover 且 Bar 6 始终随 Bar 1；再执行一次 `/aeui focuslayout comfort`，确认攻击／施法不重叠、中央空白收紧、单位框不挡人物，并验证双方状态／施法、近战／远程 Swing、DoiteDPS、ArchiTotem、Field Kit popup／Queue／换装；需要回退时 `/aeui focuslayout restore` 后 `/reload` |
