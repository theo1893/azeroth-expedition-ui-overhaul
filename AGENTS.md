...
| 单位框 | A／B 外壳 `P3 / 5/5 exhausted / rejected`；B1 Health／Power `P5`。Raid A1 仍为 `P3 / 5/5 exhausted / rejected`；Raid A2 donor 已 `5/5`，attempt 5 为最佳运行时视觉，当前 `P3 / exception-review-required`，无 accepted source/runtime/addon | 展示 Raid work 中 attempt 5 的真实 40 人排版，等待用户接受或拒绝只豁免未消费外围 field bbox 的 `sample-window-only` 例外；接受前不进入 P4/P5，禁止第六次。B1 另待 Turtle WoW P6 |
