...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v2.7、Combat Focus runtime-v2.5 与 Sidebar Group runtime-v1.0 为 `P5 / pending-game-validation`。AEUI `0.8.25` 的 `ACTION-BARS-CORE-SIM-V11` 保留既有单位框、Aura、三条计时栈、系统字体、DoiteDPS 安全带与四栏 `2×2 / 3×4` 几何。`2026-08-12` 新实机证据确认上一轮仅抢写 AutoBar handle 与仅放大姿态外层 scale 均未解决问题；当前 bridge-v2.7 把活动 AutoBar display 原生停靠到 `pfActionBarMain`，由 provider 每次 `SetupVisual` 自己写主栏相对锚点，绑定态自由坐标仅保留作可逆回退；AEUI 专用 docking token 在 logout／reload 前恢复，不污染下一次 AutoBar 先行加载。focus runtime-v2.5／profile v16 把真实 `bar11.icon_size` 从 `18` 提到 `25 UI`、local scale 固定为 `1.0`，并在 pfUI `UpdateConfig` 边界重建／重施；v14／v15 copied profile 只升级姿态合同，不改其他手调坐标。全部位图字节不变，ImageGen `0/0` | `/reload` 后确认卷袋立即位于主动作栏左侧，连续两次 `/aeui autobar apply`、开关配置页及移动主栏均不跳位；`/aeui status` 应含 `autobar-anchor-basis=provider-dock`、`autobar-provider-dock=bound`、`autobar-drag-handle=hidden-bound`。确认三枚战士姿态按钮为 `25 UI / scale 1`、点击／高亮正常；继续复测既有系统字体、Aura、Boss Debuff、计时栈、单 mover、四栏可逆 unbind、popup／Queue／换装。需要回退时 `/aeui focuslayout restore` 后 `/reload` |
