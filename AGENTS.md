...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。Field Kit accepted source 与两张 TGA 像素不变；首次实机检查发现 AutoBar Count 可见但 Item Icon 被口袋遮挡，TrinketMenu 双槽也只剩护套。根因是 A／B 装饰与 ActionButtonTemplate Icon 共用 `BACKGROUND`。AEUI `0.8.1`／`fieldkit-contract=1.1` 已把 AutoBar `24+12`、TrinketMenu `2+30` 的口袋／护套移到低于真实 Button `1` 级的非交互子 Frame；display `9/9`＋`7/7`、package pass。新增显式可逆 `/aeui autobar open／apply／restore`，只在用户主动 apply 时备份并写当前角色 `4×6` profile，不启用 provider、不改其他角色或 TrinketMenu。Field Kit 为 `P5 / runtime-v1.1 / pending-retest`，ImageGen `0`，原循环仍止于 `4/5` 与 `1/5` | Turtle WoW `/reload` 后先复测两方 Icon／Count／Cooldown／Queue 层序；再执行 apply／restore 验证精确 `4×6` 三组与可逆配置，并完成四向 popup、TrinketMenu 候选／换装／拖动／缩放／停靠及 `/aeui actionbars` fail-open；通过前不得标记 P6 |
