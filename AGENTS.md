...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit art／v1.4 drawer／v1.5 强绑定不变，仍为 `P5 / pending-retest`。第二张 `2560×1440` 实机截图证明 v1.1 虽已把全局 pfUI 切到舒适的 tier 8，但 `280 UI` 玩家框仍经 `1920×1080 → 2560×1440` 显示链放大为 `374×114 px` 并压住左卷袋。AEUI `0.8.8`／`focus-layout-contract=1.2` 保留 tier 8，只对 Player／Target、双方施法、Swing、姿态和 DoiteDPS 使用 `0.75` local display compensation，并以固定目标显示坐标取代 UIParent 虚拟尺寸推导；投影后玩家框约 `280 px`、两框内缘约 `80 px`、左卷袋净空约 `3 px`。Combat Deck／Field Kit 不缩放、不移位，TrinketMenu MainScale 保持 `1.0`；无维护循环、无重绘、ImageGen `0` | 启动验证 `version 0.8.8`、`focus-layout-contract=1.2`、`focus-ui-scale=saved`、`focus-ui-scale-tier=8`、`focus-layout-display-scale=0.75`，检查玩家／目标不再覆盖卷袋、施法、Swing、DoiteDPS、动作条、右双槽、中文文字、CD 与世界视野；需恢复完整 preset 时执行 `/aeui focuslayout comfort`。同轮继续 Field Kit popup、Queue／换装及 fail-open 复测；通过前不得标记 P6 |
