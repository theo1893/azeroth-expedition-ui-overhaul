...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。Field Kit accepted source／TGA 像素不变；v1.1 Icon 层序已实机通过，v1.2 已把遮挡主格的线性 popup 改为外置 `1×1–6／2×4–6` drawer 并加入双侧软吸附。实机随后发现主格到右抽屉的鼠标穿越会提前关闭；AEUI `0.8.3`／`fieldkit-contract=1.3` 仅在 exact 外置态加入全卷袋高度、直接隶属 `AutoBarPopupFrame` 的透明悬停通道，延后 XML Frame `OnLeave`，关闭仍由 AutoBar 原计时器负责；NATIVE、非 exact 与 AEUI 关闭完整恢复。simulation `89/89`、display `19/19`，runtime display `9/9`＋`10/10`；Field Kit 为 `P5 / runtime-v1.3 / pending-retest`，ImageGen `0`，原循环仍止于 `4/5` 与 `1/5` | Turtle WoW `/reload` 验证 `fieldkit-contract=1.3`，从卷袋上／中／下主格缓慢移入左右候选 `1／6／7／12` 时不断开、完全离开后正常关闭；再验证四种 popup 模式、非 exact 原脚本回退、两袋独立拖离／回吸附、apply／restore、TrinketMenu 候选／Queue／换装及 `/aeui actionbars` fail-open；通过前不得标记 P6 |
