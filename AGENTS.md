...
| 单位框 | `P3`。A attempt 1 已计 `1/5`：粗旧皮革、左端粗缝线／局部夹片和右端偏心铆钉可保留，但检测到双开口；bbox `1425×224`、比例误差 `24.853972%`、各向异性 `19.906433%`、hard safe core 侵入，故无 candidate/source/runtime。`UF-A1 V3-A final.r1` 已自包含收敛为单开口、约 `1284×252` 与固定安全区；B/B1 仍 `0/5 sequence-wait`。reviewer 一次无生成性能错误已线性化，不占额度 | 提交 `final.r1 / repair-prepared` 与 V3 reviewer 后执行 A attempt 2，只以同段前稿为 Image 3；A 终态后依次 B、B1。内部通过即停，第五次仍失败则耗尽。A2 继续暂停 |
