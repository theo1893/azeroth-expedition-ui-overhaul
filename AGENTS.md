...
| 任务 | 用户于 `2026-08-05` 实机确认当前 Quest Log 左右页 bug 与显示问题已修复；该活动范围保持 `P6 user-confirmed`。QS-B1 V7-A attempt 5 已于 `2026-08-07` 获用户明确接受并进入 `P5 runtime-exported / addon-integrated`：exact `128×768` source SHA `168f527f…05b8` 等比导出为 `32×192` TGA SHA `db620778…c615`，Quest runtime `1.26`／Theme `1.9` 将闭合 `28px` 载体根部与 QS-A1 火漆共同挂在 `QuestLogDetailScrollChildFrame`，通过 `13/13` 技术、`29/29` 真实排版、`6/6` 展示区与 Lua 冒烟检查。QL-D V3 手绘军需装备签模拟与 production 合同已确认／授权；attempt 1 为 `17/19` 技术、display `5/5`，但因 `2.5714` aspect、微型摊书身份、规则缝圈、密集交叉纹理与绿边拒绝，当前 `candidate-rejected / P3 / 1/5 / repairing`，V3.r1 必须固定 Image 1／2 fresh regenerate。七枚独立功能纹章仍未验收，菜单不响应，旧 Blizzard／pfQuest 按钮继续 fail-open。Tracker 与 NPC Quest／Gossip 不变 | 固定执行器执行 QL-D V3.r1 attempt 2 并完成候选内审；Turtle WoW 验证闭合态火漆跨压、滚动裁切和 TGA 方向；另行完成七纹章与代理 parity 后才可启用事务菜单或隐藏旧按钮 |
