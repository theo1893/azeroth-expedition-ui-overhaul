...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。用户于 `2026-08-09` 接受 `AB.TRINKET.KIT.V1` 第4稿与 `AB.CONSUMABLE.KIT.V1` 第1稿；两套 exact canonical 已 byte-exact 晋级为 tracked P4 source。Trinket SHA `82dd2260…c012`、循环终止 `4/5`；Consumable SHA `623f29c5…a2419`、循环终止 `1/5`。两个 SourceManifest 固定 Character V3 provenance、Alpha、四格映射、`80 px` margin 与 `16/16` display／violations `0`；接受后 ImageGen `0`，尚未切片或接入 runtime | 用户若继续 Field Kit，分别冻结四 cell crop／九宫格／旋转／拉伸／UV 与 provider adapter 合同，再走 P4→P5 display-region、静态回归及 fresh-checkout package。不得启用 AutoBar、应用 profile、改 TrinketMenu SavedVariables 或接管 provider 动态层 |
