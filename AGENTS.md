...
| 动作条／随身栏 | `AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6；Field Kit 视觉 runtime-v1.5／bridge-v2.0 与 Combat Focus runtime-v2.2 为 `P5 / pending-game-validation`。AEUI `0.8.19` 的 `ACTION-BARS-CORE-SIM-V11` 保留 V10 的 Turtle WoW 游戏原生几何：Player／Target `240×60 / 0.8`，TargetTarget `240×60 / 0.68` 并以 `8 UI` 间距中线依附 Target 右侧；Aura `23 UI`、真实 `size+7` 步进每行八枚；玩家 Cast／目标 Cast／Swing 同轴 `260×12 / 1.0`。V11 将三框局部字形改为客户端系统字体 `OUTLINE / 18 UI`，并把 DoiteDPS 时间线与资源两排作为 union 从 `TOPLEFT (850,-647)` 上移至 `(850,-615)`；姿态、消耗品／饰品低 `20 UI` 底线及精简 AutoBar 行为不变。未手调的 exact v7–v11 几何与仍匹配旧字体的 exact v12 profile 在 `/reload` 一次迁移为 v13。右侧 Paging／Vertical／Left／Right 四栏已有 `2×2` 四个 `3×4` 分区、总体 `6×8` 的 proposal-only 方案，未接入 runtime；全部位图字节不变，ImageGen `0/0` | `/reload` 后确认三框系统字体 `18`、DoiteDPS 双排不压 Player Buff，并复测 Player／卷袋、Aura 八枚、Boss 16 Debuff、三条计时栈及 pfUI unlock 生命周期；继续验证 AutoBar 动态格数、ArchiTotem、Field Kit popup／Queue／换装。四栏组合待用户确认后再实现单 group mover、独立内容配置与可逆 unbind；需要回退时 `/aeui focuslayout restore` 后 `/reload` |
