...
| 聊天 | Full V1 runtime `1.19 / P5` 保持；正式仍复用 V3 Tab／输入／未读。用户已接受 `CHAT.INPUT.DARK.V1.r3 attempt 4` 进入 `P4`；精确透明源已固化为 `assets/source/chat/input-dark-v1/ChatInput_Dark_V1_r3.png`，SHA-256 `4df36bc…cda0`，并建立 source manifest。生产实际 ImageGen `4/5`、流程错误 `4`；剩余一次停止且不转移。现行 TGA、Lua、pfUI／ChatMOD 与游戏功能均未改变，生成证据按 P6-C 门禁保留。Chat Copy／URL Copy 暂缓；右框及左右聊天信息 Panel 隐藏，小地图 Panel 保留 | 为新输入源建立确定性 P4→P5 exporter，生成最终 TGA 后再做真实排版、display-region 与静态测试；游戏设备可用时再 `/reload` 验证核心 r1.19 与新输入交互 |
