...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，固定生产 `5/5` 且禁止 attempt 6。Field Kit accepted source／TGA、v1.4 drawer 与 v1.5 强绑定不变，仍为 `P5 / pending-retest`。用户于 `2026-08-09` 要求本期先完成玩家／目标、施法、攻击计时、DoiteDPS 等位置，后期再重绘；AEUI `0.8.6`／`focus-layout-contract=1.0` 已实现 `/aeui focuslayout apply` 一次性 V3 纵栈，并写入“大奶黑牛”pfUI／DoiteDPS SavedVariables。关键状态保持 provider 原生 Alpha；Rail／连接片／口袋／护套等装饰鼠标穿透，DoiteDPS 锁定态沿用 provider 鼠标穿透，不创建中央大型透明命中层。无维护循环、无重绘、ImageGen `0`；package pass | 启动或 `/reload` 验证 `version 0.8.6`、`focus-layout-contract=1.0`、DoiteDPS→攻击计时→Aura→Player／Target→双施法条→姿态→Combat Deck 的物理纵栈、Aura 每肩六枚换行、关键状态透明度与世界点击；需要恢复时只执行一次 `/aeui focuslayout apply`。同轮继续复测 `fieldkit-contract=1.5`、唯一 mover、popup 跨格、apply／restore、Queue／换装及 fail-open；通过前不得标记 P6 |
