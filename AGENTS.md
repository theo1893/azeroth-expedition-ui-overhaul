...
| 动作条／随身栏 | `ACTION-BARS-CORE-SIM-V3` 已确认。`AB.SLOT.BASE.V1` 与 `AB.RAIL` 均为 `P6 / game-validated`，各自固定生产 `5/5` 且禁止 attempt 6。`AB.FIELDKIT.V1` 的 AutoBar `4×6 / 24 类` 三段分组与 TrinketMenu 双槽方向已确认；两个 production body、各 `5` 次预算和各自 Character V3 Image 1 上传已授权。Trinket attempt 4 canonical `82dd2260…c012` 在 `4/5` 全门禁通过；Consumable attempt 1 raw `de25567f…b8ba`／canonical `623f29c5…a2419` 在 `1/5` 同样通过 Prompt、语义、美术、技术与 `16/16` display 全门禁。两者均为 `candidate-reviewed / pending-user-acceptance`，按 pass 即停，未用预算不再消耗。纯绿 raw→canonical 修订不重绘、不新增输入图、预算不重置 | 提交 Consumable attempt 1 记录后，请用户分别接受或拒绝 Trinket 第4稿与 Consumable 第1稿；接受前不得晋级 source／runtime。若拒绝，Trinket 只余 `1` 次、Consumable 只余 `4` 次，均禁止 attempt 6；全过程不启用 AutoBar、不改 TrinketMenu SavedVariables |
