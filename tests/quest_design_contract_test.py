...
def main() -> None:
    ...
    require(
        ...
        (
            ...
            "`candidate-rejected / P3 / production 3/5 / repairing`",
            ...
            "miniature-open-book identity",
            "f65856e024882f35bbc031c9d5e2405772e7a6ee52e890a1b3873639c678235e",
            "### `QL-D V3.r2` production prompt",
            "at least eighty percent",
            "766f796cd9e5f70d7b395d43ec8475f661051c4dac5202fa6db4812892c9dc66",
            "### `QL-D V3.r3` production prompt",
            "two-envelope occupancy construction",
            "deba614615e6e768840fbea8d3354345e614e76123df6535e6ab2be67f122d51",
            "## V3 自主修复循环与授权边界",
            "authorized=true",
        ),
    ...
    assert reward_production_spec["executor"]["current_actual_imagegen_calls"] == 3
    ...
    assert len(reward_production_spec["attempt_history"]) == 3
    assert all(
    ...
    assert reward_production_spec["attempt_history"][1][
        "technical_checks"
    ] == "19/19 after authorized 18-pixel RGB-only soft despill"
    assert reward_production_spec["attempt_history"][0][
        "previous_output_image_3_eligible"
    ] is False
    assert reward_production_spec["current_prompt"] == {
        "version": "QL-D V3.r3",
        "body_sha256": "deba614615e6e768840fbea8d3354345e614e76123df6535e6ab2be67f122d51",
    ...
    assert v7a_executor["current_actual_generations"] == 5
    ...
    assert v7a_history[2]["attempt"] == 3
    assert v7a_history[2]["fixed_crop_mean_rgb"] == [
        102.68,
        86.25,
        63.73,
    ]
    assert v7a_history[2]["first_failed_gate"] == (
        "non-repeating-hand-painted-surface-language"
    )
    ...
    assert v7a_history[4]["attempt"] == 5
    assert v7a_history[4]["mode"] == (
        "final-fresh-generate-with-fixed-image-1-and-image-2-only"
    )
    assert v7a_history[4]["repo_commit_before_generation"] == "b025656"
    assert v7a_history[4]["technical_checks"] == "13/13 pass"
    assert v7a_history[4]["layout_checks"] == "29/29 pass"
    ...
    require(
        ...
        (
            ...
            "当前实际 ImageGen：`5/5 candidate-reviewed`",
            "V7-A 独立生产授权记录",
