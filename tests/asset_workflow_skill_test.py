...
def main() -> None:
    ...
    require(
        ...
        (
            ...
            "Before executing either a simulation or production prompt",
            ...
            "not authorize either ImageGen stage",
            "## Simulate before production",
            "mandatory low-cost direction gate",
            "one actual ImageGen call per simulation version by default",
            "independently from every",
    ...
    require(
        ...
        (
            ...
            "`prompt-draft → simulation-authorized`",
            "`simulation-authorized → simulation-reviewed`",
            ...
            "生成前模拟版本默认只产生 `1` 次实际 ImageGen 调用",
            "不消耗任何生产正文的",
            "模拟图不得复制",
    ...
    require(
        ...
        (
            ...
            "不得新增路线图",
            "`prompts/` 树",
            "首次模拟和首次生产执行前都先提交用户授权的 work",
            "每次模拟或生产实际生图前提交当前完整执行正文",
            "## 五次自主修复同步",
